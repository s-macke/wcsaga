...
int cfile_init(char *exe_dir, char *cdrom_dir) {
    ...
    if (!cfile_inited) {
        ...
        snprintf(Cfile_user_dir, CFILE_ROOT_DIRECTORY_LEN - 1, "%s%s%s%s%s%s", detect_home(), DIR_SEPARATOR_STR,
                 Osreg_company_name, DIR_SEPARATOR_STR, Osreg_user_dir, DIR_SEPARATOR_STR);
        cf_create_directory(Cfile_user_dir);

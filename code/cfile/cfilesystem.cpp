...
// for a defined and specifically set location to get/send pilot/campaign files and other files that must be written to home
char Home_path[CF_MAX_PATHNAME_LENGTH];
...
void cf_build_root_list(char *cdrom_dir) {
    ...
    if (Cmdline_mod) {
        for (cur_pos = Cmdline_mod; strlen(cur_pos) != 0; cur_pos += (strlen(cur_pos) + 1)) {
            // =========================================================================
            // Look for mods under the users HOME directory to use before system ones
            ...
            if ((strlen(root->path) < (CF_MAX_PATHNAME_LENGTH - 1)) && (root->path[strlen(root->path) -
                                                                                   1] != DIR_SEPARATOR_CHAR)) {
                strcat_s(root->path,
                         DIR_SEPARATOR_STR);        // put trailing backslash on for easier path construction
            }
            ...
            // =========================================================================
            // Look for mods under the system directory next
    ...
    if (*Home_path == '\0')
        snprintf(Home_path, sizeof(Home_path) - 1, "%s%s%s%s%s%s", detect_home(), DIR_SEPARATOR_STR,
                 Osreg_company_name, DIR_SEPARATOR_STR, Osreg_user_dir, DIR_SEPARATOR_STR);

    // Next, check any VP files under the current directory.
    cf_build_pack_list(root);

    // =========================================================================
    // Search the system root last
                ...
                if (!(find.attrib & _A_SUBDIR)) {
                    ...
                    if ( ext )	{
                        if ( is_ext_in_list( Pathtypes[i].extensions, ext ) )	{
                            ...
                            std::string name_ext(file->name_ext);
                            std::transform(name_ext.begin(), name_ext.end(), name_ext.begin(), tolower);
                            File_lookup.emplace(name_ext, file);
...
int cf_find_file_location_fast(char *filespec, int pathtype, int max_out, char *pack_filename, int *size, int *offset,
                               bool localize) {
    std::string filespec_lower(filespec);
    std::transform(filespec_lower.begin(), filespec_lower.end(), filespec_lower.begin(), tolower);

    boost::unordered_map<std::string, cf_file *>::iterator lookup_iter = File_lookup.find(filespec_lower);
    if (lookup_iter != File_lookup.end()) {
        cf_file *f = lookup_iter->second;
...
int cf_find_file_location(char *filespec, int pathtype, int max_out, char *pack_filename, int *size, int *offset,
    ...
    if (cf_find_file_location_fast(filespec, pathtype, max_out, pack_filename, size, offset, localize)) {
        return 1;
    }

    int i;

    // Search the hard drive for files.
    ...
    for (i = 0; i < num_search_dirs; i++) {
        ...
        for (int tested_root = 0; tested_root < Num_roots; ++tested_root) {
            if (cf_get_root(tested_root)->roottype != CF_ROOTTYPE_PATH)
                continue;

            cf_create_default_path_string_for_root(tested_root, longname, sizeof(longname) - 1, search_order[i],
                                                   filespec, localize);
...
int cf_find_file_location_ext(char *filename, const int ext_num, const char **ext_list, int pathtype, int max_out,
                              char *pack_filename, int *size, int *offset, bool localize) {
...
#if 0 // Files with different extensions are not stored during game play, so there is no need to look for them on the disk.
    // Search the hard drive for files.
    int i, num_search_dirs = 0;
    int search_order[CF_MAX_PATH_TYPES];
    ...
    for (i = 0; i < num_search_dirs; i++) {
        ...
        for (cur_ext = 0; cur_ext < ext_num; cur_ext++) {
            for (int tested_root = 0; tested_root < Num_roots; ++tested_root) {
                ...
                char longname[MAX_PATH_LEN];
                memset( longname, 0, sizeof(longname) );
                cf_create_default_path_string_for_root( tested_root, longname, sizeof(longname)-1, search_order[i], filespec, localize );
...
int cf_get_file_list(int max, char **list, int pathtype, char *filter, int sort, file_list_info *info) {
    ...
    if (find_handle != -1) {
        do {
            ...
            const char* filter_ext = strrchr(filter, '.');
            const char* find_name_ext = strrchr(find.name, '.');

            if (strcmp(filter_ext, find_name_ext) != 0)
                continue;
...
// location, 'filter' only needs to be the filter itself, with no path information.
// See above descriptions of cf_get_file_list() for more information about how it all works.
int cf_get_file_list_preallocated(int max, char arr[][MAX_FILENAME_LEN], char **list, int pathtype, char *filter,
                                  int sort, file_list_info *info) {
...
int cf_create_default_path_string(char *path, uint path_max, int pathtype, char *filename, bool localize) {
    return cf_create_default_path_string_for_root(0, path, path_max, pathtype, filename, localize);
}
...
//			root_n	  - optional, the root to use for path generation.
...
int cf_create_default_path_string_for_root(int root_n, char *path, uint path_max, int pathtype, char *filename,
                                           bool localize) {
#ifdef SCP_UNIX
    ...
    if (filename && strpbrk(filename, "/\\:")) {
        ...
        cf_root *root = cf_get_root(root_n);
        ...
        if ((pathtype == CF_TYPE_PLAYERS) || (pathtype == CF_TYPE_SINGLE_PLAYERS) ||
            (pathtype == CF_TYPE_MULTI_PLAYERS)) {
            strncpy(path, Home_path, path_max);
        } else {

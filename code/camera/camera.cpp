...
float Viewer_zoom_default = 0.46631f; // 50 degrees FOV
float Viewer_zoom_target = 0.36397f; // 40 degrees FOV
float Viewer_zoom_FRED = 0.36397f; // 40 degrees FOV
float Sexp_zoom = 0.0f;
...
void camera::set_zoom(float in_zoom, float in_zoom_time, float in_zoom_acceleration_time,
                      float in_zoom_deceleration_time) {
    if (in_zoom_time == 0.0f && in_zoom_acceleration_time == 0.0f && in_zoom_deceleration_time == 0.0f) {
    ...
    flags &= ~CAM_STATIONARY_FOV;
    if (in_zoom <= 0.0f) {
        zoom.setVD(in_zoom_time, in_zoom_acceleration_time, 0.0f);
    }

    zoom.setAVD(in_zoom, in_zoom_time, in_zoom_acceleration_time, in_zoom_deceleration_time, 0.0f);
}
...
void camera::set_position(vec3d *in_position, float in_translation_time, float in_translation_acceleration_time,
                          float in_translation_deceleration_time, float in_end_velocity) {
    ...
    pos_x.setAVD(in_position->xyz.x, in_translation_time, in_translation_acceleration_time,
                 in_translation_deceleration_time, in_end_velocity);
    pos_y.setAVD(in_position->xyz.y, in_translation_time, in_translation_acceleration_time,
                 in_translation_deceleration_time, in_end_velocity);
    pos_z.setAVD(in_position->xyz.z, in_translation_time, in_translation_acceleration_time,
                 in_translation_deceleration_time, in_end_velocity);
}
...
void camera::set_rotation(matrix *in_orientation, float in_rotation_time, float in_rotation_acceleration_time,
    ...
    for (int i = 0; i < 9; i++)
        ori[i].setAVD(in_orientation->a1d[i], in_rotation_time, in_rotation_acceleration_time,
...
void camera::set_rotation_facing(vec3d *in_target, float in_rotation_time, float in_rotation_acceleration_time,
    ...
    if (in_target != NULL) {
        ...
        if (in_target->xyz.x == position.xyz.x && in_target->xyz.y == position.xyz.y &&
        ...
        if (!(flags & CAM_STATIONARY_ORI) || object_target.IsValid() || object_host.IsValid()) {
            if (object_target.IsValid()) {
                ...
                if (object_target_submodel < 0 || pm == NULL) {
                    ...
                    model_find_world_point(&target_pos, &vmd_zero_vector, pm->id,
                                           object_target_submodel, &objp->orient, &objp->pos);
                }
...
subtitle::subtitle(int in_x_pos, int in_y_pos, char *in_text, char *in_imageanim, float in_display_time,
                   float in_fade_time, color *in_text_color, bool center_x, bool center_y, int in_width, int in_height,
                   bool in_post_shaded) {

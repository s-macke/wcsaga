...
void triggered_rotation::start(queued_animation* q)
{
	...
	for (int axis = 0; axis < 3; axis++)
	{
		...
		if (q->accel.a1d[axis] == 0.0f)
			...
			slow_angle.a1d[axis] = end_angle.a1d[axis] - (((q->vel.a1d[axis] * q->vel.a1d[axis]) / (2.0f *
																									q->accel.
																									a1d[axis])) *
														  direction.a1d[axis]);
	}
	...
	nprintf(("ModelAnim",
			 ...
			 end_angle.a1d[0], end_angle.a1d[1], end_angle.a1d[2], rot_vel.a1d[0], rot_vel.a1d[1], rot_vel.a1d[2],
			 rot_accel.a1d[0], rot_accel.a1d[1], rot_accel.a1d[2],
...
void triggered_rotation::add_queue(queued_animation* the_queue, int dir)
{
	...
	if (n_queue > 0)
	{
		...
		if (i != n_queue)
		{
			...
			if (i != (MAX_TRIGGERED_ANIMATIONS - 1))
				memcpy(&queue_tmp[i], &queue_tmp[i + 1], sizeof(queued_animation) * (MAX_TRIGGERED_ANIMATIONS - (i +
																												 1)));
	...
	if (n_queue > 0)
	{
		...
		for (i = 0;
			 (i < n_queue) && (i < MAX_TRIGGERED_ANIMATIONS) && (new_queue.start_time > queue_tmp[i].start_time); i++);
...
void model_anim_submodel_trigger_rotate(model_subsystem* psub, ship_subsys* ss)
{
	...
	*/
	...
	{
		// are we moving?
		if ((trigger->current_vel.a1d[i] != 0.0f) || ((trigger->current_ang.a1d[i] *
													   trigger->direction.a1d[i]) <= (trigger->slow_angle.a1d[i] *
																					  trigger->direction.a1d[i])))
		{
			// yep...
			...
			if ((trigger->current_ang.a1d[i] * trigger->direction.a1d[i]) <= (trigger->slow_angle.a1d[i] *
																			  trigger->direction.a1d[i]))
			{
				// speeding up
				if ((trigger->current_vel.a1d[i] * trigger->direction.a1d[i]) < (trigger->rot_vel.a1d[i] *
																				 trigger->direction.a1d[i]) &&
					(trigger->rot_accel.a1d[i] != 0.0f))
				{
			...
			else
			{
				...
				if (((trigger->current_vel.a1d[i] * trigger->direction.a1d[i]) > 0) &&
					(trigger->rot_accel.a1d[i] != 0.0f))
				{
					trigger->current_vel.a1d[i] -= (trigger->rot_accel.a1d[i] * flFrametime);
				}
				...
				else
				{
					...
					if ((trigger->current_vel.a1d[i] / fabs(trigger->current_vel.
						 a1d[i])) != (trigger->rot_accel.a1d[i] / fabs(trigger->rot_accel.a1d[i])))
					{
			...
			if ((trigger->current_ang.a1d[i] * trigger->direction.a1d[i]) > (trigger->end_angle.a1d[i] *
																			 trigger->direction.a1d[i]))
			{
...
int model_anim_instance_get_actual_time(queued_animation* properties)
{
	...
	for (int a = 0; a < 3; a++)
	{
		temp = fl2i(((3.0f * properties->vel.a1d[a] * properties->vel.a1d[a]) + (2.0f * properties->accel.a1d[a] *
			...
			for (j = 0; j < psub->n_triggers; j++)
			{
				if (psub->triggers[j].type == type)
				{
					...
					if (psub->triggers[j].reverse_start == -1)
						psub->triggers[j].reverse_start = ani_time - model_anim_instance_get_actual_time(&psub->
							triggers[j]);
...
int model_anim_get_time_type(ship_subsys* pss, int animation_type, int subtype)
{
	...
	for (i = 0; i < psub->n_triggers; i++)
	{
			...
			((psub->triggers[i].subtype == ANIMATION_SUBTYPE_ALL) || (psub->triggers[i].subtype == subtype)))
		{
			...
			if ((pss->trigger.current_vel.a1d[0] != 0.0f) || (pss->trigger.current_vel.a1d[1] != 0.0f) ||
				(pss->trigger.current_vel.a1d[2] != 0.0f))
			{
				...
				for (int a = 0; a < 3; a++)
				{
					...
					{
						a_time = fl2i(((fl_sqrt(2.0f * pss->trigger.rot_accel.a1d[a] * (pss->trigger.end_angle.a1d[a] -
																						pss->trigger.current_ang.
																						a1d[a]) +
										pss->trigger.current_vel.a1d[a] * pss->trigger.current_vel.a1d[a]) -
										pss->trigger.current_vel.a1d[a])
					...
					else
					{
						...
						{
							a_time = fl2i(((3.0f * pss->trigger.rot_vel.a1d[a]) / (2.0f * pss->trigger.rot_accel.
																				   a1d[a])
										   + (pss->trigger.current_vel.a1d[a] * pss->trigger.current_vel.a1d[a])
										   ...
										   - (pss->trigger.current_vel.a1d[a] / (pss->trigger.rot_accel.a1d[a] *
																				 direction)))
								* 1000.0f);

...
void model_allocate_interp_data(int n_verts = 0, int n_norms = 0, int n_list_verts = 0)
{
	...
	Assert((n_verts || Num_interp_verts_allocated) && (n_norms ||
													   Num_interp_norms_allocated)
													   /*&& (n_list_verts || Num_interp_list_verts_allocated)*/ );
...
{
	if (Cmdline_cell)
		model_interp_splode_defpoints(p, pm, sm, model_radius / 100);
	if (splodeing)
		model_interp_splode_defpoints(p, pm, sm, splode_level * model_radius);
...
void model_interp_flatpoly(ubyte* p, polymodel* pm)
{
	...
	for (i = 0; i < nv; i++)
	{
		...
		else
		{
			...
			{
				light_apply_rgb(&Interp_list[i]->r, &Interp_list[i]->g, &Interp_list[i]->b, Interp_verts[vertnum],
					vp(p + 8), Interp_light);
			}
			else
			{
				...
				{
					light_apply_rgb(&Interp_lighting->lights[norm].r, &Interp_lighting->lights[norm].
						g, &Interp_lighting->lights[norm].b, Interp_verts[vertnum], vp(p + 8), Interp_light);
...
void model_interp_edge_alpha(ubyte* param_r, ubyte* param_g, ubyte* param_b, vec3d* pnt, vec3d* norm, float alpha,
							 bool invert = false)
{
	vec3d r;
...
void model_interp_tmappoly(ubyte* p, polymodel* pm)
{
	...
	if (!Cmdline_nohtl)
	{
		...
		if (splodeing)
		{
			...
			for (i = 0; i < nv; i++)
			{
				...
				model_interp_edge_alpha(&Interp_list[i]->r, &Interp_list[i]->g, &Interp_list[i]->b,
					Interp_verts[verts[i].vertnum], Interp_norms[verts[i].normnum], salpha, false);
			}
			cull = gr_set_cull(0);
	...
	for (i = 0; i < nv; i++)
	{
		...
		else
		{
			...
			if ((Interp_flags & MR_NO_LIGHTING) || (tmap->is_ambient))
			{	//gets the ambient glow to work
				Interp_list[i]->r = 191;
				Interp_list[i]->g = 191;
				Interp_list[i]->b = 191;
				...
				{
					model_interp_edge_alpha(&Interp_list[i]->r, &Interp_list[i]->g, &Interp_list[i]->b,
						Interp_verts[verts[i].vertnum], Interp_norms[verts[i].normnum], Interp_warp_alpha, false);
				}

				if (Interp_flags & MR_CENTER_ALPHA)
				{
					model_interp_edge_alpha(&Interp_list[i]->r, &Interp_list[i]->g, &Interp_list[i]->b,
						Interp_verts[verts[i].vertnum], Interp_norms[verts[i].normnum], Interp_warp_alpha, true);
				}
			...
			else
			{
				...
				if (Interp_flags & MR_NO_SMOOTHING)
				{
					...
					if ((Detail.lighting > 2) && (Interp_detail_level < 2) && !Cmdline_cell && Cmdline_spec)
						light_apply_specular(&Interp_list[i]->spec_r, &Interp_list[i]->spec_g, &Interp_list[i]->spec_b,
							Interp_verts[vertnum], vp(p + 8), &View_position);
				...
				else
				{
					...
					if (!Interp_use_saved_lighting && !Interp_light_applied[norm])
					{
						...
						light_apply_rgb(&Interp_lighting->lights[norm].r, &Interp_lighting->lights[norm].
							g, &Interp_lighting->lights[norm].b, Interp_verts[vertnum], Interp_norms[norm],
							Interp_light);
						if ((Detail.lighting > 2) && (Interp_detail_level < 2) && !Cmdline_cell && Cmdline_spec)
							light_apply_specular(&Interp_lighting->lights[norm].spec_r, &Interp_lighting->lights[norm].
								spec_g, &Interp_lighting->lights[norm].spec_b, Interp_verts[vertnum],
								Interp_norms[norm], &View_position);
		...
		else
		{
			...
			if (Interp_tmap_flags & TMAP_FLAG_TEXTURED)
			{
				...
				else
				{
					...
					else if ((Interp_new_replacement_textures != NULL) && (Interp_new_replacement_textures[
																		   rt_begin_index + TM_BASE_TYPE] >= 0))
					{
					...
					else
					{
						...
						if (!(Interp_flags & MR_NO_GLOWMAPS) && (tglow->GetTexture() >= 0))
						{
							...
							if ((Interp_objnum >= 0) && (Objects[Interp_objnum].type == OBJ_SHOCKWAVE) &&
								(tglow->GetNumFrames() > 1))
							{
								GLOWMAP = tglow->GetTexture() + shockwave_get_framenum(Objects[Interp_objnum].instance,
									tglow->GetNumFrames());
							}
							else
							{
...
void model_interp_sortnorm_b2f(ubyte* p, polymodel* pm, bsp_info* sm, int do_box_check)
{
	...
	{		//facing
		...
		if (backlist)
			model_interp_sub(p + backlist, pm, sm, do_box_check);

		if (onlist)
			model_interp_sub(p + onlist, pm, sm, do_box_check);			//onlist

		if (frontlist)
			model_interp_sub(p + frontlist, pm, sm, do_box_check);

	}
	else
	{			//not facing.  draw front then back
	...
	if (postlist)
		model_interp_sub(p + postlist, pm, sm, do_box_check);		// postlist
...
int model_interp_sub(void* model_ptr, polymodel* pm, bsp_info* sm, int do_box_check)
{
	...
	while (chunk_type != OP_EOF)
	{
		...
		case OP_EOF:
			return 1;
		case OP_DEFPOINTS:
			model_interp_defpoints(p, pm, sm);
			break;
		case OP_FLATPOLY:
			model_interp_flatpoly(p, pm);
			break;
		case OP_TMAPPOLY:
			model_interp_tmappoly(p, pm);
			break;
		case OP_SORTNORM:
			model_interp_sortnorm(p, pm, sm, do_box_check);
			break;
...
void model_render_insignias(polymodel* pm, int detail_level)
{
	...
	for (idx = 0; idx < pm->num_ins; idx++)
	{
		...
		for (s_idx = 0; s_idx < pm->ins[idx].num_faces; s_idx++)
		{
			...
			vecs[0].u = pm->ins[idx].u[s_idx][0];
			vecs[0].v = pm->ins[idx].v[s_idx][0];
			vecs[1].u = pm->ins[idx].u[s_idx][1];
			vecs[1].v = pm->ins[idx].v[s_idx][1];
			vecs[2].u = pm->ins[idx].u[s_idx][2];
			vecs[2].v = pm->ins[idx].v[s_idx][2];

			if (!Cmdline_nohtl)
			{
				light_apply_rgb(&vecs[0].r, &vecs[0].g, &vecs[0].b, &pm->ins[idx].vecs[i1], &pm->ins[idx].norm[i1],
					1.5f);
				light_apply_rgb(&vecs[1].r, &vecs[1].g, &vecs[1].b, &pm->ins[idx].vecs[i2], &pm->ins[idx].norm[i2],
					1.5f);
				light_apply_rgb(&vecs[2].r, &vecs[2].g, &vecs[2].b, &pm->ins[idx].vecs[i3], &pm->ins[idx].norm[i3],
					1.5f);
...
void model_render(int model_num, matrix* orient, vec3d* pos, uint flags, int objnum, int lighting_skip,
				  int* replacement_textures)
{
	...
	int time = timestamp();
	for (int i = 0; i < pm->n_glow_point_banks; i++)
	{ //glow point blink code -Bobboau
...
float Mc_viewer_pos_factor[MAX_DETAIL_LEVEL+1] =
{
	0.080f,
	0.040f,
	0.020f,
...
float Mc_size_factor[MAX_DETAIL_LEVEL+1] =
{
	1.40f,
	1.30f,
	1.20f,
...
float Mc_framerate_lo[MAX_DETAIL_LEVEL+1] =
{
	0.0f,
	10.0f,
	15.0f,
...
float Mc_framerate_hi[MAX_DETAIL_LEVEL+1] =
{
	15.0f,
	20.0f,
	25.0f,
...
int Mc_detail_add[MAX_DETAIL_LEVEL+1] =
{
	-2,
	-1,
	+1,
...
float model_find_closest_point(vec3d* outpnt, int model_num, int submodel_num, matrix* orient, vec3d* pos,
							   vec3d* eye_pos)
{
	...
	return interp_closest_dist_to_box(&closest_pos, &eye_rel_pos, &pm->submodel[submodel_num].min, &pm->
		submodel[submodel_num].max);
}
...
#define NOISE_SCALE 0.5f
			...
			if (Interp_tertiary_thrust_glow_bitmap >= 0)
			{
				...
				tertiary_thruster_batcher.draw_bitmap(&p, 0, (w * 0.6f * Interp_tertiary_thrust_glow_rad_factor),
					(magnitude * 4), (-(D > 0) ? D : -D));
			}
			...
			if (Interp_secondary_thrust_glow_bitmap >= 0)
			{
				...
				if (d > 0.0f)
				{
					...
					secondary_thruster_batcher.draw_beam(&pnt, &norm2, wVal * Interp_secondary_thrust_glow_rad_factor *
						0.5f, d);
				}
			}
			// end secondary glows
	...
	if (Interp_secondary_thrust_glow_bitmap >= 0)
	{
		...
		secondary_thruster_batcher.render(TMAP_FLAG_GOURAUD | TMAP_FLAG_RGB | TMAP_FLAG_TEXTURED | TMAP_FLAG_CORRECT |
			TMAP_HTL_3D_UNLIT);
	}
...
void model_really_render(int model_num, matrix* orient, vec3d* pos, uint flags, int objnum)
{
	...
	if ((The_mission.flags & MISSION_FLAG_FULLNEB) && (Neb2_render_mode != NEB2_RENDER_NONE) && !(flags &
																								  MR_NO_FOGGING))
	{
	...
	if (Interp_flags & MR_AUTOCENTER)
	{
		...
		{
			auto_back.xyz.x = -((pm->submodel[pm->detail[Interp_detail_level]].max.xyz.x +
								 pm->submodel[pm->detail[Interp_detail_level]].min.xyz.x) / 2.0f);
			auto_back.xyz.y = -((pm->submodel[pm->detail[Interp_detail_level]].max.xyz.y +
								 pm->submodel[pm->detail[Interp_detail_level]].min.xyz.y) / 2.0f);
			auto_back.xyz.z = -((pm->submodel[pm->detail[Interp_detail_level]].max.xyz.z +
								 pm->submodel[pm->detail[Interp_detail_level]].min.xyz.z) / 2.0f);
			set_autocen = true;
		}
			...
			if (bank->is_on)
			{
				...
				for (j = 0; j < bank->num_points; j++)
				{
					...
					if (flick == 1)
					{
						...
						{ //this is where it rotates for the submodel parent-Bobboau
							...
							matrix inv_orientation;
							vm_copy_transpose_matrix(&inv_orientation, &pm->submodel[bank->submodel_parent].
								orientation);
						...
						case 1:
							{
								...
								moldel_calc_facing_pts(&top1, &bottom1, &fvec, &pnt, gpt->radius,
									1.0f, &View_position);
								moldel_calc_facing_pts(&top2, &bottom2, &fvec, &norm, gpt->radius,
									1.0f, &View_position);
								...
								if (The_mission.flags & MISSION_FLAG_FULLNEB)
								{
									gr_set_bitmap(bank->glow_neb_bitmap, GR_ALPHABLEND_FILTER, GR_BITBLT_MODE_NORMAL,
										1.0f);
								}
								else
								{
									gr_set_bitmap(bank->glow_bitmap, GR_ALPHABLEND_FILTER, GR_BITBLT_MODE_NORMAL,
										1.0f);
								}

								g3_draw_poly(4, verts, TMAP_FLAG_TILED | TMAP_FLAG_TEXTURED | TMAP_FLAG_CORRECT |
									TMAP_HTL_3D_UNLIT); // added TMAP_FLAG_TILED flag for beam texture tileing -Bobboau
	...
	if (Interp_flags & MR_SHOW_PATHS)
	{
		if (Cmdline_nohtl)
			...
			model_draw_paths_htl(model_num);
	}
...
void submodel_render(int model_num, int submodel_num, matrix* orient, vec3d* pos, uint flags, int objnum,
					 int* replacement_textures)
{
	// replacement textures - Goober5000
	model_set_replacement_textures(replacement_textures);
...
float get_model_closest_box_point_with_delta(vec3d* closest_box_point, vec3d* start_point, int modelnum,
											 int* is_inside, float delta)
{
	int i, idx;
...
// displaces closest point an optional amount delta to the outside of the box
// closest_box_point can be NULL.
float get_world_closest_box_point_with_delta(vec3d* closest_box_point, object* box_obj, vec3d* start_point,
											 int* is_inside, float delta)
{
...
void find_sortnorm(int offset, ubyte* bsp_data)
{
	...
	if (prelist)
		find_tri_counts(offset + prelist, bsp_data);
	if (backlist)
		find_tri_counts(offset + backlist, bsp_data);
	if (onlist)
		find_tri_counts(offset + onlist, bsp_data);
	if (frontlist)
		find_tri_counts(offset + frontlist, bsp_data);
	if (postlist)
		find_tri_counts(offset + postlist, bsp_data);
}
...
void generate_vertex_buffers(bsp_info* model, polymodel* pm)
{
	...
	for (i = 0; i < MAX_MODEL_TEXTURES; i++)
	{
		...
		if (Cmdline_normal)
		{
			memcpy((model_list.tsb) + model_list.n_verts, polygon_list[i].tsb, sizeof(tsb_t) *
				polygon_list[i].n_verts);
		}
	...
	else
	{
		...
		{
			mprintf((
				"IBX NOTICE:  Vert limit hit for IBX version 1 on subobject '%s', changing file format to version 2...\n", model->name));
		...
		for (j = 0; j < polygon_list[i].n_verts; j++)
		{
			...
			else
			{
				...
				if (use_large_buffer)
				{
					...
					Assert(same_vert(&model_list.vert[new_buffer.index_buffer.ibuffer[j]], &polygon_list[i].
						vert[j], &model_list.norm[new_buffer.index_buffer.ibuffer[j]], &polygon_list[i].norm[j]));
				}
				else
				{
					Assert(new_buffer.index_buffer.sbuffer != NULL);
					...
					Assert(same_vert(&model_list.vert[new_buffer.index_buffer.sbuffer[j]], &polygon_list[i].
						vert[j], &model_list.norm[new_buffer.index_buffer.sbuffer[j]], &polygon_list[i].norm[j]));
				}
...
void model_render_buffers(bsp_info* model, polymodel* pm, bool is_child)
{
	...
	for (uint i = 0; i < buffer_size; i++)
	{
		...
		gr_render_buffer(0, model->buffer[i].n_prim, model->buffer[i].index_buffer.sbuffer,
			model->buffer[i].index_buffer.ibuffer, Interp_tmap_flags);
...
{
	if (this->texture != original_texture)
	{

...
{
	if (vert->xyz.x < oct->min.xyz.x)
		return 0;
	if (vert->xyz.x > oct->max.xyz.x)
		return 0;

	if (vert->xyz.y < oct->min.xyz.y)
		return 0;
	if (vert->xyz.y > oct->max.xyz.y)
		return 0;

	if (vert->xyz.z < oct->min.xyz.z)
		return 0;
	if (vert->xyz.z > oct->max.xyz.z)
		return 0;
...
int model_octant_find_faces_sub(polymodel* pm, model_octant* oct, void* model_ptr, int just_count)
{
	...
	while (chunk_type != OP_EOF)
	{
		...
		case OP_DEFPOINTS:
			...
			break;
		case OP_FLATPOLY:
			moff_flatpoly(p, pm, oct, just_count);
			break;
		case OP_TMAPPOLY:
			moff_tmappoly(p, pm, oct, just_count);
			break;
		case OP_SORTNORM:
			{
				...
				if (prelist)
					model_octant_find_faces_sub(pm, oct, p + prelist, just_count);
				if (backlist)
					model_octant_find_faces_sub(pm, oct, p + backlist, just_count);
				if (onlist)
					model_octant_find_faces_sub(pm, oct, p + onlist, just_count);
				if (frontlist)
					model_octant_find_faces_sub(pm, oct, p + frontlist, just_count);
				if (postlist)
					model_octant_find_faces_sub(pm, oct, p + postlist, just_count);
			}
			break;
		case OP_BOUNDBOX:
			break;

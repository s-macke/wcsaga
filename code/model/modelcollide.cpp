...
static void mc_check_face(int nv, vec3d** verts, vec3d* plane_pnt, float face_rad, vec3d* plane_norm,
						  uv_pair* uvl_list, int ntmap, ubyte* poly)
{
	vec3d hit_point;
	float dist;
	float u, v;
...
static void mc_check_sphereline_face(int nv, vec3d** verts, vec3d* plane_pnt, float face_rad, vec3d* plane_norm,
									 uv_pair* uvl_list, int ntmap, ubyte* poly)
{
	vec3d hit_point;
	float u, v;
...
void model_collide_sortnorm(ubyte* p)
{
	...
	if (prelist)
		model_collide_sub(p + prelist);
	if (backlist)
		model_collide_sub(p + backlist);
	if (onlist)
		model_collide_sub(p + onlist);
	if (frontlist)
		model_collide_sub(p + frontlist);
	if (postlist)
		model_collide_sub(p + postlist);
}
...
bool mc_shield_check_common(shield_tri* tri)
{
	...
	if (!(Mc->flags & MC_CHECK_SPHERELINE))
	{	// Don't do this test for sphere colliding against shields
		...
		if (dist < 0.0f)
			return false; // If the ray is behind the plane there is no collision
		if (!(Mc->flags & MC_CHECK_RAY) && (dist > 1.0f))
			return false; // The ray isn't long enough to intersect the plane
...
void mc_check_shield()
{
	...
	// KeldorKatarn: This shield collision tree seems to be buggy.
	// It misses shield impacts. Deactivated for now.
...
void mc_check_subobj(int mn)
{
	...
	if ((mn < 0) || (mn >= Mc_pm->n_models))
		return;

	sm = &Mc_pm->submodel[mn];
	if (sm->no_collisions)
		return; // don't do collisions
	if (sm->nocollide_this_only)
		goto NoHit; // Don't collide for this model, but keep checking others
	...
	if (mc_ray_boundingbox(&sm->min, &sm->max, &Mc_p0, &Mc_direction, &hitpt))
	{
		...
		if (Mc->flags & MC_ONLY_BOUND_BOX)
		{
			...
			if (dist < 0.0f)
				goto NoHit; // If the ray is behind the plane there is no collision
			if (!(Mc->flags & MC_CHECK_RAY) && (dist > Mc_mag))
				goto NoHit; // The ray isn't long enough to intersect the plane
...
int model_collide(mc_info* mc_info)
{
	...
	if (Mc->num_hits)
	{
		...
		{
			model_find_world_point(&Mc->hit_point_world, &Mc->hit_point, Mc->model_num, Mc->hit_submodel, Mc->orient,
				Mc->pos);
		}
	}

...
void model_unload(int modelnum, int force)
{
	...
	if (pm->glow_point_banks)
	{ // free the glows!!! -Bobboau
...
static void set_subsystem_info(model_subsystem* subsystemp, char* props, char* dname)
{
	...
	{ // If unrecognized type, set to unknown so artist can continue working...
	...
	if ((p = strstr(props, "$rotate")) != NULL)
	{
		...
		if ((p = strstr(props, "$stepped")) != NULL)
		{
			...
			subsystemp->stepped_rotation = new
			(stepped_rotation);
			...
			subsystemp->stepped_rotation->max_turn_accel = PI2 / (fraction * (1.0f - fraction) * num_steps * t_trans *
																  t_trans);
...
void do_new_subsystem(int n_subsystems, model_subsystem* slist, int subobj_num, float rad, vec3d* pnt, char* props,
					  char* subobj_name, int model_num)
{
	int i;
...
{
	box[0].xyz.x = big_mn->xyz.x;
	box[0].xyz.y = big_mn->xyz.y;
	box[0].xyz.z = big_mn->xyz.z;
	box[1].xyz.x = big_mx->xyz.x;
	box[1].xyz.y = big_mn->xyz.y;
	box[1].xyz.z = big_mn->xyz.z;
	box[2].xyz.x = big_mx->xyz.x;
	box[2].xyz.y = big_mx->xyz.y;
	box[2].xyz.z = big_mn->xyz.z;
	box[3].xyz.x = big_mn->xyz.x;
	box[3].xyz.y = big_mx->xyz.y;
	box[3].xyz.z = big_mn->xyz.z;


	box[4].xyz.x = big_mn->xyz.x;
	box[4].xyz.y = big_mn->xyz.y;
	box[4].xyz.z = big_mx->xyz.z;
	box[5].xyz.x = big_mx->xyz.x;
	box[5].xyz.y = big_mn->xyz.y;
	box[5].xyz.z = big_mx->xyz.z;
	box[6].xyz.x = big_mx->xyz.x;
	box[6].xyz.y = big_mx->xyz.y;
	box[6].xyz.z = big_mx->xyz.z;
	box[7].xyz.x = big_mn->xyz.x;
	box[7].xyz.y = big_mx->xyz.y;
	box[7].xyz.z = big_mx->xyz.z;
}
...
//				mprintf(( "Num models = %d\n", pm->n_models ));
				...
				{
					Warning(LOCATION, "Inverted bounding box on model '%s'!  Swapping values to compensate.",
						pm->filename);
				}
		...
		case ID_SOBJ:
			{		//Subobject header
				...
				{
					Warning(LOCATION,
						"Inverted bounding box on submodel '%s' of model '%s'!  Swapping values to compensate.",
				...
				{
					if (strstr(pm->submodel[n].name, "turret") || strstr(pm->submodel[n].name, "gun") ||
				...
				if ((p = strstr(props, "$special")) != NULL)
				{
					...
					if (!stricmp(type, "subsystem"))
					{	// if we have a subsystem, put it into the list!
						do_new_subsystem(n_subsystems, subsystems, n, pm->submodel[n].rad, &pm->submodel[n].offset,
				...
				if ((pm->submodel[n].movement_type == MOVEMENT_TYPE_ROT) && (pm->submodel[n].movement_axis ==
																			 MOVEMENT_AXIS_NONE))
				{
					Warning(LOCATION, "Rotation without rotation axis defined on submodel '%s' of model '%s'!",
						pm->submodel[n].name, pm->filename);
				}
				...
				*/
				...
				if (((p = strstr(props, "$uvec:")) != NULL) || ((p = strstr(props, "$uvec=")) != NULL))
				{
					p += 6;
					...
					if (((p = strstr(props, "$fvec:")) != NULL) || ((p = strstr(props, "$fvec=")) != NULL))
					{
						parsed_string = p + 6;
					...
					else
					{
						...
						Warning(LOCATION,
							"Improper custom orientation matrix, you must define a up vector, then a forward vector");
					}
				}
				else
				{
					...
					if ((strstr(props, "$fvec:") != NULL) || (strstr(props, "$fvec=") != NULL))
					{
						Warning(LOCATION,
							"Improper custom orientation matrix, you must define a up vector, then a forward vector");
					}
				...
				if (pm->submodel[n].is_thruster && !(pm->submodel[n].no_collisions) &&
					!(pm->submodel[n].nocollide_this_only) && !(pm->submodel[n].collide_invisible))
				{
					pm->submodel[n].nocollide_this_only = true;
				}
		...
		case ID_SHLD:
			{
				...
				if (pm->shield.nverts > 0)
				{
					...
					{						// read in the vertex list
						...
						pm->shield.verts[i].normal = vmd_zero_vector;
					}
				}
				...
				if (pm->shield.ntris > 0)
				{
					...
					for (i = 0; i < pm->shield.ntris; i++)
					{
						...
						for (j = 0; j < 3; j++)
						{
							...
							vm_vec_add2(&pm->shield.verts[pm->shield.tris[i].verts[j]].normal, &pm->shield.tris[i].
								norm);
#ifndef NDEBUG
					...
					for (i = 0; i < pm->shield.nverts; i++)
					{
						vm_vec_normalize_safe(&pm->shield.verts[i].normal);
					}
				}
			}
			break;
		...
		case ID_DOCK:
			{
				...
				if (pm->n_docks > 0)
				{
					...
					for (i = 0; i < pm->n_docks; i++)
					{
						...
						{
							Warning(LOCATION,
								"Model '%s' has %d slots in dock point '%s'; models must have exactly %d slots per dock point.", filename, bay->num_slots, bay->name, 2);
						}
				...
				for (int gpb = 0; gpb < gpb_num; gpb++)
				{
					...
					if (length > 0)
					{
						...
						if (bank->glow_neb_bitmap < 0)
						{
							...
							nprintf(("Model",
									 "Glow point bank texture not found for '%s', setting as the normal one num\n",
									 pm->filename));
						...
						else
						{
							nprintf(("Model", "Glow point bank %i nebula texture num is %d for '%s'\n", gpb,
									 bank->glow_neb_bitmap, pm->filename));
						}
					}
					...
					if (subsystems)
					{
						for (snum = 0; snum < n_subsystems; snum++)
						{
							...
							if (parent == subsystemp->subobj_num)
							{
								...
								if (n_slots > MAX_TFP)
								{
									Warning(LOCATION, "Model %s has too many turret firing points on subsystem %s",
										subsystemp->name);
								}
	...
	for (i = 0; i < pm->n_models; i++)
	{
		...
		for (j = 0; j < pm->n_models; j++)
		{
			...
			{		// They only differ by one character!
				...
				if ((dl1 < 0) || (dl2 < 0) || (dl1 >= MAX_MODEL_DETAIL_LEVELS) || (dl2 >= MAX_MODEL_DETAIL_LEVELS))
	...
	for (i = 0; i < pm->n_paths; i++)
	{
		if (!strnicmp(pm->paths[i].name, NOX("$bay"), 4))
		{
			...
			if (bay_num < 1 || bay_num > MAX_SHIP_BAY_PATHS)
			{
				...
				if (bay_num < 1)
				{
					Warning(LOCATION, "Model '%s' bay path '%s' index '%d' has an invalid bay number of %d",
...
// returns 1 for invalid model, 2 for point offscreen.
// note that x1,y1,x2,y2 aren't clipped to 2d screen coordinates!
int submodel_find_2d_bound_min(int model_num, int submodel, matrix* orient, vec3d* pos, int* x1, int* y1, int* x2,
							   int* y2)
{
	polymodel* po;
	int n_valid_pts;
...
int model_rotate_gun(int model_num, model_subsystem* turret, matrix* orient, angles* base_angles, angles* gun_angles,
					 vec3d* pos, vec3d* dst, int obj_idx, bool reset)
{
	polymodel* pm;
...
void model_find_world_point(vec3d* outpnt, vec3d* mpnt, int model_num, int sub_model_num, matrix* objorient,
							vec3d* objpos)
{
	vec3d pnt;
	vec3d tpnt;
	...
	while (i >= 0)
	{
		...
		if (!child_submodel->blown_off && (child_submodel->i_replace == -1))
		{
			...
			if (child_submodel->movement_type == MOVEMENT_TYPE_ROT)
			{
				...
				for (subsys = GET_FIRST(&pship->subsys_list); subsys != END_OF_LIST(&pship->subsys_list);
...
// reference, and given the object's orient and position, 
// return the point in 3-space in outpnt.
void model_find_world_dir(vec3d* out_dir, vec3d* in_dir, int model_num, int sub_model_num, matrix* objorient,
						  vec3d* objpos)
{
	vec3d pnt;
	vec3d tpnt;
...
void model_do_childeren_dumb_rotation(polymodel* pm, int mn)
{
	while (mn >= 0)
	{
		...
		if (pm->submodel[mn].first_child > -1)
			model_do_childeren_dumb_rotation(pm, pm->submodel[mn].first_child);
...
void swap_bsp_sortnorms(polymodel* pm, ubyte* p)
{
	...
	if (prelist)
		swap_bsp_data(pm, p + prelist);
	if (backlist)
		swap_bsp_data(pm, p + backlist);
	if (onlist)
		swap_bsp_data(pm, p + onlist);
	if (frontlist)
		swap_bsp_data(pm, p + frontlist);
	if (postlist)
		swap_bsp_data(pm, p + postlist);
}
#endif // BIG_ENDIAN

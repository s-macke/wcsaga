...
void scoring_eval_harbison(ship *shipp) {
    ...
    if (!stricmp(shipp->ship_name, "alpha 2") && (!stricmp(Game_current_mission_filename,
                                                           "demo01") ||
                                                  !stricmp(Game_current_mission_filename, "sm1-01"))) {
        int death_count;
...
void scoring_eval_rank(scoring_struct *sc) {
    ...
    if (Player->flags & PLAYER_FLAGS_PROMOTED) {
        ...
        // Automatic promotions will not change the player score.
...
void scoring_level_close(int accepted) {
    ...
    if (accepted) {
        ...
        if ((The_mission.flags & MISSION_FLAG_NO_PROMOTION) || ((Game_mode &
                                                                 GM_NORMAL) && !(Game_mode & GM_CAMPAIGN_MODE))) {
...
void scoring_add_damage(object *ship_obj, object *other_obj, float damage) {
    ...
    if ((ship_obj->type == OBJ_SHIP) && (use_obj->type == OBJ_SHIP)) {
        ...
        for (idx = 0; idx < MAX_DAMAGE_SLOTS; idx++) {
            ...
            if ((sp->damage_ship_id[idx] == -1) || (ship_get_by_signature(sp->damage_ship_id[idx]) < 0) ||
...
int scoring_eval_kill(object *ship_obj) {
    ...
    if (Game_mode & GM_MULTIPLAYER) {
        ...
        if (net_player_num != -1) {
            ...
            nprintf(("Network", "Setting player %s deaths to %d\n", Net_players[net_player_num].m_player->callsign,
                    Net_players[net_player_num].m_player->stats.m_player_deaths));
    ...
    if ((max_damage_pct >= Kill_percentage) && (dead_ship->damage_ship_id[max_damage_index] >= 0)) {
        ...
        if (plr != NULL) {
            ...
            if ((Ships[Objects[plr->objnum].instance].team == dead_ship->team) &&
                !((Game_mode & GM_MULTIPLAYER) && (Netgame.type_flags & NG_TYPE_DOGFIGHT))) {
                ...
                if ((Game_mode & GM_MULTIPLAYER) && (Netgame.type_flags &
                                                     NG_TYPE_DOGFIGHT) && (multi_find_player_by_object(ship_obj) < 0)) {
                    ...
                    if (net_plr != NULL) {
                        ...
                        if (!(Netgame.type_flags & NG_TYPE_DOGFIGHT) && (Ship_info[dead_ship->ship_info_index].
                                flags & (SIF_BIG_SHIP | SIF_HUGE_SHIP))) {
                            for (idx = 0; idx < MAX_PLAYERS; idx++) {
                                if (MULTI_CONNECTED(Net_players[idx]) && (Net_players[idx].p_info.team == net_plr->
                                        p_info.team) &&
                                    (&Net_players[idx] != net_plr)) {
                                    assist_score = (int) (dead_ship->score * The_mission.ai_profile->
                                            assist_award_percentage_scale[Game_skill_level]);
                        ...
                        if ((Net_player != NULL) && (Net_player->flags & NETINFO_FLAG_AM_MASTER) &&
                            (net_plr != NULL) && (dead_plr != NULL) && (net_plr->m_player != NULL) &&
                            (dead_plr->m_player != NULL)) {
                            char dead_text[1024] = "";

                            sprintf(dead_text, "%s gets the kill for %s", net_plr->m_player->callsign,
                                    dead_plr->m_player->callsign);
...
void scoring_eval_assists(ship *sp, int killer_sig, bool is_enemy_player) {
    ...
    for (idx = 0; idx < MAX_DAMAGE_SLOTS; idx++) {
        ...
        if (((sp->damage_ship[idx] / sp->total_damage_received) >= Assist_percentage) ||
            (The_mission.ai_profile->flags & AIPF_ASSIST_SCORING_SCALES_WITH_DAMAGE)) {
            ...
            if ((plr != NULL) && (iff_x_attacks_y(Ships[Objects[plr->objnum].instance].team,
                                                  sp->team)) && (killer_sig != Objects[plr->objnum].signature)) {

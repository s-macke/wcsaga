...
#define RANK_MEDAL_REGION        0            // region number of the rank medal
#define NUM_MEDAL_REGIONS            MAX_MEDALS + 1                // the extra one is for the rank

const int FIRST_RIBBON = 7;

// coords for indiv medal bitmaps
int Medal_coords[GR_NUM_RESOLUTIONS][NUM_MEDAL_REGIONS][2] =
        {
                {                // GR_640
                        {
                                189,
                        ...
                        },                // allied defense citation
                        {
                                411,
                                234
                        },                // neb campaign victory
                        {
                                494,
                                234
                        },                // ntf campaign victory
                        {
                                283,
                                91
                        },                // wings
                ...
                {                // GR_1024
                        ...
                        },                // Cairo Liberation
                        {
                                463,
                                256
                        },                // Hyperion Service Medal
                        {
                                363,
                                256
                        },                // Freya Campaign Medal
                }
        };
...
ui_button_info Medals_buttons[GR_NUM_RESOLUTIONS][MEDALS_NUM_BUTTONS] =
        {
                { // GR_640
                        ui_button_info("MEB_18", 574, 432, -1, -1, 20),
                },
                { // GR_1024
                        ui_button_info("2_MEB_18", 919, 691, -1, -1, 20),
                }
        };
                ...
                {    // GR_640
                        {
                                ...
                                UI_XSTR_COLOR_PINK,
                                -1,
                                &Medals_buttons[GR_640][MEDALS_EXIT].button
                        },
                },
                {    // GR_1024
                        {
                                ...
                                UI_XSTR_COLOR_PINK,
                                -1,
                                &Medals_buttons[GR_1024][MEDALS_EXIT].button
                        },
                },
        };
...
static MENU_REGION Medal_regions[NUM_MEDAL_REGIONS];
...
void medal_main_init(player *pl, int mode) {
    ...
    for (idx = 0; idx < MEDALS_NUM_BUTTONS; idx++) {
        // create the object
        Medals_buttons[gr_screen.res][idx].button.create(&Medals_window, "", Medals_buttons[gr_screen.res][idx].x,
                                                         Medals_buttons[gr_screen.res][idx].y, 1, 1, 0, 1);
...
void blit_label(char *label, int *coords, int num) {
    ...
    x = (gr_screen.max_w_unscaled - sw) / 2;
...
void GetEarnedRibbons(std::vector <std::pair<medal_stuff, int> > &earnedMedals) {
    ...
    for (size_t i = FIRST_RIBBON; i < Medals.size(); ++i) {
        if (Player_score->medals[i] > 0) {
            earnedMedals.push_back(std::pair<medal_stuff, int>(Medals[i], Player_score->medals[i]));
        }
    }
}
...
int medal_main_do() {
    ...
    region = snazzy_menu_do((ubyte *) Medals_mask->data, Medals_mask_w, Medals_mask_h, NUM_MEDAL_REGIONS,
                            Medal_regions, &selected);
    switch (region) {
        case ESC_PRESSED:
            if (Medals_mode == MM_NORMAL) {
                gameseq_post_event(GS_EVENT_PREVIOUS_STATE);
        ...
        default : {
            uint idxMedal = region - 1;
            bool isRibbon = idxMedal >= FIRST_RIBBON;

            if (isRibbon) {
                size_t numRibbons = std::count_if(Player_score->medals + FIRST_RIBBON,
                                                  Player_score->medals + MAX_MEDALS,
                ...
                if (idxRibbon < numRibbons) {
                    ...
                    blit_label(earnedRibbons[idxRibbon].first.name, &Medal_coords[gr_screen.res][idxMedal][0],
                               earnedRibbons[idxRibbon].second);
                }
            } else if (Player_score->medals[region - 1] > 0) {
                blit_label(Medals[region - 1].name, &Medal_coords[gr_screen.res][region - 1][0],
                           Player_score->medals[region - 1]);
            }
        }
            break;
    } // end switch
    ...
    for (idx = 0; idx < Num_medals; idx++) {
        ...
        if (Player_score->medals[idx] > 0) {
            ...
            if (num_medals > 1) {
                ...
                std::string tmp(base);
                sprintf(base, "%s%c", tmp.c_str(), (num_medals - 2) + 'a');
            }
...
void init_snazzy_regions() {
    ...
    for (idx = 0; idx < NUM_MEDAL_REGIONS; idx++) {
...
void blit_medals() {
    ...
    for (regionIdx = idx = 0; idx < Medals.size(); idx++) {
        if (idx == RANK_MEDAL_REGION) {
            regionIdx++;
        }

        if (Player_score->medals[idx] > 0) {
            Assert(idx < Medal_bitmaps.size());
            gr_set_bitmap(Medal_bitmaps[idx]);
            Assert(regionIdx < NUM_MEDAL_REGIONS);
            gr_bitmap(Medal_coords[gr_screen.res][regionIdx][0], Medal_coords[gr_screen.res][regionIdx][1]);
        }

        if ((idx < FIRST_RIBBON) || (Player_score->medals[idx] > 0)) {
            regionIdx++;
        }
    }

            ...
            if (!ship_override && !debris_override) {
                ...
                if (debris_hit_info.heavy == pship) {
                    ...
                    if (apply_ship_damage) {
                        ship_apply_local_damage(debris_hit_info.heavy, debris_hit_info.light, &hitpos, ship_damage,
                                                quadrant_num, CREATE_SPARKS, debris_hit_info.submodel_num);
                    }
                } else {
                    // don't draw sparks using sphere hit position
                    if (apply_ship_damage) {
                        ship_apply_local_damage(debris_hit_info.light, debris_hit_info.heavy, &hitpos, ship_damage,
                                                MISS_SHIELDS, NO_SPARKS);
                    }
                }
...
int collide_asteroid_ship(obj_pair *pair) {
    ...
    if (dist < pasteroid->radius + pship->radius) {
        ...
        if (hit) {
            ...
            if (!ship_override && !asteroid_override) {
                ...
                float max_ship_impulse = (2.0f * pship->phys_info.max_vel.xyz.z + vm_vec_mag_quick(&asteroid_vel)) *
                                         (pship->phys_info.mass * pasteroid->phys_info.mass) / (pship->phys_info.mass +
                                                                                                pasteroid->phys_info.mass);
                ...
                if ((ship_damage > 500.0f) && (ship_damage > Ships[pship->instance].ship_max_hull_strength / 8.0f)) {
                    ...
                    nprintf(("AI", "Pinning damage to %s from asteroid at %7.3f (%7.3f percent)\n",
                            ...
                            100.0f * ship_damage / Ships[pship->instance].ship_max_hull_strength));
                }
                ...
                asteroid_damage =
                        asteroid_hit_info.impulse / pasteroid->phys_info.mass;    // ie, delta velocity of asteroid
                ...
                if (asteroid_hit_info.heavy == pship) {
                    ...
                    ship_apply_local_damage(asteroid_hit_info.heavy, asteroid_hit_info.light, &hitpos, ship_damage,
                                            quadrant_num, CREATE_SPARKS, asteroid_hit_info.submodel_num);
                    ...
                    ship_apply_local_damage(asteroid_hit_info.light, asteroid_hit_info.heavy, &hitpos, ship_damage,
                                            MISS_SHIELDS, NO_SPARKS);
        ...
        time = 1000.0f * (dist - pship->radius - pasteroid->radius - 10.0f) /

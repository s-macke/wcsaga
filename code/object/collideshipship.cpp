...
void collect_ship_ship_physics_info(object *heavy, object *light, mc_info *mc_info,
...
bool model_precise_collide(mc_info *mc_info, polymodel *pm_light, object *heavy, object *light, float radius1,
                           float radius2) {
    bool collision_found = false;
    float original_radius = mc_info->radius;
    vec3d hitpoint, hitpoint1;
    vec3d heavy_pos = heavy->pos;
    vec3d light_pos = light->pos;
    matrix light_orient = light->orient;
    vec3d min = pm_light->mins;
    vec3d max = pm_light->maxs;

    for (int i = 0; i < 3; ++i) {
        min.a1d[i] = (float) ((int) min.a1d[i]);
        max.a1d[i] = (float) ((int) (max.a1d[i] + 0.5f));
    }

    if (model_collide(mc_info)) {
        vm_vec_add(&hitpoint, &mc_info->hit_point_world, &heavy_pos);
        vm_vec_sub(&hitpoint, &hitpoint, &light_pos);
        vm_vec_unrotate(&hitpoint1, &hitpoint, &light_orient);

        if (fvi_point_inside_boundingbox(&min, &max, &hitpoint1)) {
            collision_found = true;
        }
    }

    mc_info->radius = radius1;
    if (!collision_found && model_collide(mc_info)) {
        vm_vec_add(&hitpoint, &mc_info->hit_point_world, &heavy_pos);
        vm_vec_sub(&hitpoint, &hitpoint, &light_pos);
        vm_vec_unrotate(&hitpoint1, &hitpoint, &light_orient);
    ...
    mc_info->radius = original_radius;
...
int ship_ship_check_collision(collision_info_struct *ship_ship_hit_info, vec3d *hitpos) {
    ...
    mc.model_num = Ship_info[heavy_shipp->ship_info_index].model_num;
    ...
    polymodel *pm_light = model_get(Ship_info[light_shipp->ship_info_index].model_num);
    float rx = fl_abs(pm_light->submodel[pm_light->detail[0]].max.xyz.x - pm_light->submodel[pm_light->detail[0]].min.
            xyz.x);
    float ry = fl_abs(pm_light->submodel[pm_light->detail[0]].max.xyz.y - pm_light->submodel[pm_light->detail[0]].min.
            xyz.y);
    float rz = fl_abs(pm_light->submodel[pm_light->detail[0]].max.xyz.z - pm_light->submodel[pm_light->detail[0]].min.
            xyz.z);
    mc.radius = MAX(rx, MAX(ry, rz)) / 2.0f;
    float radius2 = MIN(rx, MIN(ry, rz)) / 2.0f;
    float radius1 = (mc.radius + radius2) / 2.0f;
    ...
    if (!(heavy_shipp->flags2 & SF2_DONT_COLLIDE_INVIS)) {
        if ((heavy_obj->flags & OF_PLAYER_SHIP) || (light_obj->flags &
                                                    OF_PLAYER_SHIP) || (heavy_shipp->team != light_shipp->team)) {
    ...
    if (model_collide(&mc)) {
        ...
        if (ship_ship_hit_info->collide_rotate) {
            ...
            for (i = 0; i < num_rotating_submodels; i++) {
                ...
                world_find_model_point(&p0, &light_obj->last_pos, pm,
                                       submodel_list[i], &heavy_obj->last_orient, &heavy_obj->last_pos);

                pm->submodel[submodel_list[i]].angs = copy_angles;
                world_find_model_point(&p1, &light_obj->pos, pm, submodel_list[i], &heavy_obj->orient, &heavy_obj->
                        pos);
                ...
                if (model_precise_collide(&mc, pm_light, heavy_obj, light_obj, radius1, radius2)) {
                    if (mc.hit_dist < ship_ship_hit_info->hit_time) {
                        ...
                        model_find_world_point(&ship_ship_hit_info->hit_pos, &mc.hit_point, mc.model_num,
                                               mc.hit_submodel, &heavy_obj->orient, &zero);

                        // set up ship_ship_hit_info for rotating submodel
                        if (ship_ship_hit_info->edge_hit == 0) {
                            model_find_obj_dir(&ship_ship_hit_info->collision_normal, &mc.hit_normal, heavy_obj,
                                               mc.hit_submodel);
                        }
                        ...
                        model_find_world_point(&ship_ship_hit_info->light_collision_cm_pos, &int_light_pos,
                                               mc.model_num, mc.hit_submodel, &heavy_obj->orient, &zero);
                    }
                }
    ...
    if (valid_hit_occured) {
        ...
        if ((collide_obj != NULL) && (Ship_info[Ships[collide_obj->instance].ship_info_index].flags & (SIF_FIGHTER |
                                                                                                       SIF_BOMBER))) {
            char *submode_string = "";
            ai_info *aip;
        ...
        vm_vec_sub(&ship_ship_hit_info->r_light, &ship_ship_hit_info->hit_pos, &ship_ship_hit_info->
        ...
        if (heavy_shipp->team == light_shipp->team) {
            ...
            if ((Ship_info[heavy_shipp->ship_info_index].flags &
                 SIF_SMALL_SHIP) && (Ship_info[light_shipp->ship_info_index].flags & SIF_SMALL_SHIP)) {
                if ((heavy_obj->flags & OF_PLAYER_SHIP) || (light_obj->flags & OF_PLAYER_SHIP)) {
                    ...
                    vm_vec_scale_add2(&heavy_obj->phys_info.vel, &perp_rel_vel, SEP_VEL * light_obj->phys_info.mass /
                                                                                mass_sum);
                    vm_vec_scale_add2(&light_obj->phys_info.vel, &perp_rel_vel, -SEP_VEL * heavy_obj->phys_info.mass /
                                                                                mass_sum);
                ...
                vm_vec_scale_add2(&heavy_obj->phys_info.vel, &ship_ship_hit_info->light_rel_vel,
                                  SEP_VEL * light_obj->phys_info.mass / (mass_sum * rel_vel));
                vm_vec_rotate(&heavy_obj->phys_info.prev_ramp_vel, &heavy_obj->phys_info.vel, &heavy_obj->orient);
                vm_vec_scale_add2(&light_obj->phys_info.vel, &ship_ship_hit_info->light_rel_vel, -SEP_VEL *
                                                                                                 heavy_obj->phys_info.mass /
                                                                                                 (mass_sum * rel_vel));
                vm_vec_rotate(&light_obj->phys_info.prev_ramp_vel, &light_obj->phys_info.vel, &light_obj->orient);
            }
        }
...
void calculate_ship_ship_collision_physics(collision_info_struct *ship_ship_hit_info) {
    ...
    special_cruiser_asteroid_collision = check_special_cruiser_asteroid_collision(heavy,
                                                                                  light, &cruiser_mass, &cruiser_light);
    ...
    vec3d world_rotvel_heavy_m, world_rotvel_light_m, vel_from_rotvel_heavy_m, vel_from_rotvel_light_m, v_rel_m,
            vel_heavy_m, vel_light_m;
    ...
    if (ship_ship_hit_info->collide_rotate) {
        ...
        vm_vec_unrotate(&world_rotvel_heavy_m, &heavy->phys_info.rotvel,
                        &heavy->orient);    // heavy's world rotvel before collision
        vm_vec_crossprod(&vel_from_rotvel_heavy_m, &world_rotvel_heavy_m,
                         &ship_ship_hit_info->r_heavy);    // heavy's velocity from rotvel before collision
    ...
    if (ship_ship_hit_info->submodel_rot_hit == 1) {
        ...
        if (!pm->submodel[ship_ship_hit_info->submodel_num].sii->axis_set) {
            model_init_submodel_axis_pt(pm->submodel[ship_ship_hit_info->submodel_num].sii, pm->id,
    ...
    vm_vec_unrotate(&world_rotvel_light_m, &light->phys_info.rotvel,
                    &light->orient);        // light's world rotvel before collision
    vm_vec_crossprod(&vel_from_rotvel_light_m, &world_rotvel_light_m,
                     &ship_ship_hit_info->r_light);    // light's velocity from rotvel before collision
    ...
    v_rel_normal_m = vm_vec_dotprod(&v_rel_m,
                                    &ship_ship_hit_info->collision_normal);// if less than zero, colliding contact taking place
    ...
    if (heavy == Player_obj) {
        ...
        vm_vec_scale(&delta_rotvel_heavy,
                     float(COLLISION_ROTATION_FACTOR));        // hack decrease rotation (delta_rotvel)
    ...
    if (light == Player_obj) {
        ...
        vm_vec_scale(&delta_rotvel_light,
                     float(COLLISION_ROTATION_FACTOR));        // hack decrease rotation (delta_rotvel)
    ...
    vm_vec_scale_add2(&heavy->pos, &direction_light,
                      0.2f * light->phys_info.mass / (heavy->phys_info.mass + light->phys_info.mass));
    vm_vec_scale_add2(&light->pos, &direction_light, -0.2f * heavy->phys_info.mass / (heavy->phys_info.mass +
                                                                                      light->phys_info.mass));
    vm_vec_scale_add2(&heavy->pos, &ship_ship_hit_info->collision_normal, -0.1f *
                                                                          light->phys_info.mass /
                                                                          (heavy->phys_info.mass +
                                                                           light->phys_info.mass));
    vm_vec_scale_add2(&light->pos, &ship_ship_hit_info->collision_normal,
                      0.1f * heavy->phys_info.mass / (heavy->phys_info.mass + light->phys_info.mass));
...
void mcp_1(object *player_objp, object *planet_objp) {
    ...
    ship_apply_global_damage(player_objp, planet_objp, NULL,
                             PLANET_DAMAGE_SCALE * flFrametime *
                             (float) pow((planet_radius * PLANET_DAMAGE_RANGE) / dist, 3.0f));
    ...
    if (big_class > -1 && Ship_types[big_class].ship_bools & STI_SHIP_WARP_PUSHES) {
        if (small_class > -1 && Ship_types[small_class].ship_bools & STI_SHIP_WARP_PUSHABLE) {
            ...
            if (big_speed > 3 * big_obj->phys_info.max_vel.xyz.z) {
                ...
                vm_vec_scale_add(&perp, &temp, &big_obj->orient.vec.fvec, -vm_vec_dotprod(&temp, &big_obj->orient.vec.
                        fvec));
...
int collide_ship_ship(obj_pair *pair) {
    ...
    if (A->type == OBJ_WAYPOINT)
        return 1;
    if (B->type == OBJ_WAYPOINT)
        return 1;

    Assert(A->type == OBJ_SHIP);
    Assert(B->type == OBJ_SHIP);

    // Do not check collisions of capital ship vs capital ship.
    // This will never happen and it costs performance like crazy.
    ship_info *sip_a = &Ship_info[Ships[A->instance].ship_info_index];
    ship_info *sip_b = &Ship_info[Ships[B->instance].ship_info_index];

    bool check_cap_vs_cap = ((The_mission.ai_profile->flags2 & AIPF2_CAP_VS_CAP_COLLISIONS) != 0);
    bool a_is_cap = ((sip_a->flags & SIF_SMALL_SHIP) == 0);
    bool b_is_cap = ((sip_b->flags & SIF_SMALL_SHIP) == 0);

    if (!check_cap_vs_cap && a_is_cap && b_is_cap)
        return 1;
        ...
        */
        if (hit) {
            ...
            if (!a_override && !b_override) {
                ...
                if ((ship_ship_hit_info.heavy->flags & OF_NO_SHIELDS) || !ship_is_shield_up(ship_ship_hit_info.heavy,
                                                                                            quadrant_num)) {
                    quadrant_num = -1;
                }

                ship_apply_local_damage(ship_ship_hit_info.heavy, ship_ship_hit_info.light, &world_hit_pos,
                                        100.0f * damage / HeavyOne->phys_info.mass, quadrant_num, CREATE_SPARKS,
                                        ship_ship_hit_info.submodel_num, &ship_ship_hit_info.collision_normal);
                ...
                ship_apply_local_damage(ship_ship_hit_info.light, ship_ship_hit_info.heavy, &world_hit_pos, dam2,
                ...
                maybe_push_little_ship_from_fast_big_ship(ship_ship_hit_info.heavy, ship_ship_hit_info.light,

...
int ship_ship_check_collision(collision_info_struct* ship_ship_hit_info, vec3d* hitpos)
{
	...
	if (!Collide_friendly)
	{		// Collide_friendly is a global value changed via debug console
		...
		if (heavy_shipp->team == light_shipp->team)
		{
			...
			else
			{
				...
				vm_vec_scale_add2(&light_obj->phys_info.vel, &ship_ship_hit_info->light_rel_vel, -SEP_VEL *
					heavy_obj->phys_info.mass / (mass_sum * rel_vel));
				vm_vec_rotate(&light_obj->phys_info.prev_ramp_vel, &light_obj->phys_info.vel, &light_obj->orient);
			}
		}
	}
...
void calculate_ship_ship_collision_physics(collision_info_struct* ship_ship_hit_info)
{
	...
	vm_vec_scale_add2(&heavy->pos, &ship_ship_hit_info->collision_normal, -0.1f *
		light->phys_info.mass / (heavy->phys_info.mass + light->phys_info.mass));
...
void mcp_1(object* player_objp, object* planet_objp)
{
	...
	ship_apply_global_damage(player_objp, planet_objp, NULL,
		PLANET_DAMAGE_SCALE * flFrametime * (float)pow((planet_radius * PLANET_DAMAGE_RANGE) / dist, 3.0f));
...
int collide_ship_ship(obj_pair* pair)
{
	...
	//				float	q;
				...
				maybe_push_little_ship_from_fast_big_ship(ship_ship_hit_info.heavy, ship_ship_hit_info.light,
					ship_ship_hit_info.impulse, &ship_ship_hit_info.collision_normal);

...
void obj_add_pair(object* A, object* B, int check_time, int add_to_end)
{
	...
	if (Num_pairs_allocated == 0)
		return;		// don't have anything to add the pair too

	if (A == B)
		return;		// Don't check collisions with yourself

	if (!(A->flags & OF_COLLIDES))
		return;		// This object doesn't collide with anything
	if (!(B->flags & OF_COLLIDES))
		return;		// This object doesn't collide with anything
	...
	if (check_collision == collide_debris_weapon)
	{
		...
		if (!(Weapon_info[Weapons[B->instance].weapon_info_index].wi_flags & WIF_TURNS))
		{
			...
			if (vm_vec_dist_squared(&A->pos, &B->pos) > (vm_vec_mag_squared(&delta_v) * Weapons[B->instance].lifeleft *
														 Weapons[B->instance].lifeleft))
			{
				return;
			}

			// for nonplayer ships, only create collision pair if close enough
			if ((B->parent >= 0) && !(Objects[B->parent].flags &
									  OF_PLAYER_SHIP) && (vm_vec_dist(&B->pos, &A->pos) < (4.0f * A->radius + 200.0f)))
				return;
		}
	}
	...
	if (current_pair)
	{
		...
		else
		{
			...
			if (dist > 0.0f)
			{
				...
				if (time_ms > 100)
				{		// If it takes longer than 1/10th of a second, then delay it

...
#define MAX_OBJ_SOUNDS_PLAYING						32
...
void obj_snd_do_frame()
{
	...
	bool view_is_cockpit = !(Viewer_mode & VM_NOT_COCKPIT);
	bool observer_is_valid = observer_num != -1;

	if (Viewer_obj != NULL)
	{
		observer_obj = Viewer_obj;
	}
	else if (!view_is_cockpit && observer_is_valid)
	{
		// apparently Viewer_obj is still null when Viewing Other Ship externally
		observer_obj = &Objects[observer_num];
		observer_obj->pos = Eye_position;
		observer_obj->orient = Eye_matrix;
	}
	else
	{
	...
	for (osp = GET_FIRST(&obj_snd_list); osp != END_OF_LIST(&obj_snd_list); osp = GET_NEXT(osp))
	{
		Assert(osp != NULL);
		objp = &Objects[osp->objnum];
		if (Player_obj == objp && (observer_obj == Player_obj || view_is_cockpit))
		{
			// we don't play the engine sound if the view is from the player
			int sound_index = -1;
			for (int idx = 0; idx < MAX_OBJECT_SOUNDS; idx++)
			{
			...
			if (osp->ss != NULL)
			{
				if (osp->flags & OS_TURRET_BASE_ROTATION)
				{
					if (osp->ss->base_rotation_rate_pct > 0)
						rot_vol_mult = ((0.25f + (0.75f * osp->ss->base_rotation_rate_pct)) *
										osp->ss->system_info->turret_base_rotation_snd_mult);
					else
						rot_vol_mult = 0;
				}
				if (osp->flags & OS_TURRET_GUN_ROTATION)
				{
					if (osp->ss->gun_rotation_rate_pct > 0)
						rot_vol_mult = ((0.25f + (0.75f * osp->ss->gun_rotation_rate_pct)) *
										osp->ss->system_info->turret_gun_rotation_snd_mult);
					else
						rot_vol_mult = 0;
				}
			}
		...
		if (osp->instance == -1)
		{
			if (distance < Snds[osp->id].max)
			{
				...
				{
					new_vol = max_vol - (distance - Snds[osp->id].min) *
						max_vol / (Snds[osp->id].max - Snds[osp->id].min);
				}
				...
				if (go_ahead_flag)
				{
					if (ds_using_ds3d())
					{
						osp->instance = snd_play_3d(gs, &source_pos, &View_position,
							add_distance, &objp->phys_info.vel, 1, 1.0f, SND_PRIORITY_TRIPLE_INSTANCE);
						if (osp->instance != -1)
						{
							Num_obj_sounds_playing++;
						}
					...
					else
					{
						...
						osp->instance = snd_play_looping(gs, osp->pan, 0, 0, (osp->vol * speed_vol_multiplier *
																			  rot_vol_mult),

...
// go through all windows and try and find the one that matches the search string
BOOL __stdcall os_enum_windows(HWND
hwnd,
char *search_string
)
{
...
{
if (
strstr(tmp, search_string
))
{
Os_debugger_running = 1;        // found the search string!
return
FALSE;    // stop enumerating windows
}
}
...
return
TRUE;    // Say that I've realized my own palette
break;
...
case WM_LBUTTONDOWN:
mouse_mark_button(MOUSE_LEFT_BUTTON,
1);
break;
...
case WM_RBUTTONDOWN:
mouse_mark_button(MOUSE_RIGHT_BUTTON,
1);
break;
...
case WM_MBUTTONDOWN:
mouse_mark_button(MOUSE_MIDDLE_BUTTON,
1);
break;

...
void outwnd_copy_marked_selection(HWND
hwnd);

void outwnd_update_marking(LPARAM l_parm, HWND hwnd);

void outwnd_paint(HWND
hwnd);
...
void load_filter_info(void) {
    ...
    snprintf(pathname, MAX_PATH_LEN - 1, "%s%s%s%s%s%s%s%s%s", detect_home(), DIR_SEPARATOR_STR, Osreg_company_name,
             DIR_SEPARATOR_STR, Osreg_user_dir, DIR_SEPARATOR_STR, Pathtypes[CF_TYPE_DATA].path, DIR_SEPARATOR_STR,
...
void save_filter_info(void) {
    ...
    memset(pathname, 0, sizeof(pathname));
    snprintf(pathname, MAX_PATH_LEN - 1, "%s%s%s%s%s%s%s", detect_home(), DIR_SEPARATOR_STR, Osreg_company_name,
             DIR_SEPARATOR_STR, Osreg_user_dir, DIR_SEPARATOR_STR, Pathtypes[CF_TYPE_DATA].path);
    cf_create_directory(pathname);
...
client.
top = client.bottom - nTextHeight;
InvalidateRect(hOutputWnd, &client,
0);
}
...
z = LOWORD(wParam);
if (z >= ID_FILTER && z < ID_FILTER + (int)OutwndFilter.
...
z -= ID_FILTER;
OutwndFilter[z].
enabled = !OutwndFilter[z].enabled;

if (!
stricmp(OutwndFilter[z]
.name, "error"))
{
OutwndFilter[z].
enabled = 1;
...
if (OutwndFilter[i].enabled)
AppendMenu(h_sub_menu, flags
| MF_CHECKED, ID_FILTER + i, OutwndFilter[i].name);
else
AppendMenu(h_sub_menu, flags,
ID_FILTER + i, OutwndFilter[i].name);
}

AppendMenu(h_menu, MFT_STRING,
ID_COPY, "&Copy\tEnter");
AppendMenu(h_menu, MFT_STRING,
ID_FIND, "&Find Text");
AppendMenu(h_menu, MF_POPUP,
(unsigned int)h_sub_menu, "Filter &Messages");
pt.
x = LOWORD(lParam);
pt.
y = HIWORD(lParam);
...
1);
ScrollWindow(hwnd,
0, (old_vpos - vpos) * nTextHeight, NULL, NULL);
UpdateWindow(hOutputWnd);
//InvalidateRect(hOutputWnd,NULL,0);
}
break;
...
case WM_SIZE:
InvalidateRect(hOutputWnd, NULL,
0);
break;
...
if (x >=
marked_top &&x<marked_bottom
)  // starting in marked area
text_hilight(hdc);
else
text_normal(hdc);
...
if (n ==
marked_top &&n
== marked_bottom)  // special 1 line case
{
if (marked_left)
TextOut(hdc,
0, y, outtext[n], marked_left);

text_hilight(hdc);
x = marked_left * nTextWidth;
TextOut(hdc, x, y, outtext[n]
...
if (marked_right<len)
TextOut(hdc, x, y, outtext[n]
+ marked_right, len - marked_right);

x = len * nTextWidth;
TextOut(hdc, x, y, spaces,
MAX_LINE_WIDTH - len);

}
else if (n == marked_top)
{  // start marked on this line
if (marked_left)
TextOut(hdc,
0, y, outtext[n], marked_left);
...
TextOut(hdc, x, y, outtext[n]
+ marked_left, len - marked_left);

x = len * nTextWidth;
if (marked_left < MAX_LINE_WIDTH)
TextOut(hdc, x, y, spaces,
MAX_LINE_WIDTH - marked_left);

}
else if (n == marked_bottom)
{  // end marked on this line
if (marked_right)
TextOut(hdc,
0, y, outtext[n], marked_right);
...
void outwnd_update_marking(LPARAM l_parm, HWND hwnd) {
    ...
    if (marked != old_marked || marked_top != old_marked_top || marked_bottom !=
                                                                old_marked_bottom || marked_left != old_marked_left ||
        marked_right !=
        old_marked_right) {
        old_marked = marked;
...
void outwnd_init_debug_window(int display_under_freespace_window) {
    ...
    hOutputThread = CreateThread(NULL,
                                 0, (LPTHREAD_START_ROUTINE) outwnd_thread, (LPVOID) display_under_freespace_window, 0,
                                 &OutputThreadID);
...
void outwnd_init(int display_under_freespace_window) {
    ...
    */
    if (Log_fp == NULL) {
        ...
        memset(pathname, 0, sizeof(pathname));
        snprintf(pathname, MAX_PATH_LEN - 1, "%s%s%s%s%s%s%s%s%s", detect_home(), DIR_SEPARATOR_STR,
                 Osreg_company_name, DIR_SEPARATOR_STR, Osreg_user_dir, DIR_SEPARATOR_STR, Pathtypes[CF_TYPE_DATA].path,
                 DIR_SEPARATOR_STR, FreeSpace_logfilename);
...
{
case IDOK:
GetDlgItemText(hwnd, IDC_TEXT, find_text,
82);  // get the text to find
EndDialog(hwnd, IDOK
);
return 1;
...
case WM_INITDIALOG:
SendDlgItemMessage(hwnd, IDC_TEXT, EM_LIMITTEXT,
80, 0);  // limit text to 80 chars
if (
GetDlgCtrlID((HWND)
wParam) != IDC_TEXT)
{

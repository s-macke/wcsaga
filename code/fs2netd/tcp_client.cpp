...
int FS2NetD_GetPlayerData(const char *player_name, player *pl, bool can_create, bool do_send) {
    ...
    if (do_send) {
        ...
        while ((rc_total < (uint) buffer_size) && (rc_total <= sizeof(buffer)) && (timer_get_fixed_seconds() <=
                                                                                   end_time)) {
...
int FS2NetD_ValidateTableList(bool do_send) {
    ...
    if (do_send) {
        ...
        if (num_tables > (int) Table_valid_status.size()) {
            ml_printf("FS2NetD WARNING: Table list contains %i tables, but we only requested %i!  Invalid data!",

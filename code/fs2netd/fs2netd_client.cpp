...
void fs2netd_options_config_init() {
    ...
    if (!strlen(Multi_options_g.game_tracker_ip)) {
        ml_printf("NOTICE: Address for game tracker not specified, using default instead (%s).",
                  FS2NETD_DEFAULT_SERVER);
        ...
        ml_printf("NOTICE: Incompatible game tracker IP detected (gt.pxo.net), using default instead (%s)!",
                  FS2NETD_DEFAULT_SERVER);
        strncpy(Multi_options_g.game_tracker_ip, FS2NETD_DEFAULT_SERVER, MULTI_OPTIONS_STRING_LEN);
    }

    if (!strlen(Multi_options_g.user_tracker_ip)) {
        ml_printf("NOTICE: Address for user tracker not specified, using default instead (%s).",
                  FS2NETD_DEFAULT_SERVER);
        ...
        ml_printf("NOTICE: Incompatible user tracker IP detected (ut.pxo.net), using default instead (%s)!",
                  FS2NETD_DEFAULT_SERVER);
        strncpy(Multi_options_g.user_tracker_ip, FS2NETD_DEFAULT_SERVER, MULTI_OPTIONS_STRING_LEN);
    }

    if (!strlen(Multi_options_g.tracker_port)) {
        ml_printf("NOTICE: Port for game/user trackers not specified, using default instead (%s).",
                  FS2NETD_DEFAULT_PORT);
        ...
        if ((port_tmp < 1024) || (port_tmp > USHRT_MAX)) {
            ml_printf(
                    "NOTICE: The port specified for game/user trackers, '%i', is outside of the required range, %i through %i!",
                    port_tmp, 1024, USHRT_MAX);
            ml_printf("NOTICE: Port for game/user trackers is invalid, using default instead (%s).",
                      FS2NETD_DEFAULT_PORT);
            strncpy(Multi_options_g.tracker_port, FS2NETD_DEFAULT_PORT, STD_NAME_LEN);
        }
    }
    ...
    if (!strlen(Multi_options_g.pxo_ip)) {
        ml_printf("NOTICE: Address for chat server not specified, using default instead (%s).",
                  FS2NETD_DEFAULT_CHAT_SERVER);
        ...
        ml_printf("NOTICE: Incompatible chat server IP detected (chat.pxo.net), using default instead (%s)!",
                  FS2NETD_DEFAULT_CHAT_SERVER);
        strncpy(Multi_options_g.pxo_ip, FS2NETD_DEFAULT_CHAT_SERVER, MULTI_OPTIONS_STRING_LEN);
    }
    ...
    if (!strlen(Multi_options_g.pxo_banner_url)) {
        ...
        ml_printf("NOTICE: Incompatible banner URL detected (chat.pxo.net), using default instead (%s)!",
                  FS2NETD_DEFAULT_BANNER_URL);
        strncpy(Multi_options_g.pxo_banner_url, FS2NETD_DEFAULT_BANNER_URL, MULTI_OPTIONS_STRING_LEN);
    }
...
static int fs2netd_send_player() {
    ...
    if (Is_standalone) {
        do {
            rc = fs2netd_send_player_do();
        } while (!rc);
    } else {
        rc = popup_till_condition(fs2netd_send_player_do, XSTR("&Cancel", 779),
                                  XSTR("Sending player stats requests ...", 676));
    }
...
void fs2netd_store_stats() {
    ...
    if (game_hacked_data()) {
        ...
        popup(PF_USE_AFFIRMATIVE_ICON, 1, POPUP_OK,
              XSTR("You are playing with a hacked tables, your stats will not be saved", -1));
        fs2netd_store_stats_results();
        return;
    }

    if ((multi_num_players() <= 1) && (Multi_num_players_at_start <= 1)) {
        multi_display_chat_msg(XSTR("<Not enough players were present at game start or end, stats will not be saved>",
                                    1048), 0, 0);
    ...
    */
    if (!fs2netd_check_mission(Netgame.mission_name)) {
        multi_display_chat_msg(XSTR("<Server detected a non PXO validated mission. Stats will not be saved>", 1049), 0,
                               0);
        popup(PF_USE_AFFIRMATIVE_ICON, 1, POPUP_OK,
              XSTR("This is not a PXO validated mission, your stats will not be saved", 1050));
        fs2netd_store_stats_results();
        return;
    }
...
void fs2netd_update_ban_list() {
    ...
    if (Is_standalone) {
        do {
            rc = fs2netd_update_ban_list_do();
        } while (!rc);
    } else {
        rc = popup_till_condition(fs2netd_update_ban_list_do, XSTR("&Cancel", 779), XSTR("Requesting IP ban list",
                                                                                         -1));
    }
...
bool fs2netd_get_valid_missions() {
    ...
    if (Is_standalone) {
        do {
            rc = fs2netd_get_valid_missions_do();
        } while (!rc);
    } else {
        rc = popup_till_condition(fs2netd_get_valid_missions_do, XSTR("&Cancel", 779),
                                  XSTR("Starting mission validation", -1));
    }
    ...
    switch (rc) {
        ...
        case 2:
            if (!Is_standalone) {
                popup(PF_USE_AFFIRMATIVE_ICON, 1, POPUP_OK,
                      XSTR("No missions are available from the server for validation!", -1));
            }
...
int fs2netd_update_valid_tables() {
    ...
    if (Is_standalone) {
        do {
            rc = fs2netd_update_valid_tables_do();
        } while (!rc);
    } else {
        rc = popup_till_condition(fs2netd_update_valid_tables_do, XSTR("&Cancel", 779),
                                  XSTR("Starting table validation", -1));
    }
    ...
    switch (rc) {
        ...
        case 2: {
            if (!Is_standalone) {
                popup(PF_USE_AFFIRMATIVE_ICON, 1, POPUP_OK,
                      XSTR("No tables are available from the server for validation!", -1));
            }
    ...
    for (uint i = 0; i < Table_valid_status.size(); i++) {
        if (Table_valid_status[i].valid) {
            ...
            ml_printf("FS2NetD Table Check: '%s' -- INVALID (0x%x)!", Table_valid_status[i].name,
                      Table_valid_status[i].crc32);
            hacked = 1;
        }
    }

    ...
    if (strcmp(Cur_anim_filename, Cur_cmd_brief->stage[stage].ani_filename) != 0) {
        ...
        if (stream_result < 0) {
            ...
            Cur_Anim.first_frame = bm_load(
                    Cur_cmd_brief->stage[stage].ani_filename);    //if we fail here, the value is still -1
...
void cmd_brief_init(int team) {
    ...
    */
    ...
    Cmd_brief_background_bitmap = bm_load(
            Cmd_brief_fname[Uses_scroll_buttons][gr_screen.res]);    // try to load extra one first
...
void cmd_brief_do_frame(float frametime) {
    ...
    gr_string(Cmd_text_wnd_coords[Uses_scroll_buttons][gr_screen.res][CMD_X_COORD] +
              Cmd_text_wnd_coords[Uses_scroll_buttons][gr_screen.res][CMD_W_COORD] - w, Cmd_stage_y[gr_screen.res],
              buf);
    ...
    if ((Cmd_brief_text_max_lines[gr_screen.res] + Top_cmd_brief_text_line) < Num_brief_text_lines[0]) {
        ...
        int more_txt_y = Cmd_text_wnd_coords[Uses_scroll_buttons][gr_screen.res][CMD_Y_COORD] +
                         Cmd_text_wnd_coords[Uses_scroll_buttons][gr_screen.res][CMD_H_COORD] -

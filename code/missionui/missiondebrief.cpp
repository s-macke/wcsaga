...
static ui_button_info Buttons[GR_NUM_RESOLUTIONS][NUM_BUTTONS] =
        {
                ...
                { // GR_1024
                        ...
                        ui_button_info("2_DB_15", 854, 681, 822, 704, 15),        // help
                        ui_button_info("2_DB_16", 861, 728, 800, 743, 16),        // options
...
UI_XSTR Debrief_strings[GR_NUM_RESOLUTIONS][NUM_DEBRIEF_TEXT] =
        {
                { // GR_640
                        ...
                        },
                        {
                                ...
                                UI_XSTR_COLOR_PINK,
                                -1,
                                &Buttons[0][MULTI_PINFO_POPUP].button
                        },
                        {
                                "Info",
                                1311,
                ...
                { // GR_1024
                        ...
                        },
                        {
                                ...
                                UI_XSTR_COLOR_PINK,
                                -1,
                                &Buttons[1][MULTI_PINFO_POPUP].button
                        },
                        {
                                "Info",
                                1311,
...
void debrief_ui_init() {
    ...
    if (Game_mode & GM_MULTIPLAYER) {
        ...
        List_region.create(&Debrief_ui_window, "", Debrief_list_coords[gr_screen.res][0],
                           Debrief_list_coords[gr_screen.res][1], Debrief_list_coords[gr_screen.res][2],
                           Debrief_list_coords[gr_screen.res][3], 0, 1);
        List_region.hide();
        ...
        for (i = 0; i < NUM_VOLITION_CAMPAIGNS; i++) {
            if (!stricmp(Campaign.filename, Volition_campaigns[i].campaign_name)) {
                ...
                for (j = 0; j < Volition_campaigns[i].num_missions; j++) {
                    // found it!
                    if (!stricmp(Campaign.missions[Campaign.current_mission].name,
...
void debrief_award_init() {
    ...
    if (Player->stats.m_medal_earned != -1) {
        sprintf(buf, NOX("%s%.2d"), Debrief_award_filename[gr_screen.res][DB_AWARD_MEDAL],
    ...
    if ((Rank_bitmap >= 0) || (Medal_bitmap >= 0) || (Badge_bitmap >= 0) || (Wings_bitmap >= 0) ||
        (Crest_bitmap >= 0)) {
        Award_active = 1;
    } else {
        Award_active = 0;
...
void debrief_multi_list_draw() {
    ...
    while (y + font_height <= Debrief_list_coords[gr_screen.res][3]) {
        ...
        if (!((np->flags & NETINFO_FLAG_OBSERVER) && !(np->flags & NETINFO_FLAG_OBS_PLAYER))) {
            if (Netgame.type_flags & NG_TYPE_TEAM) {
                ...
                if (np->p_info.team == 0) {
                    ...
                    if (((np->state == NETPLAYER_STATE_DEBRIEF_ACCEPT) || (np->state ==
                                                                           NETPLAYER_STATE_DEBRIEF_REPLAY)) &&
                        (Multi_common_icons[MICON_TEAM0_SELECT] != -1)) {
                        gr_set_bitmap(Multi_common_icons[MICON_TEAM0_SELECT]);
                        gr_bitmap(Debrief_list_coords[gr_screen.res][0], Debrief_list_coords[gr_screen.res][1] + y -
                                                                         2);
                    }
                    ...
                    if (((np->state == NETPLAYER_STATE_DEBRIEF_ACCEPT) || (np->state ==
                                                                           NETPLAYER_STATE_DEBRIEF_REPLAY)) &&
                        (Multi_common_icons[MICON_TEAM1_SELECT] != -1)) {
                ...
                if (((np->state == NETPLAYER_STATE_DEBRIEF_ACCEPT) || (np->state == NETPLAYER_STATE_DEBRIEF_REPLAY)) &&
                    (Multi_common_icons[MICON_TEAM0_SELECT] != -1)) {
                    gr_set_bitmap(Multi_common_icons[MICON_TEAM0_SELECT]);
        ...
        if (Net_players[Multi_list[z].net_player_index].flags & NETINFO_FLAG_OBSERVER &&
            !(Net_players[Multi_list[z].net_player_index].flags & NETINFO_FLAG_OBS_PLAYER)) {
            strcat_s(str, XSTR("(O)", 438));
        }

        // bli
        gr_string(Debrief_list_coords[gr_screen.res][0] + MULTI_LIST_TEAM_OFFSET,
                  Debrief_list_coords[gr_screen.res][1] + y, str);
...
void debrief_accept(int ok_to_post_start_game_event) {
    ...
    if ((/*Cheats_enabled ||*/Turned_traitor || Must_replay_mission) && (Game_mode & GM_CAMPAIGN_MODE)) {
        ...
        z = popup(0, 3, XSTR("Return to &Debriefing", 442), XSTR("Go to &Flight Deck", 443), XSTR("&Replay Mission",
                                                                                                  444), str);
        if (z == 2) {
    ...
    if (Game_mode & GM_MULTIPLAYER) {
        ...
        if (Game_mode & GM_CAMPAIGN_MODE) {
            ...
            if (!(Game_mode & GM_MULTIPLAYER)) {
                ...
                if ((Campaign.missions[cur].has_mission_loop && (Campaign.loop_mission != -1)) &&
...
void debrief_render() {
    ...
    gr_set_clip(Debrief_text_wnd_coords[gr_screen.res][0], Debrief_text_wnd_coords[gr_screen.res][1],
                Debrief_text_wnd_coords[gr_screen.res][2], Debrief_text_wnd_coords[gr_screen.res][3]);
    y = 0;
    z = Text_offset;
...
void debrief_replay_pressed() {
    if (!Turned_traitor && !Must_replay_mission && (Game_mode & GM_CAMPAIGN_MODE)) {
        int choice;
        choice = popup(0, 2, POPUP_CANCEL, XSTR("&Replay", 451),
                       XSTR(
...
void debrief_setup_ship_kill_stats(int stage_num) {
    ...
    for (i = 0; i < MAX_SHIP_CLASSES; i++) {
        ...
        std::string ship_class(buffer);
        std::vector<Kill_pair>::iterator kill_pair = std::find_if(kills.begin(), kills.end(),
                                                                  KillClassEqual(ship_class));

        if (kill_pair == kills.end()) {
            kills.push_back(std::make_pair(ship_class, kill_arr[i]));
        } else {
            kill_pair->second += kill_arr[i];
        }
    }
    ...
    for (std::vector<Kill_pair>::const_iterator kill = kills.begin(); kill != kills.end(); ++kill) {
        ...
        kill_info->num = kill->second;

        strcpy(kill_info->text, kill->first.c_str());
        ...
        if (multi_debrief_stats_accept_code() != 1) {
            if (MULTIPLAYER_MASTER) {
                for (i = 0; i < MAX_PLAYERS; i++) {
                    if (MULTI_CONNECTED(Net_players[i]) && !MULTI_STANDALONE(Net_players[i]) &&
                        !MULTI_PERM_OBSERVER(Net_players[i]) && (Net_players[i].m_player != NULL)) {
        ...
        case KEY_ESC: {
            ...
            if (Game_mode & GM_MULTIPLAYER) {
                ...
                if (!Turned_traitor && !Must_replay_mission && (Game_mode & GM_CAMPAIGN_MODE)) {
                    ...
                    choice = popup(pf_flags, 3, POPUP_CANCEL, XSTR("&Yes", 454), XSTR("&No, retry later", 455),
                    ...
                    choice = popup(0, 2, POPUP_NO, POPUP_YES,
                                   XSTR(
...
void debrief_draw_award_text() {
    ...
    int field_width = (Medal_bitmap > 0) ? Debrief_award_text_width[gr_screen.res][DB_WITH_MEDAL] :
                      Debrief_award_text_width[gr_screen.res][DB_WITHOUT_MEDAL];

    // vertically centered within field
    start_y = Debrief_award_text_coords[gr_screen.res][1] + ((Debrief_award_text_coords[gr_screen.res][2] - (fh *
    ...
    for (i = 0; i < Debrief_award_text_num_lines; i++) {
        ...
        x = (Medal_bitmap < 0) ? (Debrief_award_text_coords[gr_screen.res][0] + (field_width - sw) /
                                                                                2)
...
void debrief_do_frame(float frametime) {
    ...
    gr_set_color_fast(&Color_normal);
    gr_printf(Debrief_title_coords[gr_screen.res][0], Debrief_title_coords[gr_screen.res][1] - 10,
    ...
    switch (Current_mode) {
        case DEBRIEF_TAB:
            if (Num_debrief_stages <= 0) {
                ...
                gr_printf(Debrief_text_wnd_coords[gr_screen.res][0], Debrief_text_wnd_coords[gr_screen.res][1],
                          XSTR("No Debriefing for mission: %s", 458), Game_current_mission_filename);
    ...
    if (Text_offset + Debrief_text_wnd_coords[gr_screen.res][3] / gr_get_font_height() < Num_text_lines) {
        ...
        gr_printf(Debrief_text_wnd_coords[gr_screen.res][0] + Debrief_text_wnd_coords[gr_screen.res][2] / 2 - w / 2,
                  Debrief_text_wnd_coords[gr_screen.res][1] + Debrief_text_wnd_coords[gr_screen.res][3],
                  XSTR("More", 459));
    }
    ...
    if (Game_mode & GM_CAMPAIGN_MODE) {
        if ((Campaign.current_mission >= 0) && (Campaign.current_mission < MAX_CAMPAIGN_MISSIONS)) {
            if ((Campaign.missions[Campaign.current_mission].name != NULL) && !stricmp(Campaign.missions[Campaign.
                    current_mission].name, "sm3-09.fs2")) {
    ...
    if (Must_replay_mission && (!Debrief_skip_popup_already_shown) && (Player->show_skip_popup) && (Game_mode &
                                                                                                    GM_NORMAL) &&
        (Game_mode & GM_CAMPAIGN_MODE) && (Player->failures_this_session >= PLAYER_MISSION_FAILURE_LIMIT) &&
        ...
        int popup_choice = popup(0, 3, XSTR("Do Not Skip This Mission", 1473),
                                 ...
                                 XSTR("You have failed this mission five times.  If you like, you may advance to the next mission.",
                                      1472));
        switch (popup_choice) {
            case 0:
...
void debrief_rebuild_player_list() {
    ...
    for (i = 0; i < MAX_PLAYERS; i++) {
        ...
        if (MULTI_CONNECTED((*np)) && !MULTI_STANDALONE((*np))) {
            ...
            gr_force_fit_string(list->callsign, CALLSIGN_LEN - 1, Debrief_list_coords[gr_screen.res][2] -
                                                                  MULTI_LIST_TEAM_OFFSET);
        }
    } // end for
}

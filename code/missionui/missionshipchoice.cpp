...
static int Ship_anim_coords[GR_NUM_RESOLUTIONS][4] =
        {
                {
                        259,
                        81,
                ...
                { // GR_640
                        {
                                ...
                                UI_XSTR_COLOR_PINK,
                                -1,
                                &Ship_select_buttons[0][SS_BUTTON_RESET].button
                        }
                },
                { // GR_1024
                        {
                                ...
                                UI_XSTR_COLOR_PINK,
                                -1,
                                &Ship_select_buttons[1][SS_BUTTON_RESET].button
                        }
                }
        };
...
char *ss_tooltip_handler(char *str) {
    ...
    } else if (!stricmp(str, NOX("@ship_velocity"))) {
        return Ship_info[Selected_ss_class].velocity_str;

    } else if (!stricmp(str, NOX("@ship_maneuverability"))) {
        return Ship_info[Selected_ss_class].maneuverability_str;

    } else if (!stricmp(str, NOX("@ship_shields"))) {
        return Ship_info[Selected_ss_class].shields_str;
...
void ship_select_init() {
    ...
    if (Game_mode & GM_MULTIPLAYER) {
        ...
        maybe_change_selected_wing_ship(Net_player->p_info.ship_index / MAX_WING_SLOTS,
    ...
    if (Ship_select_open) {
        ...
        common_buttons_maybe_reload(
                &Ship_select_ui_window);    // AL 11-21-97: this is necessary since we may returning from the hotkey
...
//
void do_mouse_over_wing_slot(int block, int slot) {
...
void ship_select_blit_ship_info() {
    ...
    ship_info *sip;
    char str[100];
    color *header = &Color_text_heading;
    color *text = &Color_text_normal;
    ...
    if ((sip->velocity_str != NULL) && strlen(sip->velocity_str)) {
        gr_string(Ship_info_coords[gr_screen.res][SHIP_SELECT_X_COORD] + 4, y_start, sip->velocity_str);
    } else {
        sprintf(str, XSTR("%d kps", 743), (int) ((sip->max_vel.xyz.z * Hud_unit_multiplier) + 0.5f));
        gr_string(Ship_info_coords[gr_screen.res][SHIP_SELECT_X_COORD] + 4, y_start, str);
    }
    y_start += 10;
    ...
    // blit the shields
    gr_set_color_fast(header);
    gr_string(Ship_info_coords[gr_screen.res][SHIP_SELECT_X_COORD], y_start, XSTR("Shields", -1));
    y_start += 10;
    gr_set_color_fast(text);
    if ((sip->shields_str != NULL) && strlen(sip->shields_str)) {
        gr_string(Ship_info_coords[gr_screen.res][SHIP_SELECT_X_COORD] + 4, y_start, sip->shields_str);
    } else {
    ...
    if ((sip->turrets_str != NULL) && strlen(sip->turrets_str)) {
        ...
        y_start += 10;
        gr_set_color_fast(text);
        gr_string(Ship_info_coords[gr_screen.res][SHIP_SELECT_X_COORD] + 4, y_start, sip->turrets_str);
        y_start += 10;
    } else {
    ...
    if ((sip->manufacturer_str != NULL) && strlen(sip->manufacturer_str)) {
        ...
        gr_string(Ship_info_coords[gr_screen.res][SHIP_SELECT_X_COORD] + 4, y_start,
...
void ship_select_do(float frametime) {
    ...
    ship_select_choice = snazzy_menu_do(ShipSelectMaskData, Shipselect_mask_w, Shipselect_mask_h, Num_mask_regions,
                                        Region, &snazzy_action, 0);
    ...
    if (ship_select_choice > -1) {
        ...
        switch (ship_select_choice) {
            ...
            case SHIP_SELECT_ICON_3:
                ...
                break;
            case WING_0_SHIP_0:
                do_mouse_over_wing_slot(0, 0);
                break;
            case WING_0_SHIP_1:
                do_mouse_over_wing_slot(0, 1);
                break;
            case WING_0_SHIP_2:
                do_mouse_over_wing_slot(0, 2);
                break;
            case WING_0_SHIP_3:
                do_mouse_over_wing_slot(0, 3);
                break;
            case WING_1_SHIP_0:
                do_mouse_over_wing_slot(1, 0);
                break;
            case WING_1_SHIP_1:
                do_mouse_over_wing_slot(1, 1);
                break;
            case WING_1_SHIP_2:
                do_mouse_over_wing_slot(1, 2);
                break;
            case WING_1_SHIP_3:
                do_mouse_over_wing_slot(1, 3);
                break;
            case WING_2_SHIP_0:
                do_mouse_over_wing_slot(2, 0);
                break;
            case WING_2_SHIP_1:
                do_mouse_over_wing_slot(2, 1);
                break;
            case WING_2_SHIP_2:
                do_mouse_over_wing_slot(2, 2);
                break;
            case WING_2_SHIP_3:
                do_mouse_over_wing_slot(2, 3);
                break;
    ...
    if (!Cmdline_ship_choice_3d) {
        if ((Selected_ss_class >= 0) && (Ss_icons[Selected_ss_class].ss_anim.num_frames > 0)) {
            generic_anim_render(&Ss_icons[Selected_ss_class].ss_anim, (help_overlay_active(SS_OVERLAY)) ? 0 :
                                                                      frametime, Ship_anim_coords[gr_screen.res][0],
    ...
    if (ss_icon_being_carried()) {
        ...
        if (Ss_icons[Carried_ss_icon.ship_class].icon_bmaps[ICON_FRAME_SELECTED] != -1) {
            ...
            if (Ss_icons[Carried_ss_icon.ship_class].model_index == -1) {
                Ss_icons[Carried_ss_icon.ship_class].model_index = model_load(sip->pof_file,
                                                                              sip->n_subsystems, &sip->subsystems[0]);
            ...
            if (Ss_icons[Carried_ss_icon.ship_class].model_index != -1) {
                draw_model_icon(Ss_icons[Carried_ss_icon.ship_class].model_index, MR_LOCK_DETAIL | MR_AUTOCENTER |
                                                                                  MR_NO_FOGGING | MR_NO_LIGHTING,
                                sip->closeup_zoom / 1.27f, sx, sy, w, h, sip);
            }
    ...
    //////////////////////////////////
    if (Cmdline_ship_choice_3d || ((Ss_icons != NULL) && (Selected_ss_class >= 0) &&
                                   (Ss_icons[Selected_ss_class].ss_anim.num_frames > 0))) {
...
void draw_ship_icon_with_number(int screen_offset, int ship_class) {
    ...
    if (ss_icon->current_icon_bitmap != -1) {
        ...
        gr_bitmap(Ship_list_coords[gr_screen.res][screen_offset][0],
        ...
        if (ss_icon->model_index != -1) {
            draw_model_icon(ss_icon->model_index, MR_LOCK_DETAIL | MR_AUTOCENTER | MR_NO_FOGGING | MR_NO_LIGHTING,
                            sip->closeup_zoom / 1.27f, Ship_list_coords[gr_screen.res][screen_offset][0],
                            Ship_list_coords[gr_screen.res][screen_offset][1], 32, 28, sip);
        }
        draw_brackets_square(Ship_list_coords[gr_screen.res][screen_offset][0],
                             Ship_list_coords[gr_screen.res][screen_offset][1],
                             Ship_list_coords[gr_screen.res][screen_offset][0] + 32,
                             Ship_list_coords[gr_screen.res][screen_offset][1] + 28);
...
void start_ship_animation(int ship_class, int play_sound) {
    ...
    if (Cmdline_ship_choice_3d || !strlen(sip->anim_filename)) {
        ...
        if (generic_anim_stream(&Ss_icons[ship_class].ss_anim) == -1) {
            ...
            Ss_icons[ship_class].ss_anim.first_frame = bm_load(
                    Ship_info[ship_class].anim_filename);    //if we fail here, the value is still -1
...
void draw_wing_block(int wb_num, int hot_slot, int selected_slot, int class_select) {
    ...
    for (i = 0; i < MAX_WING_SLOTS; i++) {
        ...
        if (bitmap_to_draw != -1) {
            ...
            gr_set_color_fast(color_to_draw);
            //gr_set_shader(shader_to_use);
            draw_model_icon(icon->model_index, MR_LOCK_DETAIL | MR_AUTOCENTER | MR_NO_FOGGING | MR_NO_LIGHTING,
                            sip->closeup_zoom / 1.27f, Wing_icon_coords[gr_screen.res][slot_index][0],
                            Wing_icon_coords[gr_screen.res][slot_index][1], 32, 28, sip);
            draw_brackets_square(Wing_icon_coords[gr_screen.res][slot_index][0],
                                 Wing_icon_coords[gr_screen.res][slot_index][1],
                                 Wing_icon_coords[gr_screen.res][slot_index][0] + 32,
                                 Wing_icon_coords[gr_screen.res][slot_index][1] + 28);
    ...
    if (ship_class == -1) {
        ...
        if (icon->icon_bmaps[bmap_num] != -1) {
            ...
            if (icon->model_index != -1) {
                ...
                draw_model_icon(icon->model_index, MR_LOCK_DETAIL | MR_AUTOCENTER | MR_NO_FOGGING | MR_NO_LIGHTING,
                                sip->closeup_zoom / 1.27f, x, y, 32, 28, sip);
            ...
            switch (ws->status) {
                ...
                case WING_SLOT_FILLED | WING_SLOT_IS_PLAYER | WING_SLOT_LOCKED:
                    ...
                    if (ws->status & WING_SLOT_IS_PLAYER) {
                        ...
                        if (wl_update_ship_weapons(Ships[Player_obj->instance].objnum, &Wss_slots[i * MAX_WING_SLOTS +
                                                                                                  j]) == -1) {
                            popup(PF_USE_AFFIRMATIVE_ICON, 1, POPUP_OK, XSTR("Player ship has no weapons", 461));
                            return -1;
                        }
                ...
                case WING_SLOT_EMPTY | WING_SLOT_IS_PLAYER:
                    if (ws->status & WING_SLOT_IS_PLAYER) {
                        popup(PF_USE_AFFIRMATIVE_ICON, 1, POPUP_OK, XSTR("Player %s must select a place in player wing",
                                                                         462), Player->callsign);
                        return -1;
                    }
                    break;
            ...
            switch (ws->status) {
                case WING_SLOT_EMPTY:
                    ...
                    if (wb->is_late) {
                        ...
                        hud_set_wingman_status_none(Ships[shipnum].wing_status_wing_index,
                                                    Ships[shipnum].wing_status_wing_pos);
                    }
                    break;
...
void update_player_ship(int si_index) {
    ...
    if (Player->last_ship_flown_si_index != si_index) {
        hud_force_reload_ship_gauges();
    }

...
brief_common_buttons Common_buttons[3][GR_NUM_RESOLUTIONS][NUM_COMMON_BUTTONS] =
{
	{	// UGH
		...
		{ // GR_1024			
			...
			brief_common_buttons("2_CB_05", 914, 681, 949, 671, 5),
...
void common_show_no_ship_error()
{
	popup(PF_USE_AFFIRMATIVE_ICON, 1, POPUP_OK,
		XSTR("At least one ship must be selected before proceeding to weapons loadout", 460));
}
...
void common_buttons_init(UI_WINDOW* ui_window)
{
	...
	{
		b = &Common_buttons[Current_screen - 1][gr_screen.res][i].button;
		b->create(ui_window, "", Common_buttons[Current_screen - 1][gr_screen.res][i].x,
			Common_buttons[Current_screen - 1][gr_screen.res][i].y, 60, 30, 0, 1);
	...
	// add some text	
	ui_window->add_XSTR("Briefing", 1504, Common_buttons[Current_screen - 1][gr_screen.res][COMMON_BRIEFING_BUTTON].xt,
		Common_buttons[Current_screen - 1][gr_screen.res][COMMON_BRIEFING_BUTTON].yt, &Common_buttons[Current_screen -
		1][gr_screen.res][COMMON_BRIEFING_BUTTON].button, UI_XSTR_COLOR_PINK);
	ui_window->add_XSTR("Ship Selection", 1067, Common_buttons[Current_screen - 1][gr_screen.res][COMMON_SS_BUTTON].xt,
		Common_buttons[Current_screen - 1][gr_screen.res][COMMON_SS_BUTTON].yt, &Common_buttons[Current_screen -
		1][gr_screen.res][COMMON_SS_BUTTON].button, UI_XSTR_COLOR_PINK);
	ui_window->add_XSTR("Weapon Loadout", 1068, Common_buttons[Current_screen -
		1][gr_screen.res][COMMON_WEAPON_BUTTON].xt, Common_buttons[Current_screen -
		1][gr_screen.res][COMMON_WEAPON_BUTTON].yt, &Common_buttons[Current_screen -
		1][gr_screen.res][COMMON_WEAPON_BUTTON].button, UI_XSTR_COLOR_PINK);
	ui_window->add_XSTR("Commit", 1062, Common_buttons[Current_screen - 1][gr_screen.res][COMMON_COMMIT_BUTTON].xt,
		Common_buttons[Current_screen - 1][gr_screen.res][COMMON_COMMIT_BUTTON].yt, &Common_buttons[Current_screen -
		1][gr_screen.res][COMMON_COMMIT_BUTTON].button, UI_XSTR_COLOR_PINK);
	ui_window->add_XSTR("Help", 928, Common_buttons[Current_screen - 1][gr_screen.res][COMMON_HELP_BUTTON].xt,
		Common_buttons[Current_screen - 1][gr_screen.res][COMMON_HELP_BUTTON].yt, &Common_buttons[Current_screen -
		1][gr_screen.res][COMMON_HELP_BUTTON].button, UI_XSTR_COLOR_PINK);
	ui_window->add_XSTR("Options", 1036, Common_buttons[Current_screen - 1][gr_screen.res][COMMON_OPTIONS_BUTTON].xt,
		Common_buttons[Current_screen - 1][gr_screen.res][COMMON_OPTIONS_BUTTON].yt, &Common_buttons[Current_screen -
		1][gr_screen.res][COMMON_OPTIONS_BUTTON].button, UI_XSTR_COLOR_PINK);
...
void common_maybe_play_cutscene(int movie_type)
{
	for (uint i = 0; i < The_mission.cutscenes.size(); i++)
	{
		if (movie_type == The_mission.cutscenes[i].type)
		{
			...
			bool is_campaign_mode = ((Game_mode & GM_CAMPAIGN_MODE) != 0);

			if (!is_campaign_mode && The_mission.cutscenes[i].is_campaign_only)
				continue;
...
{
	static char buf[MAX_FILENAME_LEN + 1] =
	{
		0
	};
...
void common_reset_buttons()
{
	...
	case ON_BRIEFING_SELECT:
		Common_buttons[Current_screen - 1][gr_screen.res][COMMON_BRIEFING_REGION].button.
			skip_first_highlight_callback();
		break;
	case ON_SHIP_SELECT:
...
void draw_model_rotating(int model_id, int x1, int y1, int x2, int y2, float* rotation_buffer, vec3d* closeup_pos,
						 float closeup_zoom, float rev_rate, int flags, bool resize)
{

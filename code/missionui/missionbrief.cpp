                ...
                { // GR_640
                        {
                                ...
                                UI_XSTR_COLOR_PINK,
                                -1,
                                &Brief_buttons[0][BRIEF_BUTTON_MULTI_LOCK].button
                        },
                        {
                                "Skip Training",
                                1442,
                                ...
                                UI_XSTR_COLOR_PINK,
                                -1,
                                &Brief_buttons[0][BRIEF_BUTTON_SKIP_TRAINING].button
                        },
                        {
                                "Exit Loop",
                                1477,
                                ...
                                UI_XSTR_COLOR_PINK,
                                -1,
                                &Brief_buttons[0][BRIEF_BUTTON_EXIT_LOOP].button
                        }
                },
                { // GR_1024
                        {
                                ...
                                UI_XSTR_COLOR_PINK,
                                -1,
                                &Brief_buttons[1][BRIEF_BUTTON_MULTI_LOCK].button
                        },
                        {
                                "Skip Training",
                                1442,
                                ...
                                UI_XSTR_COLOR_PINK,
                                -1,
                                &Brief_buttons[1][BRIEF_BUTTON_SKIP_TRAINING].button
                        },
                        {
                                "Exit Loop",
                                1477,
                                ...
                                UI_XSTR_COLOR_PINK,
                                -1,
                                &Brief_buttons[1][BRIEF_BUTTON_EXIT_LOOP].button
                        }
                }
        };
...
void brief_skip_training_pressed() {
    ...
    val = popup(PF_USE_NEGATIVE_ICON | PF_USE_AFFIRMATIVE_ICON, 2, POPUP_NO, POPUP_YES,
...
void brief_exit_loop_pressed() {
    int val = popup(PF_USE_NEGATIVE_ICON | PF_USE_AFFIRMATIVE_ICON, 2, POPUP_NO, POPUP_YES,
...
void brief_buttons_init() {
    ...
    for (i = 0; i < NUM_BRIEF_BUTTONS; i++) {
        b = &Brief_buttons[gr_screen.res][i].button;
        b->create(&Brief_ui_window, "", Brief_buttons[gr_screen.res][i].x, Brief_buttons[gr_screen.res][i].y, 60, 30,
                  0, 1);
    ...
    Closeup_close_button.create(&Brief_ui_window, "", Closeup_button_coords[gr_screen.res][BRIEF_X_COORD],
                                Closeup_button_coords[gr_screen.res][BRIEF_Y_COORD], 60, 30, 0, 1);
...
void brief_init() {
    ...
    if (Brief_inited == TRUE) {
        common_buttons_maybe_reload(
                &Brief_ui_window);    // AL 11-21-97: this is necessary since we may returning from the hotkey
...
#ifndef FS2_DEMO
    hud_anim_init(&Fade_anim, Brief_static_coords[gr_screen.res][0], Brief_static_coords[gr_screen.res][1],
    ...
    ML_objectives_init(Brief_goals_coords[gr_screen.res][BRIEF_X_COORD],
                       Brief_goals_coords[gr_screen.res][BRIEF_Y_COORD],
                       Brief_goals_coords[gr_screen.res][BRIEF_W_COORD],
                       Brief_goals_coords[gr_screen.res][BRIEF_H_COORD]);

    // set the camera target
    if (Briefing->num_stages > 0) {
        brief_set_new_stage(&Briefing->stages[0].camera_pos, &Briefing->stages[0].camera_orient, 0,
                            Current_brief_stage);
...
void brief_render(float frametime) {
    ...
    if (Briefing->num_stages > 0) {
        z = brief_render_text(Top_brief_text_line, Brief_text_coords[gr_screen.res][0],
                              Brief_text_coords[gr_screen.res][1], Brief_text_coords[gr_screen.res][3], frametime);
        if (z) {
        ...
        if ((Brief_text_max_lines[gr_screen.res] + Top_brief_text_line) < Num_brief_text_lines[0]) {
            ...
            int more_txt_y = Brief_text_coords[gr_screen.res][1] + Brief_text_coords[gr_screen.res][3] -
                             2;                // located below brief text, centered
    ...
    int title_x_offset = 28;
    gr_printf(Brief_bmap_coords[gr_screen.res][0] + title_x_offset, Brief_bmap_coords[gr_screen.res][1] -
                                                                    title_y_offset, NOX("[name: %s, mod: %s]"),
              Mission_filename, The_mission.modified);
#endif
...
void brief_do_frame(float frametime) {
    ...
    int snazzy_action = -1;
    brief_choice = snazzy_menu_do(Brief_ui_mask_data, Brief_ui_mask_w, Brief_ui_mask_h, Num_briefing_regions,
                                  Briefing_select_region, &snazzy_action, 0);
...
#ifndef NDEBUG
    if (cam_change) {
        nprintf(("General", "Camera pos: %.2f, %.2f %.2f // ", Closeup_cam_pos.xyz.x, Closeup_cam_pos.xyz.y,

...
static MENU_REGION Weapon_select_region[NUM_WEAPON_SELECT_REGIONS];    // see initialization
...
static int Wl_bank_count_draw_flags[MAX_SHIP_WEAPONS] =
        {
                0,
                0,
                0,            // primaries -- don't draw counts
                1,
                1,
                1,
                1,
                1,
                1    // secondaries -- do draw counts
        };
                ...
                { // GR_640
                        {
                                ...
                                UI_XSTR_COLOR_PINK,
                                -1,
                                &Buttons[0][WL_BUTTON_RESET].button
                        },
                        {
                                "Lock",
                                1270,
                                ...
                                UI_XSTR_COLOR_PINK,
                                -1,
                                &Buttons[0][WL_BUTTON_MULTI_LOCK].button
                        }
                },
                { // GR_1024
                        {
                                ...
                                UI_XSTR_COLOR_PINK,
                                -1,
                                &Buttons[1][WL_BUTTON_RESET].button
                        },
                        {
                                "Lock",
                                1270,
                                ...
                                UI_XSTR_COLOR_PINK,
                                -1,
                                &Buttons[1][WL_BUTTON_MULTI_LOCK].button
                        }
                }
        };
...
void weapon_buttons_init() {
    ...
    for (i = 0; i < NUM_WEAPON_BUTTONS; i++) {
        b = &Buttons[gr_screen.res][i];
        b->button.create(&Weapon_ui_window, "", Buttons[gr_screen.res][i].x, Buttons[gr_screen.res][i].y, 60, 30,
                         Weapon_button_scrollable[i]);
...
void wl_render_overhead_view(float frametime) {
    ...
    if (display_type == 0 && wl_ship->overhead_bitmap > -1) {
        ...
        if (wl_ship->model_num < 0) {
            ...
            gr_set_clip(Wl_overhead_coords[gr_screen.res][0], Wl_overhead_coords[gr_screen.res][1],
                        gr_screen.res == 0 ? 291 : 467, gr_screen.res == 0 ? 226 : 362);
            g3_start_frame(1);
            ...
            model_render(wl_ship->model_num, &object_orient, &vmd_zero_vector, MR_LOCK_DETAIL | MR_AUTOCENTER |
                                                                               MR_NO_FOGGING, -1, -1);
            ...
            for (x = 0; x < pm->n_guns; x++) {
                if ((Wss_slots[Selected_wl_slot].wep[x] == Selected_wl_class && Hot_weapon_bank < 0) ||
                    x == Hot_weapon_bank) {
                    ...
                    for (y = 0; y < pm->gun_banks[x].num_slots; y++) {
                        ...
                        gr_line(Wl_bank_coords[gr_screen.res][x][0] + 106, Wl_bank_coords[gr_screen.res][x][1] + 12,
                                xc - 4, Wl_bank_coords[gr_screen.res][x][1] + 12);
            ...
            for (x = 0; x < pm->n_missiles; x++) {
                if ((Wss_slots[Selected_wl_slot].wep[x + MAX_SHIP_PRIMARY_BANKS] == Selected_wl_class &&
                     Hot_weapon_bank < 0) || x + MAX_SHIP_PRIMARY_BANKS == Hot_weapon_bank) {
                    Assert(num_found < NUM_ICON_FRAMES);
                    gr_set_color_fast(&Icon_colors[ICON_FRAME_NORMAL + num_found]);
                    gr_circle(Wl_bank_coords[gr_screen.res][x + MAX_SHIP_PRIMARY_BANKS][0] - 50,
                              Wl_bank_coords[gr_screen.res][x + MAX_SHIP_PRIMARY_BANKS][1] + 12, 5);
                    for (y = 0; y < pm->missile_banks[x].num_slots; y++) {
                        ...
                        gr_line(Wl_bank_coords[gr_screen.res][x + MAX_SHIP_PRIMARY_BANKS][0] - 50,
                                Wl_bank_coords[gr_screen.res][x + MAX_SHIP_PRIMARY_BANKS][1] + 12, xc + 4,
                                Wl_bank_coords[gr_screen.res][x + MAX_SHIP_PRIMARY_BANKS][1] + 12);
...
void wl_reset_selected_slot() {
    ...
    if ((Game_mode & GM_MULTIPLAYER) && !MULTI_PERM_OBSERVER(Net_players[MY_NET_PLAYER_NUM]) &&
...
void wl_get_ship_class_weapons(int ship_class, int *wep, int *wep_count) {
    ...
    for (i = 0; i < sip->num_secondary_banks; i++) {
        ...
        wep_count[i + MAX_SHIP_PRIMARY_BANKS] = wl_calc_missile_fit(sip->secondary_bank_weapons[i],
                                                                    sip->secondary_bank_ammo_capacity[i]);
    }
}
...
void wl_get_parseobj_weapons(int sa_index, int ship_class, int *wep, int *wep_count) {
    ...
    for (i = 0; i < MAX_SHIP_SECONDARY_BANKS; i++) {
        if (wep[i + MAX_SHIP_PRIMARY_BANKS] >= 0) {
            wep_count[i + MAX_SHIP_PRIMARY_BANKS] = wl_calc_missile_fit(wep[i + MAX_SHIP_PRIMARY_BANKS],
                                                                        fl2i(ss->secondary_ammo[i] / 100.0f *
                                                                             sip->secondary_bank_ammo_capacity[i] +
                                                                             0.5f));
        }
    }
                ...
                if (Wss_num_wings <= 0) {
                    ...
                    if (Wl_pool[wi_index] <= 0) {
                        ...
                        for (wep_pool_index = 0; wep_pool_index < MAX_WEAPON_TYPES; wep_pool_index++) {
                            ...
                            if (!eval_weapon_flag_for_game_type(
                                    Ship_info[ship_class].allowed_weapons[wep_pool_index])) {
                                continue;
                            }


                            for (wep_precedence_index = 0; wep_precedence_index < Num_player_weapon_precedence;
                                 wep_precedence_index++) {
                        ...
                        if (Weapon_info[wi_index].subtype == WP_MISSILE) {
                            ...
                            new_wep_count = wl_calc_missile_fit(wi_index,
                                                                Ship_info[ship_class].secondary_bank_ammo_capacity[secondary_bank_index]);
                        }
...
void weapon_select_init() {
    ...
    if (Weapon_select_open) {
        ...
        common_buttons_maybe_reload(
                &Weapon_ui_window);    // AL 11-21-97: this is necessary since we may returning from the hotkey
    ...
    Weapon_select_background_bitmap = bm_load((Game_mode & GM_MULTIPLAYER) ? Weapon_select_multi_background_fname[
            Uses_apply_all_button][gr_screen.res]
                                                                           : Weapon_select_background_fname[Uses_apply_all_button][gr_screen.res]);
    if (Weapon_select_background_bitmap < 0)    // failed to load
    {
        Uses_apply_all_button = 0;    // nope, sorry
        Weapon_select_background_bitmap = bm_load((Game_mode & GM_MULTIPLAYER) ? Weapon_select_multi_background_fname[
                Uses_apply_all_button][gr_screen.res] : Weapon_select_background_fname[Uses_apply_all_button][gr_screen.
                res]);
    }
    ...
    snazzy_menu_add_region(&Weapon_select_region[Num_weapon_select_regions++], "", ICON_SHIP_SECONDARY_4, 0);
    snazzy_menu_add_region(&Weapon_select_region[Num_weapon_select_regions++], "", ICON_SHIP_SECONDARY_5, 0);
...
void wl_weapon_desc_start_wipe() {
    ...
    if (w > Weapon_title_max_width[gr_screen.res]) {
        // split
        currchar_src = (int) (((float) title_len / (float) w) *
                              Weapon_title_max_width[gr_screen.res]);            // char to start space search at
...
void weapon_select_do(float frametime) {
    ...
    wl_choice = snazzy_menu_do(WeaponSelectMaskData, Weaponselect_mask_w, Weaponselect_mask_h,
                               Num_weapon_select_regions, Weapon_select_region, &snazzy_action, 0);
        ...
        switch (wl_choice) {
            ...
            case ICON_SHIP_SECONDARY_3:
                if (do_mouse_over_ship_weapon(6))
                    wl_choice = -1;
                break;
            case ICON_SHIP_SECONDARY_4:
                if (do_mouse_over_ship_weapon(7))
                    wl_choice = -1;
                break;
            case ICON_SHIP_SECONDARY_5:
                if (do_mouse_over_ship_weapon(8))
                    wl_choice = -1;
                break;
            case WING_0_SHIP_0:
                Hot_wl_slot = 0;
    ...
    if (wl_choice > -1 && snazzy_action == SNAZZY_CLICKED) {
        switch (wl_choice) {
            ...
            case ICON_SHIP_SECONDARY_3:
                ...
                break;
            case ICON_SHIP_SECONDARY_4:
                maybe_select_new_ship_weapon(7);
                break;
            case ICON_SHIP_SECONDARY_5:
                maybe_select_new_ship_weapon(8);
                break;
            case WING_0_SHIP_0:
    ...
    if (Wl_icons[Selected_wl_class].model_index != -1) {
        ...
        generic_anim_render(&Cur_Anim, (help_overlay_active(WL_OVERLAY)) ? 0 : frametime, weapon_ani_coords[0],
                            weapon_ani_coords[1]);
    }
    ...
    if (wl_icon_being_carried()) {
        ...
        if (Wl_icons[Carried_wl_icon.weapon_class].can_use > 0) {
            ...
            if (icon->icon_bmaps[WEAPON_ICON_FRAME_SELECTED] != -1) {
                ...
                if (icon->model_index != -1) {
                    //Draw the model
                    draw_model_icon(icon->model_index, MR_LOCK_DETAIL | MR_NO_FOGGING | MR_NO_LIGHTING, 0.4f, sx, sy,
                                    w, h, NULL);
                    ...
                    int half_x, half_y;
                    char *print_name = (Weapon_info[Carried_wl_icon.weapon_class].alt_name[0]) ?
                                       Weapon_info[Carried_wl_icon.weapon_class].alt_name
                                                                                               : Weapon_info[Carried_wl_icon.weapon_class].
                                    name;
        ...
        if (Carried_wl_icon.from_bank >= MAX_SHIP_PRIMARY_BANKS) {
            if (mx == Carried_wl_icon.from_x && my == Carried_wl_icon.from_y) {
                ...
                wl_render_icon_count(num_missiles, Wl_bank_coords[gr_screen.res][Carried_wl_icon.from_bank][0],
                                     Wl_bank_coords[gr_screen.res][Carried_wl_icon.from_bank][1]);
            }
        }
        ...
        if (!Wl_icons[Carried_wl_icon.weapon_class].can_use) {
            ...
            if ((diffx > 2) || (diffy > 2)) {
                ...
                if (Lcl_gr) {
                    ...
                    strncpy(display_name, (Weapon_info[Carried_wl_icon.weapon_class].alt_name[0]) ?
                                          Weapon_info[Carried_wl_icon.weapon_class].alt_name
                                                                                                  : Weapon_info[Carried_wl_icon.weapon_class].
                                    name, NAME_LENGTH);
                    lcl_translate_wep_name(display_name);
                    popup(PF_USE_AFFIRMATIVE_ICON, 1, POPUP_OK, XSTR("A %s is unable to carry %s weaponry",
                                                                     633),
                          (Ship_info[ship_class].alt_name[0]) ? Ship_info[ship_class].alt_name :
                          Ship_info[ship_class].name, display_name);
                } else {
                    popup(PF_USE_AFFIRMATIVE_ICON, 1, POPUP_OK, XSTR("A %s is unable to carry %s weaponry",
                                                                     633),
                          (Ship_info[ship_class].alt_name[0]) ? Ship_info[ship_class].alt_name :
                          Ship_info[ship_class].name, Weapon_info[Carried_wl_icon.weapon_class].name);
                }
...
//
void wl_render_icon(int index, int x, int y, int num, int draw_num_flag, int hot_mask, int hot_bank_mask,
                    int select_mask) {
    int bitmap_id = -1;
    ...
    if (bitmap_id != -1) {
        ...
        if (icon->model_index != -1) {
            //Draw the model
            draw_model_icon(icon->model_index, MR_LOCK_DETAIL | MR_NO_FOGGING | MR_NO_LIGHTING, .5f / 1.25f, x, y, 56,
                            24, NULL);
        } else if (icon->laser_bmap != -1) {
            ...
            //Draw the weapon name, crappy last-ditch effort to not crash.
            int half_x, half_y;
            char *print_name = (Weapon_info[index].alt_name[0]) ? Weapon_info[index].alt_name : Weapon_info[index].
                    name;
...
void wl_draw_ship_weapons(int index) {
    ...
    for (i = 0; i < MAX_SHIP_WEAPONS; i++) {
        ...
        if (i < Ship_info[Wss_slots[index].ship_class].num_primary_banks || (i >= MAX_SHIP_PRIMARY_BANKS && ((i -
                                                                                                              MAX_SHIP_PRIMARY_BANKS) <
                                                                                                             Ship_info[Wss_slots[index].ship_class].num_secondary_banks))) {
            if (Weapon_slot_bitmap != -1) {
                ...
                draw_brackets_square(Wl_bank_coords[gr_screen.res][i][0], Wl_bank_coords[gr_screen.res][i][1],
                                     Wl_bank_coords[gr_screen.res][i][0] + 56,
                                     Wl_bank_coords[gr_screen.res][i][1] + 24);
            }
        }
        ...
        if ((wep[i] != -1) && (wep_count[i] >= 0)) {
            ...
            x_offset = 0;
            wl_render_icon(wep[i], Wl_bank_coords[gr_screen.res][i][0] + x_offset, Wl_bank_coords[gr_screen.res][i][1],
                           wep_count[i], Wl_bank_count_draw_flags[i], -1, i, wep[i]);
        }
    }
}
...
void draw_wl_icon_with_number(int list_count, int weapon_class) {
    ...
    wl_render_icon(weapon_class, Wl_weapon_icon_coords[gr_screen.res][list_count][0],
                   Wl_weapon_icon_coords[gr_screen.res][list_count][1],
...
//	pick_from_ship_slot()
//
// input: num	->	index into shipb banks
void pick_from_ship_slot(int num) {
...
void wl_update_parse_object_weapons(p_object *pobjp, wss_unit *slot) {
    ...
    for (i = 0; i < MAX_SHIP_SECONDARY_BANKS; i++) {
        ...
        if ((slot->wep_count[sidx] >= 0) && (slot->wep[sidx] >= 0)) {
            ...
            max_count = wl_calc_missile_fit(slot->wep[sidx],
...
void start_weapon_animation(int weapon_class) {
    ...
    if (strcmp(Cur_Anim.filename, Weapon_info[weapon_class].anim_filename) != 0) {
        ...
        if (generic_anim_stream(&Cur_Anim) == -1) {
            ...
            Cur_Anim.first_frame = bm_load(
                    Weapon_info[weapon_class].anim_filename);    //if we fail here, the value is still -1
    ...
    for (i = 0; i < MAX_SHIP_PRIMARY_BANKS; i++) {
        ...
        if ((slot->wep_count[i] > 0) && (slot->wep[i] >= 0)) {
            ...
            if (Weapon_info[swp->primary_bank_weapons[j]].wi_flags2 & WIF2_BALLISTIC) {
                ...
                swp->primary_bank_ammo[j] = wl_calc_ballistic_fit(swp->primary_bank_weapons[j],
                                                                  Ship_info[slot->ship_class].primary_bank_ammo_capacity[i]);
            } else {
...
void wl_saturate_bank(int ship_slot, int bank) {
    ...
    max_count = wl_calc_missile_fit(slot->wep[bank], Ship_info[slot->ship_class].secondary_bank_ammo_capacity[bank -
                                                                                                              3]);
...
int wl_swap_slot_slot(int from_bank, int to_bank, int ship_slot, int *sound, net_player *pl) {
    ...
    class_mismatch_flag = (IS_BANK_PRIMARY(from_bank) && IS_BANK_SECONDARY(to_bank)) ||
                          (IS_BANK_SECONDARY(from_bank) && IS_BANK_PRIMARY(to_bank));
    ...
    if (!class_mismatch_flag) {
        ...
        if (eval_weapon_flag_for_game_type(sip->restricted_loadout_flag[to_bank])) {
            if (!eval_weapon_flag_for_game_type(sip->allowed_bank_restricted_weapons[to_bank][slot->wep[from_bank]])) {
                ...
                strncpy(display_name, (Weapon_info[slot->wep[from_bank]].alt_name[0]) ? Weapon_info[slot->
                        wep[from_bank]].alt_name : Weapon_info[slot->wep[from_bank]].name, NAME_LENGTH);
    ...
    if (IS_BANK_SECONDARY(from_bank) && IS_BANK_SECONDARY(to_bank)) {
        ...
        if (slot->wep[from_bank] == slot->wep[to_bank]) {
            ...
            dest_max = wl_calc_missile_fit(slot->wep[to_bank],
                                           Ship_info[slot->ship_class].secondary_bank_ammo_capacity[to_bank - 3]);
...
int wl_grab_from_list(int from_list, int to_bank, int ship_slot, int *sound, net_player *pl) {
    ...
    if ((IS_LIST_PRIMARY(from_list) && IS_BANK_SECONDARY(to_bank)) || (IS_LIST_SECONDARY(from_list) &&
                                                                       IS_BANK_PRIMARY(to_bank))) {
        // do nothing
        *sound = SND_ICON_DROP;
        return 0;
    }
    ...
    if (to_bank < MAX_SHIP_PRIMARY_BANKS) {
        ...
        max_fit = wl_calc_missile_fit(from_list, Ship_info[slot->ship_class].secondary_bank_ammo_capacity[to_bank -
                                                                                                          MAX_SHIP_PRIMARY_BANKS]);
    }
...
int wl_swap_list_slot(int from_list, int to_bank, int ship_slot, int *sound, net_player *pl) {
    ...
    if (eval_weapon_flag_for_game_type(sip->restricted_loadout_flag[to_bank])) {
        if (!eval_weapon_flag_for_game_type(sip->allowed_bank_restricted_weapons[to_bank][from_list])) {
            ...
            strncpy(display_name, (Weapon_info[from_list].alt_name[0]) ? Weapon_info[from_list].alt_name :
                                  Weapon_info[from_list].name, NAME_LENGTH);
...
int wl_apply(int mode, int from_bank, int from_list, int to_bank, int to_list, int ship_slot, int player_index,
...
int wl_drop(int from_bank, int from_list, int to_bank, int to_list, int ship_slot, int player_index,
            bool dont_play_sound) {
    int mode;
    int update = 0;
    net_player *pl;
    ...
    if (!(Game_mode & GM_MULTIPLAYER) || MULTIPLAYER_HOST) {
        ...
        send_wss_request_packet(Net_player->player_id, from_bank, from_list, to_bank, to_list, ship_slot, -1,
                                WSS_WEAPON_SELECT);
    }
    ...
    for (cur_wing_slot = 0; cur_wing_slot < MAX_WING_SLOTS; cur_wing_slot++) {
        ...
        for (cur_bank = 0; cur_bank < MAX_SHIP_WEAPONS; cur_bank++) {
            ...
            if (cur_bank < MAX_SHIP_PRIMARY_BANKS) {
                ...
                if (cur_bank - MAX_SHIP_PRIMARY_BANKS >= source_sip->num_secondary_banks ||
                    cur_bank - MAX_SHIP_PRIMARY_BANKS >= sip->num_secondary_banks)
                    continue;
            }
            ...
            // maybe localize
            if (Lcl_gr) {
                strncpy(buf, (Weapon_info[weapon_type_to_add].alt_name[0]) ? Weapon_info[weapon_type_to_add].alt_name :
                             Weapon_info[weapon_type_to_add].name, NAME_LENGTH);
                lcl_translate_wep_name(buf);
                wep_display_name = buf;
            } else {
                wep_display_name = (Weapon_info[weapon_type_to_add].alt_name[0]) ? Weapon_info[weapon_type_to_add].
                        alt_name : Weapon_info[weapon_type_to_add].name;
            }
            ...
            if (!eval_weapon_flag_for_game_type(sip->allowed_weapons[weapon_type_to_add])
                ...
                sprintf(error_messages[cur_wing_slot * MAX_SHIP_WEAPONS + cur_bank],
                        NOX("%s is unable to carry %s weaponry"), ship_name, wep_display_name);
                error_flag = true;
                continue;
            }
            ...
            if (eval_weapon_flag_for_game_type(sip->restricted_loadout_flag[cur_bank])) {
                if (!eval_weapon_flag_for_game_type(sip->allowed_bank_restricted_weapons[cur_bank][
                                                            weapon_type_to_add])) {
                    if (cur_bank < MAX_SHIP_PRIMARY_BANKS)
                        sprintf(error_messages[cur_wing_slot * MAX_SHIP_WEAPONS + cur_bank],
                                NOX("%s is unable to carry %s weaponry in primary bank %d"), ship_name,
                                wep_display_name,
                                cur_bank + 1);
                    else
                        sprintf(error_messages[cur_wing_slot * MAX_SHIP_WEAPONS + cur_bank],
                                NOX("%s is unable to carry %s weaponry in secondary bank %d"), ship_name,
                                wep_display_name,
                                cur_bank + 1 - MAX_SHIP_PRIMARY_BANKS);
            ...
            if ((result == 0) || (result == 2)) {
                sprintf(error_messages[cur_wing_slot * MAX_SHIP_WEAPONS + cur_bank],
                        NOX("Insufficient %s available to arm %s"), (Weapon_info[weapon_type_to_add].alt_name[0]) ?
                                                                    Weapon_info[weapon_type_to_add].alt_name
                                                                                                                  : Weapon_info[weapon_type_to_add].name,
                        ship_name);
                error_flag = true;
                continue;
            }

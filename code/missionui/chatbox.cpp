    ...
    if ((Chatbox_mode_flags & CHATBOX_FLAG_BUTTONS) && (Chatbox_mode_flags & CHATBOX_FLAG_DRAW_BOX)) {
        for (idx = 0; idx < CHATBOX_NUM_BUTTONS; idx++) {
            ...
            Chatbox_buttons[gr_screen.res][idx].button.create(&Chat_window, "", Chatbox_buttons[gr_screen.res][idx].x,
                                                              Chatbox_buttons[gr_screen.res][idx].y, 60, 30,
                                                              (idx == CHATBOX_TOGGLE_SIZE) ? 0 : 1);
...
void chatbox_add_line(char *msg, int pid, int add_id) {
    ...
    // COMMAND LINE OPTION
    if (Cmdline_multi_stream_chat_to_file && Multi_chat_stream != NULL &&
        strlen(msg) > 0) { // stream to the file if we're supposed to

...
{
	{	// GR_640
		196 + CHATBOX_TEAM_ICON_SPACE,
		13,
		410 - CHATBOX_TEAM_ICON_SPACE,
		74
	},
	{	// GR_1024
		315 + CHATBOX_TEAM_ICON_SPACE,
		22,
		654 - CHATBOX_TEAM_ICON_SPACE,
		116
	}
};
...
{
	{	// GR_640
		103 + CHATBOX_TEAM_ICON_SPACE,
		149,
		380 - CHATBOX_TEAM_ICON_SPACE,
		143
	},
	{	// GR_1024
		165 + CHATBOX_TEAM_ICON_SPACE,
		244,
		654 - CHATBOX_TEAM_ICON_SPACE,
		263
	}
...
int chatbox_create(int mode_flags)
{
	...
	if (Chatbox_mode_flags & CHATBOX_FLAG_DRAW_BOX)
	{
		...
		if ((Chatbox_bitmap == -1) || (Chatbox_small_bitmap == -1) || (Chatbox_big_bitmap == -1) ||
			(Chatbox_mp_bitmap == -1))
		{
			return -1;
		}
	...
	Chat_inputbox.create(&Chat_window, Chatbox_inputbox_x, Chatbox_textenter_y, Chatbox_inputbox_w, CHATBOX_MAX_LEN,
		"", UI_INPUTBOX_FLAG_INVIS | UI_INPUTBOX_FLAG_ESC_CLR | UI_INPUTBOX_FLAG_KEYTHRU | UI_INPUTBOX_FLAG_EAT_USED,
		Chatbox_w, Color_netplayer[MY_NET_PLAYER_NUM]);
	...
	if ((Chatbox_mode_flags & CHATBOX_FLAG_BUTTONS) && (Chatbox_mode_flags & CHATBOX_FLAG_DRAW_BOX))
	{
		...
		{
			// create the button			
			Chatbox_buttons[gr_screen.res][idx].button.create(&Chat_window, "", Chatbox_buttons[gr_screen.res][idx].x,
				Chatbox_buttons[gr_screen.res][idx].y, 60, 30, (idx == CHATBOX_TOGGLE_SIZE) ? 0 : 1);
		...
		if (Chatbox_mode_flags & CHATBOX_FLAG_SMALL)
		{
			Chatbox_buttons[gr_screen.res][CHATBOX_TOGGLE_SIZE].button.set_bmaps(Chatbox_buttons[gr_screen.
				res][CHATBOX_TOGGLE_SIZE].filename);
		}
		else
		{
			Chatbox_buttons[gr_screen.res][CHATBOX_TOGGLE_SIZE].button.set_bmaps(Chatbox_buttons[gr_screen.
				res][CHATBOX_TOGGLE_SIZE + 1].filename);
		}
	}
...
int chatbox_scroll_up()
{
	if (Num_brief_chat_lines > Chatbox_max_lines)
	{
		...
		if (Brief_chat_prev_index[Brief_current_add_line] != prev && !(Num_brief_chat_lines < MAX_BRIEF_CHAT_LINES &&
																	   Brief_start_display_index == 0))
		{
...
void chatbox_add_line(char* msg, int pid, int add_id)
{
	...
	// COMMAND LINE OPTION
	if (Cmdline_multi_stream_chat_to_file && Multi_chat_stream != NULL && strlen(msg) > 0)
	{ // stream to the file if we're supposed to

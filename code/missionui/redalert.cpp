	...
	{ // GR_640
		{
			...
			UI_XSTR_COLOR_PINK,
			-1,
			&Buttons[0][RA_REPLAY_MISSION].button
		},
		{
			"Previous Mission",
			1452,
	...
	{ // GR_1024
		{
			...
			UI_XSTR_COLOR_PINK,
			-1,
			&Buttons[1][RA_REPLAY_MISSION].button
		},
		{
			"Previous Mission",
			1452,
...
void red_alert_blit_title()
{
	...
	gr_string(Ra_brief_text_wnd_coords[gr_screen.res][0] + ((Ra_brief_text_wnd_coords[gr_screen.res][2] - w) / 2),
		Ra_flash_y[gr_screen.res] - h - 5, str);
...
void red_alert_do_frame(float frametime)
{
	...
	if (timestamp_elapsed(Text_delay))
	{
		...
		if (Briefing->num_stages > 0)
		{
			finished_wipe = brief_render_text(0, Ra_brief_text_wnd_coords[gr_screen.res][RA_X_COORD],
				Ra_brief_text_wnd_coords[gr_screen.res][RA_Y_COORD],
				Ra_brief_text_wnd_coords[gr_screen.res][RA_H_COORD], frametime, 0);
		}

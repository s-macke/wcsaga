...
typedef struct starfield_bitmap_instance {
    ...
    starfield_bitmap_instance() {
        memset(this, 0, sizeof(starfield_bitmap_instance));
        star_bitmap_index = -1;
    };
} starfield_bitmap_instance;
...
//XSTR:OFF
debris_vclip Debris_vclips_normal[MAX_DEBRIS_VCLIPS] =
        {
                {
                        -1,
                        -1,
...
debris_vclip Debris_vclips_nebula[MAX_DEBRIS_VCLIPS] =
        {
                {
                        -1,
                        -1,
...
static void starfield_create_perspective_bitmap_buffer(angles *a, float scale_x, float scale_y, int div_x, int div_y,
                                                       uint tmap_flags, int env, ushort *index_buffer) {
    float p_phi = 10.0f;
    float p_theta = 10.0f;
    ...
    for (idx = 0; idx <= div_x; idx++) {
        for (s_idx = 0; s_idx <= div_y; s_idx++) {
            ...
            stars_project_2d_onto_sphere(&s_points[idx][s_idx], 1000.0f, s_phi + ((float) idx * d_phi),
                                         s_theta + ((float) s_idx * d_theta));
...
static void starfield_generate_bitmap_instance_buffers() {
    ...
    for (idx = 0; idx < sb_instances; idx++) {
        ...
        if (Starfield_bitmap_instances[idx].buffer != NULL) {
            starfield_create_perspective_bitmap_buffer(&Starfield_bitmap_instances[idx].ang,
                                                       Starfield_bitmap_instances[idx].scale_x,
                                                       Starfield_bitmap_instances[idx].scale_y,
                                                       Starfield_bitmap_instances[idx].div_x,
                                                       Starfield_bitmap_instances[idx].div_y,
                                                       TMAP_FLAG_TEXTURED | TMAP_FLAG_CORRECT | TMAP_FLAG_XPARENT, 0,
                                                       Starfield_bitmap_instances[idx].buffer);
        }
    }
    ...
    while (!check_for_eof()) {
            ...
            if ((idx = stars_find_bitmap(sbm.filename)) >= 0) {
                if (sbm.xparent == Starfield_bitmaps[idx].xparent) {
                    if (!Parsing_modular_table)
                        Warning(LOCATION, "Starfield bitmap '%s' listed more than once!!  Only using the first entry!",
                                sbm.filename);
                } else {
                    Warning(LOCATION,
                            "Starfield bitmap '%s' already listed as a %sxparent bitmap!!  Only using the xparent version!",
        ...
        while (optional_string("$Sun:")) {
            ...
            if ((idx = stars_find_sun(sbm.filename)) >= 0) {
                if (Parsing_modular_table)
                    ...
                    Warning(LOCATION, "Sun bitmap '%s' listed more than once!!  Only using the first entry!",
                            sbm.filename);
            } else {
        ...
        while (optional_string("$Debris:")) {
            ...
            if (Num_debris_normal < MAX_DEBRIS_VCLIPS) {
                ...
                Warning(LOCATION, "Could not load normal motion debris '%s'; maximum of %d exceeded.", name,
                        MAX_DEBRIS_VCLIPS);
            }
        }
        ...
        while (optional_string("$DebrisNeb:")) {
            ...
            if (Num_debris_nebula < MAX_DEBRIS_VCLIPS) {
                ...
                Warning(LOCATION, "Could not load nebula motion debris '%s'; maximum of %d exceeded.", name,
                        MAX_DEBRIS_VCLIPS);
            }
        }
...
void stars_draw_sun(int show_sun) {
    ...
    for (idx = 0; idx < num_suns; idx++) {
        ...
        if (bm->fps) {
            gr_set_bitmap(bm->bitmap_id + ((timestamp() / (int) (bm->fps) % bm->n_frames)), GR_ALPHABLEND_FILTER,
                          GR_BITBLT_MODE_NORMAL, 0.999f);
        } else {
...
void stars_draw_lens_flare(vertex *sun_vex, int sun_n) {
    ...
    dx = 2.0f * (i2fl(gr_screen.clip_right - gr_screen.clip_left) * 0.5f -
                 sun_vex->sx); // (dx,dy) is a 2d vector equal to two times the vector from the sun's position to the center fo the screen
    dy = 2.0f * (i2fl(gr_screen.clip_bottom - gr_screen.clip_top) * 0.5f -
                 sun_vex->sy); // meaning it is the vector to the opposite position on the screen
...
void stars_draw_sun_glow(int sun_n) {
    ...
    // draw the sun itself, keep track of how many we drew
    if (bm->glow_fps) {
        gr_set_bitmap(bm->glow_bitmap + ((timestamp() / (int) (bm->glow_fps) % bm->glow_n_frames)),
                      GR_ALPHABLEND_FILTER, GR_BITBLT_MODE_NORMAL, 0.5f);
    } else {
    ...
    for (idx = 0; idx < sb_instances; idx++) {
        ...
        if (Starfield_bitmaps[star_index].xparent) {
            if (Starfield_bitmaps[star_index].fps) {
                gr_set_bitmap(Starfield_bitmaps[star_index].
                        bitmap_id + ((timestamp() / (int) (Starfield_bitmaps[star_index].fps) %
                                      Starfield_bitmaps[star_index].n_frames)));
            } else {
                gr_set_bitmap(Starfield_bitmaps[star_index].bitmap_id);
            }

            if (Cmdline_nohtl) {
                g3_draw_perspective_bitmap(&Starfield_bitmap_instances[idx].ang,
                                           Starfield_bitmap_instances[idx].scale_x,
                                           Starfield_bitmap_instances[idx].scale_y,
                                           Starfield_bitmap_instances[idx].div_x, Starfield_bitmap_instances[idx].div_y,
            ...
            if (Starfield_bitmaps[star_index].fps) {
                gr_set_bitmap(Starfield_bitmaps[star_index].
                                      bitmap_id + ((timestamp() / (int) (Starfield_bitmaps[star_index].fps) %
                                                    Starfield_bitmaps[star_index].n_frames)), GR_ALPHABLEND_FILTER,
                              GR_BITBLT_MODE_NORMAL, 0.9999f);
            } else {
                gr_set_bitmap(Starfield_bitmaps[star_index].bitmap_id, GR_ALPHABLEND_FILTER, GR_BITBLT_MODE_NORMAL,
                              0.9999f);
            }
...
void stars_draw(int show_stars, int show_suns, int show_nebulas, int show_subspace, int env) {
    ...
    if (show_nebulas && (Game_detail_flags & DETAIL_FLAG_NEBULAS) && (Neb2_render_mode != NEB2_RENDER_POF) &&
    ...
    if (!env && show_stars && (Nmodel_num < 0) && (Game_detail_flags & DETAIL_FLAG_STARS) && !(The_mission.flags &
    ...
    if (!Rendering_to_env && (Game_detail_flags & DETAIL_FLAG_MOTION) && (!Fred_running) && (supernova_active() < 3) &&
        (!Cmdline_nomotiondebris)) {
        stars_draw_debris();
    }
...
int stars_add_sun_entry(starfield_list_entry *sun) {
    ...
    if (Sun_bitmaps[idx].bitmap_id < 0) {
        ...
        // maybe didn't load a static image so try for an animated one
        if (Sun_bitmaps[idx].bitmap_id < 0) {
            Sun_bitmaps[idx].bitmap_id = bm_load_animation(Sun_bitmaps[idx].filename, &Sun_bitmaps[idx].
                    n_frames, &Sun_bitmaps[idx].fps, NULL, 1);
        ...
        if (Sun_bitmaps[idx].glow_bitmap < 0) {
            ...
            // maybe didn't load a static image so try for an animated one
            if (Sun_bitmaps[idx].glow_bitmap < 0) {
                Sun_bitmaps[idx].glow_bitmap = bm_load_animation(Sun_bitmaps[idx].glow_filename, &Sun_bitmaps[idx].
                        glow_n_frames, &Sun_bitmaps[idx].glow_fps, NULL, 1);
...
int stars_add_bitmap_entry(starfield_list_entry *sle) {
    ...
    if (Starfield_bitmaps[idx].bitmap_id < 0) {
        ...
        // maybe didn't load a static image so try for an animated one
        if (Starfield_bitmaps[idx].bitmap_id < 0) {
            Starfield_bitmaps[idx].bitmap_id = bm_load_animation(Starfield_bitmaps[idx].filename, &Starfield_bitmaps[
                    idx].n_frames, &Starfield_bitmaps[idx].fps, NULL, 1);
    ...
    if (background_idx < 0 && !Fred_running) {
        ...
        if (at_least_one_bitmap) {
            if (Num_backgrounds == 1)
                Warning(LOCATION,
                        "Unable to find a sufficient number of bitmaps for this mission's background.  The background will not be displayed.");
            else if (Num_backgrounds > 1)
                Warning(LOCATION,
                        "Unable to find a sufficient number of bitmaps for any background listed in this mission.  No background will be displayed.");
        }
    }
#endif
...
int stars_get_first_valid_background() {
    ...
    for (i = 0; i < (uint) Num_backgrounds; i++) {
        ...
        if ((total_suns >= (background->suns.size() + 1) / 2) && (total_bitmaps >= (background->bitmaps.size() + 1) /
                                                                                   2))
            return i;
    }
    ...
    if (Cur_background >= 0) {
        ...
        for (j = 0; j < background->bitmaps.size(); j++) {
            if ((stars_add_bitmap_entry(&background->bitmaps[j]) < 0) && !Fred_running)
                Warning(LOCATION, "Failed to add starfield bitmap '%s' to the mission!",

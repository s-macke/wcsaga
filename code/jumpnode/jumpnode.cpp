...
void jumpnode_level_close() {
    ...
    if (Num_jump_nodes != 0)
        Warning(LOCATION, "Num_jump_nodes should be 0, but is actually %d.", Num_jump_nodes);
}
    ...
    if (Fred_running) {
        ...
        if (m_flags & JN_USE_DISPLAY_COLOR) {
            ...
            if (dist < 1000) {
                ...
                alpha_index = fl2i(HUD_COLOR_ALPHA_USER_MAX - 2 + (dist - 1000) * (HUD_COLOR_ALPHA_USER_MIN -

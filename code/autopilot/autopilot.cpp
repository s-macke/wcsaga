...
void SelectNav(int nav) {
    ...
    if (Navs[nav].flags & NP_VALIDTYPE && !(Navs[nav].flags & NP_NOSELECT)) {
        CurrentNav = nav;
    }
}
    ...
    for (i = 0; i < MAX_SHIPS; i++) {
        if (Ships[i].objnum != -1 &&
            ...
            if (Ship_info[Ships[i].ship_info_index].flags
                & (SIF_CRUISER | SIF_CAPITAL | SIF_SUPERCAP | SIF_CORVETTE | SIF_AWACS | SIF_GAS_MINER |
                   SIF_FREIGHTER | SIF_TRANSPORT)) {
                capshipPresent = true;
            ...
            if (The_mission.flags &
                MISSION_FLAG_USE_AP_CINEMATICS) {// instantly turn the ship to match the direction player is looking
                //vm_vec_sub(&norm1, Navs[CurrentNav].GetPosition(), &Player_obj->pos);
                ...
                && Player_obj != &Objects[Ships[i].objnum]) //only if not player object
            {
                ...
                if (leader_objp != &Objects[Ships[i].objnum]) {
                    ...
                    get_absolute_wing_pos_autopilot(&goal_point, leader_objp, wing_index,
            ...
            if (Ships[i].wingnum == -1 || The_mission.flags & MISSION_FLAG_USE_AP_CINEMATICS) {
                if (Navs[CurrentNav].flags & NP_WAYPOINT) {
                    ai_add_ship_goal_player(AIG_TYPE_PLAYER_SHIP, AI_GOAL_WAYPOINTS_ONCE,
                                            0, ((waypoint_list *) Navs[CurrentNav].target_obj)->name,
                                            &Ai_info[Ships[i].ai_index]);
                    //fixup has to wait until after wing goals
                } else {
                    ai_add_ship_goal_player(AIG_TYPE_PLAYER_SHIP, AI_GOAL_FLY_TO_SHIP,
                                            0, ((ship *) Navs[CurrentNav].target_obj)->ship_name,
                                            &Ai_info[Ships[i].ai_index]);
                }
        ...
        for (i = 0; i < MAX_WINGS; i++) {
            if (Wings[i].flags & WF_NAV_CARRY) {
                ...
                if (Navs[CurrentNav].flags & NP_WAYPOINT) {
                    ...
                    ai_add_wing_goal_player(AIG_TYPE_PLAYER_WING, AI_GOAL_WAYPOINTS_ONCE,
                                            0, ((waypoint_list *) Navs[CurrentNav].target_obj)->name, i);
                    ...
                    ai_add_wing_goal_player(AIG_TYPE_PLAYER_WING, AI_GOAL_FLY_TO_SHIP,
                                            0, ((ship *) Navs[CurrentNav].target_obj)->ship_name, i);
        ...
        if (capshipPresent) {
            ...
            for (i = 0; i < (int) capIndexes.size(); i++) {
                ...
                if (Ship_info[Ships[capIndexes[i]].ship_info_index].flags & (SIF_CAPITAL | SIF_SUPERCAP)) {
                    ...
                    // position below
                    vm_vec_scale(&up,
                                 capship_spreads[0]); // scale the up vector by the radius of the largest ship in this formation part
                    ...
                    // position below
                    vm_vec_scale(&up,
                                 capship_spreads[1]); // scale the up vector by the radius of the largest ship in this formation part
...
void nav_warp(bool prewarp = false) {
    ...
    vm_vec_sub(&targetPos, &tpos,
               &Player_obj->pos); //targetPos is actually a projection in a the direction toward the nav

...
void labviewer_change_model(char *model_fname, int lod = 0, int sel_index = -1) {
    ...
    if (change_model) {
        ...
        if (model_fname != NULL) {
            ...
            Lab_model_num = model_load(model_fname, (valid_ship) ? Ship_info[sel_index].n_subsystems : 0, subsystems,
                                       0);
            ...
            if (valid_ship) {
                ...
                for (int idx = 0; idx < Ship_info[sel_index].n_subsystems; idx++) {
                    model_set_instance_info(&Lab_ship_subsys[idx].submodel_info_1,
                                            Lab_ship_model_subsys[idx].turn_rate, 0.5f);
                }
            }
        } else {
...
void labviewer_render_model(float frametime) {
    ...
    if (Lab_viewer_flags & LAB_FLAG_SHOW_DEBRIS) {
        ...
        for (i = 0; i < pm->num_debris_objects; i++) {
            ...
            model_find_world_point(&world_point, &pm->submodel[pm->debris_objects[i]].offset,
                                   Lab_model_num, -1, &Lab_viewer_orient, &vmd_zero_vector);

            submodel_render(Lab_model_num, pm->debris_objects[i], &Lab_viewer_orient, &world_point,
                            Lab_model_flags, -1);
        }
    }
        ...
        if ((sip != NULL) && (Lab_viewer_flags & LAB_FLAG_SUBMODEL_ROTATE)) {
            for (i = 0; i < sip->n_subsystems; i++) {
                ...
                model_set_instance(Lab_model_num, Lab_ship_model_subsys[i].subobj_num, &Lab_ship_subsys[i].
                        submodel_info_1);
...
void labviewer_render_bitmap(float frametime) {
    ...
    g3_set_view_matrix(&Lab_viewer_pos, &vmd_identity_matrix, Lab_viewer_zoom * 1.35f);
    ...
    g3_draw_laser(&headp, wip->laser_head_radius, &vmd_zero_vector, wip->laser_tail_radius,
                  TMAP_FLAG_TEXTURED | TMAP_FLAG_XPARENT | TMAP_HTL_3D_UNLIT);
    ...
    if (!(Lab_model_flags & MR_NO_GLOWMAPS) && (wip->laser_glow_bitmap.first_frame >= 0)) {
        ...
        if ((wip->laser_color_2.red == 0) && (wip->laser_color_2.green == 0) && (wip->laser_color_2.blue == 0)) {
            ...
            gr_init_color(&c, (int) ((float) wip->laser_color_1.red + (((float) wip->laser_color_2.red - (float) wip->
                                  laser_color_1.red) * pct)),
                          (int) ((float) wip->laser_color_1.green +
                                 (((float) wip->laser_color_2.green - (float) wip->laser_color_1.
                                         green) * pct)),
                          (int) ((float) wip->laser_color_1.blue +
                                 (((float) wip->laser_color_2.blue - (float) wip->laser_color_1.
                                         blue) * pct)));
        }

        vm_vec_scale_add(&headp2, &vmd_zero_vector, &Lab_viewer_orient.vec.fvec,
                         wip->laser_length * weapon_glow_scale_l);
        vm_vec_scale_add(&tailp, &vmd_zero_vector, &Lab_viewer_orient.vec.fvec,
        ...
        if (wip->laser_glow_bitmap.num_frames > 1) {
            ...
            framenum = fl2i((current_glow_frame * wip->laser_glow_bitmap.num_frames) / wip->laser_glow_bitmap.
                    total_time);

            CLAMP(framenum, 0, wip->laser_glow_bitmap.num_frames - 1);
        }

        gr_set_bitmap(wip->laser_glow_bitmap.first_frame + framenum, GR_ALPHABLEND_FILTER, GR_BITBLT_MODE_NORMAL,
                      weapon_glow_alpha);
        g3_draw_laser_rgb(&headp2, wip->laser_head_radius * weapon_glow_scale_f, &tailp, wip->laser_tail_radius *
                                                                                         weapon_glow_scale_r, c.red,
                          c.green, c.blue, TMAP_FLAG_TEXTURED | TMAP_FLAG_XPARENT | TMAP_FLAG_RGB |
                                           TMAP_HTL_3D_UNLIT);
    }
...
void labviewer_do_render(float frametime) {
    ...
    if (frametotal != 0.0f) {
        gr_printf_no_resize(gr_screen.clip_left + 2, gr_screen.clip_bottom - gr_get_font_height(), "FPS: %i",
                            fl2i(Framerate + 0.5f));
    } else {
...
void labviewer_set_class_window(int mode) {
    ...
    if (Lab_class_toolbar == NULL) {
        Lab_class_toolbar = (Window *) Lab_screen->Add(new Window("Class Toolbar", 0, Lab_toolbar->GetHeight(), -1, -1,
                                                                  WS_NOTITLEBAR | WS_NONMOVEABLE));
    }
...
void labviewer_variables_add(int *Y, char *var_name) {
    ...
    new_text = (Text *) Lab_variables_window->AddChild(new Text(std::string((var_name)) + std::string("Editbox"), "",
                                                                VAR_POS_RIGHTX, y, VAR_POS_RIGHTWIDTH, 12,
                                                                T_EDITTABLE));
...
void labviewer_make_variables_window(Button *caller) {
    ...
    Lab_variables_window = (Window *) Lab_screen->Add(new Window("Class Variables",
                                                                 gr_screen.max_w -
                                                                 (VAR_POS_RIGHTX + VAR_POS_RIGHTWIDTH +
                                                                  25), 200));
...
void labviewer_make_render_options_window(Button *caller) {
    ...
    // model flags
    ADD_RENDER_FLAG("Wireframe", Lab_model_flags,
                    ((Cmdline_nohtl) ? MR_SHOW_OUTLINE : MR_SHOW_OUTLINE_HTL) | MR_NO_POLYS);
    ...
    // start tree
    cmp = (Tree *) Lab_render_options_window->AddChild(new Tree("Detail Options Tree", 0, y + 2, NULL,
...
void labviewer_make_desc_window(Button *caller) {
    ...
    Lab_description_window = (Window *) Lab_screen->Add(
            new Window("Description", gr_screen.max_w - gr_screen.max_w / 3 -
                                      50,
                       gr_screen.max_h - gr_screen.max_h / 6 - 50,
                       gr_screen.max_w / 3,
                       gr_screen.max_h / 6));
    Lab_description_text = (Text *) Lab_description_window->AddChild(
            new Text("Description Text", "No ship selected.", 0,
                     0));

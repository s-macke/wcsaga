...
public:
    ...
    ubyte GetColorHandle(int ColorID, int ID = CIE_HANDLE_N) {
        if (this == NULL) {
            ...
            return Handles[ID].Colors[ColorID];
        }
    }
...
public:
    ObjectClassInfoEntry() {
        Object = -1;
        Coords[0] = Coords[1] = Coords[2] = Coords[3] = INT_MAX;
    }
...
protected:
    ...
    void OnRefreshSize() {
        if (DoRefreshSize() != OF_FALSE && Parent != NULL)
            Parent->OnRefreshSize();
    }
    ...
    int GetCIEImageHandle(int id, int handleid = 0) {
        if (InfoEntry != NULL) {
            return InfoEntry->GetImageHandle(id, handleid);
        } else {
            return -1;
        }
    }
...
public:
    ...
    GUIObject(std::string in_Name = "", int x_coord = 0, int y_coord = 0, int x_width = -1, int y_height = -1,
              int in_style = 0);
...
public:
    Button(std::string in_caption, int x_coord, int y_coord, void (*in_function)(Button *caller) = NULL,
           int x_width = -1, int y_height = -1, int in_style = 0);
...
public:
    Tree(std::string in_name, int x_coord, int y_coord, void *in_associateditem = NULL, int x_width = -1,
         int y_width = -1, int in_style = 0);

    //void LoadItemList(TreeItem *in_list, unsigned int count);
    TreeItem *AddItem(TreeItem *parent, std::string in_name, int in_data = 0, bool in_delete_data = true,
                      void (*in_function)(Tree *caller) = NULL);
...
public:
    Text(std::string in_name, std::string in_content, int x_coord, int y_coord, int x_width = -1, int y_width = -1,
         int in_style = 0);
    ...
    void
    SetSaveLoc(short int *sint_ptr, int save_method, short int max_value = SHRT_MAX, short int min_value = SHRT_MIN);
...
public:
    Checkbox(std::string in_label, int x_coord, int y_coord, void (*in_function)(Checkbox *caller) = NULL,
             int x_width = -1, int y_height = DEFAULT_BUTTON_HEIGHT, int in_style = 0);
...
public:
    ImageAnim(std::string in_name, std::string in_imagename, int x_coord, int y_coord, int x_width = -1,
              int y_width = -1, int in_style = 0);

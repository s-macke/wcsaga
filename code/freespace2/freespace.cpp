...
//	int s2, e2;
...
#ifndef NDEBUG
    if (SAGA_VERSION_BUILD == 0 && SAGA_VERSION_REVIS == 0) {
        mprintf(("FreeSpace version: %i.%i\n", SAGA_VERSION_MAJOR, SAGA_VERSION_MINOR));
    } else if (SAGA_VERSION_REVIS == 0) {
        mprintf(("FreeSpace version: %i.%i.%i\n", SAGA_VERSION_MAJOR, SAGA_VERSION_MINOR, SAGA_VERSION_BUILD));
    } else {
        mprintf(("FreeSpace version: %i.%i.%i.%i\n", SAGA_VERSION_MAJOR, SAGA_VERSION_MINOR, SAGA_VERSION_BUILD, SAGA_VERSION_REVIS));
    }
    ...
    std::string root_path(full_path);

    size_t pos = root_path.rfind(".app");
    root_path = root_path.substr(0, pos-1);
    strncpy(whee, root_path.c_str(), MAX_PATH_LEN-1);
#else
...
void game_tst_frame()
{
    ...
    if(tst == 2){
        ...
        snd_play(&Snds[SND_MAIN_HALL_PA_18]);
...
DCF(dcf_fov, "Change the field of view of the main camera")
{
    ...
    if ( Dc_status )
    {
        if(cam == NULL)
            ...
            dc_printf("Zoom factor set to %6.3f (original = 0.5, John = 0.75)", cam->get_zoom());
    }
}
...
void say_view_target()
{
    ...
    if (!(Game_mode & GM_DEAD_DIED) && ((Game_mode & (GM_DEAD_BLEW_UP)) || ((Last_view_target != NULL) && (Last_view_target != view_target)))) {
        if (view_target != Player_obj){
            ...
            case OBJ_SHIP:
                {
                    ship* shipp = &Ships[Objects[Player_ai->target_objnum].instance];
                    if (shipp->flags2 & SF2_HIDE_SHIP_NAME) {
                        strcpy_s(view_target_name, "targeted ship");
                    } else {
                        strcpy_s(view_target_name, shipp->ship_name);
                    }
                    end_string_at_first_hash_symbol(view_target_name, (shipp->wingnum != -1));
                }
                break;
            case OBJ_WEAPON:
...
{
    static int detail_hardware_textures = Detail.hardware_textures;

    if (detail_hardware_textures !=  Detail.hardware_textures)
    {
        detail_hardware_textures = Detail.hardware_textures;
        game_environment_map_gen();
    }
...
camid game_render_frame_setup()
{
    ...
    static float last_zoom = Sexp_zoom;

    fov_changed = ((last_zoom != Sexp_zoom) && (Sexp_zoom > 0.0f));
...
void game_render_frame( camid cid )
{
    ...
    if(cam != NULL)
    {
        ...
        if(Sexp_zoom <= 0.0f)
            g3_set_view_matrix(&eye_pos, &eye_orient, cam->get_zoom());
        else
            g3_set_view_matrix(&eye_pos, &eye_orient, Sexp_zoom);
    }
    else
    {
...
void bars_do_frame(float frametime)
{
    if((Cutscene_bar_flags & CUB_GRADUAL) && Cutscene_bars_progress < 1.0f)
    {
        ...
        //Set clipping
        gr_set_clip(0, yborder, gr_screen.max_w, gr_screen.max_h - yborder*2, false, false );
...
//		flFrametime /= 15.0;
    ...
    if ((gameseq_get_state() == GS_STATE_GAME_PLAY) && (Use_mouse_to_fly == 2)) {
    ...
    */
    ...
    switch (new_state) {
        ...
        case GS_STATE_START_GAME:
            ...
            {
                mission_campaign_maybe_play_movie(CAMPAIGN_MOVIE_INTRO);
            }
...
void mouse_force_pos(int x, int y);
            ...
            // Moved here from HUD_init() to make sure this happens AFTER the game clock has restarted.
            hud_reset_throttle_sound_check_id();
...
int PASCAL WinMain(HINSTANCE hInst, HINSTANCE hPrev, LPSTR szCmdLine, int nCmdShow)
{
    ...
    snprintf(userdir, MAX_PATH - 1, "%s%s%s%s%s%s", detect_home(), DIR_SEPARATOR_STR, Osreg_company_name, DIR_SEPARATOR_STR, Osreg_user_dir, DIR_SEPARATOR_STR);
    cf_create_directory(userdir);
...
#ifdef GAME_ERRORLOG_TXT
#ifdef _MSC_VER
    } __except( RecordExceptionInfo(GetExceptionInformation(), "Wing Commander Saga Main Thread") ) {
...
{
    if (Steering_cursor_bitmap != -1 && (gameseq_get_state() == GS_STATE_GAME_PLAY) && (Use_mouse_to_fly == 1) &&
        !(popup_active() || popupdead_is_active())) {
        gr_set_cursor_bitmap(Steering_cursor_bitmap);
    } else {
        int				mx, my;
        animating_obj	*am;
...
void game_maybe_draw_mouse(float frametime)
{
    ...
    switch ( game_state ) {
        ...
        // case GS_STATE_MULTI_PAUSED:
        case GS_STATE_GAME_PLAY:
            if ( popup_active() || popupdead_is_active() || ((Use_mouse_to_fly == 1) && !(Viewer_mode & VM_NOT_COCKPIT)) ) {
                Mouse_hidden = 0;
            } else {
                Mouse_hidden = 1;
            }
...
// Call this function when you want to inform the player that a feature is not supported in Saga yet.
void game_feature_not_supported_by_saga_yet()
{
    popup(PF_USE_AFFIRMATIVE_ICON|PF_BODY_BIG, 1, POPUP_OK, XSTR( "Sorry, this feature is currently not supported.", -1));
}
...
//XSTR:OFF
    ...
    if (SAGA_VERSION_BUILD == 0 && SAGA_VERSION_REVIS == 0) {
        sprintf(str, "Wing Commander Saga v%i.%i", SAGA_VERSION_MAJOR, SAGA_VERSION_MINOR);
    } else if (SAGA_VERSION_REVIS == 0) {
        sprintf(str, "Wing Commander Saga v%i.%i.%i", SAGA_VERSION_MAJOR, SAGA_VERSION_MINOR, SAGA_VERSION_BUILD);
    } else {
        sprintf(str, "Wing Commander Saga v%i.%i.%i.%i", SAGA_VERSION_MAJOR, SAGA_VERSION_MINOR, SAGA_VERSION_BUILD, SAGA_VERSION_REVIS);
    }
...
{
    sprintf(str,"v%d.%d", SAGA_VERSION_MAJOR, SAGA_VERSION_MINOR);
}

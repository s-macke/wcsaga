...
int sexp_tree::save_branch(int cur, int at_root) {
    ...
    while (cur != -1) {
        if (tree_nodes[cur].type & SEXPT_OPERATOR) {
            node = alloc_sexp(tree_nodes[cur].text, SEXP_ATOM, SEXP_ATOM_OPERATOR, -1,
                              save_branch(tree_nodes[cur].child));
		...
		*/
        {
            ...
            if (item_index >= 0) {
                ...
                if (parent >= 0) {
                    ...
                    int op_type = query_operator_argument_type(op,
                                                               Replace_count); // check argument type at this position
        ...
        for (i = 0; i < Num_submenus; i++) {
            ...
            for (j = 0; j < Num_op_menus; j++) {
                if (op_menu[j].id == category_of_subcategory(op_submenu[i].id)) {
                    add_op_submenu[j].AppendMenu(MF_POPUP, (UINT) add_op_subcategory_menu[i].m_hMenu,
                                                 op_submenu[i].name);
                    replace_op_submenu[j].AppendMenu(MF_POPUP, (UINT) replace_op_subcategory_menu[i].m_hMenu,
                                                     op_submenu[i].name);
                    insert_op_submenu[j].AppendMenu(MF_POPUP, (UINT) insert_op_subcategory_menu[i].m_hMenu,
                                                    op_submenu[i].name);
...
/*#ifdef NDEBUG
                        ...
                        if (j < Num_op_menus) {
                            ...
                            replace_op_submenu[j].AppendMenu(MF_STRING | MF_GRAYED,
                                                             Operators[i].value | OP_REPLACE_FLAG, Operators[i].text);
                            insert_op_submenu[j].AppendMenu(MF_STRING, Operators[i].value | OP_INSERT_FLAG,
                                                            Operators[i].text);
                        }
...
/*#ifdef NDEBUG
                        ...
                        if (j < Num_submenus) {
                            add_op_subcategory_menu[j].AppendMenu(MF_STRING | MF_GRAYED, Operators[i].value,
                                                                  Operators[i].text);
                            replace_op_subcategory_menu[j].AppendMenu(MF_STRING | MF_GRAYED,
                                                                      Operators[i].value | OP_REPLACE_FLAG,
                                                                      Operators[i].text);
                            insert_op_subcategory_menu[j].AppendMenu(MF_STRING, Operators[i].value | OP_INSERT_FLAG,
                                                                     Operators[i].text);
                        }
                ...
                while (ptr) {
                    if (ptr->op >= 0) {
                        ...
                        if ((data_idx + 3) % 30) {
                            ...
                            add_data_menu->AppendMenu(MF_MENUBARBREAK | MF_STRING | MF_ENABLED, ID_ADD_MENU + data_idx,
                                                      ptr->text);
                        }
                    }
            ...
            if ((type != OPF_VARIABLE_NAME) && list) {
                ...
                while (ptr) {
                    if (ptr->op >= 0) {
                        ...
                        if ((data_idx + 3) % 30)
                            replace_data_menu->AppendMenu(MF_STRING | MF_ENABLED, ID_REPLACE_MENU + data_idx,
                                                          ptr->text);
                        else
                            replace_data_menu->AppendMenu(MF_MENUBARBREAK | MF_STRING | MF_ENABLED,
                                                          ID_REPLACE_MENU + data_idx, ptr->text);
                    }
...
BOOL sexp_tree::OnCommand(WPARAM wParam, LPARAM lParam) {
    ...
    for (op = 0; op < Num_operators; op++) {
        ...
        if (id == (Operators[op].value | OP_INSERT_FLAG)) {
            ...
            item_handle = tree_nodes[node].handle = insert(Operators[op].text, BITMAP_OPERATOR, BITMAP_OPERATOR, h,
                                                           tree_nodes[item_index].handle);
        ...
        case OPF_AMBIGUOUS:
            ...
            if ((query_operator_return_type(op) == OPR_AI_GOAL) && (i == (Operators[op].min - 1))) {
                item->set_data("89", (SEXPT_NUMBER | SEXPT_VALID));
                    ...
                    ((Operators[op].value == OP_HAS_DOCKED_DELAY) || (Operators[op].value == OP_HAS_UNDOCKED_DELAY)) &&
                    (i == 2)) {
                item->set_data("1", (SEXPT_NUMBER | SEXPT_VALID));
            } else if ((Operators[op].value == OP_SHIP_TYPE_DESTROYED) ||
                       (Operators[op].value == OP_GOOD_SECONDARY_TIME)) {
...
void sexp_tree::add_operator(char *op, HTREEITEM h) {
    ...
    if (item_index == -1) {
        ...
        node = allocate_node(item_index);
        set_node(node, (SEXPT_OPERATOR | SEXPT_VALID), op);
        item_handle = tree_nodes[node].handle = insert(op, BITMAP_OPERATOR, BITMAP_OPERATOR,
                                                       tree_nodes[item_index].handle);
    }
        ...
        if ((ptr->type == OBJ_SHIP) || (ptr->type == OBJ_START)) {
            if (op == OP_AI_DOCK) {
                ...
                if (((Ship_info[Ships[ptr->instance].ship_info_index].flags & SIF_HUGE_SHIP) &&    // big ship
                     !(Ships[ptr->instance].flags2 & SF2_TOGGLE_SUBSYSTEM_SCANNING)) ||
                    // which is not flagged OR
                    ((!(Ship_info[Ships[ptr->instance].ship_info_index].flags & SIF_HUGE_SHIP)) &&  // small ship
                     (Ships[ptr->instance].flags2 &
                      SF2_TOGGLE_SUBSYSTEM_SCANNING))) {                // which is flagged

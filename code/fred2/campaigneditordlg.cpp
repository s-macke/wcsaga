...
void campaign_editor::OnLoad() {
    ...
    cf_find_file_location(Campaign.missions[Cur_campaign_mission].name, CF_TYPE_MISSIONS, 512, buf, &size, &offset,
                          false);
...
BOOL campaign_editor::Create(LPCTSTR lpszClassName, LPCTSTR lpszWindowName, DWORD dwStyle, const RECT &rect,
...
void campaign_editor::OnEnChangeCampaignIntroCutscene() {
    CEdit *bc_dialog;
...
void campaign_editor::OnEnChangeCampaignEndCutscene() {
    CEdit *bc_dialog;

...
void fred_enable_htl() {
    if (!Briefing_dialog) gr_set_proj_matrix((4.0f / 9.0f) * 3.14159f * FRED_DEFAULT_HTL_FOV,
                                             gr_screen.aspect * (float) gr_screen.clip_width /
                                             (float) gr_screen.clip_height, 1.0f, FRED_DEAFULT_HTL_DRAW_DIST);
    if (Briefing_dialog) gr_set_proj_matrix((4.0f / 9.0f) * 3.14159f * FRED_BRIEFING_HTL_FOV,
                                            gr_screen.aspect * (float) gr_screen.clip_width /
                                            (float) gr_screen.clip_height, 1.0f, FRED_DEAFULT_HTL_DRAW_DIST);
...
//		if (!viewpoint || OBJ_INDEX(objp) != cur_object_index)
        {
            if (Fred_outline) {
                ...
                model_render(Ship_info[Ships[z].ship_info_index].model_num, &objp->orient, &objp->pos,
                             j | MR_SHOW_OUTLINE, -1, -1, Ships[z].ship_replacement_textures);
            } else {
                model_render(Ship_info[Ships[z].ship_info_index].model_num, &objp->orient, &objp->pos, j, -1, -1,
                             Ships[z].ship_replacement_textures);
            }
        }
...
void render_frame() {
    ...
    g3_set_view_matrix(&eye_pos, &eye_orient, Viewer_zoom_FRED); // Former 0.5f zoom. Now using different setting.
    ...
    g3_start_frame(0);     // ** Accounted for
    g3_set_view_matrix(&eye_pos, &eye_orient, Viewer_zoom_FRED);
}

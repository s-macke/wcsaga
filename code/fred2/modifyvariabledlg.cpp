...
void CModifyVariableDlg::OnSelchangeModifyVariableName() {
    ...
    m_type_campaign_persistent = ((Sexp_variables[sexp_variable_index].type & SEXP_VARIABLE_CAMPAIGN_PERSISTENT) != 0)
                                 ? true : false;
    m_type_player_persistent = ((Sexp_variables[sexp_variable_index].type & SEXP_VARIABLE_PLAYER_PERSISTENT) != 0)
                               ? true : false;
...
BOOL CModifyVariableDlg::OnInitDialog() {
    ...
    m_type_campaign_persistent = ((Sexp_variables[last_modified].type & SEXP_VARIABLE_CAMPAIGN_PERSISTENT) != 0) ? true
                                                                                                                 : false;
    m_type_player_persistent = ((Sexp_variables[last_modified].type & SEXP_VARIABLE_PLAYER_PERSISTENT) != 0) ? true
                                                                                                             : false;

...
void VoiceActingManager::OnGenerateFileNames() {
    ...
    for (i = 0; i < Cmd_briefs[0].num_stages; i++) {
        ...
        if (!m_no_replace || !strlen(filename) || !strnicmp(filename, "none.wav", 4) ||
    ...
    for (i = 0; i < Num_messages - Num_builtin_messages; i++) {
        ...
        // generate only if we're replacing or if it has a replaceable name
        if (!m_no_replace || !filename || !strlen(filename) || !strnicmp(filename, "none.wav", 4) ||
            message_filename_is_generic(filename)) {
            ...
            Messages[i + Num_builtin_messages].wave_info.name = strdup(
                    LPCTSTR(generate_filename(m_abbrev_message, i + 1, digits)));
        }
    }

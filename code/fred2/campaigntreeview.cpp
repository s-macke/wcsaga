...
void campaign_tree_view::realign_tree() {
    ...
    for (i = 0; i < Campaign.num_missions; i++) {
        ...
        for (j = 0; j < Total_links; j++)
            if (Links[j].to == z) {
                Assert(Campaign.missions[Links[j].from].level <=
                       Campaign.missions[z].level);  // links can't go up the tree, only down
    ...
    if (Campaign.type != CAMPAIGN_TYPE_SINGLE) {
        ...
        if (Campaign.num_players != 0) {
            if (Campaign.num_players != a_mission.num_players) {
                ...
                sprintf(buf,
                        "Mission \"%s\" has %d players.  Campaign has %d players.  Campaign will not play properly in FreeSpace",
                        name, a_mission.num_players, Campaign.num_players);
...
void campaign_tree_view::horizontally_align_mission(int num, int dir) {
    ...
    if ((Campaign.missions[num].pos == -1) ||
        (Campaign.missions[num].pos + 1 == total_width * 2)) {  // need to expand total_width
        for (i = 0; i < Campaign.num_missions; i++)

...
void campaign_tree_wnd::OnCpgnFileOpen() {
    ...
    CFileDialog dlg(TRUE, "fc2", NULL, OFN_HIDEREADONLY | OFN_OVERWRITEPROMPT,
                    "FreeSpace Campaign files (*.fc2)|*.fc2||", this);
    if (dlg.DoModal() == IDOK) {
        ...
        if (strlen(pathname) > MAX_PATH_LEN - 1) {
            MessageBox("Pathname is too long", "Error");
            return;
        }
        ...
        string_copy(Campaign.pathname, pathname, MAX_PATH_LEN);
        Campaign_tree_formp->load_campaign();
    }
}
...
void campaign_tree_wnd::OnCpgnFileSave() {
    ...
    */
    ...
    if (save.save_campaign_file(Campaign.pathname)) {
...
void campaign_tree_wnd::OnCpgnFileSaveAs() {
    ...
    Campaign_tree_formp->update();
    if (Campaign.pathname[0])
        old_name = Campaign.pathname;

    CFileDialog dlg(FALSE, "fc2", old_name, OFN_HIDEREADONLY | OFN_OVERWRITEPROMPT,
                    "FreeSpace Campaign files (*.fc2)|*.fc2||", this);
    if (dlg.DoModal() == IDOK) {
        name = dlg.GetFileName();
        if (strlen(name) > MAX_FILENAME_LEN - 1) {
        ...
        string_copy(Campaign.filename, name, MAX_FILENAME_LEN);
        string_copy(Campaign.pathname, dlg.GetPathName(), MAX_PATH_LEN);
        if (save.save_campaign_file(Campaign.pathname)) {
            MessageBox("An error occured while saving!", "Error", MB_OK | MB_ICONEXCLAMATION);
            return;
        }
...
int campaign_tree_wnd::error_checker() {
    ...
    for (i = 0; i < Total_links; i++) {
        ...
        if (Links[i].sexp == Locked_sexp_true) {
            if (true_at[z] >= 0)
                if (error("Mission \"%s\" branch %d is true but is not last branch", Campaign.missions[z].name,
                          true_at[z]))
                    return 1;
    ...
    if (Campaign.type != CAMPAIGN_TYPE_SINGLE) {
        for (i = 0; i < Campaign.num_missions; i++) {
            ...
            if (a_mission.num_players != Campaign.num_players) {
                if (error("Mission \"%s\" has %d players.  Multiplayer campaign allows %d", Campaign.missions[i].name,
                          a_mission.num_players, Campaign.num_players))
                    return 1;
            }
        }
    }

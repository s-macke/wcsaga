...
void briefing_editor_dlg::update_data(int update) {
    ...
    if (m_last_stage >= 0) {
                    ...
                    for (j = i + 1; j < ptr->num_icons; j++) {
                        if (icon_marked[i] && icon_marked[j]) {
                            for (l = 0; l < ptr->num_lines; l++)
                                if (((ptr->lines[l].start_icon == i) && (ptr->lines[l].end_icon == j)) ||
                                    ((ptr->lines[l].start_icon == j) && (ptr->lines[l].end_icon == i)))
                                    break;
        ...
        i = ptr->num_lines;
        while (i--) {
            if (ptr->lines[i].start_icon < 0 || ptr->lines[i].end_icon < 0 ||
                ptr->lines[i].start_icon >= ptr->num_icons || ptr->lines[i].end_icon >= ptr->num_icons) {
                ptr->num_lines--;
                for (l = i; l < ptr->num_lines; l++)
                    ptr->lines[l] = ptr->lines[l + 1];
            }
                    ...
                    for (i = m_last_stage + 1; i < Briefing->num_stages; i++) {
                        if (find_icon(m_id, i) >= 0) {
                            ...
                            sprintf(msg, "Icon ID #%d is already used in a later stage.  You can only\n"
                                         "change to that ID locally.  Icon ID has been reset back to %d", m_id,
                                    ptr->icons[m_last_icon].id);
...
//		m_icon_text = convert_multiline_string(ptr->icons[m_cur_icon].text);
    ...
    while (objp != END_OF_LIST(&obj_used_list)) {
        if (objp->flags & OF_MARKED) {
            if ((objp->type == OBJ_SHIP) || (objp->type == OBJ_START) || (objp->type == OBJ_WAYPOINT) ||
                (objp->type == OBJ_JUMP_NODE))
                valid = 1;
            else
                invalid = 1;
        }

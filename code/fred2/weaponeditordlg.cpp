...
void WeaponEditorDlg::DoDataExchange(CDataExchange *pDX) {
    ...
    DDX_CBIndex(pDX, IDC_MISSILE5, m_missile5);
    DDX_CBIndex(pDX, IDC_MISSILE6, m_missile6);
    ...
    if (pDX->m_bSaveAndValidate) {
        ...
        if (save_number((char *) (LPCSTR) str, &m_ammo1)) {
            m_ammo_max1 = (m_missile1 <= 0) ? 0 : get_max_ammo_count_for_bank(m_ship_class, 0,
                                                                              m_missile1 + First_secondary_index - 1);
            if (m_ammo1 < 0)
                m_ammo1 = 0;
        ...
        if (save_number((char *) (LPCSTR) str, &m_ammo2)) {
            m_ammo_max2 = (m_missile2 <= 0) ? 0 : get_max_ammo_count_for_bank(m_ship_class, 1,
                                                                              m_missile2 + First_secondary_index - 1);
            if (m_ammo2 < 0)
                m_ammo2 = 0;
        ...
        if (save_number((char *) (LPCSTR) str, &m_ammo3)) {
            m_ammo_max3 = (m_missile3 <= 0) ? 0 : get_max_ammo_count_for_bank(m_ship_class, 2,
                                                                              m_missile3 + First_secondary_index - 1);
            if (m_ammo3 < 0)
                m_ammo3 = 0;
        ...
        if (save_number((char *) (LPCSTR) str, &m_ammo4)) {
            m_ammo_max4 = (m_missile4 <= 0) ? 0 : get_max_ammo_count_for_bank(m_ship_class, 3,
                                                                              m_missile4 + First_secondary_index - 1);
            if (m_ammo4 < 0)
                m_ammo4 = 0;
        ...
        GetDlgItem(IDC_AMMO5)->GetWindowText(str);
        if (save_number((char *) (LPCSTR) str, &m_ammo5)) {
            m_ammo_max5 = (m_missile5 <= 0) ? 0 : get_max_ammo_count_for_bank(m_ship_class, 4,
                                                                              m_missile5 + First_secondary_index - 1);
            if (m_ammo5 < 0)
                m_ammo5 = 0;
            if (m_ammo5 > m_ammo_max5)
                m_ammo5 = m_ammo_max5;
        }

        GetDlgItem(IDC_AMMO6)->GetWindowText(str);
        if (save_number((char *) (LPCSTR) str, &m_ammo6)) {
            m_ammo_max6 = (m_missile6 <= 0) ? 0 : get_max_ammo_count_for_bank(m_ship_class, 5,
                                                                              m_missile6 + First_secondary_index - 1);
            if (m_ammo6 < 0)
                m_ammo6 = 0;
            if (m_ammo6 > m_ammo_max6)
                m_ammo6 = m_ammo_max6;
        }
        ...
        m_spin5.SetRange(0, (short) m_ammo_max5);
        m_spin6.SetRange(0, (short) m_ammo_max6);
        ...
        if (m_ammo5 != BLANK_FIELD)
            DDX_Text(pDX, IDC_AMMO5, m_ammo5);
        else
            GetDlgItem(IDC_AMMO5)->SetWindowText("");

        if (m_ammo6 != BLANK_FIELD)
            DDX_Text(pDX, IDC_AMMO6, m_ammo6);
        else
            GetDlgItem(IDC_AMMO6)->SetWindowText("");
    }
}
...
ON_CBN_SELCHANGE(IDC_MISSILE5, OnSelchangeMissile5
)
ON_CBN_SELCHANGE(IDC_MISSILE6, OnSelchangeMissile6
)
...
BOOL WeaponEditorDlg::OnInitDialog() {
    ...
    box = (CComboBox *) GetDlgItem(IDC_MISSILE5);
    box->AddString("None");
    for (i = First_secondary_index; i < end2; i++) {
        box->AddString(Weapon_info[i].name);
    }

    box = (CComboBox *) GetDlgItem(IDC_MISSILE6);
    box->AddString("None");
    for (i = First_secondary_index; i < end2; i++) {
        box->AddString(Weapon_info[i].name);
    }
...
void WeaponEditorDlg::change_selection() {
    ...
    GetDlgItem(IDC_AMMO5)->GetWindowText(a5);
    GetDlgItem(IDC_AMMO6)->GetWindowText(a6);
    ...
    if (m_last_item >= 0) {
        ...
        if (m_missile5 > 0)
            m_missile5 += First_secondary_index;

        cur_weapon->secondary_bank_weapons[4] = m_missile5 - 1;
        if (m_missile6 > 0)
            m_missile6 += First_secondary_index;

        cur_weapon->secondary_bank_weapons[5] = m_missile6 - 1;
        ...
        cur_weapon->secondary_bank_ammo[4] = m_ammo_max5 ? (m_ammo5 * 100 / m_ammo_max5) : 0;
        cur_weapon->secondary_bank_ammo[5] = m_ammo_max6 ? (m_ammo6 * 100 / m_ammo_max6) : 0;
        if (m_multi_edit) {
            if (!strlen(a1))
            ...
            if (!strlen(a5))
                cur_weapon->secondary_bank_ammo[4] = BLANK_FIELD;
            if (!strlen(a6))
                cur_weapon->secondary_bank_ammo[5] = BLANK_FIELD;
        }
    }

    m_gun1 = m_gun2 = m_gun3 = m_missile1 = m_missile2 = m_missile3 = m_missile4 = m_missile5 = m_missile6 = -1;
    m_ammo1 = m_ammo2 = m_ammo3 = m_ammo4 = m_ammo5 = m_ammo6 = BLANK_FIELD;
    m_ammo_max1 = m_ammo_max2 = m_ammo_max3 = m_ammo_max4 = m_ammo_max5 = m_ammo_max6 = 0;
    if (m_cur_item < 0) {
        ...
        GetDlgItem(IDC_MISSILE5)->EnableWindow(FALSE);
        GetDlgItem(IDC_MISSILE6)->EnableWindow(FALSE);
        ...
        GetDlgItem(IDC_AMMO5)->EnableWindow(FALSE);
        GetDlgItem(IDC_AMMO6)->EnableWindow(FALSE);
    ...
    if (cur_weapon->num_secondary_banks > 0) {
        ...
        if (m_missile1 > 0) {
            ...
            if (cur_weapon->secondary_bank_ammo[0] != BLANK_FIELD)
                m_ammo1 = (int) ((cur_weapon->secondary_bank_ammo[0] * m_ammo_max1 / 100.0f) + 0.5f);
            m_missile1 -= First_secondary_index;
        }
    ...
    if (cur_weapon->num_secondary_banks > 1) {
        ...
        if (m_missile2 > 0) {
            ...
            if (cur_weapon->secondary_bank_ammo[1] != BLANK_FIELD)
                m_ammo2 = (int) ((cur_weapon->secondary_bank_ammo[1] * m_ammo_max2 / 100.0f) + 0.5f);
            m_missile2 -= First_secondary_index;
        }
    ...
    if (cur_weapon->num_secondary_banks > 2) {
        ...
        if (m_missile3 > 0) {
            ...
            if (cur_weapon->secondary_bank_ammo[2] != BLANK_FIELD)
                m_ammo3 = (int) ((cur_weapon->secondary_bank_ammo[2] * m_ammo_max3 / 100.0f) + 0.5f);
            m_missile3 -= First_secondary_index;
        }
    ...
    if (cur_weapon->num_secondary_banks > 3) {
        ...
        if (m_missile4 > 0) {
            ...
            if (cur_weapon->secondary_bank_ammo[3] != BLANK_FIELD)
                m_ammo4 = (int) ((cur_weapon->secondary_bank_ammo[3] * m_ammo_max4 / 100.0f) + 0.5f);
            m_missile4 -= First_secondary_index;
        }
    ...
    if (cur_weapon->num_secondary_banks > 4) {
        m_missile5 = cur_weapon->secondary_bank_weapons[4] + 1;
        if (m_missile5 > 0) {
            m_ammo_max5 = get_max_ammo_count_for_bank(m_ship_class, 4, m_missile5 - 1);
            if (cur_weapon->secondary_bank_ammo[4] != BLANK_FIELD)
                m_ammo5 = (int) ((cur_weapon->secondary_bank_ammo[4] * m_ammo_max5 / 100.0f) + 0.5f);
            m_missile5 -= First_secondary_index;
        }

        GetDlgItem(IDC_MISSILE5)->EnableWindow(TRUE);
        GetDlgItem(IDC_AMMO5)->EnableWindow(TRUE);
    ...
    if (cur_weapon->num_secondary_banks > 5) {
        m_missile6 = cur_weapon->secondary_bank_weapons[5] + 1;
        if (m_missile6 > 0) {
            m_ammo_max6 = get_max_ammo_count_for_bank(m_ship_class, 5, m_missile6 - 1);
            if (cur_weapon->secondary_bank_ammo[5] != BLANK_FIELD)
                m_ammo6 = (int) ((cur_weapon->secondary_bank_ammo[5] * m_ammo_max6 / 100.0f) + 0.5f);
            m_missile6 -= First_secondary_index;
        }

        GetDlgItem(IDC_MISSILE6)->EnableWindow(TRUE);
        GetDlgItem(IDC_AMMO6)->EnableWindow(TRUE);
...
void WeaponEditorDlg::OnSelchangeMissile5() {
    UpdateData(TRUE);
    UpdateData(TRUE);
    m_ammo_max5 = get_max_ammo_count_for_bank(m_ship_class, 4, m_missile5 + First_secondary_index - 1);
    m_ammo5 = m_ammo_max5 ? (m_ammo_max5) : 0;
    change_selection();
}

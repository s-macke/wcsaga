...
void CMessageEditorDlg::update_cur_message() {
    ...
    if (m_cur_msg < 0) {
        ...
        if (Messages[m_cur_msg].persona_index != -1)
            m_persona =
                    Messages[m_cur_msg].persona_index + 1;        // add one for the "none" at the beginning of the list
        else
            m_persona = 0;
...
int CMessageEditorDlg::find_event() {
    ...
    for (i = 0; i < Num_mission_events; i++) {
        ...
        if (get_operator_const(CTEXT(node)) == OP_WHEN || get_operator_const(CTEXT(node)) == OP_EVERY_TIME
            || get_operator_const(CTEXT(node)) == OP_WHEN_ARGUMENT ||
            get_operator_const(CTEXT(node)) == OP_EVERY_TIME_ARGUMENT) {
            ...
            if (CDR(CDR(node)) == -1) {  // only 1 action
                node = CADR(node);
                if ((get_operator_const(CTEXT(node)) == OP_SEND_MESSAGE) &&
                    !stricmp(CTEXT(CDR(CDR(CDR(node)))), m_message_name)) {
                    box = (CComboBox *) GetDlgItem(IDC_SENDER);
                    str = CTEXT(CDR(node));

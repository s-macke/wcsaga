...
void ship_flags_dlg::DoDataExchange(CDataExchange *pDX) {
    ...
    DDX_Control(pDX, IDC_IS_HARMLESS, m_is_harmless);
...
ON_BN_CLICKED(IDC_IS_HARMLESS, OnBnClickedIsHarmless
)
...
BOOL ship_flags_dlg::OnInitDialog() {
    ...
    int hide_log_entries = 0, is_harmless = 0;
        ...
        if ((objp->type == OBJ_START) || (objp->type == OBJ_SHIP)) {
            if (objp->flags & OF_MARKED) {
                ...
                if (first) {
                    ...
                    hide_log_entries = (shipp->flags2 & SF2_HIDE_LOG_ENTRIES) ? 1 : 0;
                    is_harmless = (shipp->flags2 & SF2_IS_HARMLESS) ? 1 : 0;
                    ...
                    toggle_subsystem_scanning = tristate_set(shipp->flags2 & SF2_TOGGLE_SUBSYSTEM_SCANNING,
                                                             toggle_subsystem_scanning);
                    ...
                    set_class_dynamically = tristate_set(shipp->flags2 & SF2_SET_CLASS_DYNAMICALLY,
                                                         set_class_dynamically);
                    ...
                    friendly_stealth_invisible = tristate_set(shipp->flags2 & SF2_FRIENDLY_STEALTH_INVIS,
                                                              friendly_stealth_invisible);
                    ...
                    hide_log_entries = tristate_set(shipp->flags2 & SF2_HIDE_LOG_ENTRIES, hide_log_entries);
                    is_harmless = tristate_set(shipp->flags2 & SF2_IS_HARMLESS, is_harmless);
                    ...
                    destroy_before_mission = tristate_set(shipp->flags & SF_KILL_BEFORE_MISSION,
                                                          destroy_before_mission);
    ...
    m_is_harmless.SetCheck(is_harmless);
...
void ship_flags_dlg::OnBnClickedIsHarmless() {
    if (m_is_harmless.GetCheck() == 1) {
        m_is_harmless.SetCheck(0);
    } else {
        m_is_harmless.SetCheck(1);
    }
}

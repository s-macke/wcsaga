...
BOOL initial_status::OnInitDialog() {
    ...
    if (!m_multi_edit) {
        for (int dockpoint = 0; dockpoint < num_dock_points; dockpoint++) {
            z = lstDockerPoints->AddString(
                    model_get_dock_name(Ship_info[Ships[m_ship].ship_info_index].model_num, dockpoint));
            lstDockerPoints->SetItemData(z, dockpoint);
        }

        for (ptr = GET_FIRST(&Ships[m_ship].subsys_list);
             ptr != END_OF_LIST(&Ships[m_ship].subsys_list); ptr = GET_NEXT(ptr)) {
        ...
        if (m_force_shields) {
            ...
            if (m_multi_edit) {
                MessageBox(
                        ...
                        "Resetting Flag", MB_OK | MB_ICONEXCLAMATION);
            }
        }
    } else
        m_has_shields = 1;
...
void initial_status::change_subsys() {
    ...
    ship_has_scannable_subsystems = Ships[m_ship].flags2 & SF2_TOGGLE_SUBSYSTEM_SCANNING;
...
void initial_status::change_dockee(bool store_selection) {
    ...
    if (cur_dockee < 0) {
        ...
        if (dockpoint_array[cur_docker_point].dockee_point >= 0) {
            ...
            cboDockeePoints->SelectString(-1,
                                          model_get_dock_name(Ship_info[Ships[cur_dockee].ship_info_index].model_num,
                                                              dockpoint_array[cur_docker_point].dockee_point));

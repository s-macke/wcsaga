...
void CShipEditorDlg::DoDataExchange(CDataExchange *pDX) {
    ...
    DDX_Control(pDX, IDC_NO_ARRIVAL_LOG, m_no_arrival_log);
    DDX_Control(pDX, IDC_NO_DEPARTURE_LOG, m_no_departure_log);
...
ON_BN_CLICKED(IDC_NO_ARRIVAL_LOG, OnBnClickedNoArrivalLog
)
ON_BN_CLICKED(IDC_NO_DEPARTURE_LOG, OnBnClickedNoDepartureLog
)
...
BOOL
CShipEditorDlg::Create(LPCTSTR lpszClassName, LPCTSTR lpszWindowName, DWORD dwStyle, const RECT &rect, CWnd *pParentWnd,
...
void CShipEditorDlg::initialize_data(int full_update) {
    ...
    if (ship_count) {
        ...
        m_no_arrival_log.SetCheck(no_arrival_log);
        m_no_departure_log.SetCheck(no_departure_log);
    ...
    if (!cue_init) {
        ...
        GetDlgItem(IDC_NO_ARRIVAL_LOG)->EnableWindow(FALSE);
        GetDlgItem(IDC_NO_DEPARTURE_LOG)->EnableWindow(FALSE);
        ...
        GetDlgItem(IDC_NO_ARRIVAL_LOG)->EnableWindow(enable);
        GetDlgItem(IDC_NO_DEPARTURE_LOG)->EnableWindow(enable);
    }
    ...
    if ((m_ship_class >= 0) && !(Ship_info[m_ship_class].flags & SIF_CARGO) &&
        !(Ship_info[m_ship_class].flags & SIF_NO_SHIP_TYPE))
...
int CShipEditorDlg::update_ship(int ship) {
    ...
    if (Ships[ship].wingnum < 0) {
        ...
        if (m_arrival_target >= 0) {
            ...
            if (((m_arrival_location != ARRIVE_FROM_DOCK_BAY) && (m_arrival_location != ARRIVE_AT_LOCATION)) &&
                (z >= 0) && !(z & SPECIAL_ARRIVAL_ANCHOR_FLAG)) {
...
void CShipEditorDlg::OnSelchangeHotkey() {
    ...
    if (set_num < MAX_STARTING_WINGS && set_num >= 0) {

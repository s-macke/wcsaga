        ...
        if (Format_fs2_open != FSO_FORMAT_RETAIL) {
            ...
            for (uint i = 0; i < The_mission.cutscenes.size(); i++) {
                if (strlen(The_mission.cutscenes[i].cutscene_name)) {
                    ...
                    if (optional_string_fred("+campaign_only", "$", "#end")) {
                        parse_comments();
                    }
            ...
            MessageBox(NULL,
                       "Warning: This mission contains cutscene data, but you are saving in the retail mission format. This infomration will be lost",
                       "Incompatibility with retail mission format", MB_OK);
        }
    }
...
//		parse_comments();
        ...
        for (i = 0; i < num_variables; i++) {
            ...
            fout("\n\t\t%d\t\t\"%s\"\t\t\"%s\"\t\t\"%s\"", i, Sexp_variables[i].variable_name, Sexp_variables[i].text,
                 type);
...
//			parse_comments();
        ...
        for (i = MAX_SEXP_VARIABLES - num_block_vars; i < MAX_SEXP_VARIABLES; i++) {
            type = block;
            fout("\n\t\t%d\t\t\"%s\"\t\t\"%s\"\t\t\"%s\"", i, Block_variables[i].variable_name, Block_variables[i].text,
                 type);
        }
    ...
    for (i = 0; i < Num_teams; i++) {
        ...
        for (j = 0; j < Team_data[i].num_ship_choices; j++) {
            ...
            if (Team_data[i].ship_list_variables[j] != -1) {
                Assert(Team_data[i].ship_list_variables[j] > -1 &&
                       Team_data[i].ship_list_variables[j] < MAX_SEXP_VARIABLES);
            ...
            if (Team_data[i].ship_count_variables[j] != -1) {
                Assert(Team_data[i].ship_count_variables[j] > -1 &&
                       Team_data[i].ship_count_variables[j] < MAX_SEXP_VARIABLES);
        ...
        for (j = 0; j < Team_data[i].num_weapon_choices; j++) {
            ...
            if (Team_data[i].weaponry_pool_variable[j] != -1) {
                Assert(Team_data[i].weaponry_pool_variable[j] > -1 &&
                       Team_data[i].weaponry_pool_variable[j] < MAX_SEXP_VARIABLES);
            ...
            if (Team_data[i].weaponry_amount_variable[j] != -1) {
                Assert(Team_data[i].weaponry_amount_variable[j] > -1 &&
                       Team_data[i].weaponry_amount_variable[j] < MAX_SEXP_VARIABLES);
...
void CFred_mission_save::save_single_dock_instance(ship *shipp, dock_instance *dock_ptr) {
    ...
    parse_comments();
    fout(" %s", model_get_dock_name(Ship_info[other_shipp->ship_info_index].model_num,
    ...
    parse_comments();
    fout(" %s", model_get_dock_name(Ship_info[shipp->ship_info_index].model_num,
...
int CFred_mission_save::save_objects() {
    ...
    for (i = z = 0; i < MAX_SHIPS; i++) {
            ...
            if ((int) Ships[i].s_alt_classes.size()) {
                for (k = 0; k < (int) Ships[i].s_alt_classes.size(); k++) {
                    ...
                    if (Ships[i].s_alt_classes[k].variable_index != -1) {
                        fout_version("\n;;FSO 3.6.10;; $Alt Ship Class: @%s",
                                     Sexp_variables[Ships[i].s_alt_classes[k].variable_index].variable_name);
                    } else {
                        fout_version("\n;;FSO 3.6.10;; $Alt Ship Class: \"%s\"",
                                     Ship_info[Ships[i].s_alt_classes[k].ship_class].name);
                    }
        ...
        if (Format_fs2_open != FSO_FORMAT_RETAIL) {
            ...
            if (Ships[i].flags2 & SF2_IS_HARMLESS)
                fout(" \"is_harmless\"");
            fout(" )");
        }
        // -----------------------------------------------------------
...
int CFred_mission_save::save_waypoints() {
    ...
    for (jump_node *jnp = (jump_node *) Jump_nodes.get_first(); !Jump_nodes.is_end(
            jnp); jnp = (jump_node *) jnp->get_next()) {
        ...
        if (!state) {
            ...
            if ((*raw_ptr == ';') && (raw_ptr[1] != '!')) {
                ...
                if ((raw_ptr[1] == ';') && (raw_ptr[2] == 'F') && (raw_ptr[3] == 'S') && (raw_ptr[4] == 'O')) {
                    ...
                    if ((s_num == 3) && (major <= FS_VERSION_MAJOR) && (minor <= FS_VERSION_MINOR) &&
                        (build <= FS_VERSION_BUILD)) {
                        state = 3;
                    } else if ((s_num == 4) && (major <= FS_VERSION_MAJOR) && (minor <= FS_VERSION_MINOR) &&
...
int CFred_mission_save::save_music() {
    ...
    if (Mission_music[SCORE_DEBRIEF_SUCCESS] != event_music_get_spooled_music_index("Success")) {
        ...
        fout(" %s", Mission_music[SCORE_DEBRIEF_SUCCESS] < 0 ? "None"
    ...
    if (Mission_music[SCORE_DEBRIEF_AVERAGE] != event_music_get_spooled_music_index("Average")) {
        ...
        fout(" %s", Mission_music[SCORE_DEBRIEF_AVERAGE] < 0 ? "None"
...
int CFred_mission_save::save_campaign_file(char *pathname) {
    ...
    if (strlen(Campaign.intro_cutscene)) {
        if (optional_string_fred("+Campaign Intro Cutscene:"))
            parse_comments();
        else
            fout("\n+Campaign Intro Cutscene:");

        fout(" %s", Campaign.intro_cutscene);
    }

    if (strlen(Campaign.end_cutscene)) {
        if (optional_string_fred("+Campaign End Cutscene:"))
            parse_comments();
        else
            fout("\n+Campaign End Cutscene:");

        fout(" %s", Campaign.end_cutscene);
    }
    ...
    for (i = 0; i < Campaign.num_missions; i++) {
        ...
        if (Format_fs2_open != FSO_FORMAT_RETAIL) {
            ...
            fout(" %d",
                 Campaign.missions[m].flags | ((Campaign.missions[m].main_hall > 0) ? CMISSION_FLAG_BASTION : 0));
        }

    ...
    if ((Num_unknown_ship_classes > 0) || (Num_unknown_weapon_classes > 0) || (Num_unknown_loadout_classes > 0)) {
        if (flags & MPF_IMPORT_FSM) {
            ...
            sprintf(msg,
                    ...
                    The_mission.name, pathname);
            Fred_view_wnd->MessageBox(msg);
        } else {
            Fred_view_wnd->MessageBox(
...
//			Player_starts++;
    ...
    for (i = 0; i < Num_teams; i++) {
        ...
        for (j = 0; j < MAX_WEAPON_TYPES; j++) {
            if (used_pool[j] != 0) {
                Warning(LOCATION, "%s is used in wings of team %d but was not in the loadout. Fixing now",
...
void CFREDDoc::OnFileImportFSM() {
    ...
    CFileDialog dlgFile(TRUE, "fsm", NULL,
                        OFN_HIDEREADONLY | OFN_FILEMUSTEXIST | OFN_NOCHANGEDIR | OFN_ALLOWMULTISELECT,
...
void CFREDDoc::OnFileImportWeapons() {
    ...
    CFileDialog dlg(TRUE, "tbl", NULL, OFN_HIDEREADONLY | OFN_FILEMUSTEXIST | OFN_NOCHANGEDIR,
                    "FreeSpace Tables (*.tbl)|*.tbl|All files (*.*)|*.*||");

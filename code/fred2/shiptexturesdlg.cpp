...
void CShipTexturesDlg::OnOK() {
    ...
    if (query_modified()) {
        ...
        if (not_found) {
            ...
            z = MessageBox(message, ((not_found > 1) ? "Some textures were not found." : "A texture was not found."),
        ...
        for (i = 0; i < texture_count; i++) {
            ...
            if (strlen(new_texture_name[i])) {
                ...
                strcpy_s(Fred_texture_replacements[Fred_num_texture_replacements].ship_name,
                         Ships[self_ship].ship_name);
...
BOOL CShipTexturesDlg::Create(LPCTSTR lpszClassName, LPCTSTR lpszWindowName, DWORD dwStyle, const RECT &rect,

...
int wing_editor::update_data(int redraw) {
    ...
    if (cur_wing >= 0) {
        ...
        if (strcmp(old_name, str)) {
            ...
            for (i = 0; i < Wings[cur_wing].wave_count; i++) {
                if ((Objects[wing_objects[cur_wing][i]].type == OBJ_SHIP) ||
                    (Objects[wing_objects[cur_wing][i]].type == OBJ_START)) {
...
void wing_editor::update_data_safe() {
    ...
    if (m_waves < 1) {
        m_waves = 1;
        if (!bypass_errors) {
            sprintf(buf, "Number of waves illegal.  Can not have %d waves. Number of waves has as been set to 1.",
                    (int) m_waves);
            MessageBox(buf);
        }
    }
    ...
    if (m_arrival_target >= 0) {
        ...
        if (((m_arrival_location != ARRIVE_AT_LOCATION) && (m_arrival_location != ARRIVE_FROM_DOCK_BAY)) && (i >= 0) &&
            !(i & SPECIAL_ARRIVAL_ANCHOR_FLAG)) {
            ...
            if ((Wings[cur_wing].arrival_distance < d) && (Wings[cur_wing].arrival_distance > -d)) {
                if (!bypass_errors) {
                    sprintf(buf, "Ship must arrive at least %d meters away from target.\n"
                            ...
                            (int) (2.0f * Objects[Ships[i].objnum].radius));

...
int create_ship(matrix *orient, vec3d *pos, int ship_type) {
    ...
    resolve_parse_flags(&Objects[obj], Iff_info[shipp->team].default_parse_flags,
                        Iff_info[shipp->team].default_parse_flags2);
    ...
    // set orders according to whether the ship is on the player ship's team
    {
        ...
        for (temp_objp = GET_FIRST(&obj_used_list);
             temp_objp != END_OF_LIST(&obj_used_list); temp_objp = GET_NEXT(temp_objp)) {
    ...
    switch (code & SRC_MASK) {
        ...
        case SRC_WING_ORDER:
            ...
            if (query_initial_orders_conflict(n))
                Fred_main_wnd->MessageBox("One or more ships of this wing also has initial orders",
                                          "Possible conflict");

...
MessageBox (wnd,
"Fred can no longer run due to internal overlay error", NULL, MB_OK | MB_ICONERROR |MB_TASKMODAL|MB_SETFOREGROUND);
else
MessageBox (wnd,
"Error: cannot enter DOS mode for 80x40 color text mode display.", NULL, MB_OK | MB_ICONERROR|MB_TASKMODAL|MB_SETFOREGROUND);
exit(1);
}
...
void CFREDView::OnLButtonDown(UINT
nFlags,
CPoint point
)
{
...
(Marked == 1) && (Objects[cur_object_index].type == OBJ_POINT)) {
Assert(Briefing_dialog);
Briefing_dialog->
icon_select(Objects[cur_object_index]
.instance);
...
void CFREDView::OnMouseMove(UINT
nFlags,
CPoint point
)
{
...
void CFREDView::OnLButtonUp(UINT
nFlags,
CPoint point
)
{
...
sprintf(msg,
"Add cloned ships to wing %s?", Wings[Duped_wing].name);
if (
MessageBox(msg,
"Query", MB_YESNO) == IDYES) {
objp = GET_FIRST(&obj_used_list);
while (objp !=
END_OF_LIST(&obj_used_list)
)    {
if (objp->
flags &OF_MARKED
) {
...
.wingnum == -1);
sprintf(Ships[ship]
.ship_name, "%s %d", Wings[Duped_wing].name,
Wings[Duped_wing].wave_count + 1);

Wings[Duped_wing].ship_index[Wings[Duped_wing].wave_count] =
ship;
Ships[ship].
wingnum = Duped_wing;

wing_objects[Duped_wing][Wings[Duped_wing].wave_count] =
OBJ_INDEX(objp);
...
LONG CFREDView::OnMenuPopupShips(UINT
wParam,
LONG lParam
)
{
...
LONG CFREDView::OnMenuPopupEdit(UINT
wParam,
LONG lParam
)
{
        ...
        if (menu.LoadMenu(IDR_MENU_EDIT_POPUP)) {
            ...
            for (i = 0; i < (int) Species_info.size(); i++) {
                ...
                shipPopup.AppendMenu(MF_STRING | MF_POPUP | MF_ENABLED,
                                     (UINT)
                species_submenu[i].m_hMenu, Species_info[i].species_name);
            }
            ...
            pPopup->AppendMenu(MF_STRING | MF_POPUP | MF_ENABLED,
                               (UINT)
            shipPopup.m_hMenu, "New Object Type");
...
BOOL CFREDView::OnCmdMsg(UINT
nID,
int nCode,
void *pExtra, AFX_CMDHANDLERINFO
* pHandlerInfo)
{
int id = (int) nID;
...
m_new_ship_type_combo_box.
SetCurSelNEW(cur_model_index);

} else if (nCode == CN_UPDATE_COMMAND_UI)    {
// Update UI element state
((CCmdUI*) pExtra)->
SetCheck(cur_model_index
+ SHIP_TYPES == id);
((CCmdUI*) pExtra)->
Enable(TRUE);
}
...
void CFREDView::OnLButtonDblClk(UINT
nFlags,
CPoint point
)
{
...
void view_universe(int just_marked) {
    ...
    g3_set_view_matrix(&view_pos, &view_orient, Viewer_zoom_FRED);
...
void CFREDView::OnZoomSelected() {
    if (query_valid_object()) {
        if (Marked > 1)
            ...
            vm_vec_scale_add(&view_pos, &Objects[cur_object_index].pos, &view_orient.vec.fvec,
                             Objects[cur_object_index].radius * -3.0f);
    }
...
//			return 1;
    ...
    for (i = 0; i < MAX_SHIPS; i++) {
        if (Ships[i].objnum >= 0) {  // is ship being used?
            ...
            for (dock_instance *dock_ptr = Objects[Ships[i].objnum].dock_list;
                 dock_ptr != NULL; dock_ptr = dock_ptr->next) {
                ...
                if (!ship_docking_valid(i, ship) && !ship_docking_valid(ship, i)) {
                    return internal_error("Docking illegal between \"%s\" and \"%s\" (initially docked)",
                                          Ships[i].ship_name, Ships[ship].ship_name);
                }

                z = get_docking_list(Ship_info[Ships[i].ship_info_index].model_num);
                point = dock_ptr->dockpoint_used;
                if (point < 0 || point >= z) {
                    internal_error("Invalid docker point (\"%s\" initially docked with \"%s\")", Ships[i].ship_name,
                ...
                if (point < 0 || point >= z) {
                    internal_error("Invalid dockee point (\"%s\" initially docked with \"%s\")", Ships[i].ship_name,
                                   Ships[ship].ship_name);
                }
            }
        }
...
// so this object has valid information, such as the instance.
                ...
                if ((Objects[obj].type == OBJ_SHIP) || (Objects[obj].type == OBJ_START)) {
                    ...
                    if (stricmp(buf, Ships[ship].ship_name)) {
                        return internal_error("Ship \"%s\" in wing should be called \"%s\"", Ships[ship].ship_name,
                                              buf);
                    }
            ...
            if (Wings[i].threshold + Wings[i].wave_count > MAX_SHIPS_PER_WING) {
                ...
                if (error("Threshold for wing \"%s\" is higher than allowed.  Reset to %d", Wings[i].name,
                          Wings[i].threshold)) {
                    return 1;
                }
            }
...
/*	for (i=0; i<num_messages; i++) {
    ...
    Assert((Player_start_shipnum >= 0) && (Player_start_shipnum < MAX_SHIPS) &&
        ...
        for (j = 0; j < Wings[i].wave_count; j++) {
            ...
            if (j == 0) {
                ...
                if (error(
                        ...
                        Ships[Wings[i].ship_index[j]].ship_name, Ships[Wings[i].ship_index[0]].ship_name)) {
                    return 1;
                }
            }
        }
        ...
        if (ptr->type == OBJ_START) {
            ...
            if (err) {
                if (The_mission.game_type & MISSION_TYPE_MULTI_TEAMS) {
                    ...
                    if (error("Player %s should be part of %s wing", Ships[ship_instance].ship_name,
                              starting_wing_list))
                        return 1;
                }
            }
        ...
        for (j = 0; j < Wings[wing].wave_count; j++)
            for (i = 0; i < MAX_AI_GOALS; i++)
                if (!ai_query_goal_valid(Wings[wing].ship_index[j], goals[i].ai_mode)) {
                    if (Fred_view_wnd->error("Order \"%s\" isn't allowed for ship \"%s\"",
...
void CFREDView::OnRunFreeSpace() {
    ...
    std::string::size_type fred_index = processed_name.find("fred2_wcsaga", 0);
    // capitalisation!
    if (fred_index == std::string::npos) {
        fred_index = processed_name.find("Fred2_WCSaga", 0);
    }

    char *cmdline;
    char buffer[MAX_PATH_LEN];
    std::string mission_file(Mission_filename);
    std::string mission_cmdline(processed_name);

    if (!mission_file.empty()) {
        mission_cmdline.append(" -start_mission ");
        mission_cmdline.append(mission_file);
        std::string::size_type extension_index = mission_cmdline.find(".fs2");

        if (extension_index == std::string::npos) {
            mission_cmdline.append(".fs2");
        }

        strcpy(buffer, mission_cmdline.c_str());
        cmdline = buffer;
    ...
    if (fred_index != std::string::npos) {
        ...
        processed_name.erase(fred_index, 6);
        //processed_name.insert(fred_index, "FS2_Open");
        processed_name.append(".exe");

        //try to start FS2_open
        r = CreateProcess(processed_name.c_str(), cmdline, NULL, NULL, FALSE, 0, NULL, "./", &si, &pi);
        if (r) {
            return;
        }
    }
    ...
    r = CreateProcess("wcsaga_sse2.exe", cmdline, NULL, NULL, FALSE, 0, NULL, "./", &si, &pi);

    if (!r) {
        if (cmdline != NULL) {
            strcpy(buffer, mission_cmdline.c_str());
        }
        r = CreateProcess("wcsaga_sse.exe", cmdline, NULL, NULL, FALSE, 0, NULL, "./", &si, &pi);
    }
    ...
    if (!r) {
        ...
        r = CreateProcess("wcsaga.exe", cmdline, NULL, NULL, FALSE, 0, NULL, "./", &si, &pi);
    }
...
void CFREDView::OnSetGroup(UINT
nID)
{
...
int get_visible_sub_system_count(ship * shipp) {
    ...
    for (cur_subsys = GET_FIRST(&shipp->subsys_list);
         cur_subsys != END_OF_LIST(&shipp->subsys_list); cur_subsys = GET_NEXT(cur_subsys)) {
...
int get_next_visible_subsys(ship * shipp, ship_subsys * *next_subsys) {
    ...
    // look before wrap
    for (*next_subsys = GET_NEXT(*next_subsys);
         *next_subsys != END_OF_LIST(&shipp->subsys_list); *next_subsys = GET_NEXT(*next_subsys)) {
    ...
    for (*next_subsys = GET_FIRST(&shipp->subsys_list);
         *next_subsys != END_OF_LIST(&shipp->subsys_list); *next_subsys = GET_NEXT(*next_subsys)) {
        if ((*next_subsys)->system_info->subobj_num != -1) {
            Update_window = 1;
            return 1;
...
int get_prev_visible_subsys(ship * shipp, ship_subsys * *prev_subsys) {
    ...
    // look before wrap
    for (*prev_subsys = GET_PREV(*prev_subsys);
         *prev_subsys != END_OF_LIST(&shipp->subsys_list); *prev_subsys = GET_PREV(*prev_subsys)) {
    ...
    // look for first after wrap
    for (*prev_subsys = GET_LAST(&shipp->subsys_list);
         *prev_subsys != END_OF_LIST(&shipp->subsys_list); *prev_subsys = GET_PREV(*prev_subsys)) {
        if ((*prev_subsys)->system_info->subobj_num != -1) {
            Update_window = 1;
            return 1;
...
void CFREDView::OnPrevSubsys() {
    ...
    if ((cur_object_index < 0) || (Objects[cur_object_index].type != OBJ_SHIP) ||
        (&Objects[cur_object_index] != Render_subsys.ship_obj)) {
        OnCancelSubsys();
        return;
    }

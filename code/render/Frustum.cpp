...
Frustum::Frustum(float fov, float aspect_ratio, float near_dist, float far_dist, vec3d *view_position,
                 matrix *view_matrix) {
    ...
    vm_vec_scale_add(&near_point, view_position, &view_matrix->vec.fvec, near_dist);
    ...
    // Bottom plane
    vec3d bottom_helper;
    vm_vec_scale_add(&bottom_helper, &near_point, &view_matrix->vec.uvec, -height);
...
bool Frustum::sphereBehindPlane(vec3d *position, float radius, vec3d *planePoint, vec3d *planeNormal) {
    // Check the object
    float distance = 0.0f;

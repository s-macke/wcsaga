...
float Eye_zoom;			// What the viewer's zoom is
...
void g3_set_view(camera* cam)
{
	...
	if (Sexp_zoom <= 0.0f)
		g3_set_view_matrix(&pos, &ori, cam->get_zoom());
	else
		g3_set_view_matrix(&pos, &ori, Sexp_zoom);
}
...
void g3_set_view_matrix(vec3d* view_pos, matrix* view_matrix, float zoom)
{
	...
	Proj_fov = gr_zoom_to_fov(View_zoom);

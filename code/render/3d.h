...
extern float Eye_zoom;            // What the viewer's zoom is
...
int g3_draw_polygon(vec3d *pos, matrix *ori, float width, float height, int tmap_flags = TMAP_FLAG_TEXTURED, int r =
255, int g = 255, int b = 255, int a = 255);

int g3_draw_polygon(vec3d *pos, vec3d *norm, float width, float height, int tmap_flags = TMAP_FLAG_TEXTURED, int r =
255, int g = 255, int b = 255, int a = 255);
...
float g3_draw_laser(vec3d *headp, float head_width, vec3d *tailp, float tail_width, uint tmap_flags =
TMAP_FLAG_TEXTURED, float max_len = 0.0f);
...
float g3_draw_laser_rgb(vec3d *headp, float head_width, vec3d *tailp, float tail_width, int r, int g, int b,
                        uint tmap_flags = TMAP_FLAG_TEXTURED | TMAP_FLAG_RGB, float max_len = 0.0f);
...
public:
    ...
    flash_ball(int number, float min_ray_width, float max_ray_width = 0, vec3d *dir = &vmd_zero_vector,
               vec3d *pcenter = &vmd_zero_vector, float outer = PI2, float inner = 0.0f, ubyte max_r = 255,
               ubyte max_g = 255, ubyte max_b = 255, ubyte min_r = 255, ubyte min_g = 255, ubyte min_b = 255)
            : ray(NULL),
              n_rays(0) {
        initialize(number, min_ray_width, max_ray_width, dir, pcenter, outer, inner, max_r, max_g, max_b, min_r, min_g,
                   min_b);
    }
    ...
    void initialize(int number, float min_ray_width, float max_ray_width = 0, vec3d *dir = &vmd_zero_vector,
                    vec3d *pcenter = &vmd_zero_vector, float outer = PI2, float inner = 0.0f, ubyte max_r = 255,
                    ubyte max_g = 255, ubyte max_b = 255, ubyte min_r = 255, ubyte min_g = 255, ubyte min_b = 255);

    void initialize(ubyte *bsp_data, float min_ray_width, float max_ray_width = 0, vec3d *dir = &vmd_zero_vector,
                    vec3d *pcenter = &vmd_zero_vector, float outer = PI2, float inner = 0.0f, ubyte max_r = 255,
                    ubyte max_g = 255, ubyte max_b = 255, ubyte min_r = 255, ubyte min_g = 255, ubyte min_b = 255);

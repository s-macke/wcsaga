...
int joy_init() {
    ...
    Joy_inited = 1;
    int num_supported_joysticks = joyGetNumDevs();
    Cur_joystick = os_config_read_uint(NULL, "CurrentJoystick", JOYSTICKID1);

    joy_get_caps(num_supported_joysticks);

    if (num_supported_joysticks < 1) {
        mprintf(("JOYSTICK: No joystick driver detected\n"));
        return 0;
    }
    ...
    if (Cur_joystick >= 0) {
        ...
        for (int count = 0; count < 20; ++count) {
            rs = joyGetPos(Cur_joystick, &joyinfo);
        ...
        if (rs == JOYERR_NOERROR) {
            ...
            mprintf(("JOYSTICK: Joystick found and read successfully.\n"));

            JOYINFOEX joyinfoex;
            joyinfoex.dwSize = sizeof(joyinfoex);
            joyinfoex.dwFlags = JOY_RETURNALL;
            MMRESULT rs = joyGetPosEx(Cur_joystick, &joyinfoex);

            mprintf(("JOYSTICK: joyGetPosEx returned: "));

            switch (rs) {
                case JOYERR_NOERROR:
                    mprintf(("JOYERR_NOERROR\n"));
                    break;

                case MMSYSERR_NODRIVER:
                    mprintf(("MMSYSERR_NODRIVER\n"));
                    break;

                case MMSYSERR_INVALPARAM:
                    mprintf(("MMSYSERR_INVALPARAM\n"));
                    break;

                case MMSYSERR_BADDEVICEID:
                    mprintf(("MMSYSERR_BADDEVICEID\n"));
                    break;

                case JOYERR_UNPLUGGED:
                    mprintf(("JOYERR_UNPLUGGED\n"));
                    break;

                case JOYERR_PARMS:
                    mprintf(("JOYERR_PARMS\n"));
                    break;
            ...
            mprintf(("JOYSTICK: Errer reading joystick data: "));
...
BOOL CALLBACK
joy_di_enum(LPCDIDEVICEINSTANCE
lpddi,
LPVOID pvRef
)
{
...
int joystick_read_raw_axis_di(int num_axes, int *axis) {
    ...
    if (SUCCEEDED(hr)) {
        switch (num_axes) {
            case 6:
                if (joystick.axis_valid[5])
                    axis[5] = joy_state.lRz;

            case 5:
                if (joystick.axis_valid[4])
                    axis[4] = joy_state.lRy;

            case 4:
                if (joystick.axis_valid[3])
                    axis[3] = joy_state.lRx;

...
#ifdef FS2_DEMO
char *Cheat_code_demo = NOX("#w[s=7:OjcLar*\\(;");
#else
char *Cheat_code = NOX("#w[s=7:OjcLar*\\(;");
#endif
...
void process_debug_keys(int k) {
    ...
    switch (k) {
        ...
        case KEY_DEBUGGED1 + KEY_SHIFTED + KEY_C:
            ...
            HUD_printf(XSTR("Countermeasure firing: %s", 6), Countermeasures_enabled ? XSTR("ENABLED",
                                                                                            7) : XSTR("DISABLED", 8));
            break;
        ...
        case KEY_DEBUGGED + KEY_V:
            ...
            // play the message
            message_send_unique_to_player(Messages[Message_debug_index].name,
                                          Message_waves[Messages[Message_debug_index].wave_info.index].name,
                                          MESSAGE_SOURCE_SPECIAL,
                                          MESSAGE_PRIORITY_HIGH, 0, 0);
        ...
        case KEY_DEBUGGED1 + KEY_ALTED + KEY_SHIFTED + KEY_K:
            if (Player_ai->target_objnum != -1) {
                ...
                if (objp->type == OBJ_SHIP) {
                    ship_apply_local_damage(objp, Player_obj, &objp->pos, Ships[objp->instance].ship_max_hull_strength *
                                                                          0.1f + 10.0f, MISS_SHIELDS, CREATE_SPARKS);
                }
            }
            break;
        ...
        case KEY_DEBUGGED1 + KEY_SHIFTED + KEY_K:
            if ((Player_ai->target_objnum != -1) && (Player_ai->targeted_subsys != NULL)) {
                ...
                if (objp->type == OBJ_SHIP) {
                    ...
                    do_subobj_hit_stuff(objp, Player_obj, &g_subobj_pos,
                                        (float) -Player_ai->targeted_subsys->system_info->
                                                type, NULL); //100.0f);
        ...
        case KEY_DEBUGGED1 + KEY_I:
            ...
            HUD_sourced_printf(HUD_SOURCE_HIDDEN, XSTR("You are %s", 10),
                               Player_obj->flags & OF_INVULNERABLE ? XSTR("now INVULNERABLE!", 11) : XSTR(
    ...
    switch (k) {
        ...
        case KEY_F12 + KEY_SHIFTED:
            ...
            if (Player_ai->target_objnum == -1)
                ...
                HUD_sourced_printf(HUD_SOURCE_HIDDEN, XSTR("%s added to set %d. (F%d)", 27),
                                   Ships[Objects[Player_ai->target_objnum].instance].ship_name, hotkey_set,
                                   4 + hotkey_set + 1);
            }
...
void game_do_end_mission_popup() {
    ...
    if (Game_mode & GM_MULTIPLAYER) {
        ...
        choice = popup(pf_flags, 3, POPUP_NO, XSTR("&Yes, Quit", 28), XSTR("Yes, &Restart", 29),
                       XSTR("Do you really want to end the mission?", 30));
...
void game_process_keys() {
    ...
    do {
        ...
        // that this has adverse affect on anything and is actually desireable because of the
        ...
        case CYCLE_NEXT_PRIMARY:
            ...
            if (ship_select_next_primary(objp, CYCLE_PRIMARY_NEXT)) {
                ...
                if (timestamp_elapsed(shipp->weapons.next_primary_fire_stamp[shipp->weapons.current_primary_bank])) {
                    shipp->weapons.next_primary_fire_stamp[shipp->weapons.current_primary_bank] = timestamp(
                            250);    //	1/4 second delay until can fire
                }
    ...
    switch (n) {
        ...
        case VIEW_EXTERNAL_TOGGLE_CAMERA_LOCK:
            ...
            if (Viewer_mode & VM_EXTERNAL) {
                ...
                if (Viewer_mode & VM_EXTERNAL_CAMERA_LOCKED) {
                    HUD_sourced_printf(HUD_SOURCE_HIDDEN,
                                       XSTR("External camera is locked, controls will move ship", 36));
                } else {
                    HUD_sourced_printf(HUD_SOURCE_HIDDEN,
                                       XSTR("External camera is free, controls will move the camera, not the ship",
                                            37));
                }
            }
            ret = 1;
        ...
        case TIME_SLOW_DOWN:
            if (Game_mode & GM_NORMAL) {
                ...
                if ((Game_time_compression > F1_0) && !Time_compression_locked) {
    ...
    switch (n) {
        ...
        case TARGET_CLOSEST_SHIP_ATTACKING_TARGET:
            ...
            if (hud_sensors_ok(Player_ship)) {
                ...
                hud_target_closest(iff_get_attacker_mask(obj_team(&Objects[Player_ai->target_objnum])),
        ...
        case RADAR_RANGE_CYCLE:
            ...
            HUD_sourced_printf(HUD_SOURCE_HIDDEN, XSTR("Radar range set to %s", 38),
                               Radar_range_text(HUD_config.rp_dist));
            break;
        ...
        case MULTI_SELF_DESTRUCT:
            ...
            if (Net_player->flags & NETINFO_FLAG_AM_MASTER) {
                if ((Net_player->m_player->objnum >= 0) && (Net_player->m_player->objnum < MAX_OBJECTS) &&
                    (Objects[Net_player->m_player->objnum].type == OBJ_SHIP) && (Objects[Net_player->m_player->objnum].
                        instance >= 0) &&

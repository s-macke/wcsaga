...
int joy_ff_create_std_periodic(LPDIRECTINPUTEFFECT *eff, int type, int dur, int per, int ang = 0, int mag = 10000,
...
int joy_ff_create_effects() {
    joy_ff_handle_error(SWFF_CreateConstantForceEffect(pDiDevice,
                                                       &pHitEffect1,
                                                       300000,                        // Duration
                                                       0,                                // Angle
                                                       10000,                        // Magnitude
    ...
    joy_ff_create_std_periodic(&pHitEffect2,
                               SINE,
                               300000,                        // Duration
                               100000,                        // Period
                               9000,                            // Angle
    ...
    joy_ff_handle_error(SWFF_CreatePeriodicEffect(pDiDevice,
                                                  &pShootEffect,
                                                  SAWTOOTH_DOWN,
                                                  160000,                        // Duration
                                                  20000,                        // Period
    ...
    init_periodic_effect_struct(&Struct_afterburn1,
                                SINE,
                                INFINITE,                    // Duration
                                20000,                        // Period
                                0,                                // Angle
    ...
    init_periodic_effect_struct(&Struct_afterburn2,
                                SINE,
                                INFINITE,                    // Duration
                                120000,                        // Period
                                9000,                            // Angle
    ...
    init_periodic_effect_struct(&Struct_dock,
                                SQUARE_HIGH,
                                125000,                        // Duration
                                100000,                        // Period
                                9000,                            // Angle
    ...
    init_periodic_effect_struct(&Struct_explode,
                                SAWTOOTH_DOWN,
                                500000,                        // Duration
                                20000,                        // Period
                                9000,                            // Angle
    ...
    init_periodic_effect_struct(&Struct_deathroll1,
                                SINE,
                                INFINITE,                    // Duration
                                200000,                        // Period
                                0,                                // Angle
    ...
    init_periodic_effect_struct(&Struct_deathroll2,
                                SINE,
                                INFINITE,                    // Duration
                                200000,                        // Period
                                9000,                            // Angle
...
void joy_ff_adjust_handling(int speed) {
    ...
    if (pSpring) {
        if (Spring_cond_effect.DIConditionStruct[0].lPositiveCoefficient != v) {
            ...
            hr = joy_ff_handle_error(pSpring->SetParameters(&Spring_cond_effect.DIEffectStruct,
                                                            DIEP_TYPESPECIFICPARAMS), "Spring");
            if (hr == DIERR_INPUTLOST) {
                joy_reacquire_ff();
                joy_ff_handle_error(pSpring->SetParameters(&Spring_cond_effect.DIEffectStruct,
                                                           DIEP_TYPESPECIFICPARAMS), "Spring");
            }
        }
    }
...
void joy_ff_change_effect(di_periodic_effect_struct *s, LPDIRECTINPUTEFFECT eff, int gain = -1, int dur = 0,
                          int flags = -1) {
    int reload = 0;
    ...
    if (flags == -1) {
        flags = DIEP_DURATION | DIEP_SAMPLEPERIOD | DIEP_GAIN | DIEP_DIRECTION | DIEP_ENVELOPE |
...
void joy_ff_play_secondary_shoot(int gain) {
    ...
    if (gain != secondary_ff_level) {
        ...
        if (joy_ff_handle_error(SWFF_SetDuration(pSecShootEffect, 150000 + gain * 25),
                                "SecShootEffect") == DIERR_INPUTLOST) {
            joy_reacquire_ff();
...
void init_periodic_effect_struct(di_periodic_effect_struct *effect, int type, int dur, int per, int ang, int mag,
                                 int att, int fade) {
    // type-specific stuff
    DWORD dwPhase = 0;

...
float
fvi_ray_plane(vec3d *new_pnt, vec3d *plane_pnt, vec3d *plane_norm,        // Plane description, a point and a normal
...
int fvi_segment_plane(vec3d *new_pnt, vec3d *plane_pnt, vec3d *plane_norm, vec3d *p0, vec3d *p1, float rad) {
    ...
    if (t < 0.0f)
        return 0;        // intersection lies behind p0
    if (t > 1.0f)
        return 0;        // intersection lies past p1
...
bool fvi_point_inside_boundingbox(vec3d *min, vec3d *max, vec3d *p0) {
    bool inside = true;
...
//
void fvi_closest_point_on_line_segment(vec3d *closest_point, vec3d *fixed_point, vec3d *line_point1,
                                       vec3d *line_point2) {
...
//
int fvi_check_sphere_sphere(vec3d *x_p0, vec3d *x_p1, vec3d *x_s0, vec3d *x_s1, float radius_p, float radius_s,
                            float *t1, float *t2) {

...
// generate a section
void nebl_generate_section(bolt_type *bi, float width, l_node *a, l_node *b, l_section *c, l_section *cap, int pinch_a,
                           int pinch_b);
                ...
                if (Nebl_flash_count) {
                    ...
                    if (b->strikes_left == bi->num_strikes) {
                        ...
                        if (frand_range(0.0f, 1.0f) < 0.5f) {
                            ...
                            if (frand_range(0.0f, 1.0f) < 0.5f) {
                                snd_play(&Snds[SND_LIGHTNING_4], 0.0f, bang, SND_PRIORITY_DOUBLE_INSTANCE);
                            } else {
                                snd_play(&Snds[SND_LIGHTNING_3], 0.0f, bang, SND_PRIORITY_DOUBLE_INSTANCE);
                            }
                        }

                        // apply em pulse
                        if (bi->emp_intensity > 0.0f) {
                            emp_apply(&b->midpoint, 0.0f, vm_vec_dist(&b->start, &b->strike), bi->emp_intensity,
                                      bi->emp_time);
                        }
                    }
                }
    ...
    if (timestamp_elapsed(Nebl_stamp)) {
        ...
        for (idx = 0; idx < num_bolts; idx++) {
            ...
            if (!IS_VEC_NULL_SQ_SAFE(&Storm->flavor)) {
                ...
                vm_vec_interp_constant(&the_mixture, &your_basic_hot_sauce, &wing_sauce,
                                       how_much_of_that_good_wing_sauce_to_add / 1000.0f);
...
int nebl_gen(vec3d *left, vec3d *right, float depth, float max_depth, int child, l_node **l_left, l_node **l_right) {
    ...
    if (d < 0.30f || (depth > max_depth)) {
        ...
        new_left->links[0] = NULL;
        new_left->links[1] = NULL;
        new_left->links[2] = NULL;
        ...
        new_right->links[0] = NULL;
        new_right->links[1] = NULL;
        new_right->links[2] = NULL;
...
void nebl_generate_section(bolt_type *bi, float width, l_node *a, l_node *b, l_section *c, l_section *cap, int pinch_a,
    ...
    for (idx = 0; idx < 3; idx++) {
        ...
        vertex proj_vertex = c->vex[idx];
        g3_rotate_vertex(&proj_vertex, &pt);
        g3_project_vertex(&proj_vertex);

        // if first frame, keep track of the average screen pos
        if ((proj_vertex.sx >= 0) && (proj_vertex.sx < gr_screen.max_w) && (proj_vertex.sy >= 0) &&
            (proj_vertex.sy < gr_screen.max_h)) {
            Nebl_flash_x += proj_vertex.sx;
            Nebl_flash_y += proj_vertex.sy;
            Nebl_flash_count++;
        }
    }
    // calculate the glow points
    nebl_calc_facing_pts_smart(&glow_a, &glow_b, &dir_normal, &a->pos, pinch_a ? 0.5f : width * 6.0f,
                               Nebl_type->b_add);
    if (Cmdline_nohtl) {
...
void nebl_render(bolt_type *bi, l_node *whee, float width, l_section *prev) {
    ...
    if (whee->links[LINK_CHILD]) {
        // generate section
        nebl_generate_section(bi, width * 0.5f, whee, whee->links[LINK_CHILD], &child_start, &end, 0,
                              whee->links[LINK_CHILD]->links[LINK_RIGHT] == NULL ? 1 : 0);
    ...
    if (whee->links[LINK_RIGHT]->links[LINK_RIGHT] == NULL) {
        ...
        // generate section
        nebl_generate_section(bi, width, whee->links[LINK_RIGHT], whee->links[LINK_RIGHT]->links[LINK_RIGHT], &end,
                              NULL, 0, 0);

...
int palman_is_nondarkening(int r, int g, int b)
{
	...
	{
		if ((r == Palman_non_darkening[i][0]) && (g == Palman_non_darkening[i][1]) &&
			(b == Palman_non_darkening[i][2]))
		{
			return 1;
		}
	}
...
void palette_update(char* name_with_extension, int restrict_font_to_128)
{
	...
	if (tmp_checksum == gr_palette_checksum)
		return;
...
ubyte* palette_get_blend_table(float alpha)
{
	...
	if (i < 0)
		i = 0;
	if (i > NUM_BLEND_TABLES - 1)
		i = NUM_BLEND_TABLES - 1;

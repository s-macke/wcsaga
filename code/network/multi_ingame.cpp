...
void multi_handle_ingame_joiners() {
    ...
    for (idx = 0; idx < MAX_PLAYERS; idx++) {
        ...
        if (!(Net_players[idx].flags & NETINFO_FLAG_RELIABLE_CONNECTED) || !(Net_players[idx].flags &
        ...
        if ((Net_players[idx].flags & NETINFO_FLAG_MISSION_OK) && !(Net_players[idx].s_info.ingame_join_flags &
                                                                    INGAME_JOIN_FLAG_LOADING_MISSION)) {
        ...
        if ((Net_players[idx].state == NETPLAYER_STATE_MISSION_LOADED) && !(Net_players[idx].s_info.ingame_join_flags &
                                                                            INGAME_JOIN_FLAG_SENDING_SHIPS)) {
            int i;
        ...
        */
        ...
        if ((Net_players[idx].state == NETPLAYER_STATE_INGAME_SHIPS) && !(Net_players[idx].s_info.ingame_join_flags &
                                                                          INGAME_JOIN_FLAG_SENDING_POST)) {
        ...
        if ((Net_players[idx].state == NETPLAYER_STATE_POST_DATA_ACK) && !(Net_players[idx].s_info.ingame_join_flags &
                                                                           INGAME_JOIN_FLAG_PICK_SHIP)) {
...
void multi_ingame_select_init() {
    ...
    */
    ...
    for (idx = 0; idx < MULTI_INGAME_JOIN_NUM_BUTTONS; idx++) {
        // create the object
        Multi_ingame_join_buttons[gr_screen.res][idx].button.create(&Multi_ingame_window, "",
                                                                    Multi_ingame_join_buttons[gr_screen.res][idx].x,
                                                                    Multi_ingame_join_buttons[gr_screen.res][idx].y, 1,
                                                                    1, 0,
                                                                    1);
        ...
        Multi_ingame_join_buttons[gr_screen.res][idx].button.set_bmaps(Multi_ingame_join_buttons[gr_screen.res][idx].
                filename);

        // set the hotspot
        Multi_ingame_join_buttons[gr_screen.res][idx].button.link_hotspot(Multi_ingame_join_buttons[gr_screen.
                res][idx].hotspot);
    }
    ...
    Multi_ingame_select_button.create(&Multi_ingame_window, "", Mi_name_field[gr_screen.res][MI_FIELD_X],
                                      Mi_name_field[gr_screen.res][MI_FIELD_Y], Mi_width[gr_screen.res],
                                      Mi_height[gr_screen.res], 0, 1);
...
void multi_ingame_join_display_ship(object *objp, int y_start) {
    ...
    for (idx = 0; idx < wp->num_primary_banks; idx++) {
        gr_string(Mi_primary_field[gr_screen.res][MI_FIELD_X], y_start + y_spacing + (idx * 10),
                  Weapon_info[wp->primary_bank_weapons[idx]].name);
    }
    ...
    for (idx = 0; idx < wp->num_secondary_banks; idx++) {
        gr_string(Mi_secondary_field[gr_screen.res][MI_FIELD_X], y_start + y_spacing + (idx * 10),
                  Weapon_info[wp->secondary_bank_weapons[idx]].name);
    }
...
void multi_ingame_join_display_avail() {
    ...
    if (Multi_ingame_ship_selected != -1) {
        int y_start = (Mi_name_field[gr_screen.res][MI_FIELD_Y] + (Multi_ingame_ship_selected *
                                                                   Mi_spacing[gr_screen.res]));

        // draw the border
        gr_set_color_fast(&Color_bright_blue);
        gr_line(Mi_name_field[gr_screen.res][MI_FIELD_X] - 1, y_start - 1, (Mi_name_field[gr_screen.res][MI_FIELD_X] -
                                                                            1) + (Mi_width[gr_screen.res] + 2),
                y_start - 1);
        gr_line(Mi_name_field[gr_screen.res][MI_FIELD_X] - 1, y_start + Mi_spacing[gr_screen.res] -
                                                              2,
                (Mi_name_field[gr_screen.res][MI_FIELD_X] - 1) + (Mi_width[gr_screen.res] + 2),
                y_start + Mi_spacing[gr_screen.res] - 2);
        gr_line(Mi_name_field[gr_screen.res][MI_FIELD_X] - 1, y_start, Mi_name_field[gr_screen.res][MI_FIELD_X] - 1,
                y_start + Mi_spacing[gr_screen.res] - 2);
        gr_line((Mi_name_field[gr_screen.res][MI_FIELD_X] - 1) + (Mi_width[gr_screen.res] + 2),
                y_start, (Mi_name_field[gr_screen.res][MI_FIELD_X] - 1) + (Mi_width[gr_screen.res] + 2),
                y_start + Mi_spacing[gr_screen.res] - 2);
    }
    ...
    while (moveup != END_OF_LIST(&Ship_obj_list)) {
        if (!(Ships[Objects[moveup->objnum].instance].flags & (SF_DYING |
                                                               SF_DEPARTING)) && (Objects[moveup->objnum].flags &
                                                                                  OF_COULD_BE_PLAYER)) {
            // display the ship
            multi_ingame_join_display_ship(&Objects[moveup->objnum],
                                           Mi_name_field[gr_screen.res][MI_FIELD_Y] +
                                           (Multi_ingame_num_avail * Mi_spacing[gr_screen.res]));
        ...
        for (idx = 0; idx < MAX_SHIPS; idx++) {
            ...
            if (team_val != -1) {
                for (j = 0; j < MAX_PLAYERS; j++) {
                    if (MULTI_CONNECTED(Net_players[j]) && (Net_players[j].m_player->objnum == Objects[Ships[idx].
                            objnum].net_signature)) {
...
void process_ingame_ship_request_packet(ubyte *data, header *hinfo) {
    ...
    switch (code) {
        ...
        case INGAME_SR_DENY :
            ...
            // display a popup
            popup(PF_BODY_BIG | PF_USE_AFFIRMATIVE_ICON, 1, POPUP_OK, XSTR("You have been denied the requested ship",
                                                                           686));
            break;
...
void multi_ingame_send_ship_update(net_player *p) {
    ...
    while (moveup != END_OF_LIST(&Ship_obj_list)) {
        ...
        if (Objects[moveup->objnum].flags & OF_COULD_BE_PLAYER &&
            obj_team(&Objects[moveup->objnum]) == p->p_info.team) {
            // send the update

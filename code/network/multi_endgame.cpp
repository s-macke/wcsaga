        ...
        if (notify_code != MULTI_END_NOTIFY_NONE) {
            switch (notify_code) {
                ...
                case MULTI_END_NOTIFY_KICKED_INGAME_ENDED:
                    strcat_s(err_msg,
                             XSTR("You were kicked because you were ingame joining a game that has ended", 1000));
                    break;
                default :
                    Int3();
            }
            ...
            switch (error_code) {
                ...
                case MULTI_END_ERROR_WAVE_COUNT:
                    strcat_s(err_msg,
                             XSTR(
                                     "The player wings Alpha, Beta, Gamma, and Zeta must have only 1 wave.  One of these wings currently has more than 1 wave.",
                                     987));
                    break;
...
void multi_endgame_check_for_warpout() {
    ...
    if ((Game_mode & GM_IN_MISSION) &&                                        // if i'm still in the mission
        ((Netgame.game_state == NETGAME_STATE_ENDGAME) ||                // if the netgame ended
         (Netgame.game_state ==
          NETGAME_STATE_DEBRIEF))                    // if the netgame is now in the debriefing state
            ) {
        need_to_warpout = 1;
    }

...
int OO_debug_info = 0;
DCF(ood,
"switch on and off targeted object update info")
{
...
void multi_oo_build_ship_list(net_player *pl) {
    ...
    for (moveup = GET_FIRST(&Ship_obj_list); moveup != END_OF_LIST(&Ship_obj_list); moveup = GET_NEXT(moveup)) {
        ...
        if ((moveup->objnum < 0) || (Objects[moveup->objnum].instance < 0) ||
...
//		}
        ...
        objp->phys_info.speed = vm_vec_mag(
                &objp->phys_info.vel);                            //	Note, cannot use quick version, causes cumulative error, increasing speed.
        objp->phys_info.fspeed = vm_vec_dot(&objp->orient.fvec,
                                            &objp->phys_info.vel);        // instead of vector magnitude -- use only forward vector since we are only interested in forward velocity
    }
...
int multi_oo_pack_data(net_player *pl, object *objp, ubyte oo_flags, ubyte *data_out) {
    ...
    if (oo_flags & OO_ORIENT_NEW) {
        ...
        ret = (ubyte) multi_pack_unpack_rotvel(1, data + packet_size + header_bytes, &objp->orient, &objp->pos,
                                               &objp->phys_info);
        packet_size += ret;
        ...
        if (oo_flags & OO_EXTRA_PHYSICS) {
            ret = (ubyte) multi_pack_unpack_desired_rotvel(1, data + packet_size + header_bytes, &objp->orient,
                                                           &objp->pos, &objp->phys_info,
    ...
    if (oo_flags & OO_POS_NEW) {
        ...
        ret = (ubyte) multi_pack_unpack_vel(1, data + packet_size + header_bytes, &objp->orient, &objp->pos,
                                            &objp->phys_info);
        packet_size += ret;
    ...
    if (oo_flags & OO_SUBSYSTEMS_NEW) {
        ...
        if (shipp->ship_info_index < 0) {
            ...
            // now the subsystems.
            for (subsysp = GET_FIRST(&shipp->subsys_list);
                 subsysp != END_OF_LIST(&shipp->subsys_list); subsysp = GET_NEXT(subsysp)) {
...
int multi_oo_unpack_client_data(net_player *pl, ubyte *data) {
    ...
    if (pl != NULL) {
        ...
        if ((pl->player->objnum != -1) && (Objects[pl->player->objnum].type == OBJ_SHIP) &&
            (Ships[Objects[pl->player->objnum].instance].ai_index != -1)) {
    ...
    if ((tobj != NULL) && (pl != NULL) && (pl->player->objnum != -1)) {
        ...
        if (Objects[pl->player->objnum].type == OBJ_SHIP) {
            ...
            if ((t_subsys != -1) && (tobj->type == OBJ_SHIP)) {
                Ai_info[Ships[Objects[pl->player->objnum].instance].ai_index].targeted_subsys = ship_get_indexed_subsys(
                        &Ships[tobj->instance], t_subsys);
            }
        }
...
int multi_oo_unpack_data(net_player *pl, ubyte *data, ushort packet_sequence_num, ushort packet_sequence_ref,
                         int time_born) {
    int offset = 0;
    ...
    if ((pobjp == NULL) || (pobjp->type != OBJ_SHIP) || (pobjp->instance < 0) ||
        (Ships[pobjp->instance].ship_info_index < 0) || (packet_sequence_num < packet_sequence_ref)) {
    ...
    if (oo_flags & OO_ORIENT_NEW) {
        ...
        if (oo_flags & OO_EXTRA_PHYSICS) {
            int r6 = multi_pack_unpack_desired_rotvel(0, data + offset, &objp->orient, &objp->pos, &objp->phys_info,
                                                      &Ship_info[Ships[pobjp->instance].ship_info_index]);
            offset += r6;
        }
    }
    ...
    if (oo_flags & OO_HULL_NEW) {
        ...
        // TEST code
#ifndef NDEBUG
        if (OO_debug_info && (Player_ai != NULL) && (pobjp != NULL) && (pobjp->instance >= 0) &&
            (Player_ai->target_objnum == OBJ_INDEX(pobjp))) {
...
int multi_oo_maybe_update(net_player *pl, object *pobj, object *obj, ubyte *data) {
    ...
    if ((stamp == -1) || timestamp_elapsed(stamp)) {
        ...
        */
        ...
        if ((pl->p_info.options.obj_update_level == OBJ_UPDATE_HIGH) ||
            (pl->p_info.options.obj_update_level == OBJ_UPDATE_LAN)) {
            oo_flags |= OO_EXTRA_PHYSICS;
        }

        // if the object's hull/shield timestamp has expired
        if ((Ships[obj->instance].status_update_stamp[player_index] == -1) ||
            timestamp_elapsed(Ships[obj->instance].status_update_stamp[player_index])) {
        ...
        // if the object's hull/shield timestamp has expired
        if ((Ships[obj->instance].subsys_update_stamp[player_index] == -1) ||
            timestamp_elapsed(Ships[obj->instance].subsys_update_stamp[player_index])) {
...
void multi_oo_process_update(ubyte *data, header *hinfo) {
    ...
    if (Net_player->flags & NETINFO_FLAG_AM_MASTER) {
        ...
        if (player_index != -1) {
            // wrap around
            if (((packet_seq - Net_players[player_index].client_cinfo_seq) < 0) &&
                ((Net_players[player_index].client_cinfo_seq - packet_seq) > 2000)) {
        ...
        // wrap around
        if (((packet_seq - Net_player->client_server_seq) < 0) &&
            ((Net_player->client_server_seq - packet_seq) > 2000)) {

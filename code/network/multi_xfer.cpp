...
// process a header
void multi_xfer_process_header(ubyte *data, PSNET_SOCKET_RELIABLE who, ushort sig, char *filename, int file_size,
...
void multi_xfer_do() {
    ...
    for (idx = 0; idx < MAX_XFER_ENTRIES; idx++) {
        ...
        if ((Multi_xfer_entry[idx].flags & MULTI_XFER_FLAG_USED) && !(Multi_xfer_entry[idx].
                flags & (MULTI_XFER_FLAG_SUCCESS |
                         MULTI_XFER_FLAG_FAIL | MULTI_XFER_FLAG_TIMEOUT
                                                                      ))) {
...
int multi_xfer_get_status(int handle) {
    ...
    if ((Multi_xfer_entry[handle].flags & MULTI_XFER_FLAG_QUEUE) && !(Multi_xfer_entry[handle].
            flags & MULTI_XFER_FLAG_QUEUE_CURRENT)) {
...
void multi_xfer_release_handle(int handle) {
    ...
    if (xe->file != NULL) {
        ...
        if (!(xe->flags & MULTI_XFER_FLAG_SUCCESS) && (xe->flags & MULTI_XFER_FLAG_RECV) &&
            (strlen(xe->filename) > 0)) {
            cf_delete(xe->ex_filename, xe->force_dir);
        }
    }
    ...
    if (xe->flags & MULTI_XFER_FLAG_QUEUE) {
        ...
        if (!(xe->flags & MULTI_XFER_FLAG_QUEUE_CURRENT)) {
            ...
            for (idx = 0; idx < MAX_XFER_ENTRIES; idx++) {
                ...
                if ((xe_c->flags & MULTI_XFER_FLAG_USED) && (xe_c->file_socket == xe->file_socket) &&
                    (xe_c->flags & MULTI_XFER_FLAG_SEND) &&
...
void multi_xfer_process_final(xfer_entry *xe) {
    ...
    if (!cf_chksum_short(xe->ex_filename, &chksum, -1, xe->force_dir) || (chksum != xe->file_chksum)) {
        ...
        nprintf(("Network", "MULTI XFER : renaming xferred file from %s to %s (chksum %d %d)\n", xe->ex_filename,
                xe->filename, (int) xe->file_chksum, (int) chksum));
#endif

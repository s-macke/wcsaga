...
LOCAL
ubyte net_buffer[NUM_REENTRANT_LEVELS][MAX_NET_BUFFER];
LOCAL
ubyte Multi_read_count;
...
LOCAL
fix Multi_server_wait_start;				// variable to hold start time when waiting to reestablish with server
...
void multi_check_listen()
{
	...
	if (sock != INVALID_SOCKET)
	{
		...
		{
			if ((Net_players[i].flags & NETINFO_FLAG_CONNECTED) && (!memcmp(&(addr.
																			  addr), &(Net_players[i].p_info.addr.
																					   addr), 6)))
			{
				...
				// NETLOG
				ml_printf(NOX("Accepted TCP connection from %s"), Net_players[i].m_player == NULL ? NOX("Unknown") :
...
void multi_client_check_server()
{
	...
	if (Netgame.flags & NG_FLAG_SERVER_LOST)
	{
		if (!(Game_mode & GM_IN_MISSION) && !popup_active())
		{
			...
			Multi_server_wait_start = timer_get_fixed_seconds();
			rval = popup_till_condition(multi_client_wait_on_server, XSTR("Cancel", 641),
				XSTR(
				"Contact lost with server.  Stopping simulation until contact reestablished.  Press Cancel to exit game.", 642));
			...
			if ((idx != MY_NET_PLAYER_NUM) && MULTI_CONNECTED(Net_players[idx]))
			{
				...
				if (Net_players[idx].s_info.reliable_connect_time != -1)
				{
					...
					if (Net_players[idx].reliable_socket != INVALID_SOCKET)
					{
						...
						// if he's timed out
					else if (((time(NULL) - Net_players[idx].s_info.reliable_connect_time) >
							  MULTI_RELIABLE_CONNECT_WAIT) && (Net_players[idx].reliable_socket == INVALID_SOCKET))
					{
...
void multi_process_incoming()
{
	...
	while ((size = psnet_get(data, &from_addr)) > 0)
	{
		...
		if ((Net_player->flags & NETINFO_FLAG_INGAME_JOIN) && (Net_player->state !=
															   NETPLAYER_STATE_INGAME_SHIP_SELECT))
		{
	...
	if (Net_player->flags & NETINFO_FLAG_AM_MASTER)
	{
		for (idx = 0; idx < MAX_PLAYERS; idx++)
		{
			if ((Net_players[idx].flags & NETINFO_FLAG_CONNECTED) && (Net_player != NULL) &&
...
void multi_do_frame()
{
	...
	if (Game_mode & GM_IN_MISSION)
	{
		...
		else
		{
			...
			if (((f2fl(Netgame.options.mission_time_limit) > 0.0f) && (Missiontime > Netgame.options.
																	   mission_time_limit)) ||
	...
	if ((Net_player != NULL) && (Net_player->flags & NETINFO_FLAG_AM_MASTER))
	{
		...
		for (idx = 0; idx < MAX_PLAYERS; idx++)
		{
			...
			{
				if ((Multi_client_update_times[idx] < 0) || timestamp_elapsed_safe(Multi_client_update_times[idx],
					1000))
				{
...
void standalone_main_init()
{
	...
	Net_player->flags |= (NETINFO_FLAG_AM_MASTER | NETINFO_FLAG_CONNECTED | NETINFO_FLAG_DO_NETWORKING |
						  NETINFO_FLAG_MISSION_OK);
	...
	if (MULTIPLAYER_MASTER)
	{
		...
		for (idx = 0; idx < MAX_PLAYERS; idx++)
		{
				...
				(Net_players[idx].m_player != NULL))
			{
				...
				{
					gr_printf(sx, sy, "%s: %d, %d pl", Net_players[idx].m_player->callsign,
						Net_players[idx].sv_bytes_sent, 0);
					sy += 10;
				}
				else
				{
					gr_printf(sx, sy, "%s: %d, %d pl", Net_players[idx].m_player->callsign,
						Net_players[idx].sv_bytes_sent, Net_players[idx].sv_last_pl);
					sy += 10;
				}
			}
		}

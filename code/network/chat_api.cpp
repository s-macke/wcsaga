    ...
    if (!Socket_connecting) {
        ...
        if (!Socket_connected) {
            ...
            if (select(Chatsock + 1, NULL, &write_fds, NULL, &timeout) > 0) {
                ...
                snprintf(signon_str, SSIZE(signon_str), NOX("/USER %s %s %s :%s"), NOX("user"), NOX("user"),
                         NOX("user"), Chat_tracker_id);
...
char *ParseIRCMessage(char *Line, int iMode) {
    ...
    if (stricmp(szCmd, NOX("PRIVMSG")) == 0) {
        ...
        if (szCTCPCmd[0] == 0x01) {
            //Handle ctcp message
            strncpy(szRemLine, Line + iPrefixLen + strlen(szCmd) + strlen(szTarget) + strlen(szCTCPCmd) +
                               6, sizeof(szRemLine) - 1);
            ...
            if (iMode == MSG_LOCAL) {
                strncpy(szHackPrefix, Line + iPrefixLen + strlen(szCmd) + strlen(szTarget) + 4, sizeof(szHackPrefix) -
                                                                                                1);
            ...
            if (stricmp(szCTCPCmd + 1, NOX("PING")) == 0) {
                snprintf(szResponse, SSIZE(szResponse), NOX("/NOTICE %s :\001PING %s\001"), szNick,
            ...
            strncpy(szRemLine, 1 + GetWordNum(0, Line + iPrefixLen + strlen(szCmd) + strlen(szTarget) +
                                                 4), sizeof(szRemLine) - 1);
            szRemLine[strlen(szRemLine) - 1] = '\0';
    ...
    if (stricmp(szCmd, NOX("KICK")) == 0) {
        ...
        snprintf(szResponse, SSIZE(szResponse), XSTR("*** %s has kicked %s from channel %s (%s)", 637), szNick,
                 szHackPrefix, szTarget, pszTempStr);
        //Remove them to the userlist too!
        RemoveChatUser(szNick);
        return szResponse;
    ...
    if (stricmp(szCmd, NOX("NICK")) == 0) {
        ...
        snprintf(szResponse, SSIZE(szResponse), XSTR("*** %s is now known as %s", 638), szNick, GetWordNum(0,
                                                                                                           szRemLine));
        return szResponse;
    }

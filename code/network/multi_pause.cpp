	...
	{ // GR_640
		{
			...
			UI_XSTR_COLOR_PINK,
			-1,
			&Multi_paused_buttons[0][MP_EXIT_MISSION].button
		},
		{
			"Mission",
			1063,
	...
	{ // GR_1024
		{
			...
			UI_XSTR_COLOR_PINK,
			-1,
			&Multi_paused_buttons[1][MP_EXIT_MISSION].button
		},
		{
			"Mission",
			1063,
...
void multi_pause_server_eval_request(net_player* pl, int pause)
{
	...
	if ((cur_state == GS_STATE_DEBRIEF) || (cur_state == GS_STATE_MULTI_MISSION_SYNC) ||
		(cur_state == GS_STATE_BRIEFING) ||
		(cur_state == GS_STATE_STANDALONE_POSTGAME) || (cur_state == GS_STATE_MULTI_STD_WAIT) ||
		(cur_state == GS_STATE_WEAPON_SELECT) ||
	...
	else
	{
		...
		for (i = 0; i < MULTI_PAUSED_NUM_BUTTONS; i++)
		{
			...
			Multi_paused_buttons[gr_screen.res][i].button.create(&Multi_paused_window, "",
				Multi_paused_buttons[gr_screen.res][i].x, Multi_paused_buttons[gr_screen.res][i].y, 1, 1, 0, 1);

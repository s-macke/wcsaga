...
{
	return ((Multi_msg_mode != MULTI_MSG_NONE) && timestamp_elapsed(Multi_msg_stamp) &&
			Multi_msg_repeat_flag && !Multi_msg_text_enter) ? 1 : 0;
}
...
void multi_msg_show_squadmsg(net_player* source, int command, ushort target_sig, int subsys_type)
{
	...
	case DISABLE_SUBSYSTEM_ITEM:
		...
		{
			sprintf(temp_string, XSTR("Disable subsystem %s on %s", 705), Multi_msg_subsys_name[subsys_type],
				Ships[target_obj->instance].ship_name);
			strcat_s(hud_string, temp_string);
		}
		else
		{
...
int multi_msg_player_in_ship(int shipnum, net_player* pl)
{
	...
	if ((pl->m_player->objnum != -1) && (Objects[pl->m_player->objnum].type == OBJ_SHIP) &&
		(shipnum == Objects[pl->m_player->objnum].instance))
	{
		return 1;
	}
...
void multi_msg_send_squadmsg_packet(net_player* target, net_player* source, int command, ushort net_sig,
									int subsys_type)
{
	ubyte data[100];
	char s_val;
	...
	for (idx = 0; idx < MAX_PLAYERS; idx++)
	{
		if (MULTI_CONNECTED(Net_players[idx]) && !MULTI_STANDALONE(Net_players[idx]))
		{
			...
			if (multi_msg_player_in_wing(wingnum, &Net_players[idx]))
			{
				...
				{
					multi_msg_send_squadmsg_packet(&Net_players[idx], &Net_players[player_num], command, net_sig,
						subsys_type);
					sent_count--;
				}
			}

...
void multi_kick_process()
{
	...
	{
		if (MULTI_CONNECTED(Net_players[idx]) && (Net_players[idx].s_info.kick_timestamp != -1) &&
			timestamp_elapsed(Net_players[idx].s_info.kick_timestamp))
		{
...
void multi_kick_player(int player_index, int ban, int reason)
{
	...
	if (!(Game_mode & GM_STANDALONE_SERVER) && ((Net_players[player_index].flags & NETINFO_FLAG_GAME_HOST) ||
...
void multi_kick_get_text(net_player* pl, int reason, char* str)
{
	...
	case KICK_REASON_CANT_XFER:
		sprintf(str, XSTR("<%s was kicked for not having builtin mission %s>", 1004), pl->m_player->callsign,

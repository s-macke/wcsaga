	...
	case MULTI_MSG_ALL:
		for (idx = 0; idx < MAX_PLAYERS; idx++)
		{
			if (MULTI_CONNECTED(Net_players[idx]) &&													// player is connected
				...
				!(Net_players[idx].p_info.options.flags & MLO_FLAG_NO_VOICE))
			{				// is accepting sound periods
...
void multi_voice_alg_play_window(int stream_index)
{
	...
	if (Multi_voice_can_play)
	{
		...
		Multi_voice_stream[stream_index].stream_snd_handle = rtvoice_play_uncompressed(Multi_voice_stream[
			stream_index].stream_rtvoice_handle, (unsigned char*)Multi_voice_playback_buffer, buffer_offset);

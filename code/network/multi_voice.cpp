...
void multi_voice_server_process() {
    ...
    for (idx = 0; idx < MAX_PLAYERS; idx++) {
        if (MULTI_CONNECTED(Net_players[idx]) && (Net_players[idx].s_info.voice_token_timestamp != -1) &&
            timestamp_elapsed(Net_players[idx].s_info.voice_token_timestamp)) {
...
void multi_voice_player_process() {
    ...
    if (!Multi_voice_keydown && multi_voice_keydown() &&
        Multi_voice_can_record && !(Netgame.options.flags & MSO_FLAG_NO_VOICE)) {
...
void multi_voice_take_token(int stream_index) {
    ...
    if ((Multi_voice_stream[stream_index].token_status == (int) MULTI_VOICE_TOKEN_INDEX_FREE) ||
        (Multi_voice_stream[stream_index].token_status == (int) MULTI_VOICE_TOKEN_INDEX_RELEASED)) {
    ...
    if (Net_player == &Net_players[Multi_voice_stream[stream_index].token_status]) {
        ...
        // timestamp this guy so that he can't get the token back immediately
        Net_players[Multi_voice_stream[stream_index].token_status].s_info.voice_token_timestamp = timestamp(Netgame.
                options.voice_token_wait);
    }
...
void multi_voice_player_send_stream() {
    ...
    // get the data
    rtvoice_get_data((unsigned char **) &Multi_voice_record_buffer, &sound_size, &uncompressed_size, &d_gain, NULL,
                     NULL);
    gain = (float) d_gain;
    ...
    while (size_sent < sound_size) {
        ...
        if (Net_player->flags & NETINFO_FLAG_AM_MASTER) {
            multi_voice_route_data(data, packet_size, MY_NET_PLAYER_NUM, (int) msg_mode, (target_index == -1) ?
                                                                                         NULL
                                                                                                              : &Net_players[target_index]);
        } else {
        ...
        case MULTI_MSG_ALL:
            for (idx = 0; idx < MAX_PLAYERS; idx++) {
                if (MULTI_CONNECTED(Net_players[idx]) &&
                    ...
                    // not the sending player
                    (Net_player != &Net_players[idx]) &&                                                    // not me
                    (Multi_voice_player_prefs[idx] & (1 << player_index)) &&
                    // is accepting sound from this player
                    !(Net_players[idx].p_info.options.flags &
                      MLO_FLAG_NO_VOICE)) {                // is accepting sound periods
        ...
        case MULTI_MSG_FRIENDLY:
            for (idx = 0; idx < MAX_PLAYERS; idx++) {
                if (MULTI_CONNECTED(Net_players[idx]) &&
                    ...
                    (Multi_voice_player_prefs[idx] & (1 << player_index)) &&
                    // is accepting sound from the sender
                    !(Net_players[idx].p_info.options.flags &
                      MLO_FLAG_NO_VOICE)) {                // is accepting sound periods
...
int multi_voice_get_stream(int stream_id) {
    ...
    max_diff = -1;
    for (idx = 0; idx < MULTI_VOICE_MAX_STREAMS; idx++) {
        if (((max_diff_index == -1) || ((cur_time - Multi_voice_stream[idx].stream_last_heard) > max_diff)) &&
            (Multi_voice_stream[idx].token_stamp != -1)) {
            max_diff_index = idx;
...
void multi_voice_process_packet(ubyte *data, header *hinfo) {
    ...
    switch (code) {
        ...
        case MV_CODE_DATA:
            ...
            offset += multi_voice_process_data(data + offset, player_index,
                                               msg_mode, (target_index == -1) ? NULL : &Net_players[target_index]);

            // if we're the server of the game, we should also route this data to all other players
            if (Net_player->flags & NETINFO_FLAG_AM_MASTER) {
                multi_voice_route_data(data, offset, player_index,
                                       msg_mode, (target_index == -1) ? NULL : &Net_players[target_index]);
            }
            break;
...
void multi_voice_client_send_pending() {
    ...
    if (!Multi_voice_recording || (Multi_voice_current_stream_sent < 0) || (Multi_voice_current_stream_sent >
                                                                            Multi_voice_current_stream_index)) {
        return;
    }
...
#ifdef MULTI_VOICE_VERBOSE
        nprintf(("Network", "MULTI VOICE : PACKET %d %d\n", (int) str->accum_buffer_csize[sent], (int) str->
                accum_buffer_usize[sent]));
#endif
    ...
    if (Multi_voice_can_play) {
        ...
        for (idx = 0; idx < MULTI_VOICE_ACCUM_BUFFER_COUNT; idx++) {
            ...
            if (st->accum_buffer_flags[idx]) {
                ...
                rtvoice_uncompress(st->accum_buffer[idx], (int) st->accum_buffer_csize[idx],
                                   st->accum_buffer_gain[idx], (ubyte *) Multi_voice_playback_buffer + buffer_offset,
                                   st->accum_buffer_usize[idx]);
        ...
        buffer_offset = multi_voice_mix(MULTI_VOICE_POST_SOUND, Multi_voice_playback_buffer,
                                        buffer_offset, MULTI_VOICE_MAX_BUFFER_SIZE);
        ...
        Multi_voice_stream[stream_index].stream_snd_handle = rtvoice_play_uncompressed(Multi_voice_stream[
                                                                                               stream_index].stream_rtvoice_handle,
                                                                                       (unsigned char *) Multi_voice_playback_buffer,
                                                                                       buffer_offset);
...
void multi_voice_alg_process_streams() {
    ...
    for (idx = 0; idx < MULTI_VOICE_MAX_STREAMS; idx++) {
        ...
        if ((Multi_voice_stamps[idx] != -1) && multi_voice_alg_should_play(idx)) {
            ...
            if ((Net_player->flags & NETINFO_FLAG_AM_MASTER) && ((Net_player->p_info.options.flags &
                                                                  MLO_FLAG_NO_VOICE) || (player_index == -1) ||
                                                                 !(Multi_voice_player_prefs[MY_NET_PLAYER_NUM] & (1 <<
                                                                                                                    player_index)))) {
                // unset the stamp so that its not "free"

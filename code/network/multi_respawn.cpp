...
void multi_respawn_player(net_player* pl, char cur_primary_bank, char cur_secondary_bank, ubyte cur_link_status,
						  ushort ship_ets, ushort net_sign, char* parse_name, vec3d* pos = NULL);
...
void multi_respawn_handle_invul_players()
{
	...
	for (idx = 0; idx < MAX_PLAYERS; idx++)
	{
		if (MULTI_CONNECTED(Net_players[idx]) && (Objects[Net_players[idx].m_player->objnum].flags & OF_INVULNERABLE))
		{
			...
			if (((Net_players[idx].s_info.invul_timestamp != -1) && (timestamp_elapsed(Net_players[idx].s_info.
																	 invul_timestamp))) ||
				((Net_players[idx].m_player->ci.fire_primary_count > 0) || (Net_players[idx].m_player->ci.
																			fire_secondary_count > 0)))
			{
...
void multi_respawn_build_points()
{
	...
	{
		// stuff info
		if ((Ships[Objects[moveup->objnum].instance].respawn_priority > 0) && (Multi_respawn_priority_count <
																			   MAX_PRIORITY_POINTS))
		{
...
// respawn the passed player with the passed ship object and weapon link settings
void multi_respawn_player(net_player* pl, char cur_primary_bank, char cur_secondary_bank, ubyte cur_link_status,
						  ushort ship_ets, ushort net_sig, char* parse_name, vec3d* pos)
{
	int objnum;
	object* objp;
	ship* shipp;
...
void multi_respawn_process_packet(ubyte* data, header* hinfo)
{
	...
	case RESPAWN_BROADCAST:
		...
		// respawn the player
		multi_respawn_player(&Net_players[player_index], cur_primary_bank, cur_secondary_bank, cur_link_status,
			ship_ets, net_sig, parse_name, &v);
	...
	case RESPAWN_REQUEST:
		...
		{
			nprintf(("Network",
					 "Received respawn observer request from a player who shouldn't be respawning as an observer!\n"));
			Int3();
			break;
		}
		...
		else
		{
			...
			{
				multi_respawn_player(&Net_players[player_index], Net_players[player_index].s_info.cur_primary_bank,
					Net_players[player_index].s_info.cur_secondary_bank,
					Net_players[player_index].s_info.cur_link_status, Net_players[player_index].s_info.ship_ets, 0,
					Net_players[player_index].p_info.p_objp->name);
			}
		}
		break;
	}
...
void multi_respawn_server()
{
	...
	// respawn me
	multi_respawn_player(Net_player, Net_player->s_info.cur_primary_bank, Net_player->s_info.cur_secondary_bank,
		Net_player->s_info.cur_link_status, Net_player->s_info.ship_ets, 0, Net_player->p_info.p_objp->name);
...
void multi_respawn_place(object* new_obj, int team)
{
	...
	for (idx = 0; idx < Multi_respawn_priority_count; idx++)
	{
		...
		if ((Multi_respawn_priority_ships[idx].team == team) || !(Netgame.type_flags & NG_TYPE_TEAM))
		{
			...
			if ((lookup >= 0) && ((pri == NULL) || (Ships[lookup].respawn_priority > pri->respawn_priority)) &&
				(Ships[lookup].objnum >= 0) && (Ships[lookup].objnum < MAX_OBJECTS))
			{
				pri = &Ships[lookup];
	...
	if ((pri != NULL) && (pri_obj != NULL))
	{
		...
		else
		{
			...
			{
			case 0:
				vm_vec_scale_add(&new_obj->pos, &pri_obj->pos, &pri_obj->orient.vec.rvec, (pm->maxs.xyz.x -
																						   pm->mins.xyz.x));
				break;
			...
			case 2:
				vm_vec_scale_add(&new_obj->pos, &pri_obj->pos, &pri_obj->orient.vec.uvec, (pm->maxs.xyz.y -
																						   pm->mins.xyz.y));
				break;
			...
			case 4:
				vm_vec_scale_add(&new_obj->pos, &pri_obj->pos, &pri_obj->orient.vec.fvec, (pm->maxs.xyz.z -
																						   pm->mins.xyz.z));
				break;

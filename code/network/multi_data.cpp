                ...
                for (p_idx = 0; p_idx < MAX_PLAYERS; p_idx++) {
                    ...
                    if ((Net_player != &Net_players[p_idx]) &&
                        ...
                        if (multi_xfer_send_file(Net_players[p_idx].reliable_socket, Multi_data[idx].filename,
                                                 CF_TYPE_ANY, MULTI_XFER_FLAG_AUTODESTROY | MULTI_XFER_FLAG_QUEUE) <
                            0) {
...
void multi_data_send_my_junk() {
    ...
    if (ok_to_send) {
        ...
        if ((Net_player->flags & NETINFO_FLAG_AM_MASTER) && !(Game_mode & GM_STANDALONE_SERVER) &&
            (strlen(Net_player->m_player->image_filename) > 0)) {
            ...
            multi_xfer_send_file(Net_player->reliable_socket, Net_player->m_player->image_filename, CF_TYPE_ANY,
                                 MULTI_XFER_FLAG_AUTODESTROY | MULTI_XFER_FLAG_QUEUE);
        }
    }
    ...
    if (ok_to_send) {
        ...
        // host should put his own pic file in the list now
        if ((Net_player->flags & NETINFO_FLAG_AM_MASTER) && !(Game_mode & GM_STANDALONE_SERVER) &&
            (strlen(Net_player->m_player->squad_filename) > 0)) {
            multi_data_add_new(Net_player->m_player->squad_filename, MY_NET_PLAYER_NUM);
        }
            // otherwise clients should just queue up a send
        else {
            // add a file extension if necessary
            multi_xfer_send_file(Net_player->reliable_socket, Net_player->m_player->squad_filename, CF_TYPE_ANY,
                                 MULTI_XFER_FLAG_AUTODESTROY | MULTI_XFER_FLAG_QUEUE);
        }
    }
}
...
void multi_data_maybe_reload() {
    ...
    for (idx = 0; idx < MAX_PLAYERS; idx++) {
        if (MULTI_CONNECTED(Net_players[idx]) && strlen(Net_players[idx].m_player->squad_filename) &&

...
int multi_update_http_init()
{
	...
	// local file
	strcpy_s(local_file, Cfile_user_dir);
...
int multi_update_error_verifying()
{
	...
	strcat_s(out_str,
		XSTR(
		"There was an error verifying your version of FreeSpace, if you continue, you will not necessarily be up to date", 978));

	switch (popup(PF_USE_AFFIRMATIVE_ICON | PF_USE_NEGATIVE_ICON, 2, XSTR("&Go back", 1524), XSTR("&Continue", 1525),
			out_str))
	{

    ...
    while (1) {
        ...
        switch (Socket_type) {
            ...
            case NET_TCP:
                ...
                read_len = recvfrom(Unreliable_socket, (char *) packet_read.data, MAX_TOP_LAYER_PACKET_SIZE, 0,
                                    (SOCKADDR *) &ip_addr, &from_len);
                break;
...
void psnet_rel_send_ack(SOCKADDR *raddr, unsigned int sig, ubyte link_type, float time_sent) {
    ...
    switch (link_type) {
        ...
        case NET_TCP:
            ...
            ret = SENDTO(Unreliable_socket, (char *) &ack_header,
                         RELIABLE_PACKET_HEADER_ONLY_SIZE + ack_header.data_len, 0, raddr, sizeof(SOCKADDR),
                         PSNET_TYPE_RELIABLE);
            break;
        default:
...
void psnet_rel_close_socket(PSNET_SOCKET_RELIABLE *sockp) {
    ...
    switch (Reliable_sockets[*sockp].connection_type) {
        ...
        case NET_TCP:
            ...
            SENDTO(Unreliable_socket, (char *) &diss_conn_header, RELIABLE_PACKET_HEADER_ONLY_SIZE, 0,
                   &Reliable_sockets[*sockp].addr, sizeof(SOCKADDR), PSNET_TYPE_RELIABLE);
            break;
        default:
...
int psnet_rel_send(PSNET_SOCKET_RELIABLE socketid, ubyte *data, int length, int np_index) {
    ...
    if (rsocket->status != RNF_CONNECTED) {
        ...
        ml_printf("Can't send packet because of status %d in nw_SendReliable(). socket = %d", rsocket->status,
                  socketid);
        return -1;
    }
            ...
            if (send_this_packet) {
                switch (rsocket->connection_type) {
                    ...
                    case NET_TCP:
                        ...
                        bytesout = SENDTO(Unreliable_socket, (char *) &send_header,
                                          RELIABLE_PACKET_HEADER_ONLY_SIZE + rsocket->send_len[i], 0, &rsocket->addr,
...
void psnet_rel_work() {
        ...
        if ((Reliable_sockets[Serverconn].status == RNF_LIMBO) &&
            ...
            switch (Reliable_sockets[Serverconn].connection_type) {
                ...
                case NET_TCP:
                    ...
                    ret = SENDTO(Unreliable_socket, (char *) &conn_header, RELIABLE_PACKET_HEADER_ONLY_SIZE, 0,
                                 &Reliable_sockets[Serverconn].addr, sizeof(SOCKADDR), PSNET_TYPE_RELIABLE);
                    break;
                default:
    ...
    do {
        ...
        if (udp_has_data) {
            ...
            memset(&d3_rcv_addr, 0, sizeof(net_addr));
            memset(&rcv_addr, 0, sizeof(SOCKADDR));
            bytesin = RECVFROM(Unreliable_socket, (char *) &rcv_buff, sizeof(reliable_header), 0,
                               (SOCKADDR *) &rcv_addr, &addrlen, PSNET_TYPE_RELIABLE);
        ...
        if (bytesin) {
            ...
            if (rcv_buff.type == RNT_ACK) {
                ...
                if (rsocket->num_ping_samples >= MAX_PING_HISTORY) {
                    ...
                    rsocket->mean_ping =
                            ((sort_ping[MAX_PING_HISTORY / 2] + sort_ping[(MAX_PING_HISTORY / 2) + 1])) / 2;
                }
                rsocket->ping_pos++;
    ...
    for (j = 0; j < MAXRELIABLESOCKETS; j++) {
        ...
        if (rsocket->status == RNF_CONNECTED) {
            ...
            for (i = 0; i < MAXNETBUFFERS; i++) {
                // send again
                if ((rsocket->sbuffers[i]) &&
                    (fl_abs((psnet_get_time() - rsocket->timesent[i])) >= retry_packet_time)) {
                    reliable_header send_header;
                    send_header.send_time = psnet_get_time();
                    send_header.send_time = INTEL_FLOAT(&send_header.send_time);
                    ...
                    if (rsocket->connection_type == NET_TCP) {
                        rcode = SENDTO(Unreliable_socket, (char *) &send_header,
                                       RELIABLE_PACKET_HEADER_ONLY_SIZE + rsocket->send_len[i], 0, &rsocket->addr,
                                       sizeof(SOCKADDR), PSNET_TYPE_RELIABLE);
                    }
#ifdef _WIN32
            ...
            if ((rsocket->status == RNF_CONNECTED) &&
                (fl_abs((psnet_get_time() - rsocket->last_packet_sent)) > NETHEARTBEATTIME)) {
                reliable_header send_header;
                send_header.send_time = psnet_get_time();
                send_header.send_time = INTEL_FLOAT(&send_header.send_time);
                send_header.seq = 0;
                ...
                rcode = -1;
                if (rsocket->connection_type == NET_TCP) {
                    rcode = SENDTO(Unreliable_socket, (char *) &send_header, RELIABLE_PACKET_HEADER_ONLY_SIZE, 0,
                                   &rsocket->addr, sizeof(SOCKADDR), PSNET_TYPE_RELIABLE);
                }
#ifdef _WIN32
                else if(rsocket->connection_type == NET_IPX){
            ...
            if ((rsocket->status == RNF_CONNECTED) &&
                (fl_abs((psnet_get_time() - rsocket->last_packet_received)) > Nettimeout)) {
    ...
    *socket = INVALID_SOCKET;
...
#endif
            addrlen = sizeof(SOCKADDR);
            bytesin = RECVFROM(Unreliable_socket, (char *) &ack_header, sizeof(reliable_header), 0,
                               (SOCKADDR *) &rcv_addr, &addrlen, PSNET_TYPE_RELIABLE);
            if (bytesin == -1) {
                //Int3();//See Kevin
    ...
    switch (server_addr->type) {
        ...
        case NET_TCP:
            ...
            if (SOCKET_ERROR ==
                SENDTO(Unreliable_socket, (char *) &conn_header, RELIABLE_PACKET_HEADER_ONLY_SIZE, 0, addr,
                       sizeof(SOCKADDR), PSNET_TYPE_RELIABLE)) {
    ...
    do {
        ...
        if (SELECT(typeless_sock + 1, &read_fds, NULL, NULL, &timeout, PSNET_TYPE_RELIABLE)) {
            ...
            addrlen = sizeof(SOCKADDR);
            bytesin = RECVFROM(typeless_sock, (char *) &ack_header, sizeof(reliable_header), 0, (SOCKADDR *) &rcv_addr,
                               &addrlen, PSNET_TYPE_RELIABLE);
            if (bytesin == -1) {
                ml_printf("recvfrom returned an error! -- %d", WSAGetLastError());
                Int3();//See Kevin
                return;
                        ...
                        for (i = 1; i < MAXRELIABLESOCKETS; i++) {
                            if (Reliable_sockets[i].status == RNF_UNUSED) {
                                ...
                                *socket = i;
                                ...
                                Last_sent_iamhere = psnet_get_time();
                                int rcode = SENDTO(typeless_sock, (char *) &conn_header,
                                                   RELIABLE_PACKET_HEADER_ONLY_SIZE, 0, addr, sizeof(SOCKADDR),
                                                   PSNET_TYPE_RELIABLE);
                                ...
                                float f;
                                f = psnet_get_time();
                                while ((fl_abs((psnet_get_time() - f)) < 2) &&
                                       (Reliable_sockets[i].status != RNF_CONNECTING)) {
                                    psnet_rel_work();
                                }

...
void std_create_gen_dialog(char* title)
{
	...
	Multi_gen_dialog = CreateDialog(GetModuleHandle(NULL), MAKEINTRESOURCE(IDD_GEN),
		Psht, (DLGPROC)std_gen_dialog_proc);
	if (Multi_gen_dialog != NULL)
	{
...
void std_add_chat_text(char* text, int player_index, int add_id)
{
	...
	if (num_items > 19)
	{
		ret_val = SendMessage(Godstuff_player_messages, LB_SETTOPINDEX, (WPARAM)num_items -
			GODSTUFF_MAX_ITEMS, (LPARAM)0);
	}
}
...
void std_build_title_string(char* str)
{
	...
	{
		snprintf(ver_str, sizeof(ver_str) - 1, "%i.%i", SAGA_VERSION_MAJOR, SAGA_VERSION_MINOR);
	}
	else if (SAGA_VERSION_REVIS == 0)
	{
		snprintf(ver_str, sizeof(ver_str) - 1, "%i.%i.%i", SAGA_VERSION_MAJOR, SAGA_VERSION_MINOR, SAGA_VERSION_BUILD);
	}
	else
	{
		snprintf(ver_str, sizeof(ver_str) - 1, "%i.%i.%i.%i", SAGA_VERSION_MAJOR, SAGA_VERSION_MINOR,
	...
	case MSG_SYSTRAYICON:
		{
			...
			else if (lParam == WM_RBUTTONUP)
			{
				...
				int choice = TrackPopupMenuEx(stdPopup, TPM_RETURNCMD | TPM_NONOTIFY, cur_pos.x, cur_pos.y,
					Standalone_hwnd, NULL);
...
void std_init_standalone()
{
	...
	Standalone_thread = CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE)standalone_process, NULL,
		0, &Standalone_thread_id);

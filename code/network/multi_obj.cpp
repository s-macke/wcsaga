...
void multi_oo_calc_interp_splines(int ship_index, vec3d *cur_pos, matrix *cur_orient, physics_info *cur_phys_info,
                                  vec3d *new_pos, matrix *new_orient, physics_info *new_phys_info);
...
void multi_oo_build_ship_list(net_player *pl) {
    ...
    for (moveup = GET_FIRST(&Ship_obj_list); moveup != END_OF_LIST(&Ship_obj_list); moveup = GET_NEXT(moveup)) {
        ...
        if ((moveup->objnum < 0) || (Objects[moveup->objnum].instance < 0) || (Objects[moveup->objnum].type !=
                                                                               OBJ_SHIP)) {
            continue;
        }

        // if we're a standalone server, don't send any data regarding its pseudo-ship
        if ((Game_mode & GM_STANDALONE_SERVER) && ((&Objects[moveup->objnum] == Player_obj) ||
                                                   (Objects[moveup->objnum].net_signature == STANDALONE_SHIP_SIG))) {
            continue;
        }
        ...
        if ((Ships[Objects[moveup->objnum].instance].ship_info_index >= 0) && (Ships[Objects[moveup->objnum].instance].
                ship_info_index < Num_ship_classes) &&
            (Ship_info[Ships[Objects[moveup->objnum].instance].ship_info_index].flags & SIF_KNOSSOS_DEVICE)) {
            continue;
        }
...
int multi_oo_pack_data(net_player *pl, object *objp, ubyte oo_flags, ubyte *data_out) {
    ...
    if (oo_flags & OO_POS_NEW) {
        ...
        ret = (ubyte) multi_pack_unpack_vel(1, data + packet_size + header_bytes, &objp->orient, &objp->pos, &objp->
                phys_info);
        packet_size += ret;
    ...
    if (oo_flags & OO_ORIENT_NEW) {
        ...
        ret = (ubyte) multi_pack_unpack_rotvel(1, data + packet_size + header_bytes, &objp->orient, &objp->pos, &objp->
                phys_info);
        packet_size += ret;
    ...
    if (MULTIPLAYER_MASTER && (sip->flags & SIF_SUPPORT) && (shipp->ai_index >= 0) && (shipp->ai_index < MAX_AI_INFO)) {
        ...
        if ((Ai_info[shipp->ai_index].support_ship_objnum < 0) || (Ai_info[shipp->ai_index].support_ship_objnum >=
                                                                   MAX_OBJECTS)) {
            dock_sig = 0;
        } else {
...
int multi_oo_unpack_client_data(net_player *pl, ubyte *data) {
    ...
    if ((pl->m_player->objnum >= 0) && (Objects[pl->m_player->objnum].type == OBJ_SHIP) &&
        (Objects[pl->m_player->objnum].instance >= 0)) {
    ...
    if ((tobj != NULL) && (pl != NULL) && (pl->m_player->objnum != -1)) {
        ...
        if (Objects[pl->m_player->objnum].type == OBJ_SHIP) {
            ...
            if ((t_subsys != -1) && (tobj->type == OBJ_SHIP)) {
                Ai_info[Ships[Objects[pl->m_player->objnum].instance].ai_index].targeted_subsys =
                        ship_get_indexed_subsys(&Ships[tobj->instance], t_subsys);
            }
        }
...
int multi_oo_unpack_data(net_player *pl, ubyte *data) {
    ...
    if ((pobjp == NULL) || (pobjp->type != OBJ_SHIP) || (pobjp->instance < 0) || (pobjp->instance >= MAX_SHIPS) ||
        (Ships[pobjp->instance].ship_info_index < 0) || (Ships[pobjp->instance].ship_info_index >= Num_ship_classes)) {
        offset += data_size;
        return offset;
    }
    ...
    if (oo_flags & OO_POS_NEW) {
        ...
        if (oo_arrive_time_count[shipp - Ships] == 5) {
            ...
            for (idx = 0; idx < 4; idx++) {
                oo_arrive_time_avg_diff[shipp - Ships] += oo_arrive_time[shipp - Ships][idx + 1] -
                                                          oo_arrive_time[shipp - Ships][idx];
            }
    ...
    if (oo_flags & OO_POS_NEW) {
        ...
        if (oo_interp_count[shipp - Ships] < 2) {
            ...
            multi_oo_calc_interp_splines(shipp - Ships, &pobjp->pos, &pobjp->orient, &pobjp->
                    phys_info, &new_pos, &new_orient, &new_phys_info);
        }
...
int multi_oo_maybe_update(net_player *pl, object *obj, ubyte *data) {
    ...
    if ((Ships[obj->instance].np_updates[player_index].status_update_stamp == -1) ||
        timestamp_elapsed_safe(Ships[obj->instance].np_updates[player_index].status_update_stamp, OO_MAX_TIMESTAMP)) {
    ...
    // if the object's hull/shield timestamp has expired
    if ((Ships[obj->instance].np_updates[player_index].subsys_update_stamp == -1) ||
        timestamp_elapsed_safe(Ships[obj->instance].np_updates[player_index].subsys_update_stamp, OO_MAX_TIMESTAMP)) {
    ...
    if ((shipp->np_updates[player_index].pos_chksum != 0) && (shipp->np_updates[player_index].pos_chksum ==
                                                              cur_pos_chksum)) {
    ...
    if ((shipp->np_updates[player_index].orient_chksum != 0) && (shipp->np_updates[player_index].orient_chksum ==
                                                                 cur_orient_chksum)) {
        // if we otherwise would have been sending it, keep track of it (debug only)
#ifndef NDEBUG
        if (oo_flags & OO_ORIENT_NEW) {
...
void multi_oo_process() {
    ...
    for (idx = 0; idx < MAX_PLAYERS; idx++) {
        if (MULTI_CONNECTED(Net_players[idx]) && !MULTI_STANDALONE(Net_players[idx]) &&
            (Net_player != &Net_players[idx])/*&& !MULTI_OBSERVER(Net_players[idx])*/ ) {
            // now process the rest of the objects
            multi_oo_process_all(&Net_players[idx]);

            // do firing stuff for this player
            if ((Net_players[idx].m_player != NULL) && (Net_players[idx].m_player->objnum >= 0) &&
                !(Net_players[idx].flags & NETINFO_FLAG_LIMBO) && !(Net_players[idx].flags & NETINFO_FLAG_RESPAWNING)) {
                if ((Objects[Net_players[idx].m_player->objnum].flags &
                     OF_PLAYER_SHIP) && !(Objects[Net_players[idx].m_player->objnum].flags & OF_SHOULD_BE_DEAD)) {
...
void multi_oo_server_process() {
    ...
    for (idx = 0; idx < MAX_PLAYERS; idx++) {
        if (MULTI_CONNECTED(Net_players[idx]) && !MULTI_SERVER(Net_players[idx])) {
            ...
            if ((Net_players[idx].s_info.rate_stamp == -1) || timestamp_elapsed_safe(Net_players[idx].s_info.
                    rate_stamp, OO_MAX_TIMESTAMP) || (abs(timestamp_ticker - Net_players[idx].s_info.
                    rate_stamp) >= (int) (1000.0f / (float) OO_gran))) {
                Net_players[idx].s_info.rate_stamp = timestamp((int) (1000.0f / (float) OO_gran));
...
void multi_oo_calc_interp_splines(int ship_index, vec3d *cur_pos, matrix *cur_orient, physics_info *cur_phys_info,
    ...
    physics_sim(&c, &m_copy, &p_copy,
                avg_diff * oo_error);            // next point, assuming we followed our current path

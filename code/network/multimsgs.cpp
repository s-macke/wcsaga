...
void process_game_chat_packet(ubyte* data, header* hinfo)
{
	...
	if (Net_player->flags & NETINFO_FLAG_AM_MASTER)
	{
		...
		case MULTI_MSG_TARGET:
			...
			{
				send_game_chat_packet(&Net_players[player_index], msg, (int)mode, &Net_players[to_player_index], NULL,
					server_msg);
			}
			break;
			// expression mode
...
void process_deny_packet(ubyte* data, header* hinfo)
{
	...
	case JOIN_DENY_JR_TRACKER_INVAL :
		popup(PF_USE_AFFIRMATIVE_ICON, 1, POPUP_OK,
			XSTR(
			"You have been rejected because this is a Parallax Online game and you are not a registered Parallax Online pilot", 723));
		break;
	...
	case JOIN_DENY_JR_TEMP_CLOSED :
		popup(PF_USE_AFFIRMATIVE_ICON, 1, POPUP_OK,
			XSTR("You have been rejected because the netgame is forming and the host has temporarily closed it", 726));
		break;
	...
	case JOIN_DENY_JR_INGAME_JOIN :
		popup(PF_USE_AFFIRMATIVE_ICON, 1, POPUP_OK,
			XSTR("You cannot join at this time since someone else is currently joining.  Try again shortly.", 733));
		break;

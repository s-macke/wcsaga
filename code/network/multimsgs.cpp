...
void multi_io_send_force(net_player *pl) {
    ...
    if (MULTIPLAYER_MASTER) {
        psnet_send(&pl->p_info.addr, pl->s_info.unreliable_buffer, pl->s_info.unreliable_buffer_size,
                   NET_PLAYER_NUM(pl));

        // add the bytes sent to this player
        pl->sv_bytes_sent += pl->s_info.unreliable_buffer_size;
    } else {
        psnet_send(&Netgame.server_addr, pl->s_info.unreliable_buffer, pl->s_info.unreliable_buffer_size,
                   NET_PLAYER_NUM(pl));
    }
    pl->s_info.unreliable_buffer_size = 0;
}
...
void multi_io_send_reliable_force(net_player *pl) {
    ...
    // send everything in
    if (MULTIPLAYER_MASTER) {
        psnet_rel_send(pl->reliable_socket, pl->s_info.reliable_buffer, pl->s_info.reliable_buffer_size,
                       NET_PLAYER_NUM(pl));
    } else if (Net_player != NULL) {
        psnet_rel_send(Net_player->reliable_socket, pl->s_info.reliable_buffer, pl->s_info.reliable_buffer_size,
                       NET_PLAYER_NUM(pl));
    }
    pl->s_info.reliable_buffer_size = 0;
}
    ...
    if (Net_player->flags & NETINFO_FLAG_AM_MASTER) {
        switch (mode) {
            ...
            case MULTI_MSG_FRIENDLY:
                for (idx = 0; idx < MAX_PLAYERS; idx++) {
                    if (MULTI_CONNECTED(Net_players[idx]) && !MULTI_STANDALONE(Net_players[idx]) &&
                        (&Net_players[idx] != from) && (Net_players[idx].p_info.team == from->p_info.team)) {
                        multi_io_send_reliable(&Net_players[idx], data, packet_size);
                    }
                }
                break;
            ...
            case MULTI_MSG_EXPR:
                ...
                for (idx = 0; idx < MAX_PLAYERS; idx++) {
                    if (MULTI_CONNECTED(Net_players[idx]) && !MULTI_STANDALONE(Net_players[idx]) &&
                        (&Net_players[idx] != from) && multi_msg_matches_expr(&Net_players[idx], expr)) {
                        multi_io_send_reliable(&Net_players[idx], data, packet_size);
                        undeliverable = false;
                    }
                }
            ...
            switch (mode) {
                case MULTI_MSG_EXPR:
                    ...
                    if (from == Net_player) {
                        ...
                        send_game_chat_packet(Net_player, "Unable to send message, player does not exist",
                                              MULTI_MSG_TARGET, from, NULL, 1);
                    }
                    break;
            }
        }
        ...
        switch (mode) {
            ...
            case MULTI_MSG_TARGET:
                ...
                if (to_player_index != MY_NET_PLAYER_NUM) {
                    send_game_chat_packet(&Net_players[player_index], msg, (int) mode, &Net_players[to_player_index],
                                          NULL,
                                          server_msg);
                }
                break;
...
void send_accept_packet(int new_player_num, int code, int ingame_join_team) {
    ...
    for (i = 0; i < MAX_PLAYERS; i++) {
        ...
        if (!MULTI_CONNECTED(Net_players[i]) || psnet_same(&Net_players[new_player_num].p_info.addr, &(Net_players[i].
                p_info.addr)) ||
            (Net_player == &Net_players[i])) {
            continue;
        }
    ...
    if (Net_players[new_player_num].tracker_player_id >= 0) {
        ml_printf(NOX("Server accepted %s (tracker id %d) as new client"),
                  Net_players[new_player_num].m_player->callsign, Net_players[new_player_num].tracker_player_id);
    } else {
...
void send_game_active_packet(net_addr *addr) {
    ...
    flags = 0;
    if ((Netgame.mode == NG_MODE_PASSWORD) || ((Game_mode & GM_STANDALONE_SERVER) && (multi_num_players() == 0) &&
                                               (std_is_host_passwd()))) {
...
void process_netgame_update_packet(ubyte *data, header *hinfo) {
    ...
    if (ng_state == NETGAME_STATE_MISSION_SYNC) {
        ...
        if ((Netgame.game_state == NETGAME_STATE_FORMING) ||
            ((Netgame.game_state != NETGAME_STATE_FORMING) && ((gameseq_get_state() == GS_STATE_MULTI_HOST_SETUP) ||
                                                               (gameseq_get_state() == GS_STATE_MULTI_CLIENT_SETUP)))) {
            ...
            // if coming from the debriefing state
                 ...
                 ((Netgame.game_state != NETGAME_STATE_DEBRIEF) && ((gameseq_get_state() == GS_STATE_DEBRIEF) ||
                                                                    (gameseq_get_state() ==
        ...
        if ((Netgame.game_state == NETGAME_STATE_MISSION_SYNC) ||
            ((Netgame.game_state != NETGAME_STATE_MISSION_SYNC) && (gameseq_get_state() ==
...
void process_game_query(ubyte *data, header *hinfo) {
    ...
    if ((Netgame.game_state == NETGAME_STATE_STD_HOST_SETUP) || (Netgame.game_state == NETGAME_STATE_HOST_SETUP) ||
        (Netgame.game_state == 0) || (Netgame.max_players == -1)) {
        return;
    }
...
void process_netplayer_update_packet(ubyte *data, header *hinfo) {
    ...
    while (stop != 0xff) {
        ...
        if ((player_num == -1) || (Net_player == &Net_players[player_num])) {
            ...
            if ((Net_players[player_num].state == NETPLAYER_STATE_JOINING) && (new_state == NETPLAYER_STATE_JOINED) &&
    ...
    if ((Game_mode & GM_STANDALONE_SERVER) && (player_num != -1) && (Net_players[player_num].flags &
                                                                     NETINFO_FLAG_GAME_HOST)) {
...
void process_secondary_fired_packet(ubyte *data, header *hinfo, int from_player) {
    ...
    shipp->weapons.detonate_weapon_time = timestamp(
            5000);        // be sure that we don't detonate a remote weapon before it is time.
...
void send_turret_fired_packet(int ship_objnum, int subsys_index, int weapon_objnum) {
    ...
    // sanity
    if ((weapon_objnum < 0) || (Objects[weapon_objnum].type != OBJ_WEAPON) || (Objects[weapon_objnum].instance < 0) ||
        (Weapons[Objects[weapon_objnum].instance].weapon_info_index < 0)) {
        return;
    }
...
void send_mission_message_packet(int id, char *who_from, int priority, int timing, int source, int builtin_type,
                                 int multi_target, int multi_team_filter, int delay) {
    int packet_size;
...
void process_mission_message_packet(ubyte *data, header *hinfo) {
    ...
    if ((builtin_type >= 0) && (Netgame.type_flags & NG_TYPE_TEAM) && (Net_player != NULL) &&
...
void send_observer_update_packet() {
    ...
    if ((Player_obj == NULL) || (Player_obj->type != OBJ_OBSERVER) || (Net_player == NULL) || !(Net_player->flags &
...
void send_repair_info_packet(object *repaired_objp, object *repair_objp, int code) {
    ...
    nprintf(("Network", "Repair: %s sent to all players (%s/%s)\n", repair_text[cd],
            Ships[repaired_objp->instance].ship_name, (repair_objp == NULL) ? "<none>" : Ships[repair_objp->instance].
            ship_name));
}
...
void process_repair_info_packet(ubyte *data, header *hinfo) {
    ...
    nprintf(("Network", "Repair: %s received (%s/%s)\n", repair_text[code], (repaired_objp == NULL) ? "<None>" :
                                                                            Ships[repaired_objp->instance].ship_name, (repair_objp ==
                                                                                                                       NULL)
                                                                                                                      ? "<None>"
...
// ALAN begin
void send_wss_request_packet(short player_id, int from_slot, int from_index, int to_slot, int to_index,
                             int wl_ship_slot, int ship_class, int mode, net_player *p) {
    ubyte data[MAX_PACKET_SIZE];
...
void process_wss_request_packet(ubyte *data, header *hinfo) {
    ...
    if (Game_mode & GM_STANDALONE_SERVER) {
        send_wss_request_packet(player_id, from_slot, from_index, to_slot, to_index, wl_ship_slot, ship_class, mode,
                                Netgame.host);
    }
    ...
    switch (code) {
        case JOIN_DENY_JR_STATE :
            popup(PF_USE_AFFIRMATIVE_ICON, 1, POPUP_OK,
                  XSTR("You have been rejected because the game is not in an appropriate state to accept", 722));
            break;
        ...
        case JOIN_DENY_JR_PASSWD :
            popup(PF_USE_AFFIRMATIVE_ICON, 1, POPUP_OK,
                  XSTR("You have been rejected because this is a password protected game", 724));
            break;
        case JOIN_DENY_JR_CLOSED :
            popup(PF_USE_AFFIRMATIVE_ICON, 1, POPUP_OK,
                  XSTR("You have been rejected because this is a closed game and the mission is in progress", 725));
            break;
        case JOIN_DENY_JR_TEMP_CLOSED :
            popup(PF_USE_AFFIRMATIVE_ICON, 1, POPUP_OK,
                  XSTR("You have been rejected because the netgame is forming and the host has temporarily closed it",
                       726));
            break;
        case JOIN_DENY_JR_RANK_HIGH :
            popup(PF_USE_AFFIRMATIVE_ICON, 1, POPUP_OK,
                  XSTR("You have been rejected because this is a rank limited game and your rank is too high", 727));
            break;
        case JOIN_DENY_JR_RANK_LOW :
            popup(PF_USE_AFFIRMATIVE_ICON, 1, POPUP_OK,
                  XSTR("You have been rejected because this is a rank limited game and your rank is too low", 728));
            break;
        case JOIN_DENY_JR_DUP :
            popup(PF_USE_AFFIRMATIVE_ICON, 1, POPUP_OK,
                  XSTR("You have been rejected because there is an identical player already in the game", 729));
            break;
        ...
        case JOIN_DENY_JR_BANNED :
            popup(PF_USE_AFFIRMATIVE_ICON, 1, POPUP_OK,
                  XSTR("You have been rejected because you are banned from this server", 731));
            break;
        case JOIN_DENY_JR_NOOBS :
            popup(PF_USE_AFFIRMATIVE_ICON, 1, POPUP_OK,
                  XSTR("You have been rejected because this game does not allow observers", 732));
            break;
        case JOIN_DENY_JR_INGAME_JOIN :
            popup(PF_USE_AFFIRMATIVE_ICON, 1, POPUP_OK,
                  XSTR("You cannot join at this time since someone else is currently joining.  Try again shortly.",
                       733));
            break;
        ...
        case JOIN_DENY_JR_TYPE :
            popup(PF_USE_AFFIRMATIVE_ICON, 1, POPUP_OK,
                  XSTR("You cannot join a game in progress unless it is a dogfight mission.", 1433));
            break;
    }
    ...
    if (Game_mode & GM_STANDALONE_SERVER) {
        ...
        for (idx = 0; idx < MAX_PLAYERS; idx++) {
            if (MULTI_CONNECTED(Net_players[idx]) && !MULTI_STANDALONE(Net_players[idx]) &&
                ...
                multi_server_update_player_weapons(&Net_players[idx], &Ships[Objects[Net_players[idx].m_player->
                        objnum].instance]);
            }
        }
...
void send_client_update_packet(net_player *pl) {
    ...
    } else if ((Game_mode & GM_IN_MISSION) && !(pl->flags & NETINFO_FLAG_INGAME_JOIN) && !(NETPLAYER_IS_OBSERVER(pl)) &&
               !(NETPLAYER_IS_DEAD(pl)) && (Objects[pl->m_player->objnum].type == OBJ_SHIP)) {
    ...
    if (val & UPDATE_HULL_INFO) {
        ...
        // now the subsystems.
        for (subsysp = GET_FIRST(&shipp->subsys_list); subsysp != END_OF_LIST(&shipp->subsys_list);
...
void process_client_update_packet(ubyte *data, header *hinfo) {
    ...
    if (have_hull_info) {
        ...
        ship_info *sip;
        ship *shipp;
        ubyte hull_percent, shield_percent[MAX_SHIELD_SECTIONS], n_subsystems, subsystem_percent[MAX_MODEL_SUBSYSTEMS],
                threats;
        ubyte ub_tmp;
        ship_subsys *subsysp;
        object *objp;
        ...
        if (!(Net_player->flags & NETINFO_FLAG_INGAME_JOIN) && (Player_ship != NULL) && (Player_obj != NULL) &&
            (Net_player != NULL)) {
            shipp = Player_ship;
            objp = Player_obj;
            sip = &Ship_info[shipp->ship_info_index];
    ...
    if (!popup_active() && !(Net_player->flags & NETINFO_FLAG_RESPAWNING) &&
...
void process_emp_effect(ubyte *data, header *hinfo) {
    ...
    if ((objp != NULL) && (objp->type == OBJ_SHIP)) {
        ...
        if (!(Net_player->flags & NETINFO_FLAG_OBSERVER) && (Player_obj != NULL) && (Player_obj->type == OBJ_SHIP) &&
            (Player_obj == objp)) {
...
void send_beam_fired_packet(object *shooter, ship_subsys *turret, object *target, int beam_info_index,
                            beam_info *override, ubyte fighter_beam, int bank_point) {
    ubyte data[MAX_PACKET_SIZE];
    int packet_size = 0;
    short u_beam_info;
    char subsys_index;
...
void process_beam_fired_packet(ubyte *data, header *hinfo) {
    ...
    if ((fire_info.shooter == NULL) || (fire_info.shooter->type != OBJ_SHIP) || (fire_info.shooter->instance < 0) ||
...
void send_weapon_or_ammo_changed_packet(int ship_index, int bank_type, int bank_number, int ammo_left, int rearm_limit,
...
void send_player_pain_packet(net_player *pl, int weapon_info_index, float damage, vec3d *force, vec3d *hitpos,
    ...
    ADD_VECTOR((*force));
    ADD_VECTOR((*hitpos));
    short_quadrant = (short) quadrant;
    ADD_SHORT(short_quadrant);
...
void process_player_pain_packet(ubyte *data, header *hinfo) {
    ...
    ship_hit_pain((float) udamage, (int) quadrant);
    ...
    if (captain_change) {
        ...
        for (idx = 0; idx < MAX_PLAYERS; idx++) {
            if (MULTI_CONNECTED(Net_players[idx]) && (Net_players[idx].player_id == player_id)) {
                HUD_printf("%s is the new captain of team %d", Net_players[idx].m_player->callsign,
...
void process_self_destruct_packet(ubyte *data, header *hinfo) {
    ...
    if ((Objects[Net_players[np_index].m_player->objnum].instance < 0) || (Objects[Net_players[np_index].m_player->
            objnum].instance >= MAX_SHIPS)) {
        return;
    }

...
void multi_options_read_config()
{
	...
	Multi_options_g.port = (Cmdline_network_port >= 0) ? (ushort)Cmdline_network_port : forced_port ==
		0 ? (ushort)DEFAULT_GAME_PORT : forced_port;
	...
	else
	{
		while (!cfeof(in))
		{
			...
			if (Is_standalone)
			{
				...
				else // set the standalone server's permanent name
				if (SETTING("+name"))
				{
					NEXT_TOKEN()
					;
				...
				else // standalone won't allow voice transmission
				if (SETTING("+no_voice"))
				{
					Multi_options_g.std_voice = 0;
				}
				else // set the max # of players on the standalone
				...
				else // set the standalone host password
				if (SETTING("+passwd"))
				{
					NEXT_TOKEN()
					;
				...
				else // standalone pxo login user
				...
				else // standalone pxo login password
			...
			else // port to use for the game/user tracker (FS2NetD)
			...
			else // ip addr of pxo chat server
			...
			else // url of pilot rankings page
			if (SETTING("+rank_url"))
			{
				NEXT_TOKEN()
				;
			...
			else // url of pxo account create page
			...
			else // url of pxo account verify page
			...
			else // set the max datarate for high updates
			if (SETTING("+datarate"))
			{
				NEXT_TOKEN()
				;

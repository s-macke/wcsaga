...
void multi_ts_init_graphics() {
    ...
    for (idx = 0; idx < MULTI_TS_NUM_BUTTONS; idx++) {
        ...
        if ((idx == MULTI_TS_SHIPS_UP) || (idx == MULTI_TS_SHIPS_DOWN)) {
            Multi_ts_buttons[gr_screen.res][idx].button.create(&Multi_ts_window, "",
                                                               Multi_ts_buttons[gr_screen.res][idx].x,
                                                               Multi_ts_buttons[gr_screen.res][idx].y, 1, 1, 1, 1);
        } else {
            Multi_ts_buttons[gr_screen.res][idx].button.create(&Multi_ts_window, "",
                                                               Multi_ts_buttons[gr_screen.res][idx].x,
                                                               Multi_ts_buttons[gr_screen.res][idx].y, 1, 1, 0, 1);
        }
    ...
    Multi_ts_window.add_XSTR("Briefing", 765, Multi_ts_buttons[gr_screen.res][MULTI_TS_BRIEFING].xt,
                             Multi_ts_buttons[gr_screen.res][MULTI_TS_BRIEFING].yt,
                             &Multi_ts_buttons[gr_screen.res][MULTI_TS_BRIEFING].
    ...
    Multi_ts_window.add_XSTR("Commit", 1062, Multi_ts_buttons[gr_screen.res][MULTI_TS_COMMIT].xt,
                             Multi_ts_buttons[gr_screen.res][MULTI_TS_COMMIT].yt,
                             &Multi_ts_buttons[gr_screen.res][MULTI_TS_COMMIT].button,
                             UI_XSTR_COLOR_PINK);
    Multi_ts_window.add_XSTR("Lock", 1270, Multi_ts_buttons[gr_screen.res][MULTI_TS_LOCK].xt,
                             Multi_ts_buttons[gr_screen.res][MULTI_TS_LOCK].yt,
...
void multi_ts_blit_wings() {
    ...
    for (idx = 0; idx < MAX_WSS_SLOTS; idx++) {
        ...
        if (Multi_ts_team[Net_player->p_info.team].multi_ts_flag[idx] == MULTI_TS_FLAG_EMPTY) {
            ...
            ss_blit_ship_icon(Multi_ts_slot_icon_coords[idx][gr_screen.res][MULTI_TS_X_COORD],
                              Multi_ts_slot_icon_coords[idx][gr_screen.res][MULTI_TS_Y_COORD],
                              Wss_slots[idx].ship_class,
            ...
            if ((Netgame.type_flags & NG_TYPE_TEAM) && (Multi_ts_team[Net_player->p_info.team].multi_ts_player[idx] !=
                ...
                gr_string(Multi_ts_slot_icon_coords[idx][gr_screen.res][MULTI_TS_X_COORD] - 5,
                          Multi_ts_slot_icon_coords[idx][gr_screen.res][MULTI_TS_Y_COORD] - 5,
                          XSTR("C", 737));  // [[ Team captain ]]
            }
        }
    }
}
...
void multi_ts_handle_mouse() {
    ...
    switch (region_type) {
        case MULTI_TS_PLAYER_LIST:
            if ((Multi_ts_hotspot_index != region_index) && (region_index >= 0) &&
                (Multi_ts_team[Net_player->p_info.team].
    ...
    switch (Multi_ts_carried_from_type) {
        ...
        case MULTI_TS_PLAYER_LIST:
            ...
            strcpy_s(callsign, Multi_ts_team[Net_player->p_info.team].multi_ts_player[Multi_ts_carried_from_index]->
                    m_player->callsign);
            gr_force_fit_string(callsign, CALLSIGN_LEN,
                                Multi_ts_slot_text_coords[Multi_ts_carried_from_index][gr_screen.
...
void multi_ts_commit_pressed() {
    ...
    switch (multi_ts_ok_to_commit()) {
        ...
        case 1:
            gamesnd_play_iface(SND_GENERAL_FAIL);
            popup(PF_USE_AFFIRMATIVE_ICON | PF_BODY_BIG, 1, POPUP_OK,
                  XSTR("You have not yet assigned all necessary ships",
                       752));
            break;

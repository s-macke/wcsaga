...
static int Multi_ts_slot_icon_coords[MULTI_TS_NUM_SHIP_SLOTS][GR_NUM_RESOLUTIONS][2] =
{							// x,y
	{		// 	
		{
			128,
...
static int Multi_ts_slot_text_coords[MULTI_TS_NUM_SHIP_SLOTS][GR_NUM_RESOLUTIONS][3] =
{	// x,y,width
	{		// alpha
		{
			112,
...
static int Multi_ts_avail_coords[MULTI_TS_AVAIL_MAX_DISPLAY][GR_NUM_RESOLUTIONS][2] =
{							// x,y coords
	{		// 
		{
			23,
...
int multi_ts_can_perform(int from_type, int from_index, int to_type, int to_index, int ship_class,
						 int player_index = -1);
...
int multi_ts_disabled_slot(int slot_num, int player_index)
{
	...
	else
	{
		...
		if (Netgame.type_flags & NG_TYPE_TEAM)
		{
			...
			if (pl->flags & NETINFO_FLAG_TEAM_CAPTAIN)
			{
				if ((Multi_ts_team[pl->p_info.team].multi_ts_player[slot_num] != NULL) &&
					(Objects[Multi_ts_team[pl->p_info.team].multi_ts_objnum[slot_num]].flags &
					 OF_PLAYER_SHIP) && (slot_num != pl->p_info.ship_index))
				{
					return 1;
				}
...
int multi_ts_disabled_high_slot(int slot_index, int player_index)
{
	...
	if (multi_ts_disabled_slot(slot_index, player_index) && !Multi_ts_team[pl->p_info.team].multi_players_locked &&
		(slot_index == pl->p_info.ship_index))
	{
		return 1;
	}
...
void multi_ts_assign_players_all()
{
	...
	multi_assign_player_ship(NET_PLAYER_INDEX(Netgame.host), &Objects[Ships[shipnum].objnum],
	...
	while (objp != END_OF_LIST(&obj_used_list))
	{
		...
		if ((objp->flags & OF_PLAYER_SHIP) && stricmp(Ships[objp->instance].ship_name, name_lookup))
		{
			...
			if (Netgame.type_flags & NG_TYPE_TEAM)
			{
				...
				{
					if (MULTI_CONNECTED(Net_players[idx]) && !MULTI_STANDALONE(Net_players[idx]) &&
						!MULTI_OBSERVER(Net_players[idx]) && (Net_players[idx].p_info.ship_index == -1) &&
						(Net_players[idx].p_info.team == team_index))
					{
						found = 1;
						break;
					}
	...
	for (idx = 0; idx < MAX_PLAYERS; idx++)
	{
		if (MULTI_CONNECTED(Net_players[idx]) && !MULTI_STANDALONE(Net_players[idx]) && !MULTI_OBSERVER(Net_players[
			idx]))
		{
	...
	for (idx = 0; idx < MULTI_TS_MAX_TVT_TEAMS; idx++)
	{
		for (s_idx = 0; s_idx < MULTI_TS_NUM_SHIP_SLOTS; s_idx++)
		{
			...
			else if (Multi_ts_team[idx].multi_ts_flag[s_idx] == MULTI_TS_FLAG_EMPTY)
			{
				...
				// delete the ship
				ship_add_exited_ship(&Ships[Objects[Multi_ts_deleted_objnums[Multi_ts_num_deleted]].instance],
					SEF_PLAYER_DELETED);
				obj_delete(Multi_ts_deleted_objnums[Multi_ts_num_deleted]);
				ship_wing_cleanup(Objects[Multi_ts_deleted_objnums[Multi_ts_num_deleted]].
					instance, &Wings[Ships[Objects[Multi_ts_team[idx].multi_ts_objnum[s_idx]].instance].wingnum]);
	...
	for (idx = 0; idx < MULTI_TS_MAX_TVT_TEAMS; idx++)
	{
		for (s_idx = 0; s_idx < MULTI_TS_NUM_SHIP_SLOTS; s_idx++)
		{
				...
				!MULTI_CONNECTED((*Multi_ts_team[idx].multi_ts_player[s_idx])))
			{
				...
				obj_set_flags(&Objects[Multi_ts_team[idx].multi_ts_objnum[s_idx]],
					Objects[Multi_ts_team[idx].multi_ts_objnum[s_idx]].flags | OF_COULD_BE_PLAYER);
			}
		}
	}
}
...
void multi_ts_init_graphics()
{
	...
	for (idx = 0; idx < MULTI_TS_NUM_BUTTONS; idx++)
	{
		...
		{
			Multi_ts_buttons[gr_screen.res][idx].button.create(&Multi_ts_window, "",
				Multi_ts_buttons[gr_screen.res][idx].x, Multi_ts_buttons[gr_screen.res][idx].y, 1, 1, 1, 1);
		}
		else
		{
			Multi_ts_buttons[gr_screen.res][idx].button.create(&Multi_ts_window, "",
				Multi_ts_buttons[gr_screen.res][idx].x, Multi_ts_buttons[gr_screen.res][idx].y, 1, 1, 0, 1);
		}
	...
	// add some text	
	Multi_ts_window.add_XSTR("Briefing", 765, Multi_ts_buttons[gr_screen.res][MULTI_TS_BRIEFING].xt,
		Multi_ts_buttons[gr_screen.res][MULTI_TS_BRIEFING].yt, &Multi_ts_buttons[gr_screen.res][MULTI_TS_BRIEFING].
		button, UI_XSTR_COLOR_GREEN);
	Multi_ts_window.add_XSTR("Ship Selection", 1067, Multi_ts_buttons[gr_screen.res][MULTI_TS_SHIP_SELECT].xt,
		Multi_ts_buttons[gr_screen.res][MULTI_TS_SHIP_SELECT].yt, &Multi_ts_buttons[gr_screen.
		res][MULTI_TS_SHIP_SELECT].button, UI_XSTR_COLOR_GREEN);
	Multi_ts_window.add_XSTR("Weapon Loadout", 1068, Multi_ts_buttons[gr_screen.res][MULTI_TS_WEAPON_SELECT].xt,
		Multi_ts_buttons[gr_screen.res][MULTI_TS_WEAPON_SELECT].yt, &Multi_ts_buttons[gr_screen.
		res][MULTI_TS_WEAPON_SELECT].button, UI_XSTR_COLOR_GREEN);
	Multi_ts_window.add_XSTR("Commit", 1062, Multi_ts_buttons[gr_screen.res][MULTI_TS_COMMIT].xt,
		Multi_ts_buttons[gr_screen.res][MULTI_TS_COMMIT].yt, &Multi_ts_buttons[gr_screen.res][MULTI_TS_COMMIT].button,
		UI_XSTR_COLOR_PINK);
	Multi_ts_window.add_XSTR("Lock", 1270, Multi_ts_buttons[gr_screen.res][MULTI_TS_LOCK].xt,
		Multi_ts_buttons[gr_screen.res][MULTI_TS_LOCK].yt, &Multi_ts_buttons[gr_screen.res][MULTI_TS_LOCK].button,
		UI_XSTR_COLOR_GREEN);
...
void multi_ts_blit_wings()
{
	...
	for (idx = 0; idx < MAX_WSS_SLOTS; idx++)
	{
		...
		{
			ss_blit_ship_icon(Multi_ts_slot_icon_coords[idx][gr_screen.res][MULTI_TS_X_COORD],
				Multi_ts_slot_icon_coords[idx][gr_screen.res][MULTI_TS_Y_COORD], -1, 0);
		}
		else
		{
			ss_blit_ship_icon(Multi_ts_slot_icon_coords[idx][gr_screen.res][MULTI_TS_X_COORD],
				Multi_ts_slot_icon_coords[idx][gr_screen.res][MULTI_TS_Y_COORD], Wss_slots[idx].ship_class,
				multi_ts_slot_bmap_num(idx));

			// if this is a team vs team game, and the slot is occupised by a team captain, put a c there
			if ((Netgame.type_flags & NG_TYPE_TEAM) && (Multi_ts_team[Net_player->p_info.team].multi_ts_player[idx] !=
														NULL) && (Multi_ts_team[Net_player->p_info.team].
																  multi_ts_player[idx]->flags &
																  NETINFO_FLAG_TEAM_CAPTAIN))
			{
				gr_set_color_fast(&Color_bright);
				gr_string(Multi_ts_slot_icon_coords[idx][gr_screen.res][MULTI_TS_X_COORD] - 5,
					Multi_ts_slot_icon_coords[idx][gr_screen.res][MULTI_TS_Y_COORD] - 5, XSTR("C", 737));  // [[ Team captain ]]
			}
		}
	}
}
...
void multi_ts_blit_wing_callsigns()
{
	...
	for (idx = 0; idx < MAX_WSS_SLOTS; idx++)
	{
		...
		else
		{
			...
			pobj = mission_parse_get_arrival_ship(Ships[Objects[Multi_ts_team[Net_player->p_info.team].
				multi_ts_objnum[idx]].instance].ship_name);
		...
		// blit the string
		if ((Multi_ts_hotspot_type == MULTI_TS_PLAYER_LIST) && (Multi_ts_hotspot_index == idx) &&
			(Multi_ts_team[Net_player->p_info.team].multi_ts_player[idx] != NULL))
		{
		...
		gr_string(Multi_ts_slot_text_coords[idx][gr_screen.res][MULTI_TS_X_COORD] +
			((Multi_ts_slot_text_coords[idx][gr_screen.res][MULTI_TS_W_COORD] - callsign_w) / 2),
			Multi_ts_slot_text_coords[idx][gr_screen.res][MULTI_TS_Y_COORD], callsign);
	}
}
...
void multi_ts_blit_avail_ships()
{
	...
	for (idx = 0; idx < Num_ship_classes; idx++)
	{
		if (Ss_pool[idx] > 0)
		{
			...
			{
				// blit the icon 
				ss_blit_ship_icon(Multi_ts_avail_coords[display_count][gr_screen.res][MULTI_TS_X_COORD],
					Multi_ts_avail_coords[display_count][gr_screen.res][MULTI_TS_Y_COORD], idx,
					multi_ts_avail_bmap_num(display_count));

				// blit the ship count available
				sprintf(count, "%d", Ss_pool[idx]);
				gr_set_color_fast(&Color_normal);
				gr_string(Multi_ts_avail_coords[display_count][gr_screen.res][MULTI_TS_X_COORD] - 20,
					Multi_ts_avail_coords[display_count][gr_screen.res][MULTI_TS_Y_COORD], count);
...
int multi_ts_region_type(int region)
{
	if ((region == TSWING_0_SHIP_0) || (region == TSWING_0_SHIP_1) || (region == TSWING_0_SHIP_2) ||
		(region == TSWING_0_SHIP_3) ||
		(region == TSWING_1_SHIP_0) || (region == TSWING_1_SHIP_1) || (region == TSWING_1_SHIP_2) ||
		(region == TSWING_1_SHIP_3) ||
		(region == TSWING_2_SHIP_0) || (region == TSWING_2_SHIP_1) || (region == TSWING_2_SHIP_2) ||
		(region == TSWING_2_SHIP_3))
	{
		return MULTI_TS_SLOT_LIST;
	}

	if ((region == TSWING_0_NAME_0) || (region == TSWING_0_NAME_1) || (region == TSWING_0_NAME_2) ||
		(region == TSWING_0_NAME_3) ||
		(region == TSWING_1_NAME_0) || (region == TSWING_1_NAME_1) || (region == TSWING_1_NAME_2) ||
		(region == TSWING_1_NAME_3) ||
		(region == TSWING_2_NAME_0) || (region == TSWING_2_NAME_1) || (region == TSWING_2_NAME_2) ||
		(region == TSWING_2_NAME_3))
	{
		return MULTI_TS_PLAYER_LIST;
	}

	if ((region == TSWING_LIST_0) || (region == TSWING_LIST_1) || (region == TSWING_LIST_2) ||
		(region == TSWING_LIST_3))
	{
...
void multi_ts_blit_ship_info()
{
	...
	if ((sip->velocity_str != NULL) && strlen(sip->velocity_str))
	{
		gr_set_color_fast(&Color_bright);
		gr_string(Multi_ts_ship_info_coords[gr_screen.res][MULTI_TS_X_COORD] + 150, y_start, sip->velocity_str);
	}
	y_start += 10;
	...
	// blit the shields
	gr_set_color_fast(&Color_normal);
	gr_string(Multi_ts_ship_info_coords[gr_screen.res][MULTI_TS_X_COORD], y_start, XSTR("Shields", -1));
	if ((sip->shields_str != NULL) && strlen(sip->shields_str))
	{
		gr_set_color_fast(&Color_bright);
		gr_string(Multi_ts_ship_info_coords[gr_screen.res][MULTI_TS_X_COORD] + 150, y_start, sip->shields_str);
	}
	y_start += 10;
	...
	// blit the turrets
	gr_set_color_fast(&Color_normal);
	gr_string(Multi_ts_ship_info_coords[gr_screen.res][MULTI_TS_X_COORD], y_start, XSTR("Turrets", -1));
	if ((sip->turrets_str != NULL) && strlen(sip->turrets_str))
	{
		gr_set_color_fast(&Color_bright);
		gr_string(Multi_ts_ship_info_coords[gr_screen.res][MULTI_TS_X_COORD] + 150, y_start, sip->turrets_str);
	}
	y_start += 10;
...
void multi_ts_blit_status_bar()
{
	...
	if (blit)
	{
		...
		gr_string(Multi_ts_status_coords[gr_screen.res][MULTI_TS_X_COORD] + ((Multi_ts_status_coords[gr_screen.
																			  res][MULTI_TS_W_COORD] - text_w) / 2),
			Multi_ts_status_coords[gr_screen.res][MULTI_TS_Y_COORD], text);
	}
}
...
void multi_ts_init_players()
{
	...
	for (idx = 0; idx < MAX_PLAYERS; idx++)
	{
		if (MULTI_CONNECTED(Net_players[idx]) && !MULTI_STANDALONE(Net_players[idx]))
		{
			...
			{
				Multi_ts_team[Net_players[idx].p_info.team].multi_ts_player[Net_players[idx].p_info.
					ship_index] = &Net_players[idx];
			}
		}
	}
}
...
void multi_ts_get_shipname(char* ship_name, int team, int slot_index)
{
	...
	{
		Assert(team == 0);
		sprintf(ship_name, "%s %d", Starting_wing_names[slot_index / MULTI_TS_NUM_SHIP_SLOTS_TEAM],
			slot_index % MULTI_TS_NUM_SHIP_SLOTS_TEAM);
	}
}
	...
	if (Netgame.type_flags & NG_TYPE_TEAM)
	{
		for (idx = 0; idx < MULTI_TS_MAX_TVT_TEAMS; idx++)
		{
			for (s_idx = 0; s_idx < MULTI_TS_NUM_SHIP_SLOTS_TEAM; s_idx++)
			{
				...
				if ((Multi_ts_team[idx].multi_ts_objnum[s_idx] != -1) && (Wss_slots_teams[idx][s_idx].
																		  ship_class == -1))
				{
					Multi_ts_team[idx].multi_ts_flag[s_idx] = MULTI_TS_FLAG_EMPTY;
				}
				else if ((Multi_ts_team[idx].multi_ts_objnum[s_idx] != -1) && (Wss_slots_teams[idx][s_idx].
																			   ship_class != -1))
				{
...
void multi_ts_handle_mouse()
{
	...
	snazzy_region = snazzy_menu_do(Multi_ts_mask_data, Multi_ts_mask_w, Multi_ts_mask_h, Multi_ts_snazzy_regions,
		Multi_ts_region, &snazzy_action, 0);
	...
	{
	case MULTI_TS_PLAYER_LIST:
		if ((Multi_ts_hotspot_index != region_index) && (region_index >= 0) && (Multi_ts_team[Net_player->p_info.team].
																				multi_ts_player[region_index] != NULL))
		{
			gamesnd_play_iface(SND_USER_SELECT);
		}
		break;
	...
	if (!Multi_ts_carried_flag && Multi_ts_clicked_flag &&
		mouse_down(MOUSE_LEFT_BUTTON) && ((Multi_ts_clicked_x != mouse_x) || (Multi_ts_clicked_y != mouse_y)))
	{
	...
	if (Multi_ts_carried_flag && !mouse_down(MOUSE_LEFT_BUTTON))
	{
		...
		multi_ts_drop(Multi_ts_carried_from_type, Multi_ts_carried_from_index, region_type, region_index,
			Multi_ts_carried_ship_class);
	}
}
...
int multi_ts_can_perform(int from_type, int from_index, int to_type, int to_index, int ship_class, int player_index)
{
	...
	case TS_SWAP_PLAYER_PLAYER:
		...
		if ((Multi_ts_team[pl->p_info.team].multi_ts_player[from_index] == NULL) ||
			(Multi_ts_team[pl->p_info.team].multi_ts_player[to_index] == NULL))
		{
			return 0;
		}
		...
		{
			pobj = mission_parse_get_arrival_ship(Ships[Objects[Multi_ts_team[pl->p_info.team].
				multi_ts_objnum[to_index]].instance].ship_name);
			if ((pobj == NULL) || !(pobj->flags & OF_PLAYER_SHIP))
			{
				return 0;
			}
...
void multi_ts_drop(int from_type, int from_index, int to_type, int to_index, int ship_class, int player_index)
{
	...
	{
		send_wss_request_packet(Net_player->player_id, from_type, from_index, to_type, to_index, -1, ship_class,
			WSS_SHIP_SELECT);
	}
}
...
int multi_ts_swap_player_player(int from_index, int to_index, int* sound, int player_index)
{
	...
	Multi_ts_team[pl->p_info.team].multi_ts_player[to_index] = Multi_ts_team[pl->p_info.team].
		multi_ts_player[from_index];
	...
	if (Multi_ts_team[pl->p_info.team].multi_ts_player[from_index] != NULL)
	{
		...
		Multi_ts_team[pl->p_info.team].multi_ts_player[from_index]->p_info.ship_class = Wss_slots_teams[pl->p_info.
			team][from_index].ship_class;

		multi_assign_player_ship(NET_PLAYER_INDEX(Multi_ts_team[pl->p_info.team].multi_ts_player[from_index]),
			&Objects[Multi_ts_team[pl->p_info.team].multi_ts_objnum[from_index]],
			Wss_slots_teams[pl->p_info.team][from_index].ship_class);
	}
	if (Multi_ts_team[pl->p_info.team].multi_ts_player[to_index] != NULL)
	{
		Multi_ts_team[pl->p_info.team].multi_ts_player[to_index]->p_info.ship_index = to_index;
		Multi_ts_team[pl->p_info.team].multi_ts_player[to_index]->p_info.ship_class = Wss_slots_teams[pl->p_info.
			team][to_index].ship_class;

		multi_assign_player_ship(NET_PLAYER_INDEX(Multi_ts_team[pl->p_info.team].multi_ts_player[to_index]), &Objects[
			Multi_ts_team[pl->p_info.team].multi_ts_objnum[to_index]],
			Wss_slots_teams[pl->p_info.team][to_index].ship_class);
	}
...
void multi_ts_blit_carried_icon()
{
	...
	{
	case MULTI_TS_SLOT_LIST:
		offset_x = Multi_ts_slot_icon_coords[Multi_ts_carried_from_index][gr_screen.res][MULTI_TS_X_COORD] -
			Multi_ts_clicked_x;
		offset_y = Multi_ts_slot_icon_coords[Multi_ts_carried_from_index][gr_screen.res][MULTI_TS_Y_COORD] -
			Multi_ts_clicked_y;
		...
		break;
	case MULTI_TS_AVAIL_LIST:
		offset_x = Multi_ts_avail_coords[Multi_ts_carried_from_index][gr_screen.res][MULTI_TS_X_COORD] -
			Multi_ts_clicked_x;
		offset_y = Multi_ts_avail_coords[Multi_ts_carried_from_index][gr_screen.res][MULTI_TS_Y_COORD] -
			Multi_ts_clicked_y;
	...
	case MULTI_TS_PLAYER_LIST:
		...
		strcpy_s(callsign, Multi_ts_team[Net_player->p_info.team].multi_ts_player[Multi_ts_carried_from_index]->
			m_player->callsign);
		gr_force_fit_string(callsign, CALLSIGN_LEN, Multi_ts_slot_text_coords[Multi_ts_carried_from_index][gr_screen.
			res][MULTI_TS_W_COORD]);
		gr_get_string_size(&callsign_w, NULL, callsign);

		// calculate the offsets
		offset_x = (Multi_ts_slot_text_coords[Multi_ts_carried_from_index][gr_screen.res][MULTI_TS_X_COORD] -
					Multi_ts_clicked_x) + ((Multi_ts_slot_text_coords[Multi_ts_carried_from_index][gr_screen.
											res][MULTI_TS_W_COORD] - callsign_w) / 2);
		offset_y = Multi_ts_slot_text_coords[Multi_ts_carried_from_index][gr_screen.res][MULTI_TS_Y_COORD] -
			Multi_ts_clicked_y;

		gr_set_color_fast(&Color_normal);
		gr_string(x + offset_x, y + offset_y,
			Multi_ts_team[Net_player->p_info.team].multi_ts_player[Multi_ts_carried_from_index]->m_player->callsign);
		break;
	default :
		break;
	}
...
void multi_ts_commit_pressed()
{
	...
	if (!Multi_ts_team[Net_player->p_info.team].multi_players_locked)
	{
		if (multi_num_players() != 1)
		{
			popup(PF_USE_AFFIRMATIVE_ICON | PF_BODY_BIG, 1, POPUP_OK,
				XSTR("Players have not yet been assigned to their ships", 751));
			return;
		}
		else
		{
	...
	case 1:
		gamesnd_play_iface(SND_GENERAL_FAIL);
		popup(PF_USE_AFFIRMATIVE_ICON | PF_BODY_BIG, 1, POPUP_OK, XSTR("You have not yet assigned all necessary ships",
			752));
		break;
	...
	case 2:
		gamesnd_play_iface(SND_GENERAL_FAIL);
		popup(PF_USE_AFFIRMATIVE_ICON | PF_BODY_BIG, 1, POPUP_OK, XSTR("There are ships without primary weapons!",
			753));
		break;
	...
	case 3:
		gamesnd_play_iface(SND_GENERAL_FAIL);
		popup(PF_USE_AFFIRMATIVE_ICON | PF_BODY_BIG, 1, POPUP_OK, XSTR("There are ships without secondary weapons!",
			754));
		break;
	}
}
...
int multi_ts_ok_to_commit()
{
	...
	for (idx = 0; idx < MULTI_TS_NUM_SHIP_SLOTS; idx++)
	{
		...
		if ((Multi_ts_team[Net_player->p_info.team].multi_ts_objnum[idx] != -1) &&
			...
			((Multi_ts_team[Net_player->p_info.team].multi_ts_player[idx] != NULL) &&
			 (Multi_ts_team[Net_player->p_info.team].multi_ts_flag[idx] == MULTI_TS_FLAG_EMPTY)))
		{
			return 1;
		}
			...
			!multi_ts_disabled_slot(idx))
		{
			...
			for (s_idx = 0; s_idx < MAX_SHIP_WEAPONS; s_idx++)
			{
				...
				if ((Wss_slots_teams[Net_player->p_info.team][idx].wep[s_idx] >= 0) &&
					(Wss_slots_teams[Net_player->p_info.team][idx].wep_count[s_idx] > 0))
				{
		...
		for (idx = 0; idx < MAX_WSS_SLOTS; idx++)
		{
			if (Multi_ts_team[team].multi_ts_flag[idx] != MULTI_TS_FLAG_NONE)
			{
				...
				if (val)
				{
					...
					Multi_ts_team[team].multi_ts_player[idx]->p_info.ship_class = Wss_slots_teams[team][idx].
						ship_class;
...
void process_pslot_update_packet(ubyte* data, header* hinfo)
{
	...
	case TS_CODE_LOCK_TEAM:
		...
		if (Net_player->flags & NETINFO_FLAG_AM_MASTER)
		{
			...
			for (idx = 0; idx < MAX_PLAYERS; idx++)
			{
				if (MULTI_CONNECTED(Net_players[idx]) && (&Net_players[idx] != Net_player) &&
					(&Net_players[idx] != &Net_players[player_index]))
				{

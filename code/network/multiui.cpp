...
void multi_join_notify_new_game()
{
	...
	Multi_join_slider.set_numberItems(Active_game_count > Mj_max_game_items[gr_screen.res] ? Active_game_count -
		Mj_max_game_items[gr_screen.res] : 0);
...
void multi_join_game_init()
{
	...
	for (idx = 0; idx < MULTI_JOIN_NUM_BUTTONS; idx++)
	{
		...
		Multi_join_buttons[gr_screen.res][idx].button.create(&Multi_join_window, "",
			Multi_join_buttons[gr_screen.res][idx].x, Multi_join_buttons[gr_screen.res][idx].y, 1, 1, 0, 1);
	...
	Multi_join_select_button.create(&Multi_join_window, "", Mj_list_area_coords[gr_screen.res][MJ_X_COORD],
		Mj_list_area_coords[gr_screen.res][MJ_Y_COORD], Mj_list_area_coords[gr_screen.res][MJ_W_COORD],
	...
	Multi_join_slider.create(&Multi_join_window, Mj_slider_coords[gr_screen.res][MJ_X_COORD],
		...
		Mj_slider_name[gr_screen.res], &multi_join_list_scroll_up, &multi_join_list_scroll_down, NULL);
...
void multi_join_game_do_frame()
{
	...
	if (!psnet_rel_check())
	{
		popup(PF_USE_AFFIRMATIVE_ICON, 1, POPUP_OK,
			XSTR(
			"Network Error.  Try exiting and restarting FreeSpace to clear the error.  Otherwise, please reboot your machine.", 756));
		gameseq_post_event(GS_EVENT_MAIN_MENU);
		return;
	}
...
void multi_start_game_init()
{
	...
	for (idx = 0; idx < MULTI_SG_NUM_BUTTONS; idx++)
	{
		...
		Multi_sg_buttons[gr_screen.res][idx].button.create(&Multi_sg_window, "",
			Multi_sg_buttons[gr_screen.res][idx].x, Multi_sg_buttons[gr_screen.res][idx].y, 1, 1, 0, 1);
	...
	Multi_sg_rank_button.create(&Multi_sg_window, "", Msg_rank_list_coords[gr_screen.res][MSG_X_COORD],
		Msg_rank_list_coords[gr_screen.res][MSG_Y_COORD], Msg_rank_list_coords[gr_screen.res][MSG_W_COORD],
	...
	Multi_sg_game_name.create(&Multi_sg_window, Msg_title_coords[gr_screen.res][MSG_X_COORD],
		Msg_title_coords[gr_screen.res][MSG_Y_COORD], Msg_title_coords[gr_screen.res][MSG_W_COORD], MAX_GAMENAME_LEN,
	...
	Multi_sg_game_passwd.create(&Multi_sg_window, Msg_passwd_coords[gr_screen.res][MSG_X_COORD],
		Msg_passwd_coords[gr_screen.res][MSG_Y_COORD], Msg_passwd_coords[gr_screen.res][MSG_W_COORD], 16, "",
		UI_INPUTBOX_FLAG_ESC_CLR | UI_INPUTBOX_FLAG_PASSWD | UI_INPUTBOX_FLAG_INVIS, -1, &Color_normal);
...
void multi_create_game_init()
{
	...
	Multi_create_player_select_button.create(&Multi_create_window, "", Mc_players_coords[gr_screen.res][MC_X_COORD],
		Mc_players_coords[gr_screen.res][MC_Y_COORD], Mc_players_coords[gr_screen.res][MC_W_COORD],
	...
	Multi_create_list_select_button.create(&Multi_create_window, "", Mc_list_coords[gr_screen.res][MC_X_COORD],
		Mc_list_coords[gr_screen.res][MC_Y_COORD], Mc_list_coords[gr_screen.res][MC_W_COORD],
...
void multi_create_game_do()
{
	...
	{	//
		...
		if (flags)
		{ //only continue if mission is multiplayer mission
			netgame_info* ng;
			ng = &Netgame;
...
int multi_create_ok_to_commit()
{
		...
		if (MULTI_IS_TRACKER_GAME)
		{
			...
			else
			{
				...
				if (popup(PF_USE_AFFIRMATIVE_ICON | PF_USE_NEGATIVE_ICON, 2, XSTR("&Back", 995), XSTR("&Continue",
					780),
					XSTR(
					"One or more players has hacked data files. If you continue, stats will not be stored at the end of the mission", 1273)) <= 0)
				{
					return 0;
				}
			}
	...
	if (MULTI_IS_TRACKER_GAME)
	{
																	   ...
																	   valid_status != MVALID_STATUS_VALID))
		{
			if (popup(PF_USE_AFFIRMATIVE_ICON | PF_USE_NEGATIVE_ICON, 2, XSTR("&Back", 995), XSTR("&Continue", 780),
				XSTR(
				"You have selected a mission which is either invalid or unknown to PXO. Your stats will not be saved if you continue", 996)) <= 0)
			{
				return 0;
			}
		}
...
void multi_host_options_init()
{
	...
	for (idx = 0; idx < MULTI_HO_NUM_BUTTONS; idx++)
	{
		...
		Multi_ho_buttons[gr_screen.res][idx].button.create(&Multi_ho_window, "",
			Multi_ho_buttons[gr_screen.res][idx].x, Multi_ho_buttons[gr_screen.res][idx].y, 1, 1, 0, 1);
	...
	for (idx = 0; idx < MULTI_HO_NUM_SLIDERS; idx++)
	{
		...
		Multi_ho_sliders[gr_screen.res][idx].slider.create(&Multi_ho_window, Multi_ho_sliders[gr_screen.res][idx].x,
			Multi_ho_sliders[gr_screen.res][idx].y, Multi_ho_sliders[gr_screen.res][idx].dots,
			Multi_ho_sliders[gr_screen.res][idx].filename, Multi_ho_sliders[gr_screen.res][idx].hotspot,
			NULL, -1, -1, -1, NULL, -1, -1, -1, Multi_ho_sliders[gr_screen.res][idx].dot_w);
	}

	// create the respawn count input box
	Multi_ho_respawns.create(&Multi_ho_window, Ho_rsp_coords[gr_screen.res][MULTI_HO_X_COORD],
		Ho_rsp_coords[gr_screen.res][MULTI_HO_Y_COORD], Ho_rsp_coords[gr_screen.res][MULTI_HO_W_COORD], 6, "",
		UI_INPUTBOX_FLAG_ESC_FOC | UI_INPUTBOX_FLAG_INVIS | UI_INPUTBOX_FLAG_NO_LETTERS, -1, &Color_bright);
	...
	Multi_ho_time_limit.create(&Multi_ho_window, Ho_time_coords[gr_screen.res][MULTI_HO_X_COORD],
		Ho_time_coords[gr_screen.res][MULTI_HO_Y_COORD], Ho_time_coords[gr_screen.res][MULTI_HO_W_COORD], 3, "",
		UI_INPUTBOX_FLAG_ESC_FOC | UI_INPUTBOX_FLAG_INVIS | UI_INPUTBOX_FLAG_NO_LETTERS, -1, &Color_bright);
	...
	Multi_ho_voice_wait.create(&Multi_ho_window, Ho_vw_coords[gr_screen.res][MULTI_HO_X_COORD],
		Ho_vw_coords[gr_screen.res][MULTI_HO_Y_COORD], Ho_vw_coords[gr_screen.res][MULTI_HO_W_COORD], 1, "",
		UI_INPUTBOX_FLAG_ESC_FOC | UI_INPUTBOX_FLAG_INVIS | UI_INPUTBOX_FLAG_NO_LETTERS, -1, &Color_bright);
	...
	Multi_ho_kill_limit.create(&Multi_ho_window, Ho_kill_coords[gr_screen.res][MULTI_HO_X_COORD],
		Ho_kill_coords[gr_screen.res][MULTI_HO_Y_COORD], Ho_kill_coords[gr_screen.res][MULTI_HO_W_COORD], 4, "",
		UI_INPUTBOX_FLAG_ESC_FOC | UI_INPUTBOX_FLAG_INVIS | UI_INPUTBOX_FLAG_NO_LETTERS, -1, &Color_bright);
	...
	Multi_ho_obs.create(&Multi_ho_window, Ho_obs_coords[gr_screen.res][MULTI_HO_X_COORD],
		Ho_obs_coords[gr_screen.res][MULTI_HO_Y_COORD], Ho_obs_coords[gr_screen.res][MULTI_HO_W_COORD], 1, "",
		UI_INPUTBOX_FLAG_ESC_FOC | UI_INPUTBOX_FLAG_INVIS | UI_INPUTBOX_FLAG_NO_LETTERS, -1, &Color_bright);
...
void multi_game_client_setup_init()
{
	...
	for (idx = 0; idx < MULTI_JW_NUM_BUTTONS; idx++)
	{
		...
		Multi_jw_buttons[gr_screen.res][idx].button.create(&Multi_jw_window, "",
			Multi_jw_buttons[gr_screen.res][idx].x, Multi_jw_buttons[gr_screen.res][idx].y, 1, 1, 0, 1);
	...
	Multi_jw_plist_select_button.create(&Multi_jw_window, "", Mjw_players_coords[gr_screen.res][MJW_X_COORD],
		Mjw_players_coords[gr_screen.res][MJW_Y_COORD], Mjw_players_coords[gr_screen.res][MJW_W_COORD],
...
void multi_sync_common_init()
{
	...
	for (idx = 0; idx < Multi_sync_button_count; idx++)
	{
		...
		Multi_sync_buttons[gr_screen.res][idx].button.create(&Multi_sync_window, "",
			Multi_sync_buttons[gr_screen.res][idx].x, Multi_sync_buttons[gr_screen.res][idx].y, 1, 1, 0, 1);
	...
	Multi_sync_plist_button.create(&Multi_sync_window, "", Ms_status_coords[gr_screen.res][MS_X_COORD],
		Ms_status_coords[gr_screen.res][MS_Y_COORD], Ms_status_coords[gr_screen.res][MS_W_COORD],
...
void multi_sync_display_name(char* name, int index, int np_index)
{
	...
	if (Netgame.type_flags & NG_TYPE_TEAM)
	{
		gr_force_fit_string(fit, CALLSIGN_LEN, Ms_status2_coords[gr_screen.res][MS_X_COORD] -
			Ms_status_coords[gr_screen.res][MS_X_COORD] - 20 - Ms_cd_icon_offset[gr_screen.res] -
...
void multi_sync_create_launch_button()
{
	if (!Multi_launch_button_created)
	{
		...
		Multi_sync_buttons[gr_screen.res][MS_LAUNCH].button.create(&Multi_sync_window, "",
			Multi_sync_buttons[gr_screen.res][MS_LAUNCH].x, Multi_sync_buttons[gr_screen.res][MS_LAUNCH].y, 1, 1, 0,
			1);
	...
	else
	{
		...
		if ((Multi_debrief_stats_accept_code == -1) && !(MULTI_IS_TRACKER_GAME))
		{
			res = popup(PF_BODY_BIG | PF_USE_AFFIRMATIVE_ICON | PF_USE_NEGATIVE_ICON, 2, XSTR("&Cancel", 779),
				XSTR("&Leave", 848), XSTR("Are you sure you want to leave the netgame before stats are stored?", 849));
...
void multi_debrief_server_process()
{
	...
	if ((Multi_debrief_time >= Multi_debrief_resend_time) && !multi_netplayer_state_check3(NETPLAYER_STATE_DEBRIEF,
		NETPLAYER_STATE_DEBRIEF_ACCEPT, NETPLAYER_STATE_DEBRIEF_REPLAY, 1))
	{
	...
	if (multi_netplayer_state_check3(NETPLAYER_STATE_DEBRIEF, NETPLAYER_STATE_DEBRIEF_ACCEPT,
		NETPLAYER_STATE_DEBRIEF_REPLAY) && (Netgame.type_flags & NG_TYPE_TEAM) && !Multi_debrief_reported_tvt &&
...
void multi_passwd_init()
{
	...
	for (idx = 0; idx < MULTI_PWD_NUM_BUTTONS; idx++)
	{
		...
		Multi_pwd_buttons[gr_screen.res][idx].button.create(&Multi_pwd_window, "",
			Multi_pwd_buttons[gr_screen.res][idx].x, Multi_pwd_buttons[gr_screen.res][idx].y, 1, 1, 0, 1);
	...
	Multi_pwd_passwd.create(&Multi_pwd_window, Mpwd_coords[gr_screen.res][MPWD_X_COORD],
		Mpwd_coords[gr_screen.res][MPWD_Y_COORD], Mpwd_coords[gr_screen.res][MPWD_W_COORD], MAX_PASSWD_LEN, "",
		UI_INPUTBOX_FLAG_ESC_CLR | UI_INPUTBOX_FLAG_INVIS, -1, &Color_normal);

...
void send_game_chat_packet(net_player *from, char *msg, int msg_mode, net_player *to = NULL, char *expr = NULL,
                           int server_msg = 0);
...
void send_mission_message_packet(int id, char *who_from, int priority, int timing, int source, int builtin_type,
                                 int multi_target, int multi_team_filter, int delay = 0);
...
void send_wss_request_packet(short player_id, int from_slot, int from_index, int to_slot, int to_index,
                             int wl_ship_slot, int ship_class, int mode, net_player *p = NULL);

...
int multi_find_player_by_net_signature(ushort net_signature) {
    ...
    for (idx = 0; idx < MAX_PLAYERS; idx++) {
        // compare against each player's object signature
        if (MULTI_CONNECTED(Net_players[idx]) && (Objects[Net_players[idx].m_player->objnum].net_signature ==
                                                  net_signature)) {
            // found the player
            return idx;
        }
...
int multi_find_player_by_ship_name(char *ship_name, bool inc_respawning) {
    ...
    for (idx = 0; idx < MAX_PLAYERS; idx++) {
        if (MULTI_CONNECTED(Net_players[idx]) && !MULTI_OBSERVER(Net_players[idx]) &&
            (Net_players[idx].m_player != NULL) && (Net_players[idx].m_player->objnum >= 0) &&
            (Net_players[idx].m_player->objnum < MAX_OBJECTS) && (Objects[Net_players[idx].m_player->objnum].type ==
                                                                  OBJ_SHIP) &&
            (Objects[Net_players[idx].m_player->objnum].instance >= 0) && (Objects[Net_players[idx].m_player->objnum].
                instance < MAX_SHIPS) && !stricmp(ship_name,
                                                  Ships[Objects[Net_players[idx].m_player->objnum].instance].ship_name)) {
            return idx;
        }
    }
...
int multi_get_player_ship(int np_index) {
    ...
    // cool?
    if (MULTI_CONNECTED(Net_players[np_index]) && !MULTI_OBSERVER(Net_players[np_index]) &&
        !MULTI_STANDALONE(Net_players[np_index]) &&
        (Net_players[np_index].m_player != NULL) && (Net_players[np_index].m_player->objnum >= 0) &&
        (Net_players[np_index].m_player->objnum < MAX_OBJECTS) && (Objects[Net_players[np_index].m_player->objnum].
            type == OBJ_SHIP) &&
        (Objects[Net_players[np_index].m_player->objnum].instance >= 0) && (Objects[Net_players[np_index].m_player->
...
int multi_create_player(int net_player_num, player *pl, char *name, net_addr *addr, int ship_class, short id) {
    ...
    Assert(net_player_num <
           MAX_PLAYERS);                // probably shoudln't be able to even get into this routine if no room
    ...
    if (ship_class == -1) {
        ...
        if (i == Num_ship_classes) {
            if (Num_ship_classes) {
                player_ship_class = 0;
                Warning(LOCATION, "Invalid default player ship specified in ship tables. Setting to %s",
...
void delete_player(int player_num, int kicked_reason) {
    ...
    psnet_rel_close_socket(
            &(Net_players[player_num].reliable_socket));                    // close out the reliable socket
    ...
    if ((Net_player->flags & NETINFO_FLAG_AM_MASTER) && (Net_players[player_num].flags & NETINFO_FLAG_OBSERVER)) {
        ...
        if ((Net_players[player_num].m_player->objnum >= 0) && (Net_players[player_num].m_player->objnum <
                                                                MAX_OBJECTS) && (Objects[Net_players[player_num].
                m_player->objnum].type == OBJ_SHIP) &&
            (Objects[Net_players[player_num].m_player->objnum].instance >= 0) && (Objects[Net_players[player_num].
...
void multi_pack_orient_matrix(ubyte *data, matrix *m) {
    ...
    if (m->vec.rvec.xyz.z < 0)
        data[16] |= (1 << 0);    // X
    if (m->vec.uvec.xyz.z < 0)
        data[16] |= (1 << 1);    // Y
    if (m->vec.fvec.xyz.z < 0)
        data[16] |= (1 << 2);    // V
    if (m->vec.fvec.xyz.x < 0)
        data[16] |= (1 << 3);    // Z
    if (m->vec.fvec.xyz.y < 0)
        data[16] |= (1 << 4);    // W
...
void multi_unpack_orient_matrix(ubyte *data, matrix *m) {
    ...
    m->vec.rvec.xyz.z = fl_sqrt(fl_abs(1 - (m->vec.rvec.xyz.x *
                                            m->vec.rvec.xyz.x) - (m->vec.rvec.xyz.y * m->vec.rvec.xyz.y))); // X
    m->vec.uvec.xyz.z = fl_sqrt(fl_abs(1 - (m->vec.uvec.xyz.x *
                                            m->vec.uvec.xyz.x) - (m->vec.uvec.xyz.y * m->vec.uvec.xyz.y))); // Y
    m->vec.fvec.xyz.z = fl_sqrt(fl_abs(1 - (m->vec.rvec.xyz.z *
                                            m->vec.rvec.xyz.z) - (m->vec.uvec.xyz.z * m->vec.uvec.xyz.z))); // V
    m->vec.fvec.xyz.x = fl_sqrt(fl_abs(1 - (m->vec.rvec.xyz.x *
                                            m->vec.rvec.xyz.x) - (m->vec.uvec.xyz.x * m->vec.uvec.xyz.x))); // Z
    m->vec.fvec.xyz.y = fl_sqrt(fl_abs(1 - (m->vec.rvec.xyz.y *
                                            m->vec.rvec.xyz.y) - (m->vec.uvec.xyz.y * m->vec.uvec.xyz.y))); // W
    ...
    if (pl->flags & NETINFO_FLAG_INGAME_JOIN) {
        if (!ok) {
            ...
            if (Netgame.options.flags & MSO_FLAG_INGAME_XFER) {
                ...
                pl->s_info.xfer_handle = multi_xfer_send_file(pl->reliable_socket, Netgame.mission_name,
                                                              CF_TYPE_MISSIONS);
            }
...
int multi_netplayer_state_check(int state, int ignore_standalone) {
    ...
    for (idx = 0; idx < MAX_PLAYERS; idx++) {
        if (MULTI_CONNECTED(Net_players[idx]) && (Net_player->player_id != Net_players[idx].player_id)) {
            if (ignore_standalone && ((Net_players[idx].flags & NETINFO_FLAG_AM_MASTER) && !(Net_players[idx].flags &
...
int multi_num_players() {
    ...
    for (idx = 0; idx < MAX_PLAYERS; idx++) {
        ...
        if (MULTI_CONNECTED(Net_players[idx]) && !MULTI_STANDALONE(Net_players[idx]) &&
            !MULTI_PERM_OBSERVER(Net_players[idx])) {
            count++;
        }
    }
...
int multi_eval_join_request(join_request *jr, net_addr *addr) {
            ...
            switch (Netgame.game_state) {
                ...
                case NETGAME_STATE_IN_MISSION:
                    if (MULTIPLAYER_STANDALONE) {
                        send_game_chat_packet(&Net_players[MY_NET_PLAYER_NUM], knock_message, MULTI_MSG_TARGET,
                                              Netgame.host, NULL, 1);
                    } else {
	...
	*/
    ...
    if ((MULTI_IS_TRACKER_GAME && fs2netd_player_banned(addr)) || ((Game_mode & GM_STANDALONE_SERVER) &&
                                                                   std_player_is_banned(jr->callsign))) {
        // maybe we should log this
        sprintf(knock_message, "Banned user %s with IP: %s attempted to join server", knock_callsign,
...
void multi_warpout_all_players() {
    ...
    for (idx = 0; idx < MAX_PLAYERS; idx++) {
        ...
        if (MULTI_CONNECTED(Net_players[idx]) && (Net_player != &Net_players[idx]) &&
            (Objects[Net_players[idx].m_player->objnum].type == OBJ_SHIP)) {
    ...
    if ((Net_player->flags & NETINFO_FLAG_OBSERVER) || (Net_player->flags & NETINFO_FLAG_RESPAWNING) ||
        (Net_player->flags & NETINFO_FLAG_OBSERVER) || ((Player_obj->type == OBJ_SHIP) && (Player_ship->flags &
                                                                                           SF_CANNOT_WARP))) {
...
int multi_get_highest_rank() {
    ...
    for (idx = 0; idx < MAX_PLAYERS; idx++) {
        if (MULTI_CONNECTED(Net_players[idx]) && !MULTI_STANDALONE(Net_players[idx]) &&
            !MULTI_PERM_OBSERVER(Net_players[idx]) && (Net_players[idx].m_player->stats.rank > max_rank)) {
...
void multi_file_xfer_notify(int handle) {
    ...
    if ((strlen(filename) > 0) && !cf_access(filename, cf_type, 00) && (cf_access(filename, cf_type, 02) == -1)) {
        ...
        popup(PF_USE_AFFIRMATIVE_ICON, 1, XSTR("&Ok", 713),
              XSTR(
...
void multi_process_valid_join_request(join_request *jr, net_addr *who_from, int ingame_join_team) {
    ...
    if (jr->flags & JOIN_FLAG_AS_OBSERVER) {
        ...
        send_accept_packet(net_player_num, (Net_players[net_player_num].flags & NETINFO_FLAG_INGAME_JOIN) ?
                                           ACCEPT_OBSERVER | ACCEPT_INGAME : ACCEPT_OBSERVER);
    } else {
...
int multi_kill_limit_reached() {
    ...
    for (idx = 0; idx < MAX_PLAYERS; idx++) {
        if (MULTI_CONNECTED(Net_players[idx]) && !MULTI_STANDALONE(Net_players[idx]) && !MULTI_OBSERVER(Net_players[
                                                                                                                idx]) &&
            (Net_players[idx].m_player->stats.m_kill_count_ok >= Netgame.options.kill_limit)) {
...
int multi_get_connection_speed() {
    int cspeed;
    char *connection_speed = os_config_read_string(NULL, "ConnectionSpeed", "Fast");
...
void multi_spew_pxo_checksums(int max_files, char *outfile) {
    ...
    if (file_names != NULL) {
        ...
        for (idx = 0; idx < count; idx++) {
            ...
            fprintf(out, "\"%s\",%u,\"%s\",%d,\"%s\"\r\n", filename, checksum, gametype, The_mission.num_players,
                    description);
        }
...
int
multi_pack_unpack_desired_vel(int write, ubyte *data, matrix *orient, vec3d *pos, physics_info *pi, ship_info *sip) {
    bitbuffer buf;
...
int multi_pack_unpack_desired_rotvel(int write, ubyte *data, matrix *orient, vec3d *pos, physics_info *pi,
                                     ship_info *sip) {
    bitbuffer buf;
    int fields = 0;
...
void send_debrief_event() {
    ...
    if ((Game_mode & GM_MULTIPLAYER) && IS_MISSION_MULTI_DOGFIGHT && !(The_mission.flags &
                                                                       MISSION_FLAG_TOGGLE_DEBRIEFING)) {
        gameseq_post_event(GS_EVENT_MULTI_DOGFIGHT_DEBRIEF);
    }

...
int opengl_create_texture_sub(int bitmap_handle, int bitmap_type, int bmap_w, int bmap_h, int tex_w, int tex_h,
                              ubyte *data = NULL, tcache_slot_opengl *t = NULL, int base_level = 0, int resize = 0,
                              int reload = 0);
...
void opengl_tcache_init() {
    ...
    if (GL_max_texture_width < 1024) {
        Error(LOCATION,
              "A minimum texture size of \"1024x1024\" is required for FS2_Open but only \"%ix%i\" was found.  Can not continue.",
    ...
    if (Is_Extension_Enabled(OGL_EXT_FRAMEBUFFER_OBJECT)) {
        ...
        if (GL_max_renderbuffer_size < 128) {
            mprintf((
                            "WARNING: Max dimensions of FBO, %ix%i, is less the required minimum!!  Extension will be disabled!\n",
...
int opengl_create_texture_sub(int bitmap_handle, int bitmap_type, int bmap_w, int bmap_h, int tex_w, int tex_h,
                              ubyte *data, tcache_slot_opengl *t, int base_level, int resize, int reload) {
    int ret_val = 1;
    int byte_mult = 0;
    ...
    switch (bitmap_type) {
        case TCACHE_TYPE_COMPRESSED: {
            if (block_size > 0) {
                ...
                if (!reload) {
                    vglCompressedTexImage2D(GL_TEXTURE_2D, 0, intFormat, mipmap_w, mipmap_h, 0, dsize,
                                            bmp_data + doffset);
                } else {
                    vglCompressedTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, mipmap_w, mipmap_h, intFormat, dsize,
                                               bmp_data + doffset);
                }
                ...
                for (i = 1; i < (mipmap_levels - base_level); i++) {
                    ...
                    if (!reload) {
                        vglCompressedTexImage2D(GL_TEXTURE_2D, i, intFormat, mipmap_w, mipmap_h, 0, dsize,
                                                bmp_data + doffset);
                    } else {
                        vglCompressedTexSubImage2D(GL_TEXTURE_2D, i, 0, 0, mipmap_w, mipmap_h, intFormat, dsize,
                                                   bmp_data + doffset);
                    }
                }
            } else {
        ...
        case TCACHE_TYPE_AABITMAP: {
            ...
            if (!reload || (GL_state.Texture.GetTarget() == GL_TEXTURE_RECTANGLE_ARB)) {
                glTexImage2D(GL_state.Texture.GetTarget(), 0, GL_ALPHA, tex_w, tex_h, 0, GL_ALPHA, GL_UNSIGNED_BYTE,
                             texmem);
            } else { // faster anis
                glTexSubImage2D(GL_state.Texture.GetTarget(), 0, 0, 0, tex_w, tex_h, GL_ALPHA, GL_UNSIGNED_BYTE,
                                texmem);
            }
        ...
        case TCACHE_TYPE_INTERFACE: {
            ...
            if (!reload || (GL_state.Texture.GetTarget() == GL_TEXTURE_RECTANGLE_ARB)) {
                glTexImage2D(GL_state.Texture.GetTarget(), 0, intFormat, mipmap_w, mipmap_h, 0, glFormat,
                             texFormat, (resize) ? texmem : bmp_data);
            } else { // faster anis
                glTexSubImage2D(GL_state.Texture.GetTarget(), 0, 0, 0, mipmap_w, mipmap_h, glFormat,
                                texFormat, (resize) ? texmem : bmp_data);
            }
        ...
        case TCACHE_TYPE_CUBEMAP: {
            ...
            for (i = 0; i < 6; i++) {
                ...
                if (block_size > 0) {
                    ...
                    if (!reload) {
                        vglCompressedTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, 0, intFormat, mipmap_w, mipmap_h,
                                                0, dsize, bmp_data + doffset);
                    } else {
                        vglCompressedTexSubImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, 0, 0, 0, mipmap_w, mipmap_h,
                                                   intFormat, dsize, bmp_data + doffset);
                    }
                    ...
                    for (j = 1; j < mipmap_levels; j++) {
                        ...
                        if (!reload) {
                            vglCompressedTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, j, intFormat, mipmap_w,
                                                    mipmap_h, 0, dsize, bmp_data + doffset);
                        } else {
                            vglCompressedTexSubImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, j, 0, 0, mipmap_w, mipmap_h,
                                                       intFormat, dsize, bmp_data + doffset);
                        }
                    }
                }
                    ...
                    if (!reload) {
                        glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, 0, intFormat, mipmap_w, mipmap_h, 0, glFormat,
                                     texFormat, bmp_data + doffset);
                    } else { // faster anis
                        glTexSubImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, 0, 0, 0, mipmap_w, mipmap_h, glFormat,
                                        texFormat, bmp_data + doffset);
                    }
                    ...
                    for (j = 1; j < mipmap_levels; j++) {
                        ...
                        if (!reload) {
                            glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, j, intFormat, mipmap_w, mipmap_h, 0,
                                         glFormat, texFormat, bmp_data + doffset);
                        } else {
                            glTexSubImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, j, 0, 0, mipmap_w, mipmap_h, glFormat,
                                            texFormat, bmp_data + doffset);
                        }
                    }
                }
        ...
        default: {
            ...
            if (!reload) {
                glTexImage2D(GL_TEXTURE_2D, 0, intFormat, mipmap_w, mipmap_h, 0, glFormat,
                             texFormat, (texmem != NULL) ? texmem : bmp_data + doffset);
            } else { // faster anis
                glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, mipmap_w, mipmap_h, glFormat,
                                texFormat, (texmem != NULL) ? texmem : bmp_data + doffset);
            }
            ...
            for (i = 1; i < (mipmap_levels - base_level); i++) {
                ...
                if (!reload) {
                    glTexImage2D(GL_TEXTURE_2D, i, intFormat, mipmap_w, mipmap_h, 0, glFormat, texFormat,
                                 bmp_data + doffset);
                } else {
                    glTexSubImage2D(GL_TEXTURE_2D, i, 0, 0, mipmap_w, mipmap_h, glFormat, texFormat,
                                    bmp_data + doffset);
                }
            }
...
int opengl_create_texture(int bitmap_handle, int bitmap_type, tcache_slot_opengl *tslot) {
    ...
    if ((Detail.hardware_textures < 4) && (bitmap_type != TCACHE_TYPE_AABITMAP) &&
        ...
        if (max_levels == 1) {
            ...
            int factor = (16 >> Detail.hardware_textures);
            max_w = MAX(max_w / factor, 1);
            max_h = MAX(max_h / factor, 1);
    ...
    // call the helper
    int ret_val = opengl_create_texture_sub(bitmap_handle, bitmap_type, bmp->w, bmp->h, final_w,
                                            final_h, (ubyte *) bmp->data, tslot, base_level, resize, reload);
...
int
gr_opengl_tcache_set_internal(int bitmap_handle, int bitmap_type, float *u_scale, float *v_scale, int tex_unit = 0) {
    int ret_val = 1;
...
int gr_opengl_preload(int bitmap_num, int is_aabitmap) {
    ...
    retval = gr_opengl_tcache_set(bitmap_num, (is_aabitmap) ? TCACHE_TYPE_AABITMAP : TCACHE_TYPE_NORMAL, &u_scale,
                                  &v_scale);
...
int opengl_export_image(int slot, int width, int height, int alpha, int num_mipmaps, ubyte *image_data) {
    ...
    for (int i = 0; i < faces; i++) {
        for (int j = 0; j < ts->mipmap_levels; j++) {
            glGetTexImage(target + i, j, (alpha) ? GL_BGRA : GL_BGR, (alpha) ? GL_UNSIGNED_INT_8_8_8_8_REV :
                                                                     GL_UNSIGNED_BYTE, image_data + m_offset);
...
int opengl_set_render_target(int slot, int face, int is_static) {
    ...
    if (ts->texture_target == GL_TEXTURE_CUBE_MAP) {
        ...
        vglFramebufferTexture2DEXT(GL_FRAMEBUFFER_EXT, GL_COLOR_ATTACHMENT0_EXT, GL_TEXTURE_CUBE_MAP_POSITIVE_X + face,
                                   ts->texture_id, 0);
    } else {
        Assert(face <= 0);
        vglFramebufferTexture2DEXT(GL_FRAMEBUFFER_EXT, GL_COLOR_ATTACHMENT0_EXT, ts->texture_target, ts->texture_id,
                                   0);
    }
...
int opengl_make_render_target(int handle, int slot, int *w, int *h, ubyte *bpp, int *mm_lvl, int flags) {
    ...
    if ((fbo != NULL) && (fbo->width == *w) && (fbo->height == *h)) {
        ...
        if (flags & BMP_FLAG_CUBEMAP) {
            ...
            for (int i = 0; i < 6; i++) {
                glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, 0, GL_RGB8, *w, *h, 0, GL_BGR, GL_UNSIGNED_BYTE,
                             NULL);
            }
    ...
    if (flags & BMP_FLAG_CUBEMAP) {
        vglFramebufferTexture2DEXT(GL_FRAMEBUFFER_EXT, GL_COLOR_ATTACHMENT0_EXT, GL_TEXTURE_CUBE_MAP_POSITIVE_X,
                                   ts->texture_id, 0);
    } else {
        vglFramebufferTexture2DEXT(GL_FRAMEBUFFER_EXT, GL_COLOR_ATTACHMENT0_EXT, GL_state.Texture.GetTarget(),
                                   ts->texture_id, 0);
    }
...
namespace opengl {
    ...
    texture *texture::create(int w, int h, format fmt) {
        ...
        switch (fmt) {
            case (fmt_color) :
                ...
                break;
            case (fmt_depth) :
                glTexImage2D(GL_TEXTURE_2D, 0, GL_DEPTH_COMPONENT32, t->width, t->height, 0, GL_DEPTH_COMPONENT,
    ...
    render_buffer::render_buffer(int w, int h)
            : width(w),
              height(h) {
        Assert(w > 0 && h > 0);
    ...
    void render_target::attach_texture(unsigned int slot, const texture *tex) {
        ...
        tex->bind();
        vglFramebufferTexture2DEXT(GL_FRAMEBUFFER_EXT, GL_COLOR_ATTACHMENT0_EXT + slot, GL_TEXTURE_2D, tex->get_id(),
                                   0);

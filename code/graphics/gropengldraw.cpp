...
void opengl_aabitmap_ex_internal(int x, int y, int w, int h, int sx, int sy, bool resize, bool mirror) {
    ...
    if (do_resize) {
        ...
        gr_resize_screen_posf(&fw, &fh, true);
    }

    x2 = x1 + fw;
    y2 = y1 + fh;

    glColor4ub(gr_screen.current_color.red, gr_screen.current_color.green, gr_screen.current_color.blue,
...
void gr_opengl_aabitmap_ex(int x, int y, int w, int h, int sx, int sy, bool resize, bool mirror) {
    ...
    if (Keeping_aspect_ratio) {
        std::pair<int, int> offset = gr_get_alignment_offset(true);

        clip_left -= offset.first;
        clip_right += offset.second;
    }
...
void gr_opengl_aaline(vertex *v1, vertex *v2) {
    ...
    FL_CLIPLINE(x1, y1, x2, y2, (float) gr_screen.clip_left, (float) gr_screen.clip_top, (float) gr_screen.clip_right,
                (float) gr_screen.clip_bottom, return, clipped = 1, swapped = 1);
...
void gr_opengl_gradient(int x1, int y1, int x2, int y2, bool resize) {
    ...
    INT_CLIPLINE(x1, y1, x2, y2, gr_screen.clip_left, gr_screen.clip_top, gr_screen.clip_right, gr_screen.clip_bottom,
                 return, clipped = 1, swapped = 1);
...
void opengl_draw_primitive(int nv, vertex **verts, uint flags, float u_scale, float v_scale, int r, int g, int b,
                           int a, int override_primary = 0) {

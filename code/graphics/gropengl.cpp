...
void gr_opengl_flip() {
    ...
    if (mouse_is_visible()) {
        ...
        if ((Use_mouse_to_fly == 1) && gameseq_get_state() == GS_STATE_GAME_PLAY) {
            mx -= 16;
            my -= 16;
        }
...
void gr_opengl_print_screen(char *filename) {
    ...
    snprintf(tmp, MAX_PATH_LEN - 1, "%s%s%s%s%s%sscreenshots", detect_home(), DIR_SEPARATOR_STR, Osreg_company_name,
             DIR_SEPARATOR_STR, Osreg_user_dir, DIR_SEPARATOR_STR);
    cf_create_directory(tmp);

    memset(tmp, 0, sizeof(tmp));
    snprintf(tmp, MAX_PATH_LEN - 1, "%s%s%s%s%s%sscreenshots%s%s.tga", detect_home(), DIR_SEPARATOR_STR,
             Osreg_company_name, DIR_SEPARATOR_STR, Osreg_user_dir, DIR_SEPARATOR_STR, DIR_SEPARATOR_STR, filename);
    //#else
...
void opengl_save_mouse_area(int x, int y, int w, int h) {
    ...
    if (Use_PBOs) {
        ...
        glReadBuffer(GL_BACK);
        glReadPixels(x, gr_screen.max_h - y - 1 - h, w, h, GL_read_format, GL_UNSIGNED_INT_8_8_8_8_REV,
                     GL_saved_mouse_data);
    }
...
int gr_opengl_save_screen() {
    ...
    if (Use_PBOs) {
        ...
        glReadPixels(0, 0, gr_screen.max_w, gr_screen.max_h, GL_read_format, GL_UNSIGNED_INT_8_8_8_8_REV,
                     opengl_screen_tmp);
...
int opengl_init_display_device() {
    ...
    if (!Cmdline_no_set_gamma && (GL_original_gamma_ramp == NULL)) {
        ...
        if (GL_original_gamma_ramp == NULL) {
            ...
            for (ushort x = 0; x < 256; x++) {
                GL_original_gamma_ramp[x] = GL_original_gamma_ramp[x + 256] = GL_original_gamma_ramp[x +
                                                                                                     512] =
                        (x << 8) | x;
            }
        }
    ...
    if ((Cmdline_fullscreen_window || Cmdline_window || os_config_read_uint(NULL, "Fullscreen",
                                                                            1) == 0) && !Cmdline_no_grab) {
        SDL_WM_GrabInput(SDL_GRAB_ON);
    }
...
bool gr_opengl_init() {
    ...
    if ((major < REQUIRED_GL_MAJOR_VERSION) || ((major <= REQUIRED_GL_MAJOR_VERSION) &&
                                                (minor < REQUIRED_GL_MINOR_VERSION))) {
        Error(LOCATION,
              "Current GL Version of %i.%i is less than the required version of %i.%i.\nSwitch video modes or update your drivers.",
              major, minor, REQUIRED_GL_MAJOR_VERSION, REQUIRED_GL_MINOR_VERSION);
    }

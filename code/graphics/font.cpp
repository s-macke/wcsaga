...
char *gr_force_fit_string(char *str, int max_str, int max_width) {
    ...
    if (w > max_width) {
        ...
        while (w > max_width) {
            Assert(strlen(str) >=
                   4);  // if this is hit, a bad max_width was passed in and the calling function needs fixing.
...
void parse_fonts_tbl(char *only_parse_first_font, size_t only_parse_first_font_size) {
    ...
    // double check
    if (Num_fonts < 3) {
        Error(LOCATION, "There must be at least three fonts in %s!", (filename) ? filename :

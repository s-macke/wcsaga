...
static void clean_filename_str(char* dest, char* src)
{
	strcpy(dest, src);
...
int generic_anim_stream(generic_anim* ga)
{
	...
	rval = cf_find_file_location_ext(ga->filename, NUM_TYPES, ext_list, CF_TYPE_ANY, sizeof(full_path) - 1,
	...
	{
		int success = bm_load_and_parse_eff(ga->filename, CF_TYPE_ANY, &ga->num_frames, &anim_fps, &ga->keyframe, &ga->
			eff.type);
		if (success != 0)
			return -1;

		clean_filename_str(clean_filename, ga->filename);

		if (ga->num_frames <= 0)
			return -1;

		success = bm_load_eff_frame(clean_filename, 0, CF_TYPE_ANY, ga->eff.type, &ga->bitmap_id, &ga->width, &ga->
			height);
		if (success != 0)
			return -1;

		ga->eff.bitmaps = static_cast<int*>(vm_malloc(ga->num_frames * sizeof(int)));
		ga->eff.bitmaps[0] = ga->bitmap_id;
		ga->eff.loaded = 1;
		for (int i = 1; i < ga->num_frames; ++i)
		{
			ga->eff.bitmaps[i] = -1;
		}

		/* TODO: do we want to use these at all? Ignoring for now... */
		ga->buffer = NULL;
		ga->keyoffset = 0;
	}
	...
	if (ga->type == BM_TYPE_ANI)
	{
		...
		if (ga->ani.animation->num_keys == 2)
		{
			...
			if (key1 < 0 || key1 >= ga->num_frames)
				key1 = -1;
			if (key2 < 0 || key2 >= ga->num_frames)
				key2 = -1;
	...
	{
		// keyframe identified on the above block, proceeding...
	}
...
void free_eff_instance(generic_anim* ga)
{
	if (ga->type == BM_TYPE_EFF)
	{
		if (ga->eff.bitmaps)
		{
			for (int i = 0; i < ga->num_frames; ++i)
			{
				if (ga->eff.bitmaps[i] != -1)
				{
					bm_release(ga->eff.bitmaps[i]);
				}
			}

			vm_free(ga->eff.bitmaps);
			ga->eff.bitmaps = NULL;
		}
	}
}
...
void generic_anim_unload(generic_anim* ga)
{
	if (ga->num_frames > 0)
	{
		if (ga->streaming)
		{
			...
			else if (ga->type == BM_TYPE_EFF)
			{
				free_eff_instance(ga);
			}
		}
		else
		{
...
void generic_render_ani_stream(generic_anim* ga)
{
	...
	if (ga->direction & GENERIC_ANIM_DIRECTION_BACKWARDS)
	{
		...
		{
			ga->ani.instance->file_offset = ga->ani.animation->file_offset + ga->ani.animation->keys[ga->
		...
		if (ga->ani.animation->flags & ANF_STREAMED)
		{
			ga->ani.instance->file_offset = unpack_frame_from_file(ga->ani.instance, ga->buffer,
				ga->width * ga->height, (ga->ani.instance->xlate_pal) ? ga->ani.animation->palette_translation : NULL,
				0, bpp);
		}
		else
		{
		...
		for (i = ga->previous_frame + 1; i <= ga->current_frame; i++)
				{
					...
					{
						ga->ani.instance->file_offset = unpack_frame_from_file(ga->ani.instance, ga->buffer,
							ga->width * ga->height, (ga->ani.instance->xlate_pal) ? ga->ani.animation->
							palette_translation : NULL, 0, bpp);
					}
					else
					{
						ga->ani.instance->data = unpack_frame(ga->ani.instance, ga->ani.instance->data, ga->buffer,
...
void generic_render_eff_stream(generic_anim* ga)
{
	...
	if (ga->eff.bitmaps[ga->current_frame] != -1)
	{
		ga->bitmap_id = ga->eff.bitmaps[ga->current_frame];
	}
	else
	{
		char clean_filename[MAX_FILENAME_LEN];
		...
		int success = bm_load_eff_frame(clean_filename, ga->current_frame, CF_TYPE_ANY, ga->eff.type, &ga->bitmap_id,
			NULL, NULL);
		Assert(success == 0);

		ga->eff.bitmaps[ga->current_frame] = ga->bitmap_id;
	}
...
bool eff_fully_loaded(generic_anim* ga)
{
	bool ret = false;

	if (ga->type == BM_TYPE_EFF)
	{
		ret = ga->eff.loaded >= ga->num_frames / 2;
	}

	return ret;
}

void eff_preload(generic_anim* ga)
{
	if (ga->type == BM_TYPE_EFF)
	{
		if (ga->eff.loaded < ga->num_frames)
		{
			char clean_filename[MAX_FILENAME_LEN];
			clean_filename_str(clean_filename, ga->filename);

			int success = bm_load_eff_frame(clean_filename, ga->eff.loaded, CF_TYPE_ANY,
				ga->eff.type, &ga->eff.bitmaps[ga->eff.loaded], NULL, NULL);
			Assert(success == 0);
...
void generic_anim_render(generic_anim* ga, float frametime, int x, int y, bool keep_aspect_ratio)
{
	float keytime = 0.0;
	...
	if ((ga->direction & GENERIC_ANIM_DIRECTION_PAUSED) == 0 && (ga->type != BM_TYPE_EFF || (ga->type == BM_TYPE_EFF &&
																							 eff_fully_loaded(ga))))
	{
		if (ga->direction & GENERIC_ANIM_DIRECTION_BACKWARDS)
		{
	...
	if (ga->type == BM_TYPE_EFF && !eff_fully_loaded(ga))
	{
		eff_preload(ga);
	}

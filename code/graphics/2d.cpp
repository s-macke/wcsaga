...
int Gr_inited = 0;
bool Keeping_aspect_ratio = false;
Alignment Active_alignment = ALIGNMENT_CENTER;
...
std::pair<int, int> gr_get_alignment_offset(bool useForClipping)
{
	float offsetLeft = ((gr_screen.max_w_unscaled * Gr_resize_X) - (gr_screen.max_w_unscaled * Gr_resize_Y)) * 0.5f;
	float offsetRight = offsetLeft;

	if (Active_alignment == ALIGNMENT_RIGHT)
	{
		offsetLeft *= 2.0f;
		offsetRight = 0.0f;
	}
	else if (Active_alignment == ALIGNMENT_LEFT)
	{
		offsetLeft = 0.0f;
		offsetRight *= 2.0f;
	}
	...
	if (useForClipping)
	{
		offsetLeft = MAX(0.0f, offsetLeft);
		offsetRight = MAX(0.0f, offsetRight);
	}

	return std::make_pair(static_cast<int>(offsetLeft + 0.5f), static_cast<int>(offsetRight + 0.5f));
}
...
bool gr_resize_screen_pos(int* x, int* y, bool isDimension/*= false*/ )
{
	...
	if (x)
	{
		...
		{
			xy_tmp = static_cast<float>(*x);

			xy_tmp *= Gr_resize_Y;

			if (!isDimension)
			{
				xy_tmp += gr_get_alignment_offset().first;
			}

			(*x) = static_cast<int>(xy_tmp);
		}
		else
		{
			xy_tmp = static_cast<float>(*x);
...
bool gr_unsize_screen_pos(int* x, int* y)
{
	...
	if (x)
	{
		xy_tmp = static_cast<float>(*x) * Gr_unsize_X;
		(*x) = fl2i(xy_tmp);
	}

	if (y)
	{
		xy_tmp = static_cast<float>(*y) * Gr_unsize_Y;
		(*y) = fl2i(xy_tmp);
	}
...
bool gr_resize_screen_posf(float* x, float* y, bool isDimension/*= false*/ )
{
	...
	if (x)
	{
		if (Keeping_aspect_ratio)
		{
			...
			if (!isDimension)
			{
				(*x) += gr_get_alignment_offset().first;
			}
		}
		else
		{
...
bool gr_init(int d_mode, int d_width, int d_height, int d_depth)
{
	...
	// Use dummy to disallow anything but 32bit.
	int dummyDepth = 32;
	...
	if (sscanf(ptr + 5, "(%dx%d)x%d ", &width, &height, &dummyDepth) != 3)
	{
	...
	bool has_sparky_hi = (cf_exists_full("2_ChoosePilot-m.pcx", CF_TYPE_ANY) && cf_exists_full("2_TechShipData-m.pcx",
						  CF_TYPE_ANY));
	...
	if (Web_cursor_bitmap < 0)
	{
		...
		//if it still hasn't loaded then this usually means that the executable isn't in the correct directory.
		if ((Web_cursor_bitmap = bm_load_animation("cursorweb")) < 0)
		{
			Error(LOCATION, "\nGame files not found.  This is most likely due to one of three reasons:\n"
				"\t1) You're running Wing Commander Saga from somewhere other than its installation folder;\n"
				"\t2) You've somehow corrupted your Wing Commander Saga installation, e.g. by modifying or removing files;\n"
				"\t3) You haven't installed Wing Commander Saga at all.\n"
				"Number 1 can be fixed by simply moving the Wing Commander Saga executable file to the correct folder.  Numbers 2 and 3 can be fixed by installing or reinstalling Wing Commander Saga.\n");
		}
	}
...
void gr_bitmap(int _x, int _y, bool allow_scaling)
{
	...
	if (allow_scaling && (gr_screen.custom_size || (gr_screen.rendering_to_texture != -1)))
	{
		...
		gr_resize_screen_posf(&w, &h, true);
	}
...
void gr_bitmap_list(bitmap_2d_list* list, int n_bm, bool allow_scaling)
{
	for (int i = 0; i < n_bm; i++)
	{
		...
		if (allow_scaling && (gr_screen.custom_size || (gr_screen.rendering_to_texture != -1)))
		{
			...
			gr_resize_screen_pos(&l->w, &l->h, true);
		}
	}
...
{
	return (float)atanf(zoom) * 2.0f;
}

float gr_fov_to_zoom(float fov)
{
	CLAMP(fov, 0.0f, 89.9999f);

	return (float)tanf(fov * 0.5f);
}

KeepAspectRatio::KeepAspectRatio(bool keep)
	: previousValue(Keeping_aspect_ratio)
{
	Keeping_aspect_ratio = keep;
}

KeepAspectRatio::~KeepAspectRatio()
{
	Keeping_aspect_ratio = previousValue;
}

ActivateAlignment::ActivateAlignment(Alignment newAlignment)
	: previousAlignment(Active_alignment)
{
	Active_alignment = newAlignment;
}

ActivateAlignment::~ActivateAlignment()
{
	Active_alignment = previousAlignment;
}

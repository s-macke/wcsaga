...
int gr_opengl_bm_load(ubyte type, int n, char* filename, CFILE* img_cfp, int* w, int* h, int* bpp, ubyte* c_type,
					  int* mm_lvl, int* size)
{
	int dds_ct;
...
static int opengl_bm_lock_ani_compress(int handle, int bitmapnum, bitmap_entry* be, bitmap* bmp, ubyte bpp,
									   ubyte flags)
{
	anim* the_anim;
	...
	nprintf(("BMPMAN", "Attempting to compress '%s' with %d frames, original size %.3fM ... ",
			 bm_bitmaps[first_frame].filename, nframes, ((float)(size * nframes) / 1024.0f) / 1024.0f));
...
static int opengl_bm_lock_compress(int handle, int bitmapnum, bitmap_entry* be, bitmap* bmp, int bpp, int flags)
{
	...
	nprintf(("BMPMAN", "Attempting to compress '%s', original size %.3fM ... ",
			 be->filename, ((float)(bmp->w * bmp->h * byte_size) / 1024.0f) / 1024.0f));
...
int gr_opengl_bm_lock(char* filename, int handle, int bitmapnum, ubyte bpp, ubyte flags)
{
	...
	if ((bmp->data == 0) && !opengl_texture_slot_valid(bitmapnum, handle))
	{
		...
		try_compress = (Cmdline_img2dds && Texture_compression_available && is_power_of_two(bmp->w,
						bmp->h) && !(flags & BMP_AABITMAP) && !Is_standalone);
...
void gr_opengl_bm_save_render_target(int n)
{
	...
	{
		dds_save_image(bmp->w, bmp->h, bmp->true_bpp,
			be->num_mipmaps, (ubyte*)bmp->data, (bmp->flags & BMP_FLAG_CUBEMAP));
	}
}

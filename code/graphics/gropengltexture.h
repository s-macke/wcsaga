 ...
 */
    class texture_pool {
    private:
        static SCP_multimap<std::pair < int, int>, texture*>
        pool;
    public:
        /** Acquire texture.
        ...
        static texture *acquire(int w, int h) {
            Assert(w > 0 && h > 0);
            SCP_multimap < std::pair < int, int >, texture * > ::iterator
            itr = pool.find(std::pair<int, int>(w, h));
            if (itr != pool.end()) {
 ...
 */
    ...
    private:
        ...
        static SCP_multimap<std::pair < int, int>, render_buffer*>
        pool;
    public:
        ~render_buffer();
        ...
        static render_buffer *acquire(int w, int h) {
            ...
            SCP_multimap < std::pair < int, int >, render_buffer * > ::iterator
            itr = pool.find(std::pair<int, int>(w, h));
            if (itr != pool.end()) {
 ...
 */
    ...
    private:
        ...
        static SCP_multimap<std::pair < int, int>, render_target*>
        pool;
    ...
    public:
        ...
        void set_depth(texture *tex) {
            ...
            depth_t = tex;
            vglFramebufferTexture2DEXT(GL_FRAMEBUFFER_EXT, GL_DEPTH_ATTACHMENT_EXT, GL_TEXTURE_2D, depth_t->get_id(),
                                       0);
        }
        ...
        static render_target *acquire(int w, int h) {
            ...
            SCP_multimap < std::pair < int, int >, render_target * > ::iterator
            itr = pool.find(std::pair<int, int>(w, h));
            if (itr != pool.end()) {

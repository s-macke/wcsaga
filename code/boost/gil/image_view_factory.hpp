...
/// Useful in constructing a color converted view over a given image view
        ...
        class color_convert_deref_fn
                : public deref_base<color_convert_deref_fn<SrcConstRefP, DstP, CC>, DstP, DstP, const DstP &, SrcConstRefP, DstP, false> {
        private:
...
/// \ingroup ImageViewTransformationsSubimage
        template<typename View>
        inline View subimage_view(const View &src, const typename View::point_t &topleft,
...
/// \ingroup ImageViewTransformationsSubsampled
        ...
        subsampled_view(const View &src, typename View::coord_t xStep, typename View::coord_t yStep) {
...
/// \brief single-channel (grayscale) view of the N-th channel of a given image_view
        ...
        namespace detail {
            ...
            struct __nth_channel_view_basic<View, true> {
                ...
                static type make(const View &src, int n) {
                    typedef typename type::x_iterator x_iterator_t;
                    return interleaved_view(src.width(), src.height(), (x_iterator_t) &(src(0, 0)[n]),
                                            src.pixels().row_size());
                }
            };
            ...
            struct nth_channel_deref_fn {
                BOOST_STATIC_CONSTANT(bool, is_mutable= pixel_is_reference<SrcP>::value &&
        ...
        namespace detail {
            ...
            public:
                ...
                static type make(const View &src) {
                    ...
                    typedef typename type::x_iterator x_iterator_t;
                    typedef typename iterator_adaptor_get_base<x_iterator_t>::type x_iterator_base_t;
                    x_iterator_t sit(x_iterator_base_t(&gil::at_c<K>(src(
                    0, 0))),src.pixels().pixel_size());
                    return type(src.dimensions(), locator_t(sit, src.pixels().row_size()));
                }
            };
            ...
            public:
                ...
                static type make(const View &src) {
                    typedef typename type::x_iterator x_iterator_t;
                    return interleaved_view(src.width(), src.height(), (x_iterator_t) &gil::at_c<K>(src(0, 0)),
                                            src.pixels().row_size());
                }
            };

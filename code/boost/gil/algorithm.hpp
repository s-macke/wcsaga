...
/// \brief Copy when both src and dst are interleaved and of the same type can be just memmove
    ...
    copy(boost::gil::pixel<T, Cs> *first, boost::gil::pixel<T, Cs> *last,
        ...
        return (boost::gil::pixel<T, Cs> *) std::copy((unsigned char *) first, (unsigned char *) last,
                                                      (unsigned char *) dst);
    }
...
/// \brief Copy when both src and dst are planar pointers is copy for each channel
    ...
    copy(boost::gil::planar_pixel_iterator<IC1, Cs> first, boost::gil::planar_pixel_iterator<IC1, Cs> last,
...
/// \ingroup CopyPixels
            ...
            struct copier_n {
                ...
                operator()(I src, typename std::iterator_traits<I>::difference_type n, O dst) const {
...
/// \brief  std::copy(I1,I1,I2) with I1 and I2 being a iterator_from_2d
    ...
    copy1(boost::gil::iterator_from_2d<IL> first, boost::gil::iterator_from_2d<IL> last,
...
/// resolves to fill of each row using the underlying pixel iterator, which is still faster
    ...
    void fill(boost::gil::iterator_from_2d<IL> first, boost::gil::iterator_from_2d<IL> last, const V &val) {
        ...
        if (first.is_1d_traversable()) {
            ...
            while (n > 0) {
                std::ptrdiff_t numToDo = std::min<const std::ptrdiff_t>(n, (std::ptrdiff_t) (first.width() -
                                                                                             first.x_pos()));
...
/// \brief invokes the destructor on every pixel of an image view
        ...
        namespace detail {
            ...
            struct std_destruct_t {
                template<typename It>
                void operator()(It first, It last) const { destruct_range(first, last); }
            };
...
/// std::uninitialized_copy for pairs of planar iterators
            ...
            void uninitialized_copy_aux(It1 first1, It1 last1,
                ...
                try {
                    ...
                    while (channel < num_channels<pixel_t>::value) {
                        std::uninitialized_copy(dynamic_at_c(first1, channel), dynamic_at_c(last1, channel),
...
/// GIL pixels are bitwise comparable, so memcmp is used. User-defined pixels that are not bitwise comparable need to provide an overload
            ...
            struct equal_n_fn<const pixel<T, Cs> *, const pixel<T, Cs> *> {
                ...
                operator()(const pixel<T, Cs> *i1, std::ptrdiff_t n, const pixel<T, Cs> *i2) const {
...
///  User-defined channels that are not bitwise comparable need to provide an overload
            ...
            struct equal_n_fn<planar_pixel_iterator<IC, Cs>, planar_pixel_iterator<IC, Cs> > {
                GIL_FORCEINLINE bool operator()(const planar_pixel_iterator<IC, Cs> i1, std::ptrdiff_t n,
...
/// Both source and destination ranges are delimited by image iterators
            ...
            struct equal_n_fn<boost::gil::iterator_from_2d<Loc1>, boost::gil::iterator_from_2d<Loc2> > {
                GIL_FORCEINLINE bool operator()(boost::gil::iterator_from_2d<Loc1> i1, std::ptrdiff_t n,
...
/// to memcmp. Otherwise it resolves to copying each row using the underlying pixel iterator
    template<typename Loc1, typename Loc2>
    GIL_FORCEINLINE
    bool equal(boost::gil::iterator_from_2d<Loc1> first, boost::gil::iterator_from_2d<Loc1> last,
        ...
        if (first.is_1d_traversable()) {
            if (first2.is_1d_traversable())
                return boost::gil::detail::equal_n_fn<typename Loc1::x_iterator, typename Loc2::x_iterator>()(first.x(),
                                                                                                              n,
                                                                                                              first2.x());
            else
                return boost::gil::detail::equal_n_fn<typename Loc1::x_iterator, boost::gil::iterator_from_2d<Loc2> >()(
                        first.x(), n, first2);
        } else {
            if (first2.is_1d_traversable())
                return boost::gil::detail::equal_n_fn<boost::gil::iterator_from_2d<Loc1>, typename Loc2::x_iterator>()(
                        first, n, first2.x());
            else
                return boost::gil::detail::equal_n_fn<boost::gil::iterator_from_2d<Loc1>, boost::gil::iterator_from_2d<Loc2> >()(
                        first, n, first2);
        }
    }
} // namespace std

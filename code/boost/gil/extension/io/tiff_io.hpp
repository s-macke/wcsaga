...
// This code will be simplified...
                ...
                void apply(const View &view) {
                    ...
                    switch (photometric) {
                            ...
                            switch (bps) {
                                ...
                                case 16: {
                                    ...
                                    for (int y = 0; y < view.height(); ++y) {
                                        io_error_if(TIFFReadScanline(_tp, &row.front(), y) != 1);
                                        std::transform(row.begin(), row.begin() + view.width(), view.row_begin(y),
                                                       color_convert_deref_fn<gray16_ref_t, typename View::value_type, CC>(
                                                               _cc));
                                    }
                                    break;
                                }
                                case 32: {
                                    ...
                                    for (int y = 0; y < view.height(); ++y) {
                                        io_error_if(TIFFReadScanline(_tp, &row.front(), y) != 1);
                                        std::transform(row.begin(), row.begin() + view.width(), view.row_begin(y),
                                                       color_convert_deref_fn<gray32f_ref_t, typename View::value_type, CC>(
                                                               _cc));
                                    }
                                    break;
                                }
                                default:
                                    io_error(
                                            "tiff_reader_color_convert::apply(): unknown combination of color type and bit depth");
                            }
                            break;
                        }
                            ...
                            switch (bps) {
                                ...
                                case 16: {
                                    ...
                                    for (int y = 0; y < view.height(); ++y) {
                                        io_error_if(TIFFReadScanline(_tp, &row.front(), y) != 1);
                                        std::transform(row.begin(), row.begin() + view.width(), view.row_begin(y),
                                                       color_convert_deref_fn<rgb16_ref_t, typename View::value_type, CC>(
                                                               _cc));
                                    }
                                    break;
                                }
                                case 32: {
                                    ...
                                    for (int y = 0; y < view.height(); ++y) {
                                        io_error_if(TIFFReadScanline(_tp, &row.front(), y) != 1);
                                        std::transform(row.begin(), row.begin() + view.width(), view.row_begin(y),
                                                       color_convert_deref_fn<rgb32f_ref_t, typename View::value_type, CC>(
                                                               _cc));
                                    }
                                    break;
                                }
                        ...
                        default: {
                            ...
                            io_error_if(
                                    !TIFFReadRGBAImage(_tp, dims.x, dims.y, (uint32 * ) & gil::view(rgbaImg)(0, 0), 0),
            ...
            public:
                ...
                void apply(const View &view) {
                    ...
                    io_error_if(TIFFSetField(_tp, TIFFTAG_PHOTOMETRIC,
                                             tiff_write_support_private<typename channel_type<View>::type,
                    ...
                    io_error_if(TIFFSetField(_tp, TIFFTAG_BITSPERSAMPLE,
                                             tiff_write_support_private<typename channel_type<View>::type,

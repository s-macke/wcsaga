...
// lbourdev: What is the advantage of having channel and colorspace together? Are there cases where they are interrelated?
            ...
            struct jpeg_read_support_private<bits8, gray_t> {
                BOOST_STATIC_ASSERT(BITS_IN_JSAMPLE
                ==8);
            ...
            public:
                ...
                void apply(const View &view) {
                    jpeg_start_decompress(
                            &_cinfo);    // lbourdev: Can this return an error? You need to check and throw. Check all other library methods that can return an error state...
...
// This code will be simplified...
            ...
            public:
                ...
                void apply(const View &view) {
                    ...
                    io_error_if(view.dimensions() != get_dimensions(),
                                "jpeg_reader_color_covert::apply(): input view dimensions don't match the image file");
                        ...
                        case JCS_CMYK: {
                            ...
                            for (int y = 0; y < view.height(); ++y) {
                                io_error_if(jpeg_read_scanlines(&_cinfo, (JSAMPARRAY) & row_address, 1) != 1,
                                            "jpeg_reader_color_covert::apply(): fail to read JPEG file");
                                std::transform(row.begin(), row.end(), view.row_begin(y),
                                               color_convert_deref_fn<cmyk8_ref_t, typename View::value_type, CC>(_cc));
                            }
                            break;
                        }
                        default:

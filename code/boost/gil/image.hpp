...
////////////////////////////////////////////////////////////////////////////////////////
        ...
        public:
            ...
            void
            recreate(x_coord_t width, y_coord_t height, std::size_t alignment = 0, const Alloc alloc_in = Alloc()) {
        ...
        private:
            ...
            std::size_t total_allocated_size_in_bytes(const point_t &dimensions) const {
                ...
                return (size_in_units + byte_to_memunit<typename view_t::x_iterator>::value - 1) /
            ...
            void allocate_(const point_t &dimensions,
                           mpl::false_) {  // if it throws and _memory!=0 the client must deallocate _memory
                ...
                unsigned char *tmp = (_align_in_bytes > 0) ? (unsigned char *) align((std::size_t) _memory,
                                                                                     _align_in_bytes) : _memory;
                _view = view_t(dimensions, typename view_t::locator(typename view_t::x_iterator(tmp),
                                                                    get_row_size_in_memunits(dimensions.x)));
            }

            void allocate_(const point_t &dimensions,
                           mpl::true_) {   // if it throws and _memory!=0 the client must deallocate _memory
...
/// \brief Returns the non-constant-pixel view of an image
        ...
        view(image<Pixel, IsPlanar, Alloc> &img) { return img._view; }

/// \brief Returns the constant-pixel view of an image
        template<typename Pixel, bool IsPlanar, typename Alloc>
        inline
        const typename image<Pixel, IsPlanar, Alloc>::const_view_t

...
// typedefs required by ConstRandomAccess2DImageViewConcept
            ...
            image_view(const View &iv)                                    : _dimensions(iv.dimensions()),
                                                                            _pixels(iv.pixels()) {}
            ...
            template<typename L2>
            image_view(coord_t width, coord_t height, const L2 &loc)        : _dimensions(x_coord_t(width),
                                                                                          y_coord_t(height)),
                                                                              _pixels(loc) {}
            ...
            template<typename View>
            bool operator==(const View &v) const { return pixels() == v.pixels() && dimensions() == v.dimensions(); }
            ...
            reference
            operator[](difference_type i) const { return begin()[i]; }        // potential performance problem!
            ...
            template<std::size_t D>
            typename axis<D>::iterator axis_iterator(const point_t &p) const { return _pixels.axis_iterator<D>(p); }

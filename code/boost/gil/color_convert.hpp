...
// The default implementation of to_luminance uses float0..1 as the intermediate channel type
            ...
            struct rgb_to_luminance_fn {
                GrayChannelValue
                operator()(const RedChannel &red, const GreenChannel &green, const BlueChannel &blue) const {
        ...
        struct default_color_converter_impl<rgb_t, cmyk_t> {
            ...
            void operator()(const P1 &src, P2 &dst) const {
                ...
                if (x > 0.0001f) {
                    ...
                    get_color(dst, cyan_t()) = (T2) ((get_color(dst, cyan_t()) - get_color(dst, black_t())) *
                                                     x1);                // c = (c - k) / x
                    get_color(dst, magenta_t()) = (T2) ((get_color(dst, magenta_t()) - get_color(dst, black_t())) *
                                                        x1);                // m = (m - k) / x
                    get_color(dst, yellow_t()) = (T2) ((get_color(dst, yellow_t()) - get_color(dst, black_t())) *
                                                       x1);                // y = (y - k) / x
                } else {

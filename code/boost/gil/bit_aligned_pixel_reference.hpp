...
/////////////////////////////
        ...
        public:
            ...
            template<bool M>
            bit_range(const bit_range<RangeSize, M> &br) : _current_byte(br._current_byte),
...
/// \brief Heterogeneous pixel reference corresponding to non-byte-aligned bit range. Models ColorBaseConcept, PixelConcept, PixelBasedConcept
        ...
        struct bit_aligned_pixel_reference {
            BOOST_STATIC_CONSTANT(int,
                                  bit_size = (mpl::accumulate < ChannelBitSizes, mpl::int_ < 0 >, mpl::plus < mpl::_1,
                                          mpl::_2 > > ::type::value));
            ...
            bit_aligned_pixel_reference(
                    const bit_aligned_pixel_reference<BitField, ChannelBitSizes, Layout, IsMutable2> &p) : _bit_range(
                    p._bit_range) {}

            // Grayscale references can be constructed from the channel reference
            explicit bit_aligned_pixel_reference(
                    const typename kth_element_type<bit_aligned_pixel_reference, 0>::type channel0) : _bit_range(
                    static_cast<data_ptr_t>(&channel0), channel0.first_bit()) {
            ...
            bit_aligned_pixel_reference(packed_pixel<BF, CR, Layout> &p) : _bit_range(
                    static_cast<data_ptr_t>(&gil::at_c<0>(p)), gil::at_c<0>(p).first_bit()) {
        ...
        private:
            ...
            template<typename B, typename C, typename L, bool M> friend
            struct bit_aligned_pixel_reference;

            template<typename Pixel>
            static void
            check_compatible() { gil_function_requires<PixelsCompatibleConcept<Pixel, bit_aligned_pixel_reference> >(); }
...
/////////////////////////////
        ...
        namespace detail {
            ...
            struct sum_k : public mpl::plus<sum_k<IntegralVector, K - 1>, typename mpl::at_c<IntegralVector,
                    K - 1>::type> {
            };
...
// Constructs a homogeneous bit_aligned_pixel_reference given a channel reference
        ...
        private:
            ...
            typedef typename detail::k_copies<size_t::value,
                    mpl::integral_c < unsigned, NumBits> >::type channel_bit_sizes_t;
        public:
...
// Having three overloads allows us to swap between different (but compatible) models of PixelConcept
    ...
    inline
    void swap(typename boost::gil::bit_aligned_pixel_reference<B, C, L, true>::value_type &x,
    ...
    template<typename B, typename C, typename L>
    inline
    void swap(const boost::gil::bit_aligned_pixel_reference<B, C, L, true> x,
              const boost::gil::bit_aligned_pixel_reference<B, C, L, true> y) {
        boost::gil::swap_proxy<typename boost::gil::bit_aligned_pixel_reference<B, C, L, true>::value_type>(x, y);
    }
}   // namespace std

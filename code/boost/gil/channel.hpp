...
/// \brief The value of a subbyte channel. Models: ChannelValueConcept
        ...
        public:
            ...
            packed_channel_value(Scalar v) : _value(integer_t(v) %
                                                    num_values) {}     // suppress GCC implicit conversion warnings in channel regression file
...
/// \brief A mutable subbyte channel reference whose bit offset is fixed at compile time. Models ChannelConcept
        ...
        public:
            ...
            operator=(const packed_dynamic_channel_reference<BitField, NumBits, Mutable1> &ref) const {
                set_unsafe(ref.get());
                return *this;
            }
...
/// \brief swap for packed_channel_reference
    ...
    inline
    void swap(typename boost::gil::packed_channel_reference<BF, FB, NB, M>::value_type &x,
...
/// \brief swap for packed_channel_reference
    template<typename BF, int FB, int NB, bool M>
    inline
    void swap(const boost::gil::packed_channel_reference<BF, FB, NB, M> x,
              const boost::gil::packed_channel_reference<BF, FB, NB, M> y) {
        boost::gil::swap_proxy<typename boost::gil::packed_channel_reference<BF, FB, NB, M>::value_type>(x, y);
    }
}   // namespace std
...
/// \brief swap for packed_dynamic_channel_reference
    ...
    inline
    void swap(typename boost::gil::packed_dynamic_channel_reference<BF, NB, M>::value_type &x,
...
/// \brief swap for packed_dynamic_channel_reference
    template<typename BF, int NB, bool M>
    inline
    void swap(const boost::gil::packed_dynamic_channel_reference<BF, NB, M> x,
              const boost::gil::packed_dynamic_channel_reference<BF, NB, M> y) {
        boost::gil::swap_proxy<typename boost::gil::packed_dynamic_channel_reference<BF, NB, M>::value_type>(x, y);
    }
}   // namespace std

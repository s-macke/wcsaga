	...
	{	// GR_640
		{
			...
			497,
			403
		},		// upper right pixel of text, lower right pixel of button (for tiny popup)
		{
			464,
			413,
			497,
...
void popupdead_start()
{
	...
	// create base window
	Popupdead_window.create(Popupdead_background_coords[gr_screen.res][0],
		Popupdead_background_coords[gr_screen.res][1], 1, 1, 0);
	...
	if (Game_mode & GM_NORMAL)
	{
		...
		if (0)
		{ //((Player->show_skip_popup) && (!Popupdead_skip_already_shown) && (Game_mode & GM_CAMPAIGN_MODE) && (Game_mode & GM_NORMAL) && (Player->failures_this_session >= PLAYER_MISSION_FAILURE_LIMIT)) {
	...
	else
	{
		...
		if (Net_player->flags & NETINFO_FLAG_LIMBO)
		{
			...
			if (((Net_player->flags & NETINFO_FLAG_AM_MASTER) && (multi_num_players() > 1)) || (Net_player->flags &
	...
	for (i = 0; i < Popupdead_num_choices; i++)
	{
		...
		b->create(&Popupdead_window, "", Popupdead_button_coords[gr_screen.res][i][0],
			Popupdead_button_coords[gr_screen.res][i][1], 30, 20, 0, 1);
		...
		b->create(&Popupdead_window, "", lx, Popupdead_region_coords[gr_screen.res][i][1],
			Popupdead_region_coords[gr_screen.res][i][2] - lx, Popupdead_region_coords[gr_screen.res][i][3] -
			Popupdead_region_coords[gr_screen.res][i][1], 0, 1);
		b->hide();
	}
...
int popupdead_do_frame(float frametime)
{
	...
	*/
	...
	if ((!Popupdead_skip_already_shown) && (Player->show_skip_popup) && (Game_mode & GM_NORMAL) && (Game_mode &
																									GM_CAMPAIGN_MODE)
	...
	if ((Game_mode & GM_MULTIPLAYER) && (The_mission.max_respawn_delay >= 0) && (timestamp_elapsed(Popupdead_timer)) &&
		(choice < 0))
	{

...
int asteroid_check_collision(object *pasteroid, object *other_obj, vec3d *hitpos,
    ...
    if (asteroid_hit_info->heavy ==
        ship_obj) {    // ship is heavier, so asteroid is sphere. Check sphere collision against ship poly model
...
void asteroid_hit(object *asteroid_obj, object *other_obj, vec3d *hitpos, float damage) {
    ...
    if (asteroid_obj->hull_strength < 0.0f) {
        if (asp->final_death_time <= 0) {
            ...
            asp->final_death_time = timestamp(fl2i(explosion_life * 1000.0f) /
                                              5);    // Wait till 30% of vclip time before breaking the asteroid up.
            if (hitpos) {

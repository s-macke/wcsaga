...
extern bool Highlight_tagged_ships;
...
void radar_init_orb() {
    ...
    Radar_gauge.first_frame = bm_load_animation(Current_radar_global->Radar_fname[gr_screen.res], &Radar_gauge.
            num_frames);
        ...
        for (j = 0; j < MAX_RADAR_LEVELS; j++) {
            {
                if (radar_iff_color[i][j][0] >= 0) {
                    gr_init_alphacolor(&Radar_colors[i][j], radar_iff_color[i][j][0], radar_iff_color[i][j][1],
                                       radar_iff_color[i][j][2], radar_iff_color[i][j][3]);
                } else {
                    gr_init_alphacolor(&Radar_colors[i][j], Radar_color_rgb[i][j].r, Radar_color_rgb[i][j].g,
                                       Radar_color_rgb[i][j].b, 255);
                }
            }
        }
    }
...
void radar_stuff_blip_info_orb(object *objp, int is_bright, color **blip_color, int *blip_type) {
    ...
    switch (objp->type) {
        case OBJ_SHIP:
            ...
            } else if (Highlight_tagged_ships && ship_is_tagged(objp)) {
            ...
            } else if (objp->flags & OF_TARGETABLE_AS_BOMB) {
...
void radar_plot_object_orb(object *objp) {
    ...
    switch (objp->type) {
        ...
        case OBJ_WEAPON: {
            ...
            if ((!(Weapon_info[Weapons[objp->instance].weapon_info_index].wi_flags2 & WIF2_SHOW_FRIENDLY)) &&
...
void radar_draw_range_orb() {
    ...
    switch (HUD_config.rp_dist) {
        ...
        case RR_SHORT:
            gr_printf(Current_radar_global->Radar_dist_coords[gr_screen.res][RR_SHORT][0],
                      Current_radar_global->Radar_dist_coords[gr_screen.res][RR_SHORT][1], XSTR("2k", 467));
            break;

        case RR_LONG:
            gr_printf(Current_radar_global->Radar_dist_coords[gr_screen.res][RR_LONG][0],
                      Current_radar_global->Radar_dist_coords[gr_screen.res][RR_LONG][1], XSTR("10k", 468));
            break;

        case RR_INFINITY:
            sprintf(buf, NOX("%c"), Lcl_special_chars);
            gr_printf(Current_radar_global->Radar_dist_coords[gr_screen.res][RR_INFINITY][0],
                      Current_radar_global->Radar_dist_coords[gr_screen.res][RR_INFINITY][1], buf);
            break;
...
void radar_orb_setup_view() {
    ...
    HUD_set_clip(Current_radar_global->Radar_coords[gr_screen.res][0],
                 Current_radar_global->Radar_coords[gr_screen.res][1], w, h);
    g3_start_frame(1);
...
void radar_blit_gauge_orb() {
    ...
    gr_aabitmap(Current_radar_global->Radar_coords[gr_screen.res][0],
                Current_radar_global->Radar_coords[gr_screen.res][1]);
}

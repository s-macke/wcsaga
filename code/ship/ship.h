...
typedef struct ship_weapon {
    ...
    int burst_counter[MAX_SHIP_WEAPONS];
} ship_weapon;
...
typedef struct ship_info {
    ...
    char *velocity_str;                            // string used by tooltips
    char *maneuverability_str;                    // string used by tooltips
    char *shields_str;                            // string used by tooltips
...
#define MAX_WING_FLAGS                10            // total number of flags in the wing structure -- used for parsing wing flags
...
extern void do_shield_effect(object *ship_objp, mc_info *mc, bool shield_collision = true);
...
extern void ship_get_eye(vec3d *eye_pos, matrix *eye_orient, object *obj,
                         bool do_slew = true);        // returns in eye the correct viewing position for the given object
//extern camid ship_get_followtarget_eye(object *obj);
extern ship_subsys *
ship_get_indexed_subsys(ship *sp, int index, vec3d *attacker_pos = NULL);    // returns index'th subsystem of this ship
extern int ship_get_index_from_subsys(ship_subsys *ssp, int objnum, int error_bypass = 0);

extern int ship_get_subsys_index(ship *sp, char *ss_name,
                                 int error_bypass = 0);        // returns numerical index in linked list of subsystems
...
1, float *dot_out = NULL, vec3d *vec_out = NULL);
...
extern int ship_get_random_player_wing_ship(int flags = SHIP_GET_ANY_SHIP, float max_dist = 0.0f,
                                            int persona_index = -1, int get_first = 0, int multi_team = -1);

extern int ship_get_random_ship_in_wing(int wingnum, int flags = SHIP_GET_ANY_SHIP, float max_dist = 0.0f,
                                        int get_first = 0);
...
extern float ship_get_primary_weapon_range(ship *shipp);

extern float ship_get_secondary_weapon_range(ship *shipp);
...
extern int ship_explode_area_calc_damage(vec3d *pos1, vec3d *pos2, float inner_rad, float outer_rad, float max_damage,

...
void do_subobj_destroyed_stuff(ship *ship_p, ship_subsys *subsys, vec3d *hitpos) {
    ...
    if (objp->radius > 100.0f) {
        ...
        for (i = 0; i < num_fireballs; i++) {
            ...
            fireball_create(&temp_vec, fireball_type, FIREBALL_MEDIUM_EXPLOSION, OBJ_INDEX(objp), fireball_rad,
                            0, &fb_vel);
        }
    }
    ...
    if ((subsys->system_info->dead_snd != -1) && !(subsys->subsys_snd_flags & SSSF_DEAD)) {
        obj_snd_assign(ship_p->objnum, subsys->system_info->dead_snd, &subsys->system_info->pnt, 0, OS_SUBSYS_DEAD,
                       subsys);
...
float do_subobj_hit_stuff(object *ship_obj, object *other_obj, vec3d *hitpos, float damage,
    ...
    if ((other_obj != NULL) && (other_obj->type == OBJ_SHOCKWAVE))    // Goober5000 check for NULL
    {
        ...
        if ((Ship_info[ship_p->ship_info_index].flags & (SIF_SMALL_SHIP)) && !(The_mission.ai_profile->flags &
    ...
    int count = 0;
    for (subsys = GET_FIRST(&ship_p->subsys_list); subsys != END_OF_LIST(&ship_p->subsys_list);
         subsys = GET_NEXT(subsys)) {
#ifndef NDEBUG
    ...
    switch (killer_objp->type) {
        ...
        case OBJ_WEAPON:
            ...
            if ((Game_mode & GM_MULTIPLAYER) && (Objects[killer_objp->parent].flags & OF_PLAYER_SHIP)) {
                ...
                ship *shipp = &Ships[Objects[killer_objp->parent].instance];
                strcpy_s(p->killer_parent_name, shipp->ship_name);
                end_string_at_first_hash_symbol(p->killer_parent_name, (shipp->wingnum != -1));
            }
            break;
...
int choose_next_spark(object *ship_obj, vec3d *hitpos) {
    ...
    for (spark_num = 0; spark_num < num_sparks; spark_num++) {
        if (shipp->sparks[spark_num].submodel_num != -1) {
            ...
            model_find_world_point(&world_hitpos[spark_num], &shipp->sparks[spark_num].pos, sip->model_num,
                                   shipp->sparks[spark_num].submodel_num, &ship_obj->orient, &ship_obj->pos);
        } else {
...
void ship_hit_create_sparks(object *ship_obj, vec3d *hitpos, int submodel_num) {
    ...
    if (instancing) {
        ...
        model_find_world_point(&temp_zero, &vmd_zero_vector, sip->model_num,
                               submodel_num, &ship_obj->orient, &ship_obj->pos);
        model_find_world_point(&temp_x, &vmd_x_vector, sip->model_num,
                               submodel_num, &ship_obj->orient, &ship_obj->pos);
        model_find_world_point(&temp_y, &vmd_y_vector, sip->model_num,
                               submodel_num, &ship_obj->orient, &ship_obj->pos);
        model_find_world_point(&temp_z, &vmd_z_vector, sip->model_num,
                               submodel_num, &ship_obj->orient, &ship_obj->pos);
        ship_model_stop(ship_obj);
...
void ship_generic_kill_stuff(object *objp, float percent_killed) {
    ...
    if (objp->flags & OF_PLAYER_SHIP) {
        ...
        if (!(Ai_info[Ships[objp->instance].ai_index].ai_flags & AIF_KAMIKAZE) && (delta_time <
                                                                                   MIN_PLAYER_DEATHROLL_TIME))
    ...
    if (Ship_info[sp->ship_info_index].death_roll_time >= 0) {
        sp->death_time = sp->final_death_time = timestamp(Ship_info[sp->ship_info_index].death_roll_time);
    } else {
...
static void ship_do_damage(object *ship_obj, object *other_obj, vec3d *hitpos, float damage, int quadrant,
                           int wash_damage = 0) {
    ship *shipp;
    ...
    if (other_obj) {
        other_obj_is_weapon = ((other_obj->type == OBJ_WEAPON) && (other_obj->instance >= 0) &&
                               (other_obj->instance < MAX_WEAPONS));
        other_obj_is_beam = ((other_obj->type == OBJ_BEAM) && (other_obj->instance >= 0) &&
                             (other_obj->instance < MAX_BEAMS));
        other_obj_is_shockwave = ((other_obj->type == OBJ_SHOCKWAVE) && (other_obj->instance >= 0) &&
                                  (other_obj->instance < MAX_SHOCKWAVES));
        other_obj_is_asteroid = ((other_obj->type == OBJ_ASTEROID) && (other_obj->instance >= 0) &&
                                 (other_obj->instance < MAX_ASTEROIDS));
        other_obj_is_debris = ((other_obj->type == OBJ_DEBRIS) && (other_obj->instance >= 0) &&
                               (other_obj->instance < MAX_DEBRIS_PIECES));
        other_obj_is_ship = ((other_obj->type == OBJ_SHIP) && (other_obj->instance >= 0) &&
                             (other_obj->instance < MAX_SHIPS));
    } else {
    ...
    if (other_obj_is_weapon)
        damage *= weapon_get_damage_scale(&Weapon_info[Weapons[other_obj->instance].weapon_info_index], other_obj,
                                          ship_obj);
    ...
    if (other_obj_is_weapon) {
        ...
        if ((Game_mode & GM_MULTIPLAYER) && ((Netgame.type_flags &
                                              NG_TYPE_TEAM) || (Netgame.type_flags & NG_TYPE_DOGFIGHT))) {
        } else {
    ...
    if (other_obj) {
        ...
        if (other_obj->type == OBJ_BEAM) {
            Assert((beam_get_weapon_info_index(other_obj) >= 0) && (beam_get_weapon_info_index(other_obj) <
                                                                    Num_weapon_types));
            if (((Weapon_info[beam_get_weapon_info_index(other_obj)].subtype != WP_LASER) ||
                 special_check) && (Player_obj != NULL) && (ship_obj == Player_obj)) {
                ship_hit_pain(damage, quadrant);
            }
        }
        if (other_obj->type == OBJ_WEAPON) {
            Assert((Weapons[other_obj->instance].weapon_info_index > -1) && (Weapons[other_obj->instance].
                    weapon_info_index < Num_weapon_types));
            if (((Weapon_info[Weapons[other_obj->instance].weapon_info_index].subtype != WP_LASER) ||
                 special_check) && (Player_obj != NULL) && (ship_obj == Player_obj)) {
                ship_hit_pain(damage, quadrant);
            }
        }
    ...
    if ((damage > 0.0f) || (subsystem_damage > 0.0f)) {
        ...
        if (damage > 0.0) {
                    ...
                    case OBJ_BEAM://give kills for fighter beams-Bobboau
                    {
                        ...
                        if (bobjn >= 0) {
                            if (!(The_mission.ai_profile->flags & AIPF_INCLUDE_BEAMS_IN_STAT_CALCS) &&
                                !(Ship_info[Ships[Objects[bobjn].instance].ship_info_index].flags & (SIF_FIGHTER |
                                                                                                     SIF_BOMBER)) &&
            ...
            if (ship_obj->hull_strength <= 0.0f) {
                        ...
                        if (sip->flags & (SIF_SMALL_SHIP)) {
                            if (other_obj) {    // Goober5000 check for NULL
                                if (other_obj->type == OBJ_BEAM) {
                                    ...
                                    if ((beam_weapon_info_index > -1) && (Weapon_info[beam_weapon_info_index].
                                            wi_flags & (WIF_HUGE))) {
                                        // Flag as vaporized
                ...
                if (!(shipp->flags & SF_DYING) && !MULTIPLAYER_CLIENT &&
                    !(Game_mode & GM_DEMO_PLAYBACK)) {  // if not killed, then kill
...
// Goober5000
void ship_apply_tag(int ship_num, int tag_level, float tag_time, object *target, vec3d *start, int ssm_index,
                    int ssm_team) {
...
void ship_apply_local_damage(object *ship_obj, object *other_obj, vec3d *hitpos, float damage, int quadrant,
                             bool create_spark, int submodel_num, vec3d *hit_normal) {
    Assert(ship_obj);    // Goober5000
    ...
    if (other_obj->type == OBJ_WEAPON && (!(Game_mode & GM_MULTIPLAYER) || ((Game_mode &
                                                                             GM_MULTIPLAYER) && (Net_player->flags &
    ...
    if (!MULTIPLAYER_CLIENT && !(Game_mode & GM_DEMO_PLAYBACK) && ((other_obj->type == OBJ_SHIP) ||
    ...
    // maybe tag the ship
    if (!MULTIPLAYER_CLIENT && (other_obj->type == OBJ_WEAPON) && (Weapon_info[Weapons[other_obj->instance].
            weapon_info_index].wi_flags & WIF_TAG)) {
...
void ship_apply_global_damage(object *ship_obj, object *other_obj, vec3d *force_center, float damage) {
    ...
    if (force_center) {
        ...
        if (Cmdline_nohtl && (other_obj != NULL) && (other_obj->type == OBJ_SHOCKWAVE) &&
            (Ship_info[Ships[ship_obj->instance].ship_info_index].flags & SIF_HUGE_SHIP)) {
...
void ship_hit_pain(float damage, int quadrant) {
    if (!(Player_obj->flags & OF_INVULNERABLE)) {
        if (quadrant >= 0) {
            game_flash(-damage / 300.0f, -damage / 300.0f, damage / 150.0f);
        } else {
            game_flash(damage / 150.0f, -damage / 300.0f, -damage / 300.0f);
        }
    }

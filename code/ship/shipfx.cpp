...
void shipfx_subsystem_maybe_create_live_debris(object *ship_obj, ship *ship_p, ship_subsys *subsys, vec3d *exp_center,
                                               float exp_mag) {
    // initializations
    ...
    if (pm->submodel[submodel_num].movement_type == MOVEMENT_TYPE_ROT) {
        ...
        // get the rotvel
        void model_get_rotating_submodel_axis(vec3d *model_axis, vec3d *world_axis, int modelnum, int submodel_num,
                                              object *obj);
        ...
        model_find_world_point(&world_axis_pt, &sii->pt_on_axis, pm->id,
    ...
    for (i = 0; i < num_live_debris; i++) {
        ...
        model_find_world_point(&start_world_pos, &pm->submodel[live_debris_submodel].offset, pm->id,
                               live_debris_submodel, &ship_obj->orient, &ship_obj->pos);

        // convert to model coord of underlying submodel
        // set angle to zero
        pm->submodel[submodel_num].angs = zero_angs;
        world_find_model_point(&start_model_pos, &start_world_pos, pm,
                               submodel_num, &ship_obj->orient, &ship_obj->pos);

        // rotate from submodel coord to world coords
        // reset angle to current angle
        pm->submodel[submodel_num].angs = copy_angs;
        model_find_world_point(&end_world_pos, &start_model_pos, pm->id,
                               submodel_num, &ship_obj->orient, &ship_obj->pos);
        ...
        fireball_create(&end_world_pos, fireball_type, FIREBALL_MEDIUM_EXPLOSION, OBJ_INDEX(ship_obj),
                        pm->submodel[live_debris_submodel].rad);

        // create debris
        live_debris_obj = debris_create(ship_obj, pm->id, live_debris_submodel, &end_world_pos, exp_center, 1,
                                        exp_mag);
    ...
    for (int idx = 0; idx < pm->num_debris_objects; idx++) {
        if (pm->submodel[pm->debris_objects[idx]].is_live_debris) {
            ...
            if (!pm->submodel[parent].blown_off) {
                ...
                ship_subsys *pss = NULL;
                for (pss = GET_FIRST(&shipp->subsys_list); pss != END_OF_LIST(&shipp->subsys_list);
                     pss = GET_NEXT(pss)) {
...
void shipfx_blow_up_hull(object *obj, int model, vec3d *exp_center) {
    ...
    for (i = 0; i < pm->num_debris_objects; i++) {
        if (!pm->submodel[pm->debris_objects[i]].is_live_debris) {
            ...
            model_find_world_point(&tmp, &pm->submodel[pm->debris_objects[i]].offset, model,
                                   0, &obj->orient, &obj->pos);
...
// accordingly.
void shipfx_flash_create(object *objp, int model_num, vec3d *gun_pos, vec3d *gun_dir, int is_primary,
    ...
    if ((Weapon_info[weapon_info_index].wi_flags & WIF_MFLASH) && !(Weapon_info[weapon_info_index].wi_flags &
                                                                    WIF_FLAK)) {
...
DCF(particle_width,
"Multiplier for angular width of the particle spew")
{
if (Dc_command) {
...
DCF(particle_num,
"Multiplier for the number of particles created")
{
if (Dc_command) {
dc_get_arg(ARG_FLOAT);
if ((Dc_arg_float >= 0) && (Dc_arg_float <= 5)) {
...
void shipfx_emit_spark(int n, int sn) {
    ...
    if (shipp->sparks[spark_num].submodel_num != -1) {
        ...
        model_find_world_point(&outpnt, &shipp->sparks[spark_num].pos, sip->model_num,
                               shipp->sparks[spark_num].submodel_num, &obj->orient, &obj->pos);
        ship_model_stop(obj);
    } else {
        ...
        if (sn > -1) {
            ...
            if (si->flags & SIF_FIGHTER) {
                if (hull_percent > 0.6f) {
                    ...
                    float spark_duration = (float) pow(2.0f, -5.0f * (hull_percent - 1.3f)) * (1.0f + 0.6f * (frand() -
                                                                                                              0.5f));    // +- 30%
            ...
            pe.normal_variance =
                    0.2f * spark_width_scale;        //	How close they stick to that normal 0=good, 1=360 degree
...
"Multiplier time between fireball in big ship explosion")
{
if (Dc_command) {
dc_get_arg(ARG_FLOAT);
...
static void split_ship_init(ship *shipp, split_ship *split_ship) {
    ...
    snd_play_3d(&Snds[SND_SHOCKWAVE_EXPLODE], &parent_ship_obj->pos, &View_position, 0.0f, NULL, 0, 1.0f,
                SND_PRIORITY_SINGLE_INSTANCE, NULL, 3.0f);
    ...
    vm_vec_copy_scale(&split_ship->back_ship.phys_info.rotvel, &rotvel, -(front_length * front_length) / (back_length *
                                                                                                          back_length
    ));
...
static void half_ship_render_ship_and_debris(clip_ship *half_ship, ship *shipp) {
    ...
    for (int i = 0; i < pm->num_debris_objects; i++) {
        ...
        if (half_ship->draw_debris[i] == DEBRIS_DRAW) {
            ...
            if (half_ship->explosion_vel > 0) {
                if (half_ship->cur_clip_plane_pt > tmp1.xyz.z + pm->submodel[pm->debris_objects[i]].max.xyz.z - 0.1f *
                                                                                                                half_ship->explosion_vel) {
                    create_debris = 1;
                }
                ...
                if (half_ship->cur_clip_plane_pt < tmp1.xyz.z + pm->submodel[pm->debris_objects[i]].min.xyz.z - 0.1f *
                                                                                                                half_ship->explosion_vel) {
                    create_debris = 1;
                }
                // is the debris visible
            ...
            if (!is_live_debris) {
                ...
                submodel_render(pm->id, pm->debris_objects[i], &half_ship->orient, &tmp, render_flags, -1,
            ...
            if (create_debris) {
                ...
                if (!is_live_debris) {
                    object *debris_obj;
                    debris_obj = debris_create(half_ship->parent_obj, pm->id,
                                               pm->debris_objects[i], &tmp, &half_ship->local_pivot, 1, 1.0f);
    ...
    model_render(pm->id, &half_ship->orient, &orig_ship_world_center, render_flags, -1, -1,
                 shipp->ship_replacement_textures);
}
...
void shipfx_debris_limit_speed(debris *db, ship *shipp) {
    ...
    if (sip->debris_min_rotspeed >= 0.0f && sip->debris_max_rotspeed >= 0.0f) {
        float debris_rotspeed = ((sip->debris_max_rotspeed - sip->debris_min_rotspeed) * frand()) +
...
void do_sub_expl_sound(float radius, vec3d *sound_pos, int *sound_handle) {
    ...
    if (handle == -1) {
        ...
        sound_handle[handle_index] = snd_play_3d(&Snds[sound_index], sound_pos, &View_position, 0.0f, NULL, 0, 0.6f,
                                                 SND_PRIORITY_MUST_PLAY, NULL, sound_range);
        ...
        if (get_sound_time_played(Snds[sound_index].id, handle) > 400) {
            ...
            sound_handle[handle_index] = snd_play_3d(&Snds[sound_index], sound_pos, &View_position, 0.0f, NULL, 0,
                                                     0.6f, SND_PRIORITY_MUST_PLAY, NULL, sound_range);
        }
...
static void maybe_fireball_wipe(clip_ship *half_ship, int *sound_handle) {
    ...
    if (timestamp_elapsed(half_ship->next_fireball)) {
        if (half_ship->length_left > 0.2f * fl_abs(half_ship->explosion_vel)) {
            ...
            if (rad < 1) {
                rad = half_ship->parent_obj->radius * frand_range(0.6f, 0.8f);
                rad *= 1.5f;
                rad = MAX(rad, half_ship->parent_obj->radius);
            } else {
            ...
            fireball_create(&model_clip_plane_pt, fireball_type, FIREBALL_LARGE_EXPLOSION,
                            OBJ_INDEX(half_ship->parent_obj), rad, 0, &half_ship->parent_obj->phys_info.vel, 0.0f, -1,
                            NULL,
                            low_res_fireballs);
...
int shipfx_large_blowup_do_frame(ship *shipp, float frametime) {
    ...
    if (timestamp_elapsed(the_split_ship->explosion_flash_timestamp)) {
        if (!the_split_ship->explosion_flash_started) {
            ...
            if (objp->flags & OF_WAS_RENDERED) {
                float excess_dist = vm_vec_dist(&Player_obj->pos, &objp->pos) - 2.0f * objp->radius -
                                    Player_obj->radius;
    ...
    physics_sim(&the_split_ship->front_ship.local_pivot, &the_split_ship->front_ship.orient, &the_split_ship->
            front_ship.phys_info, frametime);
    physics_sim(&the_split_ship->back_ship.local_pivot, &the_split_ship->back_ship.orient, &the_split_ship->back_ship.
            phys_info, frametime);
...
void engine_wash_ship_process(ship *shipp) {
            ...
            for (j = 0; j < bank->num_points; j++) {
                ...
                if (dot_to_ship > 0) {
                    ...
                    if (dist_sqr < max_wash_dist * max_wash_dist) {
                        ...
                        if (dist_sqr < ((radius_mult * radius_mult) * (bank->points[j].radius *
                                                                       bank->points[j].radius))) {
    ...
    if (shipp->wash_intensity > 0) {
        ...
        if (!do_damage) {
            ...
            damage = (0.001f * 0.003f * ENGINE_WASH_CHECK_INTERVAL * shipp->ship_max_hull_strength *
                      shipp->wash_intensity);
        }
        ...
        {
                {
                        "Animation",
                        WV_ANIMATION,
                        CombinedVariable::TYPE_STRING
                },
                {
                        "Radius",
                        WV_RADIUS,
                        CombinedVariable::TYPE_FLOAT
                },
                {
                        "Speed",
                        WV_SPEED,
...
WarpEffect::WarpEffect(object *n_objp, int n_direction) {
    this->clear();
    if (n_objp != NULL && n_objp->type == OBJ_SHIP && n_objp->instance > -1 &&
        Ships[n_objp->instance].ship_info_index > -1) {
        objp = n_objp;
...
int WE_Default::warpStart() {
    ...
    if (direction == WD_WARP_OUT) {
        ...
        // create fireball
        warp_objnum = fireball_create(&pos, fireball_type, FIREBALL_WARP_EFFECT, OBJ_INDEX(objp), radius, 1, NULL,
                                      warp_time, shipp->ship_info_index, NULL, 0, 0, sip->warpout_snd_start,
                                      sip->warpout_snd_end);
        ...
        // create fireball
        warp_objnum = fireball_create(&pos, fireball_type, FIREBALL_WARP_EFFECT, OBJ_INDEX(objp), radius, 0, NULL,
                                      warp_time, shipp->ship_info_index, NULL, 0, 0, sip->warpin_snd_start,
                                      sip->warpin_snd_end);
    } else {
        Int3();
    }
...
int WE_Default::warpFrame(float frametime) {
    if (direction == WD_WARP_IN) {
        if ((shipp->flags & SF_ARRIVING_STAGE_1) && timestamp_elapsed(stage_time_end)) {
            ...
            float warp_time = shipfx_calculate_warp_time(objp, WD_WARP_IN);
            float speed = shipfx_calculate_warp_dist(objp) /
                          warp_time;        // How long it takes to move through warp effect
        ...
        if (objp == Player_obj) {
            ...
            if (timed_out) {
                ...
                if (delta_ms > 1000.0f * frametime) {
                    nprintf(("AI", "Frame %i: Ship %s missed departue cue by %7.3f seconds.\n", Framecount,
                            shipp->ship_name, -(float) delta_ms / 1000.0f));
                }
            }
...
int WE_BSG::warpStart() {
    ...
    if (gs_start_index > -1) {
        ...
        snd_start = snd_play_3d(snd_start_gs, &objp->pos, &View_position, 0.0f, NULL, 0, 1,
                                SND_PRIORITY_SINGLE_INSTANCE, NULL, snd_range_factor);
    }
    if (gs_end_index > -1) {
...
int WE_BSG::warpShipRender() {
    ...
    if (anim > -1) {
        ...
        if (anim_frame < anim_nframes) {
            ...
            batcher.render(TMAP_FLAG_GOURAUD | TMAP_FLAG_RGB | TMAP_FLAG_TEXTURED | TMAP_FLAG_CORRECT |
                           TMAP_HTL_3D_UNLIT);
        }
    }
...
int WE_BSG::warpEnd() {
    if (snd_start > -1)
        snd_stop(snd_start);
    if (snd_end_gs != NULL)
        snd_end = snd_play_3d(snd_end_gs, &objp->pos, &View_position, 0.0f, NULL, 0, 1.0f,
                              SND_PRIORITY_SINGLE_INSTANCE, NULL, snd_range_factor);
...
WE_Homeworld::WE_Homeworld(object *n_objp, int n_direction)
    ...
    if (direction == WD_WARP_IN)
        stage_duration[3] = sip->warpin_time - (stage_duration[1] + stage_duration[2] + stage_duration[4] +
                                                stage_duration[5]);
    else if (direction == WD_WARP_OUT)
        stage_duration[3] = sip->warpout_time - (stage_duration[1] + stage_duration[2] + stage_duration[4] +
                                                 stage_duration[5]);
...
int WE_Homeworld::warpStart() {
    ...
    if (gs_index > -1) {
        ...
        snd = snd_play_3d(snd_gs, &pos, &View_position, 0.0f, NULL, 0, 1, SND_PRIORITY_SINGLE_INSTANCE, NULL,
                          snd_range_factor);
    }
...
int WE_Homeworld::warpFrame(float frametime) {
    ...
    //Process stage
    float progress = ((float) timestamp() - (float) stage_time_start) / ((float) stage_time_end - (float)
            stage_time_start);
    switch (stage) {
        case 1:
        ...
        case 2:
            break;
        case 3:
            vm_vec_scale_add(&pos, &objp->pos, &objp->orient.vec.fvec,
                             z_offset_max - progress * (z_offset_max - z_offset_min));
            break;
        case 4:
            break;
        case 5:
...
int WE_Homeworld::warpShipRender() {
    ...
    int frame = 0;
    if (anim_fps > 0)
        frame = fl2i((int) (((float) (timestamp() - (float) total_time_start) / 1000.0f) * (float) anim_fps) %
                     anim_nframes);

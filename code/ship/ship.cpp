...
#include
"weapon/flak.h"                                //phreak addded 11/05/02 for flak primaries
#include
...
#define
NUM_SHIP_SUBSYSTEM_SETS            20        // number of subobject sets to use (because of the fact that it's a linked list,
//     we can't easily go fully dynamic)

#define
NUM_SHIP_SUBSYSTEMS_PER_SET        200    // Reduced from 1000 to 400 by MK on 4/1/98.  DTP; bumped from 700 to 2100
...
static ship_subsys* Ship_subsystems[NUM_SHIP_SUBSYSTEM_SETS] =
{
NULL
};
ship_subsys ship_subsys_free_list;
static boost::unordered_map<std::string, ship_subsys*> ship_subsys_map[MAX_SHIPS];
...
#define
COLLISION_FRICTION_FACTOR    0.0        // ratio of maximum friction impulse to repulsion impulse
#define
COLLISION_ROTATION_FACTOR    1.0        // increase in rotation from collision
#define
SHIP_REPAIR_SUBSYSTEM_RATE    0.01f
...
#define
SHIP_OBJ_USED    (1<<0)                // flag used in ship_obj struct
#define
MAX_SHIP_OBJS    MAX_SHIPS            // max number of ships tracked in ship list
...
{
"untargetable",
MSS_FLAG_UNTARGETABLE,
0
},
{
"carry no damage",
MSS_FLAG_CARRY_NO_DAMAGE,
0
},
{
"use multiple guns",
MSS_FLAG_USE_MULTIPLE_GUNS,
0
},
{
"fire down normals",
MSS_FLAG_FIRE_ON_NORMAL,
0
},
{
"check hull",
MSS_FLAG_TURRET_HULL_CHECK,
0
},
{
"fixed firingpoints",
MSS_FLAG_TURRET_FIXED_FP,
0
},
{
"salvo mode",
MSS_FLAG_TURRET_SALVO,
0
},
{
"no subsystem targeting",
MSS_FLAG_NO_SS_TARGETING,
0
},
{
"fire on target",
MSS_FLAG_FIRE_ON_TARGET,
0
},
{
"reset when idle",
MSS_FLAG_TURRET_RESET_IDLE,
0
},
{
"carry shockwave",
MSS_FLAG_CARRY_SHOCKWAVE,
0
}
};
...
{
"in tech database",
SIF_IN_TECH_DATABASE | SIF_IN_TECH_DATABASE_M,
0
},
{
"in tech database multi",
SIF_IN_TECH_DATABASE_M,
0
},
{
"dont collide invisible",
SIF_SHIP_CLASS_DONT_COLLIDE_INVIS,
0
},
{
"big damage",
...
{
"surface shields",
SIF2_SURFACE_SHIELDS,
1
},
{
"show ship",
SIF2_SHOW_SHIP_MODEL,
1
},
{
"generate icon",
SIF2_GENERATE_HUD_ICON,
1
},
{
"no weapon damage scaling",
SIF2_DISABLE_WEAPON_DAMAGE_SCALING,
1
},
{
"gun convergence",
SIF2_GUN_CONVERGENCE,
1
},
{
"no thruster geometry noise",
SIF2_NO_THRUSTER_GEO_NOISE,
1
},
{
"intrinsic no shields",
SIF2_INTRINSIC_NO_SHIELDS,
1
},
{
"no primary linking",
SIF2_NO_PRIMARY_LINKING,
1
},
...
#define
SHIP_MULTITEXT_LENGTH 4096
#define
DEFAULT_DELTA_BANK_CONST    0.5f
...
sip->type_str = sip->velocity_str = sip->maneuverability_str = sip->shields_str = sip->armor_str = sip->
...
sip->closeup_zoom = Viewer_zoom_target;
...
bool is_mothership_gone(wing* wingp)
{
bool is_mothership_gone = false;

if (wingp != NULL && wingp->arrival_location == ARRIVE_FROM_DOCK_BAY)
{
Assert(wingp->arrival_anchor >= 0);
char* name = Parse_names[wingp->arrival_anchor];

// see if ship is yet to arrive.  If so, then return -1 so we can evaluate again later.
if (!mission_parse_get_arrival_ship(name))
{
bool mother_ship_not_in_mission = (ship_name_lookup(name) == -1);
bool mother_ship_departed = (mission_log_get_time(LOG_SHIP_DEPARTED, name, NULL, NULL) != 0);
bool mother_ship_destroyed = (mission_log_get_time(LOG_SELF_DESTRUCTED, name, NULL, NULL) != 0) ||
(mission_log_get_time(LOG_SHIP_DESTROYED, name, NULL, NULL) != 0);

if (mother_ship_not_in_mission || mother_ship_destroyed || mother_ship_departed)
{
is_mothership_gone = true;
}
}
}

return is_mothership_gone;
}
...
Warning(LOCATION, "Error:  Ship template %s already exists. All ship template names must be unique.",
sip->name);
...
{
stuff_malloc_string(&sip->velocity_str, F_MESSAGE);
}
...
{
stuff_malloc_string(&sip->shields_str, F_MESSAGE);
}
...
// Code added here by SS to parse the optional strings for length, gun_mounts, missile_banks and turrets
...
if (optional_string("+Turrets:"))
{
stuff_malloc_string(&sip->turrets_str, F_MESSAGE);
}
...
{
Warning(LOCATION,
"Debris min speed (%f) on %s '%s' is greater than debris max speed (%f), and will be set to debris max speed.", sip->debris_min_speed, info_type_name, sip->name, sip->debris_max_speed);
sip->debris_min_speed = sip->debris_max_speed;
}
if (sip->debris_min_rotspeed > sip->debris_max_rotspeed && sip->debris_max_rotspeed >= 0.0f)
{
Warning(LOCATION,
"Debris min rotation speed (%f) on %s '%s' is greater than debris max rotation speed (%f), and will be set to debris max rotation speed.", sip->debris_min_rotspeed, info_type_name, sip->name, sip->debris_max_rotspeed);
...
{
Warning(LOCATION,
"Debris min lifetime (%f) on %s '%s' is greater than debris max lifetime (%f), and will be set to debris max lifetime.", sip->debris_min_lifetime, info_type_name, sip->name, sip->debris_max_lifetime);
...
{
Warning(LOCATION,
"Debris min hitpoints (%f) on %s '%s' is greater than debris max hitpoints (%f), and will be set to debris max hitpoints.", sip->debris_min_hitpoints, info_type_name, sip->name, sip->debris_max_hitpoints);
...
if (optional_string("$Death-Roll Time:"))
{
stuff_int(&sip->death_roll_time);
}
...
{
Warning(LOCATION,
"$Allowed PBanks bank-specific loadout for %s exceeds permissible number of primary banks.  Ignoring the rest...", sip->name);
bank--;
break;
}
...
{
Warning(LOCATION,
"$Allowed Dogfight PBanks bank-specific loadout for %s exceeds permissible number of primary banks.  Ignoring the rest...", sip->name);
bank--;
break;
}
...
{
Assertion((sip->primary_bank_weapons[i] >= 0), "%s. No $Default PBanks supplied for bank %d",
parse_error_text, i);
}
}
...
pbank_capacity_count = stuff_int_list(sip->primary_bank_ammo_capacity, MAX_SHIP_PRIMARY_BANKS,
RAW_INTEGER_TYPE);
strcpy_s(parse_error_text, temp_error);
if (pbank_capacity_count != sip->num_primary_banks)
{
Warning(LOCATION,
"Primary bank capacities have not been completely specified for ship class %s... fix this!!",
sip->name);
}
}
...
{
Warning(LOCATION,
"$Allowed SBanks bank-specific loadout for %s exceeds permissible number of secondary banks.  Ignoring the rest...", sip->name);
bank--;
break;
}
...
if (allowed_weapons[i] >= 0)        // MK, Bug fix, 9/6/99.  Used to be "allowed_weapons" not "allowed_weapons[i]".
{
sip->allowed_bank_restricted_weapons[MAX_SHIP_PRIMARY_BANKS + bank][allowed_weapons[i]] |=
REGULAR_WEAPON;
}
}
}
...
{
Warning(LOCATION,
"$Allowed Dogfight SBanks bank-specific loadout for %s exceeds permissible number of secondary banks.  Ignoring the rest...", sip->name);
bank--;
break;
}
...
sip->num_secondary_banks = stuff_int_list(sip->secondary_bank_weapons, MAX_SHIP_SECONDARY_BANKS,
WEAPON_LIST_TYPE);
strcpy_s(parse_error_text, temp_error);
...
sbank_capacity_count = stuff_int_list(sip->secondary_bank_ammo_capacity, MAX_SHIP_SECONDARY_BANKS,
RAW_INTEGER_TYPE);
strcpy_s(parse_error_text, temp_error);
if (sbank_capacity_count != sip->num_secondary_banks)
{
Warning(LOCATION,
"Secondary bank capacities have not been completely specified for ship class %s... fix this!!",
sip->name);
}
}
...
Warning(LOCATION, "Use of '%s' flag for ship '%s' - this flag is no longer needed.",
Ship_flags[idx].name, sip->name);
...
{
Warning(LOCATION, "Ship class %s has an afterburner but has no afterburner fuel. Setting fuel to 1",
sip->name);
...
if (res == -1)
{
Warning(LOCATION, "Could not find weapon type '%s' to use as countermeasure on ship class '%s'",
sip->name);
}
...
{
Warning(LOCATION,
"Ship %s is missing the colon after \"$Stealth\". Note that you may also use the ship flag \"stealth\".",
sip->name);
sip->flags |= SIF_STEALTH;
}
...
{
Warning(LOCATION, "Too many maneuvering thrusters on ship '%s'; maximum is %d", sip->name,
MAX_MAN_THRUSTERS);
}
}
else
...
sip->ship_iff_info[iff_data[0]][iff_data[1]] = iff_init_color(iff_color_data[0], iff_color_data[1],
iff_color_data[2]);
}
...
{
Warning(LOCATION,
"Number of subsystems for ship entry '%s' (%d) exceeds max of %d; only the first %d will be used", sip->name, sip->n_subsystems, n_subsystems, MAX_MODEL_SUBSYSTEMS);
break;
}
...
Warning(LOCATION, "Invalid engine wash name %s specified for subsystem %s in ship class %s",
name_tmp, sp->subobj_name, sip->name);
}
...
{
Warning(LOCATION,
"\"fixed firingpoints\" flag used without \"use multiple guns\" flag on a subsystem on ship %s.\n\"use multiple guns\" flags added by default\n", sip->name);
sp->flags |= MSS_FLAG_USE_MULTIPLE_GUNS;
}
...
{
sip->n_subsystems += n_subsystems;
sip->subsystems = (model_subsystem*)vm_realloc(sip->subsystems, sizeof(model_subsystem) *
sip->n_subsystems);
}
...
{        // the values should be different for Fred
...
{
char err_msg[512];
sprintf(err_msg, "Unable to allocate ship subsystems. Maximum is %d. No subsystems have been assigned to %s.",
(NUM_SHIP_SUBSYSTEM_SETS * NUM_SHIP_SUBSYSTEMS_PER_SET), shipp->ship_name);
...
{
obj_snd_assign(shipp->objnum, ship_system->system_info->turret_base_rotation_snd, &ship_system->
system_info->pnt, 0, OS_TURRET_BASE_ROTATION, ship_system);
ship_system->subsys_snd_flags |= SSSF_TURRET_ROTATION;
}
if (ship_system->system_info->turret_gun_rotation_snd != -1)
{
obj_snd_assign(shipp->objnum, ship_system->system_info->turret_gun_rotation_snd, &ship_system->
system_info->pnt, 0, OS_TURRET_GUN_ROTATION, ship_system);
ship_system->subsys_snd_flags |= SSSF_TURRET_ROTATION;
}
}
if ((ship_system->system_info->flags & MSS_FLAG_ROTATES) && (ship_system->system_info->
rotation_snd != -1) &&
!(ship_system->subsys_snd_flags & SSSF_ROTATE))
{
obj_snd_assign(shipp->objnum, ship_system->system_info->rotation_snd, &ship_system->system_info->pnt,
0, OS_SUBSYS_ROTATION, ship_system);
ship_system->subsys_snd_flags |= SSSF_ROTATE;
}
}
else
...
ship_subsys_map[Objects[objnum].instance].clear();
...
ship_system->system_info->flags &= ~MSS_FLAG_TURRET_MATRIX;

ship_subsys_map[Objects[objnum].instance][ship_system->system_info->subobj_name] = ship_system;
...
if ((ship_system->system_info->flags & MSS_FLAG_TURRET_SALVO) && (ship_system->system_info->flags &
MSS_FLAG_TURRET_FIXED_FP))
{
Warning(LOCATION,
...
if ((ship_system->system_info->flags & MSS_FLAG_TURRET_FIXED_FP) && (model_system->turret_num_firing_points <
2))
{
Warning(LOCATION,
...
if ((ship_system->system_info->flags & MSS_FLAG_TURRET_FIXED_FP) && !(ship_system->system_info->flags &
MSS_FLAG_USE_MULTIPLE_GUNS))
{
Warning(LOCATION,
...
if ((ship_system->system_info->flags & MSS_FLAG_TURRET_FIXED_FP) && (number_of_weapons > model_system->
turret_num_firing_points))
{
Warning(LOCATION,
...
for (k = 0; k < MAX_SHIP_SECONDARY_BANKS; k++)
{
ship_system->weapons.secondary_bank_ammo[k] = (Fred_running ? 100 : ship_system->weapons.
secondary_bank_capacity[k]);
...
for (k = 0; k < MAX_SHIP_PRIMARY_BANKS; k++)
{
ship_system->weapons.primary_bank_ammo[k] = (Fred_running ? 100 : ship_system->weapons.
primary_bank_capacity[k]);
}
...
mtr->man_batcher.render(TMAP_FLAG_GOURAUD | TMAP_FLAG_RGB | TMAP_FLAG_TEXTURED | TMAP_FLAG_CORRECT |
TMAP_HTL_3D_UNLIT);
...
model_add_arc(sip->model_num, -1, &shipp->arc_pts[i][0], &shipp->arc_pts[i][1],
shipp->arc_type[i]);
}
}
}
...
{    // because it looks more realistic, but I don't think the AI uses side_thrust or vert_thrust
...
&& (mtp->start_snd < 0 || (snd_get_duration(mtp->start_snd) < timestamp() -
shipp->thrusters_start[i]))
)
{
...
bmap_frame += (int)(((float)(timestamp() - shipp->thrusters_start[i]) /
1000.0f) * (float)mtp->tex_fps) % mtp->tex_nframes;
...
!ship_subsys_disrupted(shipp, SUBSYSTEM_ENGINE) &&
!(pi->flags & PF_GLIDING))
{
mst_info mst;
...
MULTI_CONNECTED(Net_players[np_index]) && (Net_players[np_index].m_player != NULL))
{
...
{
if (Weapon_info[swp->primary_bank_weapons[i]].external_model_num == -1 || !sip->
draw_primary_models[i])
continue;
...
model_render(Weapon_info[swp->primary_bank_weapons[i]].
external_model_num, &vmd_identity_matrix, &bank->pnt[k], render_flags);
...
{
if (Weapon_info[swp->secondary_bank_weapons[i]].external_model_num == -1 || !sip->
draw_secondary_models[i])
continue;
...
vm_vec_scale_add2(&secondary_weapon_pos, &vmd_z_vector, -(1.0f -
shipp->secondary_point_reload_pct[i][
k]) * model_get(Weapon_info[swp->
secondary_bank_weapons[i]].external_model_num)->rad);

model_render(Weapon_info[swp->secondary_bank_weapons[i]].
external_model_num, &vmd_identity_matrix, &secondary_weapon_pos, render_flags);
}
}
...
model_render(sip->model_num, &obj->orient, &obj->pos, render_flags | MR_LOCK_DETAIL,
OBJ_INDEX(obj), -1, shipp->ship_replacement_textures);
}
else
{
model_render(sip->model_num, &obj->orient, &obj->pos, render_flags, OBJ_INDEX(obj), -1,
...
ship_subsys_map[Objects[shipp->objnum].instance].clear();
}
...
mprintf(("Cleaning up ship '%s' of wing '%s'\n", Ships[shipnum].ship_name, wingp->name));
...
mprintf(("Current ship count of wing '%s': %i\n", wingp->name, wingp->current_count));
...
if (is_mothership_gone(wingp))
{
mprintf(("The mothership of wing '%s' is gone. Setting current wave to last wave.\n", wingp->name));
wingp->current_wave = wingp->num_waves;
}

mprintf(("Wing '%s' has no ships left.\n", wingp->name));

maybe_handle_destroyed_or_departed(wingp, team);
}
}

void maybe_handle_destroyed_or_departed(wing* wingp, int team)
{
if (wingp->flags & WF_WING_GONE)
return;

bool is_last_wave = wingp->current_wave == wingp->num_waves;

if (is_last_wave)
{
mprintf(("Wing '%s' has reached its last wave.\n", wingp->name));
mprintf(("Wing '%s' statistics:\n", wingp->name));
mprintf(("\tTotal destroyed: %i\n", wingp->total_destroyed));
mprintf(("\tTotal departed: %i\n", wingp->total_departed));
mprintf(("\tTotal vanished: %i\n", wingp->total_vanished));
mprintf(("\tTotal arrived: %i\n", wingp->total_arrived_count));
}

bool are_all_ships_gone = wingp->total_destroyed + wingp->total_departed + wingp->total_vanished == wingp->total_arrived_count;
...
if (is_last_wave && are_all_ships_gone)
{
mprintf(("Wing '%s' is gone.\n", wingp->name));
...
mprintf(("Logging wing '%s' as destroyed.\n", wingp->name));
}
...
// log file code checks for this case.
mission_log_add_entry(LOG_WING_DEPARTED, wingp->name, NULL, team);

mprintf(("Logging wing '%s' as departed.\n", wingp->name));
}
...
!(Ships[Objects[so->objnum].instance].flags & (SF_DEPARTING | SF_DYING)))
{
...
if (!(shipp->flags & SF_IGNORE_COUNT) || ((shipp->wingnum != -1) && !(Wings[shipp->wingnum].flags &
WF_IGNORE_COUNT)))
...
if ((exp_objp->hull_strength <= KAMIKAZE_HULL_ON_DEATH) && (Ai_info[Ships[exp_objp->instance].ai_index].ai_flags &
AIF_KAMIKAZE) && (shipp->special_exp_damage == -1))
{
...
if (ship_explode_area_calc_damage(&exp_objp->pos, &objp->pos, inner_rad, outer_rad, max_damage,
max_blast, &damage, &blast) == -1)
{
continue;
}
...
bool vaporize_ship = (shipp->flags & SF_VAPORIZE) != 0;
...
// play death sound
snd_play_3d(&Snds[SND_VAPORIZED], &objp->pos, &View_position, objp->radius, NULL, 0, 1.0f,
...
fireball_create(&outpnt, fireball_type, FIREBALL_LARGE_EXPLOSION, OBJ_INDEX(objp), rad,
0, &objp->phys_info.vel);
...
fireball_create(&outpnt, fireball_type, FIREBALL_MEDIUM_EXPLOSION, OBJ_INDEX(objp), rad,
0, &objp->phys_info.vel);
}
}
...
snd_play_3d(&Snds[sound_index], &objp->pos, &View_position, objp->radius, NULL, 0, 1.0f,
SND_PRIORITY_MUST_PLAY);
if (objp == Player_obj)
joy_ff_explode();
...
// Sanity checks
if (weaponp->thruster_frame < 0.0f)
weaponp->thruster_frame = 0.0f;
if (weaponp->thruster_frame > 100.0f)
weaponp->thruster_frame = 0.0f;
...
if (framenum < 0)
framenum = 0;
if (framenum >= flame_anim->num_frames)
framenum = flame_anim->num_frames - 1;
...
// Sanity checks
if (weaponp->thruster_glow_frame < 0.0f)
weaponp->thruster_glow_frame = 0.0f;
if (weaponp->thruster_glow_frame > 100.0f)
weaponp->thruster_glow_frame = 0.0f;
...
if (framenum < 0)
framenum = 0;
if (framenum >= glow_anim->num_frames)
framenum = glow_anim->num_frames - 1;
...
// element.
#define
SHIP_REPAIR_SUBSYSTEM_RATE    0.01f    // percent repair per second for a subsystem
#define
SUBSYS_REPAIR_THRESHOLD        0.1    // only repair subsystems that have > 10% strength
...
#define
PLAYER_MAX_DIST_WARNING            700000            // distance in KM at which player gets warning to return to battle
#define
PLAYER_DISTANCE_MAX_WARNINGS    3                // maximum number of warnings player can receive before mission ends
#define
PLAYER_MAX_DIST_END                750000            // distance from starting loc at which we end mission
#define
PLAYER_WARN_DELTA_TIME            10000            //ms
#define
PLAYER_DEATH_DELTA_TIME            5000            //ms
...
if (!(p->flags & PLAYER_FLAGS_FORCE_MISSION_OVER) && ((p->distance_warning_count > PLAYER_DISTANCE_MAX_WARNINGS) ||
...
!MULTI_OBSERVER(Net_players[idx]) && (Objects[Net_players[idx].m_player->objnum].type !=
OBJ_GHOST))
{
// if bad, blow him up
...
{
max_missiles = get_max_ammo_count_for_bank(shipp->ship_info_index, i,
shipp->weapons.secondary_bank_weapons[i]);
...
{
if (vm_vec_dist_quick(&obj->pos, &Objects[aip->support_ship_objnum].pos) < (obj->radius +
Objects[aip->
support_ship_objnum].
radius) * 1.25f)
return;
...
sip->num_primary_banks, sip->name);
...
Error(LOCATION,
XSTR(
"There is a limit of %d ships in the mission at once.  Please be sure that you do not have more than %d ships present in the mission at the same time.", 1495), SHIPS_LIMIT, SHIPS_LIMIT);
return -1;
}
}
...
{
Ship_info[idx].model_num = model_load(Ship_info[idx].pof_file,
Ship_info[idx].n_subsystems, &Ship_info[idx].subsystems[0]);
}
}
...
{
objnum = obj_create(OBJ_SHIP, -1, n, orient, pos, model_get_radius(sip->model_num), OF_RENDERS | OF_COLLIDES |
OF_PHYSICS);
}
Assert(objnum >= 0);
...
{
if (ss->system_info->subobj_num == ptindex)
{            // when these are equal, fix up the ref
...
sp->afterburner_fuel = MAX(0, sip->afterburner_fuel_capacity - (sip_orig->afterburner_fuel_capacity -
sp->afterburner_fuel));
...
shipp->primary_rotate_rate[bank_to_stop] -= Weapon_info[swp->primary_bank_weapons[bank_to_stop]].
weapon_submodel_rotate_accell * flFrametime;
if (shipp->primary_rotate_rate[bank_to_stop] < 0.0f)
...
if ((winfo_p->wi_flags & WIF_BEAM) && (winfo_p->tag_level == 3) && (shipp->flags &
SF_TRIGGER_DOWN) &&
...
swp->next_primary_fire_stamp[bank_to_fire] = timestamp((int)(next_fire_delay / pm->gun_banks[bank_to_fire].
num_slots));
swp->last_primary_fire_stamp[bank_to_fire] = timestamp();
...
{        // the big change I made for fighter beams, if there beams fill out the Fire_Info for a targeting laser then fire it, for each point in the weapon bank -Bobboau
float t;
if (winfo_p->burst_shots > swp->burst_counter[bank_to_fire])
{
...
shipp->beam_sys_info.turret_fov = (float)cos((winfo_p->field_of_fire != 0.0f)?winfo_p->field_of_fire:
180);
...
{
get_subsystem_world_pos(&Objects[aip->target_objnum],
aip->targeted_subsys, &target_position);
}
else
{
...
vm_vec_scale_add(&predicted_target_pos, &target_position, &target_velocity_vec,
time_to_target);
polish_predicted_target_pos(winfo_p, &Objects[aip->
target_objnum], &target_position, &predicted_target_pos,
dist_to_target, &last_delta_vec, 1);
...
vm_vec_scale_add(&predicted_target_pos, &obj->pos, &plr_to_target_vec,
dist_mult);
}
...
weapon_objnum = weapon_create(&firing_pos, &firing_orient, weapon, OBJ_INDEX(obj),
new_group_id);
...
if ((winfo_p->muzzle_flash >= 0) && (((shipp == Player_ship) &&
(vm_vec_mag(&Player_obj->phys_info.vel) >=
45)) || (shipp != Player_ship)))
{
...
if ((winfo_p->wi_flags & WIF_SHUDDER) && (obj == Player_obj) && !(Game_mode &
...
num_fired++;
shipp->last_fired_point[bank_to_fire] = (shipp->last_fired_point[bank_to_fire] +
1) % num_slots;
}
}
...
int force_level = (int)((wip->armor_factor + wip->shield_factor * 0.2f) * (wip->damage *
wip->damage -
7.5f) * 0.45f +
0.6f) * 10 + 2000;
...
if ((shipp->weapons.primary_bank_weapons[0] >= 0) && (Weapon_info[shipp->weapons.primary_bank_weapons[0]].
wi_flags & WIF_BEAM) && (Weapon_info[shipp->weapons.
primary_bank_weapons[0]].b_info.
beam_type == BEAM_TYPE_C))
{
bank0_laser = 1;
}
if ((shipp->weapons.primary_bank_weapons[1] >= 0) && (Weapon_info[shipp->weapons.primary_bank_weapons[1]].
wi_flags & WIF_BEAM) && (Weapon_info[shipp->weapons.
primary_bank_weapons[1]].b_info.
beam_type == BEAM_TYPE_C))
{
bank1_laser = 1;
}
...
if ((aip->target_objnum != -1) && (vm_vec_dist_quick(&obj->pos, &Objects[aip->target_objnum].pos) >
max_dist))
{
...
strcpy_s(missile_name, wip->name);
end_string_at_first_hash_symbol(missile_name);
HUD_sourced_printf(HUD_SOURCE_HIDDEN, XSTR("Cannot fire %s without a lock", 488),
missile_name);
}
...
// of weapon_create
weapon_num = weapon_create(&firing_pos, &firing_orient, weapon, OBJ_INDEX(obj), -1,
...
{            //DTP here we switch to the next valid bank, but we can't call weapon_info on next fire_wait
...
{    //DTP, this is simply a copy of the manual cycle functions
...
Weapon_info[swp->primary_bank_weapons[swp->current_primary_bank]].name, swp->current_primary_bank + 1);
gamesnd_play_error_beep();
}
return 0;
}
...
if (objp == Player_obj)
{
HUD_sourced_printf(HUD_SOURCE_HIDDEN, XSTR("This ship has only one secondary weapon: %s", 493),
...
if (Fred_running || ignore_count)
{  // current_count not used for Fred..
...
#define
SHIELD_REPAIR_RATE    0.20f            //	Percent of shield repaired per second.
#define
HULL_REPAIR_RATE        0.15f            //	Percent of hull repaired per second.
#define
SUBSYS_REPAIR_RATE    0.10f            // Percent of subsystems repaired per second.

#define
REARM_NUM_MISSILES_PER_BATCH 4        // how many missiles are dropped in per load sound
#define
REARM_NUM_BALLISTIC_PRIMARIES_PER_BATCH    100    // how many bullets are dropped in per load sound
...
shield_rep_time = (sp->ship_max_shield_strength - shield_get_strength(objp)) / (sp->ship_max_shield_strength *
sip->sup_shield_repair_rate);
...
{
hull_rep_time = (max_hull_repair - objp->hull_strength) / (sp->ship_max_hull_strength *
sip->sup_hull_repair_rate);
}
...
num_reloads = (swp->primary_bank_start_ammo[i] - swp->primary_bank_ammo[i]) /
...
REARM_NUM_BALLISTIC_PRIMARIES_PER_BATCH != 0)
{
num_reloads++;
}
...
if ((aip->rearm_first_missile == TRUE) && (i == swp->num_secondary_banks -
1) && (banks_full != swp->num_secondary_banks))
...
if ((aip->rearm_first_ballistic_primary == TRUE) && (i == swp->num_primary_banks -
1) && (primary_banks_full != swp->
num_primary_banks))
{
// Goober5000
int sound_index;
...
if ((subsys_all_ok && shields_full && (The_mission.flags & MISSION_FLAG_SUPPORT_REPAIRS_HULL) &&
hull_ok) || (subsys_all_ok && shields_full && !(The_mission.flags & MISSION_FLAG_SUPPORT_REPAIRS_HULL)))
{
if (objp == Player_obj)
{
...
if ((banks_full == swp->num_secondary_banks) &&
(!(sip->flags & SIF_BALLISTIC_PRIMARIES) || ((sip->flags & SIF_BALLISTIC_PRIMARIES) &&
...
if (Ship_info[i].velocity_str != NULL)
{
vm_free(Ship_info[i].velocity_str);
Ship_info[i].velocity_str = NULL;
}
...
if (Ship_info[i].shields_str != NULL)
{
vm_free(Ship_info[i].shields_str);
Ship_info[i].shields_str = NULL;
}
...
if (Ship_info[i].turrets_str != NULL)
{
vm_free(Ship_info[i].turrets_str);
Ship_info[i].turrets_str = NULL;
}
}
...
if ((flags == SHIP_GET_NO_PLAYERS || flags == SHIP_GET_UNSILENCED) && (Objects[Ships[ship_index].objnum].
...
{
if (aip->target_objnum >= 0 && Objects[aip->target_objnum].type == OBJ_SHIP && Objects[aip->target_objnum].instance >= 0)
{
strcat(outbuf, Ships[Objects[aip->target_objnum].instance].ship_name);
}
else
{
strcpy(outbuf, XSTR("no orders", 495));
...
#define
SHIP_CARGO_CHECK_INTERVAL    1000
...
for (ship_so = GET_FIRST(&Ship_obj_list); ship_so != END_OF_LIST(&Ship_obj_list);
ship_so = GET_NEXT(ship_so))
{
...
limit_squared = (cargo_objp->radius + CARGO_RADIUS_DELTA) * (cargo_objp->radius +
CARGO_RADIUS_DELTA);
...
//
#define
PLAYER_ALLOW_WARN_INTERVAL        60000        // minimum time between warnings
#define
PLAYER_CHECK_WARN_INTERVAL        300        // how often we check for warnings
#define
PLAYER_MAX_WARNINGS                2            // max number of warnings player can receive in a mission
#define
PLAYER_MIN_WARN_DIST                100        // minimum distance attacking ship can be from player and still allow warning
#define
PLAYER_MAX_WARN_DIST                1000        // maximum distance attacking ship can be from plyaer and still allow warning
...
#define
PLAYER_MAX_PRAISES                    10            // max number of praises player can receive in a mission
...
if ((Ship_info[deader_sp->ship_info_index].class_type > 0) && !(Ship_types[Ship_info[deader_sp->ship_info_index].
class_type].message_bools &
STI_MSG_PRAISE_DESTRUCTION))
{
return;
}
...
message_send_builtin_to_player(MESSAGE_PRAISE_SELF, killer_sp, MESSAGE_PRIORITY_HIGH, MESSAGE_TIME_SOON, 0,
0, -1, -1);

}

#define
PLAYER_ASK_HELP_INTERVAL            60000        // minimum time between praises
#define
PLAYER_MAX_ASK_HELP                10            // max number of warnings player can receive in a mission
#define
ASK_HELP_SHIELD_PERCENT            0.1        // percent shields at which ship will ask for help
#define
ASK_HELP_HULL_PERCENT                0.3        // percent hull at which ship will ask for help
#define
AWACS_HELP_HULL_HI                    0.75        // percent hull at which ship will ask for help
#define
AWACS_HELP_HULL_LOW                0.25        // percent hull at which ship will ask for help
...
if (objp->hull_strength < ((AWACS_HELP_HULL_LOW + 0.01f * (static_rand(objp - Objects) & 5)) *
...
else if (objp->hull_strength < ((AWACS_HELP_HULL_HI + 0.01f * (static_rand(objp - Objects) & 5)) *
sp->ship_max_hull_strength))
{
...
#define
PLAYER_SCREAM_INTERVAL        60000
#define
PLAYER_MAX_SCREAMS                10
...
#define
PLAYER_LOW_AMMO_MSG_INTERVAL        250000
#define
PLAYER_REQUEST_REPAIR_MSG_INTERVAL    240000
#define
PLAYER_MAX_LOW_AMMO_MSGS            5
...
if (model_anim_start_type(sp, TRIGGER_TYPE_PRIMARY_BANK, i, 1))
{
swp->primary_animation_done_time[i] = model_anim_get_time_type(sp, TRIGGER_TYPE_PRIMARY_BANK,
i);
swp->primary_animation_position[i] = MA_POS_SET;
}
else
...
{
swp->secondary_animation_done_time[i] = model_anim_get_time_type(sp,
TRIGGER_TYPE_SECONDARY_BANK, i);
...
{
Error(LOCATION,
"Attempt to page in new subsystems subsystems failed because mission file contains more than %d subsystems"
...
Warning(LOCATION, "Ship '%s' does not have subsystem '%s' linked into the model file, '%s'.",
...
(ship_find_repair_ship(objp) == NULL))
return 0;
}
...
if (aip != NULL){
aip->artillery_objnum = -1; aip->artillery_sig = -1;    aip->artillery_lock_time = 0.0f;} continue; }
...
{
#if
defined(MULTIPLAYER_BETA_BUILD) || defined(FS2_DEMO)
return;
#else
ai_info* aip = NULL;
...
Assert((Weapon_info[shipp->weapons.primary_bank_weapons[shipp->weapons.current_primary_bank]].wi_flags &
WIF_BEAM) && (Weapon_info[shipp->weapons.primary_bank_weapons[shipp->weapons.current_primary_bank]].
b_info.beam_type == BEAM_TYPE_C));
if (!(Weapon_info[shipp->weapons.primary_bank_weapons[shipp->weapons.current_primary_bank]].wi_flags &
WIF_BEAM) || (Weapon_info[shipp->weapons.primary_bank_weapons[shipp->weapons.current_primary_bank]].
b_info.beam_type != BEAM_TYPE_C))
{
continue;
...
if ((psub->weapon_rotation_pbank != swp->current_primary_bank) || (swp->primary_bank_ammo[swp->
...
AT_TYPE_ADDITIVE            0
#define
AT_TYPE_MULTIPLICATIVE            1
#define
AT_TYPE_EXPONENTIAL            2
#define
AT_TYPE_EXPONENTIAL_BASE        3
#define
AT_TYPE_CUTOFF                4
#define
AT_TYPE_REVERSE_CUTOFF            5
#define
AT_TYPE_INSTANT_CUTOFF            6
#define
AT_TYPE_INSTANT_REVERSE_CUTOFF        7
...
if (len >= NAME_LENGTH)
{
Warning(LOCATION, "Armor name %s is %d characters too long, and will be truncated", in_name,
len - NAME_LENGTH);
}
...
{
Warning(LOCATION,
"Armor '%s', damage type %d: Armor has a different number of calculation types than arguments (%d, %d)", Name, DamageTypes.size(), adt.Calculations.size(), adt.Arguments.size());
}
...
float ship_get_primary_weapon_range(ship* shipp)
{
float prange = 0.0f;
...
if (shipp->flags & SF_PRIMARY_LINKED)
{
for (int bank = 0; bank < swp->num_primary_banks; ++ bank)
{
weapon_info* wip = &Weapon_info[swp->primary_bank_weapons[bank]];

prange = MIN(prange, wip->max_speed * wip->lifetime);
}
}
else
{
...
prange = wip->max_speed * wip->lifetime;
}
}

        ...
        if (Num_ship_classes >= MAX_SHIP_CLASSES) {
            if (!warning_too_many_ship_classes) {
                Warning(LOCATION,
                        ...
                        buf, MAX_SHIP_CLASSES, Num_ship_classes);
...
int parse_ship_values(ship_info *sip, bool isTemplate, bool first_time, bool replace) {
    ...
    if (sip->debris_min_speed > sip->debris_max_speed && sip->debris_max_speed >= 0.0f) {
        Warning(LOCATION,
                "Debris min speed (%f) on %s '%s' is greater than debris max speed (%f), and will be set to debris max speed.",
    ...
    if (sip->debris_min_rotspeed > sip->debris_max_rotspeed && sip->debris_max_rotspeed >= 0.0f) {
        Warning(LOCATION,
                "Debris min rotation speed (%f) on %s '%s' is greater than debris max rotation speed (%f), and will be set to debris max rotation speed.",
    ...
    if (sip->debris_min_lifetime > sip->debris_max_lifetime && sip->debris_max_lifetime >= 0.0f) {
        Warning(LOCATION,
                "Debris min lifetime (%f) on %s '%s' is greater than debris max lifetime (%f), and will be set to debris max lifetime.",
    ...
    if (sip->debris_min_hitpoints > sip->debris_max_hitpoints && sip->debris_max_hitpoints >= 0.0f) {
        Warning(LOCATION,
                "Debris min hitpoints (%f) on %s '%s' is greater than debris max hitpoints (%f), and will be set to debris max hitpoints.",
    ...
    if (optional_string("$Allowed PBanks:")) {
        ...
        while (check_for_string("(")) {
            ...
            if (bank >= MAX_SHIP_PRIMARY_BANKS) {
                Warning(LOCATION,
                        "$Allowed PBanks bank-specific loadout for %s exceeds permissible number of primary banks.  Ignoring the rest...",
                        sip->name);
                bank--;
                break;
            }
    ...
    if (optional_string("$Allowed Dogfight PBanks:")) {
        ...
        while (check_for_string("(")) {
            ...
            if (bank >= MAX_SHIP_PRIMARY_BANKS) {
                Warning(LOCATION,
                        "$Allowed Dogfight PBanks bank-specific loadout for %s exceeds permissible number of primary banks.  Ignoring the rest...",
                        sip->name);
                bank--;
                break;
            }
    ...
    if (optional_string("$Allowed SBanks:")) {
        ...
        while (check_for_string("(")) {
            ...
            if (bank >= MAX_SHIP_SECONDARY_BANKS) {
                Warning(LOCATION,
                        "$Allowed SBanks bank-specific loadout for %s exceeds permissible number of secondary banks.  Ignoring the rest...",
                        sip->name);
                bank--;
                break;
            }
    ...
    if (optional_string("$Allowed Dogfight SBanks:")) {
        ...
        while (check_for_string("(")) {
            ...
            if (bank >= MAX_SHIP_SECONDARY_BANKS) {
                Warning(LOCATION,
                        "$Allowed Dogfight SBanks bank-specific loadout for %s exceeds permissible number of secondary banks.  Ignoring the rest...",
                        sip->name);
                bank--;
                break;
            }
            ...
            case 1: {
                ...
                if (sp == NULL) {
                    if (sip->n_subsystems + n_subsystems >= MAX_MODEL_SUBSYSTEMS) {
                        Warning(LOCATION,
                                "Number of subsystems for ship entry '%s' (%d) exceeds max of %d; only the first %d will be used",
                                sip->name, sip->n_subsystems, n_subsystems, MAX_MODEL_SUBSYSTEMS);
                        break;
                    }
    ...
    if (shipp->flags & SF_DYING) {
        ...
        if (timestamp_elapsed(shipp->final_death_time)) {
            ...
            if (ship_get_exp_propagates(shipp)) {
                ...
                shipp->end_death_time = shipp->really_final_death_time = timestamp(fl2i(explosion_life * 1000.0f) /
                                                                                   5);    // Wait till 30% of vclip time before breaking the ship up.
    ...
    while (1) {
        ...
        for (so = GET_FIRST(&Ship_obj_list); so != END_OF_LIST(&Ship_obj_list); so = GET_NEXT(so)) {
            ...
            if (compare_shipp->create_time == new_create_time) {
                if ((unsigned int) sanity_counter == 0 &&
                    (unsigned int) last_smctu_initial_time == shipp->create_time) {
...
void ship_init_afterburners(ship *shipp) {
    ...
    for (int i = 0; i < pm->n_thrusters; i++) {
        ...
        for (int j = 0; j < bank->num_points; j++) {
            ...
            ci->w_start = bank->points[j].radius *
...
int ship_create(matrix *orient, vec3d *pos, int ship_type, char *ship_name) {
    ...
    sip->model_num = model_load(sip->pof_file, sip->n_subsystems,
    ...
    ai_clear_ship_goals(
            &Ai_info[Ships[n].ai_index]);        // only do this one here.  Can't do it in init_ai because it might wipe out goals in mission file
...
int ship_fire_primary(object *obj, int stream_weapons, int force) {
    ...
    for (i = 0; i < num_primary_banks; i++) {
        ...
        if (pm->n_guns > 0) {
            ...
            if (winfo_p->wi_flags &
                WIF_BEAM) {        // the big change I made for fighter beams, if there beams fill out the Fire_Info for a targeting laser then fire it, for each point in the weapon bank -Bobboau
                float t;
                if (winfo_p->burst_shots > swp->burst_counter[bank_to_fire]) {
                ...
                shipp->beam_sys_info.turret_fov = (float) cos(
                        (winfo_p->field_of_fire != 0.0f) ? winfo_p->field_of_fire :
                        180);
                    ...
                    for (j = 0; j < points; j++) {
                        ...
                        for (int s = 0; s < sub_shots; s++) {
                            ...
                            if (!(sip->flags2 & SIF2_GUN_CONVERGENCE)) {
                                ...
                                } else if ((sip->aiming_flags & AIM_FLAG_AUTO_CONVERGENCE) &&
                                           (aip->target_objnum != -1)) {
...
int ship_fire_secondary(object *obj, int allow_swarm) {
    ...
    if ((obj->flags & OF_PLAYER_SHIP) && (swp->secondary_bank_ammo[bank] <= 0)) {
        ...
        if (ship_select_next_valid_secondary_bank(
                swp)) {            //DTP here we switch to the next valid bank, but we can't call weapon_info on next fire_wait
            ...
            if (timestamp_elapsed(
                    shipp->weapons.next_secondary_fire_stamp[shipp->weapons.current_secondary_bank])) {    //DTP, this is simply a copy of the manual cycle functions
                shipp->weapons.next_secondary_fire_stamp[shipp->weapons.current_secondary_bank] = timestamp(
                        1000);    //Bumped from 250 to 1000 because some people seem to be to triggerhappy :).
...
int ship_select_next_primary(object *objp, int direction) {
    ...
    if (swp->num_primary_banks == 0) {
        ...
        if (objp == Player_obj) {
            HUD_sourced_printf(HUD_SOURCE_HIDDEN, XSTR("This ship has only one primary weapon: %s", 491),
                               Weapon_info[swp->primary_bank_weapons[swp->current_primary_bank]].name,
                               swp->current_primary_bank + 1);
            gamesnd_play_error_beep();
        }
        return 0;
    ...
    if (sip->flags & SIF_BALLISTIC_PRIMARIES) {
        ...
        if ((swp->current_primary_bank != original_bank) ||
            ((shipp->flags & SF_PRIMARY_LINKED) != original_link_flag)) {
            if (objp == Player_obj) {
...
char *ship_return_orders(char *outbuf, ship *sp) {
    ...
    switch (aigp->ai_mode) {
        ...
        case AI_GOAL_CHASE_ANY: {
            if (aip->target_objnum >= 0 && Objects[aip->target_objnum].type == OBJ_SHIP &&
                Objects[aip->target_objnum].instance >= 0) {
...
void ship_page_out_textures(int ship_index, bool release) {
    ...
    // thruster/particle bitmaps
    for (i = 0; i < (int) sip->normal_thruster_particles.size(); i++) PAGE_OUT_TEXTURE(
            sip->normal_thruster_particles[i].thruster_bitmap.first_frame);

    for (i = 0; i < (int) sip->afterburner_thruster_particles.size(); i++) PAGE_OUT_TEXTURE(
            sip->afterburner_thruster_particles[i].thruster_bitmap.first_frame);
}
    ...
    do {
        ...
        if (!no_content) {
            if (adt.Calculations.size() != adt.Arguments.size()) {
                Warning(LOCATION,
                        "Armor '%s', damage type %d: Armor has a different number of calculation types than arguments (%d, %d)",

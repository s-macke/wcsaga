    ...
    for (moveup = GET_FIRST(&Ship_obj_list); moveup != END_OF_LIST(&Ship_obj_list); moveup = GET_NEXT(moveup)) {
        ...
        for (ship_system = GET_FIRST(&shipp->subsys_list); ship_system != END_OF_LIST(&shipp->subsys_list);
            ...
            if ((ship_system->system_info != NULL) && (ship_system->system_info->flags & MSS_FLAG_AWACS)) {
                ...
                Awacs_team[shipp->team] += ship_system->awacs_intensity * (ship_system->current_hits /
...
#define FULLY_TARGETABLE        (viewer_has_primitive_sensors ? ((distance < viewer->primitive_sensor_range) ? MARGINALLY_TARGETABLE : UNTARGETABLE) : ALWAYS_TARGETABLE)
    ...
    if ((viewer == Player_ship) && (Game_mode & GM_MULTIPLAYER) && (Net_player != NULL) &&
    ...
    if (stealth_ship) {
        ...
        float scan_nebula_range = Neb2_awacs * Species_info[Ship_info[viewer->ship_info_index].species].
                awacs_multiplier;
        ...
        if (check_huge_ship) {
            if (check_world_pt_in_expanded_ship_bbox(&Objects[viewer->objnum].pos, target, scan_nebula_range)) {
                if (check_world_pt_in_expanded_ship_bbox(&Objects[viewer->objnum].pos, target,
                                                         MARGINALLY_TARGETABLE * scan_nebula_range))
                    return FULLY_TARGETABLE;
        ...
        for (int en_team = 0; en_team < MAX_IFFS; en_team++) {
            ...
            for (en_idx = 0; en_idx < en_count; en_idx++) {
                ...
                for (idx = 0; idx < cur_count; idx++) {
                    ...
                    if (awacs_get_level(&Objects[Ships[en_team_ships[en_idx]].objnum], &Ships[cur_team_ships[idx]],
                                        (idx == 0)) > 1.0f) {
...
int ship_is_visible_by_team(object *target, ship *viewer) {
    ...
    if ((Hud_max_targeting_range > 0) && (vm_vec_dist_quick(&target->pos, &Objects[viewer->objnum].pos) >
                                          Hud_max_targeting_range))
        return 0;

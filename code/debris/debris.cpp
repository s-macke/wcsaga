...
static void debris_start_death_roll(object* debris_obj, debris* debris_p)
{
	if (debris_p->is_hull)
	{
		...
		fireball_create(&debris_obj->pos, fireball_type, FIREBALL_LARGE_EXPLOSION, OBJ_INDEX(debris_obj),
			debris_obj->radius * 1.75f);
		...
		{
			snd_play_3d(&Snds[SND_MISSILE_IMPACT], &debris_obj->pos, &View_position, debris_obj->radius);
...
void debris_render(object* obj)
{
	...
	if (vm_vec_dist_quick(&obj->pos, &Eye_position) < obj->radius * 50.0f)
	{
		for (i = 0; i < MAX_DEBRIS_ARCS; i++)
		{
			...
			{
				model_add_arc(db->model_num, db->submodel_num, &db->arc_pts[i][0], &db->arc_pts[i][1],
					MARC_TYPE_NORMAL);
			}
		}
	}
...
object* debris_create(object* source_obj, int model_num, int submodel_num, vec3d* pos, vec3d* exp_center,
					  int hull_flag, float exp_force)
{
	...
	if (hull_flag)
	{
		if (sip->debris_min_lifetime >= 0.0f && sip->debris_max_lifetime >= 0.0f)
		{
			...
			db->lifeleft = ((sip->debris_max_lifetime - sip->debris_min_lifetime) * frand()) +
	...
	if (hull_flag)
	{
		...
		{
			obj->hull_strength = ((sip->debris_max_hitpoints - sip->debris_min_hitpoints) * frand()) +
	...
	if (hull_flag)
	{
		...
		t = 1000 * Objects[db->source_objnum].radius / 3 + myrand() % (fl2i(1000 * 3 * Objects[db->source_objnum].
																	   radius));
...
int debris_check_collision(object* pdebris, object* other_obj, vec3d* hitpos, collision_info_struct* debris_hit_info)
{
	...
	if (debris_hit_info->heavy == ship_obj)
	{	// ship is heavier, so debris is sphere. Check sphere collision against ship poly model
				...
				for (i = 0; i < num_rotating_submodels; i++)
				{
					...
					if (model_collide(&mc))
					{
						if (mc.hit_dist < debris_hit_info->hit_time)
						{
							...
							model_find_world_point(&debris_hit_info->hit_pos, &mc.hit_point, mc.model_num,
								mc.hit_submodel, &heavy_obj->orient, &zero);

							// set up debris_hit_info for rotating submodel
							if (debris_hit_info->edge_hit == 0)
							{
								model_find_obj_dir(&debris_hit_info->collision_normal, &mc.hit_normal, heavy_obj,
									mc.hit_submodel);
							}
							...
							model_find_world_point(&debris_hit_info->light_collision_cm_pos, &int_light_pos,

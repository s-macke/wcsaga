...
int
version_compare(char *filename, int *u_major, int *u_minor, int *u_build, int *l_major, int *l_minor, int *l_build) {
    ...
    char verbuffer[MAX_LINE_LENGTH + 1] =
            {
                    0
            };
    ...
    while (!feof(f)) {
        ...
        if (buffer[0] == VERSION_FILE_COMMENT_CHAR)
            continue;

...
int verify_pilot_file(char* filename, int single, int* rank)
{
	...
	if (file_version < LOWEST_COMPATIBLE_PLAYER_FILE_VERSION)
	{
		...
		sprintf(warning_text,
			"ATTENTION: Detected out-of-date player file \"%s\".  If you press OK, this file will be deleted!  Back this file up now if you do not want to lose it.  Contact a coder to resolve this error.", pname);
...
#ifndef _WIN32
		snprintf(confirm_string, 300,
			"This will cause the pilot '%s' to be converted to FSO format. The conversion process is not reversible and you will no longer be able to use this pilot in earlier versions of FreeSpace (including FS2 retail). Do you want to continue?", callsign);
#else
...
void pilot_read_techroom_data(CFILE* file, int pfile_version)
{
	...
	if (pfile_version < 242)
	{
		...
		if (Player_file_version <= FS2_DEMO_PLAYER_FILE_VERSION)
		{
...
int read_pilot_file(char* callsign, int single, player* p)
{
	...
	if (Player_file_version < LOWEST_COMPATIBLE_PLAYER_FILE_VERSION)
	{
		...
		char warning_text[256];
		sprintf(warning_text,
			"ATTENTION: Detected out-of-date player file \"%s\".  If you press OK, this file will be deleted!  Back this file up now if you do not want to lose it.  Contact a coder to resolve this error.", filename);
		Error(LOCATION, warning_text);
	...
	int si_index = ship_info_lookup(ship_name);
	...
	p->last_ship_flown_si_index = si_index;
...
int write_pilot_file_core(player* p)
{
	...
	if (!is_multi)
	{
		snprintf(pathname, MAX_PATH_LEN - 1, "%s%s%s%s%s%s%s", detect_home(), DIR_SEPARATOR_STR, Osreg_company_name,
			DIR_SEPARATOR_STR, Osreg_user_dir, DIR_SEPARATOR_STR, Pathtypes[CF_TYPE_SINGLE_PLAYERS].path);
	...
	{
		snprintf(pathname, MAX_PATH_LEN - 1, "%s%s%s%s%s%s%s", detect_home(), DIR_SEPARATOR_STR, Osreg_company_name,
			DIR_SEPARATOR_STR, Osreg_user_dir, DIR_SEPARATOR_STR, Pathtypes[CF_TYPE_MULTI_PLAYERS].path);
		cf_create_directory(pathname);
...
int write_pilot_file(player* the_player)
{
	...
	do
	{
		...
		if (pilot_write_rval)
		{
			int popup_rval = popup(PF_TITLE_RED | PF_TITLE_BIG, 3, XSTR("&Retry", 41), XSTR("&Ignore", 42),
				XSTR("&Quit Game", 43),
				XSTR(
...
void write_stats_block(CFILE* file, scoring_struct* stats, int multi)
{
	...
	while (total && !stats->kills[total - 1])
...
void init_new_pilot(player* p, int reset)
{
	if (reset)
	{
		...
		// WC Saga - Always set to highest level by default.
		int cur_speed = NUM_DEFAULT_DETAIL_LEVELS - 1;
...
{
	return popup(PF_USE_AFFIRMATIVE_ICON | PF_USE_NEGATIVE_ICON | PF_TITLE_RED | PF_TITLE_BIG, 2, XSTR("&No", 47),
		XSTR("&Yes", 48), XSTR("Warning\nA duplicate pilot exists\nOverwrite?", 49));
}
...
void pilot_load_pic_list()
{
	...
	Num_pilot_images = cf_get_file_list_preallocated(MAX_PILOT_IMAGES, Pilot_images_arr, Pilot_image_names,
		CF_TYPE_PLAYER_IMAGES, NOX("*.pcx"));
...
void pilot_load_squad_pic_list()
{
	...
	// load pilot images from the player images directory
	Num_pilot_squad_images = cf_get_file_list_preallocated(MAX_PILOT_IMAGES, Pilot_squad_images_arr,
		Pilot_squad_image_names, CF_TYPE_SQUAD_IMAGES, NOX("*.pcx"));

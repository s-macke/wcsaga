...
int verify_pilot_file(char *filename, int single, int *rank) {
    ...
    if (file_version < LOWEST_COMPATIBLE_PLAYER_FILE_VERSION) {
        ...
        sprintf(warning_text,
                "ATTENTION: Detected out-of-date player file \"%s\".  If you press OK, this file will be deleted!  Back this file up now if you do not want to lose it.  Contact a coder to resolve this error.",
                pname);
...
int pilot_file_upgrade_check(char *callsign, int single) {
    ...
    if (cf_exists(pname, CF_TYPE_SINGLE_PLAYERS)) {
        ...
        snprintf(confirm_string, 300,
                 "This will cause the pilot '%s' to be converted to FSO format. The conversion process is not reversible and you will no longer be able to use this pilot in earlier versions of FreeSpace (including FS2 retail). Do you want to continue?",
                 callsign);
#else

...
grid* brief_create_grid(grid* gridp, vec3d* forward, vec3d* right, vec3d* center, int nrows, int ncols,
						float square_size);
...
void brief_render_fade_outs(float frametime)
{
	...
	for (i = 0; i < Num_fade_icons; i++)
	{
		...
		if (!(tv.flags & PF_OVERFLOW))
		{  // make sure point projected before drawing text
...
void brief_render_icon_line(int stage_num, int line_num)
{
	...
	{
		Warning(LOCATION, "Start icon (%d/%d) missing for line %d in briefing stage %d", bl->start_icon, bs->num_icons,
			line_num, stage_num);
		//Remove line
		bs->num_lines--;
	...
	{
		Warning(LOCATION, "End icon (%d/%d) missing for line %d in briefing stage %d", bl->end_icon, bs->num_icons,
			line_num, stage_num);
		//Remove line
		bs->num_lines--;
		for (int i = line_num; i < bs->num_lines; i++)
	...
	for (i = 0; i < 2; i++)
	{
		...
		if (!(icon_vertex[i].flags & PF_OVERFLOW))
		{  // make sure point projected before drawing text
			icon_status[i] = 1;
		}
	}
...
void brief_render_icon(int stage_num, int icon_num, float frametime, int selected, float w_scale_factor,
					   float h_scale_factor)
{
	...
	{  // make sure point projected before drawing text
		...
		if ((bx < 0) || (bx > gr_screen.max_w_unscaled) || (by < 0) || (by > gr_screen.max_h_unscaled) &&
			!Fred_running)
		{
			bi->x = bx;
			bi->y = by;
...
void brief_render_map(int stage_num, float frametime)
{
	...
	gr_set_clip(bscreen.map_x1 + 1, bscreen.map_y1 + 1, bscreen.map_x2 - bscreen.map_x1 - 1,
	...
	g3_start_frame(0);
	g3_set_view_matrix(&Current_cam_pos, &Current_cam_orient, Viewer_zoom_target);
...
void brief_blit_stage_num(int stage_num, int stage_max)
{
	...
	{
		gr_printf(Brief_stage_text_coords_multi[gr_screen.res][0], Brief_stage_text_coords_multi[gr_screen.res][1],
			buf);
	}
	else
	{
...
ubyte brief_return_color_index(char c)
{
	...
	default:	//Zacam: Changed fron an Int3() in order to provide better feedback while still allowing play.
		Warning(LOCATION,
			"Unrecognized or undefined case character: '$%c' used in Briefing in mission: '%s'. Tell Zacam.", c,
			Mission_filename);
	} // end switch
...
void brief_set_new_stage(vec3d* pos, matrix* orient, int time, int stage_num)
{
	...
	if ((Last_new_stage != -1) && (num_movers == 0) && not_objv)
	{
		...
		{
			if (!vm_vec_cmp(&Briefing->stages[stage_num].camera_orient.vec.fvec, &Briefing->stages[Last_new_stage].
				camera_orient.vec.fvec))
			{
				new_time = 0;
			}
		}

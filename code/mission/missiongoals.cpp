...
int goal_text::add(char *text) {
    ...
    count = split_str(text, Goal_screen_text_w - Goal_screen_text_coords[gr_screen.res][GOAL_SCREEN_X_COORD] +
                            Goal_screen_icon_xcoord[gr_screen.res], m_line_sizes + m_num_lines, m_lines + m_num_lines,
                      max);
    m_num_lines += count;
    return count;
}
...
int ML_objectives_init(int x, int y, int w, int h) {
    ...
    Goal_screen_text_x = x - Goal_screen_icon_xcoord[gr_screen.res] + Goal_screen_text_coords[gr_screen.
            res][GOAL_SCREEN_X_COORD];
...
void ML_render_objectives_key() {
    ...
    if (Lcl_gr) {
        ...
        gr_bitmap(Objective_key_icon_coords_gr[gr_screen.res][0][0],
                  Objective_key_icon_coords_gr[gr_screen.res][0][1]);
        gr_set_bitmap(Goal_incomplete_bitmap);
        gr_bitmap(Objective_key_icon_coords_gr[gr_screen.res][1][0],
                  Objective_key_icon_coords_gr[gr_screen.res][1][1]);
        gr_set_bitmap(Goal_failed_bitmap);
        gr_bitmap(Objective_key_icon_coords_gr[gr_screen.res][2][0],
                  Objective_key_icon_coords_gr[gr_screen.res][2][1]);

        gr_string(Objective_key_text_coords_gr[gr_screen.res][0][0], Objective_key_text_coords_gr[gr_screen.res][0][1],
                  XSTR("Complete", 1437));
        gr_string(Objective_key_text_coords_gr[gr_screen.res][1][0], Objective_key_text_coords_gr[gr_screen.res][1][1],
                  XSTR("Incomplete", 1438));
        gr_string(Objective_key_text_coords_gr[gr_screen.res][2][0], Objective_key_text_coords_gr[gr_screen.res][2][1],
                  XSTR("Failed", 1439));
    } else {
        gr_set_bitmap(Goal_complete_bitmap);
        ...
        gr_string(Objective_key_text_coords[gr_screen.res][0][0], Objective_key_text_coords[gr_screen.res][0][1],
                  XSTR("Complete", 1437));
        gr_string(Objective_key_text_coords[gr_screen.res][1][0], Objective_key_text_coords[gr_screen.res][1][1],
                  XSTR("Incomplete", 1438));
        gr_string(Objective_key_text_coords[gr_screen.res][2][0], Objective_key_text_coords[gr_screen.res][2][1],
                  XSTR("Failed", 1439));
    }
}
...
void mission_goal_status_change(int goal_num, int new_status) {
    ...
    if (new_status == GOAL_FAILED) {
        ...
        if (type != BONUS_GOAL) {
            ...
            if ((Game_mode & GM_NORMAL) || ((Net_player != NULL) && (Net_player->p_info.team ==
                                                                     Mission_goals[goal_num].team))) {
        ...
        if ((Game_mode & GM_NORMAL) || ((Net_player != NULL) && (Net_player->p_info.team == Mission_goals[goal_num].
                team))) {
            hud_add_objective_messsage(type, new_status);
            // cue for Event Music
...
void mission_process_event(int event) {
    ...
    if (Mission_events[event].chain_delay >= 0) {  // this indicates it's chained
        if (event > 0) {
            if (!Mission_events[event - 1].result || ((fix) Mission_events[event - 1].timestamp +
                                                      i2f(Mission_events[event].chain_delay) > Missiontime)) {
                sindex = -1;  // bypass evaluation
            }
        }

        if ((event < Num_mission_events - 1) &&
            Mission_events[event + 1].result && (Mission_events[event + 1].chain_delay >= 0)) {
            sindex = -1;  // bypass evaluation
        }
    }
    ...
    if (result && (Mission_events[event].trigger_count > 0) &&
        (Mission_events[event].flags & MEF_USING_TRIGGER_COUNT)) {
    ...
    if (result || timestamp_valid(Mission_events[event].timestamp)) {
        ...
        else if (bump_timestamp || (!(Mission_events[event].repeat_count == -1 &&
                                      Mission_events[event].trigger_count > 0))) {
    ...
    if (MULTIPLAYER_MASTER && ((store_flags != Mission_events[event].flags) || (store_formula != Mission_events[event].
            formula) ||
                               (store_result != Mission_events[event].result) || (store_count != Mission_events[event].
            count))) {
...
int mission_goals_met() {
    ...
    for (i = 0; i < Num_goals; i++) {
        ...
        if (((Mission_goals[i].type & GOAL_TYPE_MASK) == PRIMARY_GOAL) || ((Mission_goals[i].type &
                                                                            GOAL_TYPE_MASK) == SECONDARY_GOAL)) {
            if (Mission_goals[i].satisfied == GOAL_INCOMPLETE) {
                all_goals_met = 0;
                break;
            } else if (Mission_goals[i].satisfied == GOAL_FAILED) {
...
"List and change the status of mission goals")
{
int i, type;
...
i++) {
type = Mission_goals[i].type & GOAL_TYPE_MASK;
dc_printf("%2d. %32s(%10s) -- ", i, Mission_goals[i].name,
Goal_type_text(type)
);
if (Mission_goals[i].satisfied == GOAL_COMPLETE)
dc_printf("satisfied.\n");
...
num = Dc_arg_int;
dc_get_arg(ARG_TRUE
|ARG_FALSE|ARG_STRING);
if (
Dc_arg_type &ARG_TRUE
)
Mission_goals[num].
satisfied = GOAL_COMPLETE;
else if (
Dc_arg_type &ARG_FALSE
)
Mission_goals[num].
satisfied = GOAL_FAILED;
else if (
Dc_arg_type &ARG_NONE
)
Mission_goals[num].
satisfied = GOAL_INCOMPLETE;
else if (

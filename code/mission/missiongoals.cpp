...
int goal_text::add(char* text)
{
	...
	count = split_str(text, Goal_screen_text_w - Goal_screen_text_coords[gr_screen.res][GOAL_SCREEN_X_COORD] +
		Goal_screen_icon_xcoord[gr_screen.res], m_line_sizes + m_num_lines, m_lines + m_num_lines, max);
	m_num_lines += count;
	return count;
}
...
void mission_show_goals_init()
{
	...
	for (i = 0; i < Num_goals; i++)
	{
		...
		{  // don't count invalid goals here
			continue;
		}
...
void mission_process_event(int event)
{
	...
	if (Mission_events[event].chain_delay >= 0)
	{  // this indicates it's chained
		if (event > 0)
		{

...
LOCAL
UI_WINDOW Campaign_window;
LOCAL
UI_LISTBOX Campaign_listbox;
LOCAL
UI_BUTTON Campaign_okb, Campaign_cancelb;
...
int mission_campaign_savefile_save()
{
	...
	while (total && !pl->stats.kills[total - 1])
	{  // find last used element
		total--;
	}
	...
	for (i = 0; i < sexp_variable_count(); i++)
	{
		...
		if (Sexp_variables[i].type & SEXP_VARIABLE_PLAYER_PERSISTENT)
		{
			...
			if (index < 0)
			{
				...
				{
					Error(LOCATION,
						"Too many player-persistent variables (last valid variable: %s, max number of variables: %d).  The remaining ones will not be saved.  Press OK to continue.", Player->player_variables[Player->num_variables - 1].variable_name, Player->num_variables);
					return;
				}

...
int mission_campaign_load(char *filename, player *pl, int load_savefile) {
    ...
    char strippedFilename[MAX_FILENAME_LEN];

    filename = cf_add_ext(filename, FS_CAMPAIGN_FILE_EXT);

    /// Extract filename if necessary.
    std::string filenametemp(filename);
    std::replace(filenametemp.begin(), filenametemp.end(), '\\', '/');

    // This is safe as npos == -1 will wrap around to 0.
    size_t filenameStart = filenametemp.find_last_of('/') + 1;

    strncpy(strippedFilename, filenametemp.substr(filenameStart, filenametemp.length() - filenameStart).c_str(),
            MAX_FILENAME_LEN - 1);
    strippedFilename[MAX_FILENAME_LEN - 1] = 0;
    ...
    if ((rval = setjmp(parse_abort)) != 0) {
        ...
        if (!Fred_running && !(pl->flags & PLAYER_FLAGS_IS_MULTI)) {
            if (mission_campaign_savefile_load(strippedFilename, pl)) {
        ...
        strncpy(Campaign.filename, strippedFilename, MAX_FILENAME_LEN - 1);
        strncpy(Campaign.pathname, filename, MAX_PATH_LEN - 1);
        Campaign.filename[MAX_FILENAME_LEN - 1] = 0;
        Campaign.pathname[MAX_PATH_LEN - 1] = 0;
        ...
        read_file_text(Campaign.pathname);
        reset_parse();
        memset(&Campaign, 0, sizeof(Campaign));

        // copy filename to campaign structure minus the extension
        len = strlen(strippedFilename) - 4;
        Assert(len < MAX_FILENAME_LEN);
        strncpy(Campaign.filename, strippedFilename, len);
        Campaign.filename[len] = 0;

        len = strlen(filename);
        Assert(len < MAX_PATH_LEN);
        strncpy(Campaign.pathname, filename, len);
        Campaign.pathname[len] = 0;
        ...
        if (optional_string("+Campaign Intro Cutscene:")) {
            stuff_string(Campaign.intro_cutscene, F_NAME, NAME_LENGTH);
        }

        Campaign.end_cutscene[0] = 0;
        if (optional_string("+Campaign End Cutscene:")) {
            stuff_string(Campaign.end_cutscene, F_NAME, NAME_LENGTH);
        }
    ...
    if (!Fred_running && load_savefile && (Campaign.type == CAMPAIGN_TYPE_SINGLE)) {
        ...
        mission_campaign_savefile_load(strippedFilename, pl);
    }
...
int mission_campaign_savefile_load(char *cfilename, player *pl) {
    ...
    if (version < CAMPAIGN_FILE_COMPATIBLE_VERSION) {
        Warning(LOCATION,
                "Campaign save file too old -- not compatible.  Deleting file.\nYou can continue from here without trouble\n\n");
        cfclose(fp);
        cf_delete(filename, CF_TYPE_SINGLE_PLAYERS);
        return 0;
    }
    ...
    // the actual check
    Assert(((Game_mode & GM_MULTIPLAYER) && (type_sig == (int) CAMPAIGN_MULTI_PLAYER_SIG)) || (!(Game_mode &
                                                                                                 GM_MULTIPLAYER) &&
                                                                                               (type_sig == (int)
                                                                                                       CAMPAIGN_SINGLE_PLAYER_SIG)));
    ...
    for (i = 0; i < Campaign.num_missions_completed; i++) {
        ...
        if (version >= 13) {
            ...
            if (Campaign.missions[num].num_saved_variables > 0) {
                Campaign.missions[num].saved_variables = (sexp_variable *) vm_malloc(Campaign.missions[num].
                        num_saved_variables * sizeof(sexp_variable));

                Assert(Campaign.missions[num].saved_variables != NULL);

                if (Campaign.missions[num].saved_variables != NULL)
                    memset(Campaign.missions[num].saved_variables, 0, sizeof(sexp_variable) * Campaign.missions[num].
                            num_saved_variables);
...
int mission_campaign_next_mission() {
    if ((Campaign.next_mission == -1) ||
        (strlen(Campaign.name) == 0)) // will be set to -1 when there is no next mission
        return -1;
...
void mission_campaign_eval_next_mission() {
    ...
    if (Campaign.missions[cur].has_mission_loop) {
        ...
        if (Campaign.missions[cur].mission_loop_formula != -1) {
            flush_sexp_tree(Campaign.missions[cur].mission_loop_formula);  // force formula to be re-evaluated
            eval_sexp(
                    Campaign.missions[cur].mission_loop_formula);  // this should reset Campaign.next_mission to proper value
        }
...
void mission_campaign_maybe_play_movie(int type) {
    ...
    Assert(type >= 0 && type < CAMPAIGN_MAX_MOVIE_TYPES);
    ...
    char *filename = NULL;
    switch (type) {
        case CAMPAIGN_MOVIE_INTRO:
            if (strlen(Campaign.intro_cutscene))
                filename = Campaign.intro_cutscene;
            break;

        case CAMPAIGN_MOVIE_END_CAMPAIGN:
            if (strlen(Campaign.end_cutscene))
                filename = Campaign.end_cutscene;
            break;
...
void mission_campaign_end_do() {
    ...
    if (!stricmp(Campaign.filename, "freespace2")) {
        ...
        mission_campaign_maybe_play_movie(CAMPAIGN_MOVIE_END_CAMPAIGN);
    }
        ...
        if (Sexp_variables[i].type & SEXP_VARIABLE_PLAYER_PERSISTENT) {
            ...
            if (index < 0) {
                ...
                if (Player->num_variables == MAX_SEXP_VARIABLES) {
                    Error(LOCATION,
                          "Too many player-persistent variables (last valid variable: %s, max number of variables: %d).  The remaining ones will not be saved.  Press OK to continue.",
                          Player->player_variables[Player->num_variables - 1].variable_name, Player->num_variables);
                    return;
                }

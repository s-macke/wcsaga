...
void training_obj_display()
{
	...
	hud_aabitmap(Directive_gauge[0].first_frame, Directive_coords[gr_screen.res][DIRECTIVE_COORDS_TOP][0] +
		fl2i(HUD_offset_x), Directive_coords[gr_screen.res][DIRECTIVE_COORDS_TOP][1] + fl2i(HUD_offset_y));
	...
	// print out title
	emp_hud_printf(Directive_coords[gr_screen.res][DIRECTIVE_COORDS_TITLE][0] + fl2i(HUD_offset_x),
		Directive_coords[gr_screen.res][DIRECTIVE_COORDS_TITLE][1] + fl2i(HUD_offset_y), EG_OBJ_TITLE,
		XSTR("directives", 422));
	...
	for (i = 0; i < end; i++)
	{
		...
		hud_aabitmap(Directive_gauge[1].first_frame, bx, by);
	...
	hud_aabitmap(Directive_gauge[2].first_frame, bx, by);
...
void training_mission_init()
{
	...
	if (The_mission.game_type & MISSION_TYPE_TRAINING)
	{
		Player->flags &= ~(PLAYER_FLAGS_MATCH_TARGET | PLAYER_FLAGS_MSG_MODE | PLAYER_FLAGS_AUTO_TARGETING |
						   PLAYER_FLAGS_AUTO_MATCH_SPEED | PLAYER_FLAGS_LINK_PRIMARY | PLAYER_FLAGS_LINK_SECONDARY);
	}
}
...
void sort_training_objectives()
{
	...
	for (i = 0; i < offset; i++)
	{
		...
		if ((Game_mode & GM_MULTIPLAYER) && (Netgame.type_flags & NG_TYPE_TEAM) && (Net_player != NULL))
		{
			if ((Mission_events[TRAINING_OBJ_LINES_MASK(i)].team != -1) && (Net_player->p_info.team !=
																			Mission_events[TRAINING_OBJ_LINES_MASK(i)].
																			team))
			{
				continue;
			}
...
void training_check_objectives()
{
	...
	for (event_idx = 0; event_idx < Num_mission_events; event_idx++)
	{
		...
		if ((event_status != EVENT_UNBORN) &&
			Mission_events[event_idx].objective_text && (timestamp() > Mission_events[event_idx].born_on_date + 3000))
		{
...
void message_translate_tokens(char* buf, char* text)
{
	...
	*buf = 0;
	...
	toke2 = strchr(text, '#');
	while (toke1 || toke2)
	{  // is either token types present?
		if (!toke2 || (toke1 && (toke1 < toke2)))
		{  // found $ before #
			...
			else
			{
				...
				if (ptr)
				{  // was key translated properly?
					if (!stricmp(ptr, NOX("none")) && (Training_bind_warning != Missiontime))
					{
						if (The_mission.game_type & MISSION_TYPE_TRAINING)
						{
							r = popup(PF_TITLE_BIG | PF_TITLE_RED, 2, XSTR("&Bind Control", 424),
								XSTR("&Abort mission", 425),
								XSTR(
							...
							if (r)
							{  // do they want to abort the mission?
...
{
	if ((m < 0) || !Messages[m].message[0])
	{  // remove current message from the screen
	...
	Training_num_lines = split_str(Training_buf, TRAINING_LINE_WIDTH, Training_line_sizes, Training_lines,
		MAX_TRAINING_MESSAGE_LINES);
	...
	if (message_play_training_voice(Messages[m].wave_info.index) < 0)
	{
		if (length > 0)
			...
			Training_message_timestamp = timestamp(TRAINING_TIMING_BASE + strlen(Messages[m].message) *
				TRAINING_TIMING);  // no voice file playing
...
void message_training_queue_check()
{
	...
	for (i = 0; i < Training_message_queue_count; i++)
	{
		...
		{
			message_training_setup(Training_message_queue[i].num, Training_message_queue[i].length,
				Training_message_queue[i].special_message);
...
void message_training_display()
{
	...
	gr_shade(Training_message_window_coords[gr_screen.res][0], Training_message_window_coords[gr_screen.res][1],
		TRAINING_MESSAGE_WINDOW_WIDTH, Training_num_lines * height + height);

	gr_set_color_fast(&Color_bright_blue);
	mode = count = 0;
	Training_message_visible = 1;
	for (i = 0; i < Training_num_lines; i++)
	{  // loop through all lines of message
		str = Training_lines[i];
		z = 0;
		int w, h;
		gr_get_string_size(&w, &h, str);
		x = (gr_screen.max_w_unscaled - w) / 2;

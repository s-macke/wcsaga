...
#define HOTKEY_SAVE_TIME				250		// save sets this number of milliseconds into the mission
...
static hotkey_buttons Buttons[GR_NUM_RESOLUTIONS][NUM_BUTTONS] =
{
	...
	},
	{
		...
		hotkey_buttons("2_HKB_02", 24, 548, 2),     // "Cancel"
		hotkey_buttons("2_HKB_03", 135, 548, 3), // "Clear"
		hotkey_buttons("2_HKB_04", 258, 548, 4), // "Reset"
		hotkey_buttons("2_HKB_05", 862, 8, 5),
		hotkey_buttons("2_HKB_06", 862, 71, 6),
		hotkey_buttons("2_HKB_07", 854, 681, 7), // "Help"
		hotkey_buttons("2_HKB_08", 862, 728, 8), // "Options"
		hotkey_buttons("2_HKB_09", 920, 692, 9)  // "Accept"
	}
	//XSTR:ON
};
...
{
	{
		// GR_640
		{
			...
			UI_XSTR_COLOR_PINK,
			-1,
			&Buttons[GR_640][CANCEL_BUTTON].button
		},
		{
			"Clear",
			1517,
			...
			UI_XSTR_COLOR_PINK,
			-1,
			&Buttons[GR_640][CLEAR_BUTTON].button
		},
		{
			"Reset",
			1518,
			...
			UI_XSTR_COLOR_PINK,
			-1,
			&Buttons[GR_640][RESET_BUTTON].button
		},
		{
			"Help",
			1519,
			...
			UI_XSTR_COLOR_PINK,
			-1,
			&Buttons[GR_640][HELP_BUTTON].button
		},
		{
			"Options",
			1520,
			...
			UI_XSTR_COLOR_PINK,
			-1,
			&Buttons[GR_640][OPTIONS_BUTTON].button
		},
		{
			"Accept",
			1521,
			...
			UI_XSTR_COLOR_PINK,
			-1,
			&Buttons[GR_640][ACCEPT_BUTTON].button
		}
	},
	{
		// GR_1024
		{
			...
			UI_XSTR_COLOR_PINK,
			-1,
			&Buttons[GR_1024][CANCEL_BUTTON].button
		},
		{
			"Clear",
			1517,
			...
			UI_XSTR_COLOR_PINK,
			-1,
			&Buttons[GR_1024][CLEAR_BUTTON].button
		},
		{
			"Reset",
			1518,
			...
			UI_XSTR_COLOR_PINK,
			-1,
			&Buttons[GR_1024][RESET_BUTTON].button
		},
		{
			"Help",
			1519,
			...
			UI_XSTR_COLOR_PINK,
			-1,
			&Buttons[GR_1024][HELP_BUTTON].button
		},
		{
			"Options",
			1520,
			...
			UI_XSTR_COLOR_PINK,
			-1,
			&Buttons[GR_1024][OPTIONS_BUTTON].button
		},
		{
			"Accept",
			1521,
			...
			UI_XSTR_COLOR_PINK,
			-1,
			&Buttons[GR_1024][ACCEPT_BUTTON].button
		}
	}
};
...
void mission_hotkey_maybe_restore()
{
	...
	for (i = 0; i < Num_hotkeys_saved; i++)
	{
		...
		if (index != -1)
		{
			hud_target_hotkey_add_remove(Hotkey_saved_info[i].setnum, &Objects[Ships[index].objnum],
				HOTKEY_USER_ADDED);
		 ...
		 p_objp = GET_NEXT(p_objp))
	{
		...
		for (i = 0; i < Num_hotkeys_saved; i++)
		{
			...
			if (!stricmp(p_objp->name, Hotkey_saved_info[i].name))
			{
				if (p_objp->wingnum >= 0)
				{
					Wings[p_objp->wingnum].hotkey = Hotkey_saved_info[i].setnum;
				}
				else
				{
					p_objp->hotkey = Hotkey_saved_info[i].setnum;
				}
...
void mission_hotkey_maybe_save_sets()
{
	...
	for (i = 0; i < MAX_KEYED_TARGETS; i++)
	{
			 ...
			 p_objp = GET_NEXT(p_objp))
		{
			...
			if (((p_objp->wingnum >= 0) && (Wings[p_objp->wingnum].hotkey == i)) ||
				(p_objp->hotkey == i))
			{
				hkp->setnum = i;
				strcpy(hkp->name, p_objp->name);
				hkp++;
				Num_hotkeys_saved++;
			}
		}
...
{
	// The restoring check that used to be here is now irrelevant since restoring and saving
	// of hotkeys is now handled differently (possibly better).
...
int hotkey_line_add_sorted(char* text, int type, int index, int start)
{
	...
	z = Num_lines - 1;
	while ((z >= start) && ((Hotkey_lines[z].type == HOTKEY_LINE_SUBSHIP) || (stricmp(text,
																			  Hotkey_lines[z].label) < 0)))
		z--;
...
int hotkey_build_team_listing(int enemy_team_mask, int y, bool list_enemies)
{
	...
	for (so = GET_FIRST(&Ship_obj_list); so != END_OF_LIST(&Ship_obj_list); so = GET_NEXT(so))
	{
		...
		if (Ship_info[Ships[shipnum].ship_info_index].class_type < 0 || !(Ship_types[Ship_info[Ships[shipnum].
																		  ship_info_index].class_type].hud_bools &
																		  STI_HUD_HOTKEY_ON_LIST))
			continue;
	...
	for (i = 0; i < Num_wings; i++)
	{
		if (Wings[i].current_count && (Wings[i].ship_index[Wings[i].special_ship] >= 0) &&
			iff_matches_mask(Ships[Wings[i].ship_index[Wings[i].special_ship]].team, team_mask))
		{
...
{
	if (Hotkey_lines[Num_lines - 1].y + gr_get_font_height() > Hotkey_lines[Scroll_offset].y +
		Hotkey_list_coords[gr_screen.res][3])
	{
		Scroll_offset++;
		while (!hotkey_line_query_visible(Selected_line) || (Hotkey_lines[Selected_line].type == HOTKEY_LINE_HEADING))
		{
...
void mission_hotkey_do_frame(float frametime)
{
	...
	for (i = 0; i < LIST_BUTTONS_MAX; i++)
	{
		...
		if (List_buttons[i].pressed())
		{
			...
			if ((z >= Hotkey_wing_icon_x[gr_screen.res]) && (z < (Hotkey_wing_icon_x[gr_screen.
																  res])+ Hotkey_function_field_width[gr_screen.res]))
			{
				expand_wing();
			}
		}
	...
	gr_printf(Hotkey_function_name_coords[gr_screen.res][0] + (Hotkey_function_name_coords[gr_screen.res][2] - w) / 2,
		Hotkey_function_name_coords[gr_screen.res][1], buf);
	...
	while (hotkey_line_query_visible(line))
	{
		...
		case HOTKEY_LINE_HEADING:
			...
			gr_line(Hotkey_ship_x[gr_screen.res] + w + 1, i, Hotkey_list_coords[gr_screen.res][0] +
				Hotkey_list_coords[gr_screen.res][2], i);
			break;
		...
		if (Hotkey_lines[line].type != HOTKEY_LINE_HEADING)
		{
			...
			List_buttons[line - Scroll_offset].update_dimensions(Hotkey_list_coords[gr_screen.res][0], y,
				Hotkey_list_coords[gr_screen.res][0] + Hotkey_list_coords[gr_screen.res][2] -
				Hotkey_list_coords[gr_screen.res][0], font_height);
		...
		if (hotkeys)
		{
			for (i = 0; i < MAX_KEYED_TARGETS; i++)
			{
				if (hotkeys & (1 << i))
				{
					gr_printf(Hotkey_list_coords[gr_screen.res][0] + Hotkey_function_field_width[gr_screen.res] * i, y,
						Scan_code_text[Key_sets[i]]);
				}
			}
			/*
						*buf = 0;
		...
		{
			// indent
			gr_force_fit_string(buf, 255, Hotkey_list_coords[gr_screen.res][0] + Hotkey_list_coords[gr_screen.
				res][2] - (Hotkey_ship_x[gr_screen.res] + 20));
			gr_printf(Hotkey_ship_x[gr_screen.res] + 20, y, buf);
		}
		else
		{
			gr_force_fit_string(buf, 255, Hotkey_list_coords[gr_screen.res][0] + Hotkey_list_coords[gr_screen.res][2] -
				Hotkey_ship_x[gr_screen.res]);

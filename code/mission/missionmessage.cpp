...
LOCAL
        pmessage
Playing_messages[MAX_PLAYING_MESSAGES];
...
int message_select_persona(ship *shipp, int persona_needed, bool use_only_autoassign_personas);
...
void persona_parse() {
    ...
    if (optional_string("+autoassign")) {
        Personas[Num_personas].auto_assign = true;
    } else {
        Personas[Num_personas].auto_assign = false;
    }
...
bool message_filename_is_generic(char *filename) {
    if (!strnicmp(filename, "cuevoice.wav", 8))
        return true;
    if (!strnicmp(filename, "emptymsg.wav", 8))
        return true;
    if (!strnicmp(filename, "generic.wav", 7))
        return true;
    if (!strnicmp(filename, "msgstart.wav", 8))
        return true;
...
int message_calc_anim_start_frame(int time, anim *ani, int reverse) {
    ...
    if (start_frame < 0) {
        mprintf((
                        "animation '%s' is %i frames too short for the corresponding death message wave. Start frame reset to 0.\n"
                                , ani->name, -start_frame));
        start_frame = 0;
    }
...
void message_play_anim(message_q *q) {
    ...
    if ((q->message_num < Num_builtin_messages) || !(_strnicmp(HEAD_PREFIX_STRING, ani_name,
        ...
        if (persona_index >= 0) {
            if (Personas[persona_index].flags & (PERSONA_FLAG_WINGMAN | PERSONA_FLAG_SUPPORT | PERSONA_FLAG_LARGE)) {
                // get a random head
                ...
                std::string tmp(ani_name);
                sprintf(ani_name, "%s%c", tmp.c_str(), 'a' + rand_index);
                subhead_selected = TRUE;
            } else if (Personas[persona_index].flags & (PERSONA_FLAG_COMMAND)) {
                // get a random head
...
void message_queue_process() {
    ...
    if (Num_messages_playing > 0) {
        ...
        while (i < Num_messages_playing) {
            ...
            if ((Playing_messages[i].anim != NULL) && (Playing_messages[i].wave == -1)) {
                const float frames_per_character = 1.37154f;

                int length = strlen(Messages[Playing_messages[i].id].message);
                int length_in_frames = (int) ((length * frames_per_character) + 0.5f);
                int frames_played = (Playing_messages[i].anim->frame_num - Playing_messages[i].anim->start_at);

                if (frames_played > length_in_frames)
                    text_done = true;
            }
            ...
            if (((Playing_messages[i].wave != -1) && wave_done) || text_done) {
                if (!ani_done) {
                    anim_stop_playing(Playing_messages[i].anim);
                }
            }
            ...
            if ((Playing_messages[i].builtin_type != MESSAGE_OOPS) && (Playing_messages[i].builtin_type !=
                                                                       MESSAGE_HAMMER_SWINE)) {
                if ((Player_ship->team == Iff_traitor) && (!(Game_mode & GM_MULTIPLAYER) || !(Netgame.type_flags &
                                                                                              NG_TYPE_DOGFIGHT))) {
            ...
            if (Playing_messages[i].shipnum != -1) {
                if ((Ships[Playing_messages[i].shipnum].flags & SF_DYING) && (Playing_messages[i].builtin_type !=
                    ...
                    //
                    Assert(shipnum >= 0);
                    if (!(Ships[shipnum].flags & SF_SHIP_HAS_SCREAMED) && !(Ships[shipnum].flags2 &
                                                                            SF2_NO_DEATH_SCREAM)) {
    ...
    while (MessageQ_num > 0) {
        ...
        if (timestamp_valid(q->window_timestamp) && timestamp_elapsed(q->window_timestamp) && !q->group) {
            ...
            if (q->message_num <
                Num_builtin_messages) {            // we should only ever remove builtin messages this way
    ...
    if (Num_messages_playing) {
        ...
        if (q->builtin_type == MESSAGE_HAMMER_SWINE) {
            ...
            return;
                   ...
                   (q->priority > MESSAGE_PRIORITY_LOW)) {
    ...
    if (q->builtin_type == MESSAGE_WINGMAN_SCREAM) {
        if (!((Ship_info[Ships[Message_shipnum].ship_info_index].flags &
               SIF_SMALL_SHIP) || (Ships[Message_shipnum].flags2 & SF2_ALWAYS_DEATH_SCREAM))) {
            goto all_done;
        }
    }
    ...
    if (Message_shipnum >= 0) {
        ...
        if (shipp->callsign_index >= 0) {
            ...
            end_string_at_first_hash_symbol(who_from, (shipp->wingnum != -1));
        }
    }
...
void message_queue_message(int message_num, int priority, int timing, char *who_from, int source, int group, int delay,
                           int builtin_type) {
    int i, m_persona;
    ...
    MessageQ[i].flags = 0;
    if (!stricmp(who_from, The_mission.command_sender) && (m_persona != -1) && (Personas[m_persona].flags &
...
int message_get_persona(ship *shipp) {
    ...
    if (shipp != NULL) {
        ...
        if (shipp->persona_index != -1) {
            ...
            i = message_select_persona(shipp, persona_needed, true);

            if (i == -1) {
                // No autoassignment personas found. Let's try and ignore the auto_assign flag.
                i = message_select_persona(shipp, persona_needed, false);
            }

            if (i == -1) {
                // We didn't find anything, so just use the first one for safety.
                i = 0;
            }
        }
    }

    // Assign now, if not yet assigned.
    if (shipp->persona_index == -1) {
        shipp->persona_index = i;
    }
...
int message_select_persona(ship *shipp, int persona_needed, bool use_only_autoassign_personas) {
    int selected_persona = -1;
    SCP_vector<int> slist;

    SCP_vector <std::pair<Persona *, int> > tempPersonas;

    for (int i = 0; i < Num_personas; i++) {
        tempPersonas.push_back(std::pair<Persona *, int>(&Personas[i], i));
    }

    for (int i = 0; i < Num_personas - 1; i++) {
        int r = i + (rand() % (Num_personas - i)); // Random remaining position.
        std::pair < Persona * , int > temp = tempPersonas[i];
        tempPersonas[i] = tempPersonas[r];
        tempPersonas[r] = temp;
    }
    ...
    for (int i = 0; i < Num_personas; i++) {
        ...
        if (tempPersonas[i].first->species != Ship_info[shipp->ship_info_index].species)
            continue;

        // this Persona is supposed to be auto assigned if otherwise possible
        if (use_only_autoassign_personas && !tempPersonas[i].first->auto_assign)
            continue;

        // check the ship types, and don't try to assign those which don't type match
        if (tempPersonas[i].first->flags & persona_needed) {
            if (!(tempPersonas[i].first->flags & PERSONA_FLAG_USED)) {
                tempPersonas[i].first->flags |= PERSONA_FLAG_USED;
                selected_persona = tempPersonas[i].second;
                break;
            } else {
                // otherwise add it to our list of valid options to randomly select from
                slist.push_back(tempPersonas[i].second);
            }
        }
    }
    ...
    if (!slist.empty()) {
        int i = (rand() % slist.size());
        selected_persona = slist[i];
    }
...
void message_send_builtin_to_player(int type, ship *shipp, int priority, int timing, int group, int delay,
    ...
    if (!(Game_mode & GM_MULTIPLAYER) || ((multi_target == -1) || (multi_target == MY_NET_PLAYER_NUM))) {
        ...
        if ((multi_team_filter < 0) || !(Netgame.type_flags & NG_TYPE_TEAM) || ((Net_player != NULL) &&
    ...
    if (MULTIPLAYER_MASTER) {
        ...
        if (multi_target == -1) {
            if (multi_message_should_broadcast(type)) {
                send_mission_message_packet(message_index, who_from, priority, timing, source, type, -1,
                                            multi_team_filter);
            }
        } else {
            send_mission_message_packet(message_index, who_from, priority, timing, source, type, multi_target,
                                        multi_team_filter);
        }
    }
}

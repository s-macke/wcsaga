...
void fix_old_special_explosions(p_object* p_objp, int variable_index)
{
	...
	for (i = variable_index; i < (variable_index + BLOCK_EXP_SIZE); i++)
	{
		...
		{
			Warning(LOCATION,
				"%s is using the old special explosions method but does not appear to have variables for all the values", p_objp->name);
			return;
		}
	}
...
int parse_object(mission* pm, int flag, p_object* p_objp)
{
	...
	while (optional_string("$Alt Ship Class:"))
	{
		...
		if (new_alt_class.ship_class < 0)
		{
			...
			{
				Warning(LOCATION,
					"Ship \"%s\" has an invalid Alternate Ship Class type (ships.tbl probably changed). Skipping this entry", p_objp->name);
				continue;
			}
			else
			{
...
int parse_wing_create_ships(wing* wingp, int num_to_create, int force, int specific_instance)
{
	...
	if ((objnum != -1) && (Game_mode & GM_IN_MISSION))
	{		// if true, we have created at least one new ship.
		int ship_num;
...
int mission_did_ship_arrive(p_object* objp)
{
	...
	if (should_arrive)
	{ 		// has the arrival criteria been met?
		int object_num;
...
void mission_add_to_arriving_support(object* requester_objp)
{
	...
	if (i != Num_arriving_repair_targets)
	{		// found the ship before reaching the end -- ignore it!
		return;
	}

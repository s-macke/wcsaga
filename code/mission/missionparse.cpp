...
int mission_set_arrival_location(int anchor, int location, int distance, int objnum, int path_mask, vec3d *new_pos,
                                 matrix *new_orient);
...
void parse_player_info2(mission *pm) {
    ...
    for (nt = 0; nt < Num_teams; nt++) {
        ...
        for (i = 0; i < total; i += 4) {
            ...
            if (Game_mode & GM_CAMPAIGN_MODE || ((Game_mode & GM_MULTIPLAYER) && !(Net_player->flags &
        ...
        if (optional_string("+Default_ship:")) {
            ...
            if (-1 == ptr->default_ship) {
                WarningEx(LOCATION, "Mission: %s\nUnknown default ship %s!  Defaulting to %s.", pm->name, str,
                          Ship_info[ptr->ship_list[0]].name);
            }
    ...
    if (nt != Num_teams)
        Error(LOCATION, "Not enough ship/weapon pools for mission.  There are %d teams and only %d pools.", Num_teams,
              nt);
}
...
void parse_single_cutscene(mission *pm, int type) {
    ...
    if (optional_string("+campaign_only")) {
        scene.is_campaign_only = true;
    } else {
        scene.is_campaign_only = false;
    }
...
void parse_briefing(mission *pm, int flags) {
    ...
    if (nt != Num_teams)
        Error(LOCATION, "Not enough briefings in mission file.  There are %d teams and only %d briefings.", Num_teams,
              nt);
}
...
void position_ship_for_knossos_warpin(p_object *p_objp) {
    ...
    float dist = fvi_ray_plane(&new_point, &knossos_objp->pos, &knossos_objp->orient.vec.fvec, &p_objp->pos, &p_objp->
...
void parse_dock_one_docked_object(p_object *pobjp, p_object *parent_pobjp) {
    ...
    parent_dockpoint = model_find_dock_name_index(Ship_info[Ships[parent_objp->instance].ship_info_index].model_num,
...
int parse_create_object_sub(p_object *p_objp) {
    ...
    if (shipp->special_hitpoints > 0) {
        ...
        for (ss = GET_FIRST(&shipp->subsys_list); ss != END_OF_LIST(&shipp->subsys_list) && ss != NULL;
             ss = GET_NEXT(ss)) {
    ...
    for (i = 0; i < p_objp->num_texture_replacements; i++) {
        ...
        for (j = 0; j < pm->n_textures; j++) {
            ...
            if (tnum > -1)
                shipp->ship_replacement_textures[j * TM_NUM_TYPES + tnum] = p_objp->replacement_textures[i].
                        new_texture_id;
        }
    }
    ...
    // other flag checks
    ////////////////////////
    if (p_objp->ship_max_shield_strength == 0.0f || (!Fred_running && !(p_objp->flags2 & P2_OF_FORCE_SHIELDS_ON) &&
                                                     (sip->flags2 & SIF2_INTRINSIC_NO_SHIELDS)))
    ...
    if ((p_objp->flags & P_OF_PLAYER_START) && (Fred_running || ((Game_mode & GM_MULTIPLAYER) && !(Game_mode &
                                                                                                   GM_IN_MISSION))))
...
#else
                                                                                                                            if (Fred_running && (shipp->wingnum != -1) && (Wings[shipp->wingnum].flags & WF_NO_ARRIVAL_LOG))
		shipp->flags2 |= SF2_NO_ARRIVAL_LOG;

	if (Fred_running && (shipp->wingnum != -1) && (Wings[shipp->wingnum].flags & WF_NO_DEPARTURE_LOG))
		shipp->flags2 |= SF2_NO_DEPARTURE_LOG;
#endif
    // ditto for Kazan
        ...
        if (!stricmp(sssp->name, NOX("Pilot"))) {
            ...
            for (j = 0; j < wp->num_primary_banks; j++) {
                if (Fred_running) {
                    ...
                    wp->primary_bank_ammo[j] = fl2i(capacity / Weapon_info[wp->primary_bank_weapons[j]].cargo_size +
                                                    0.5f);
                }
            }
            ...
            for (j = 0; j < wp->num_secondary_banks; j++) {
                if (Fred_running) {
                    ...
                    int capacity = fl2i(sssp->secondary_ammo[j] / 100.0f * sip->secondary_bank_ammo_capacity[j] +
                                        0.5f);
                    wp->secondary_bank_ammo[j] = fl2i(capacity / Weapon_info[wp->secondary_bank_weapons[j]].
                            cargo_size + 0.5f);
                }
            }
        ...
        while (ptr != END_OF_LIST(&shipp->subsys_list)) {
            ...
            if (!subsystem_stricmp(ptr->system_info->subobj_name, sssp->name)) {
                if (Fred_running) {
                    ...
                    ptr->max_hits = ptr->system_info->max_subsys_strength * (shipp->ship_max_hull_strength /
    ...
    if (Fred_running) {
        ...
        for (iLoop = 0; iLoop < MAX_SHIELD_SECTIONS; iLoop++) {
            Objects[objnum].shield_quadrant[iLoop] = (float) (p_objp->initial_shields *
                                                              get_max_shield_quad(&Objects[objnum]) / 100.0f);
        }
    ...
    if ((Game_mode & GM_IN_MISSION) && (!Fred_running)) {
        ...
        if (!Game_restoring) {
            ...
            if (shipp->wingnum == -1) {
                ...
                mission_set_arrival_location(p_objp->arrival_anchor, location, p_objp->arrival_distance, objnum,
                                             p_objp->arrival_path_mask, NULL, NULL);
        ...
        if ((shipp->wingnum == -1) && (shipp->hotkey != -1))
            mission_hotkey_mf_add(shipp->hotkey, shipp->objnum, HOTKEY_LATE_ARRIVAL);
        else if ((shipp->wingnum != -1) && (Wings[shipp->wingnum].hotkey != -1))
            mission_hotkey_mf_add(Wings[shipp->wingnum].hotkey, shipp->objnum, HOTKEY_LATE_ARRIVAL);
...
void resolve_parse_flags(object *objp, int parse_flags, int parse_flags2) {
...
#else
                                                                                                                            if (Fred_running && (parse_flags2 & P2_SF2_HIDE_SHIP_NAME))
		shipp->flags2 |= SF2_HIDE_SHIP_NAME;
#endif

    if (parse_flags2 & P2_SF2_IS_HARMLESS)
...
#ifndef _DEBUG
    if (parse_flags2 & P2_SF2_HIDE_LOG_ENTRIES)
    ...
    if (parse_flags2 & P2_SF2_NO_DEPARTURE_LOG)
        shipp->flags2 |= SF2_NO_DEPARTURE_LOG;
#else
                                                                                                                            if (Fred_running && (parse_flags2 & P2_SF2_HIDE_LOG_ENTRIES))
		shipp->flags2 |= SF2_HIDE_LOG_ENTRIES;

	if (Fred_running && (parse_flags2 & P2_SF2_NO_ARRIVAL_LOG))
		shipp->flags2 |= SF2_NO_ARRIVAL_LOG;

	if (Fred_running && (parse_flags2 & P2_SF2_NO_DEPARTURE_LOG))
		shipp->flags2 |= SF2_NO_DEPARTURE_LOG;
#endif
}
...
void fix_old_special_explosions(p_object *p_objp, int variable_index) {
    ...
    Assertion(!(p_objp->use_special_explosion),
              "Mission appears to be using both the new and old method of special explosions for %s. Old method values used",
              p_objp->name);
    ...
    for (i = variable_index; i < (variable_index + BLOCK_EXP_SIZE); i++) {
        if (!(Block_variables[i].type & SEXP_VARIABLE_BLOCK)) {
            Warning(LOCATION,
                    "%s is using the old special explosions method but does not appear to have variables for all the values",
                    p_objp->name);
            return;
        }
    }
...
void fix_old_special_hits(p_object *p_objp, int variable_index) {
    ...
    Assertion(((p_objp->special_hitpoints == 0) && (p_objp->special_shield == -1)),
              "Mission appears to be using both the new and old method of special hitpoints for %s", p_objp->name);

    // check all the variables are valid
    for (i = variable_index; i < (variable_index + BLOCK_HIT_SIZE); i++) {
        if (!(Block_variables[i].type & SEXP_VARIABLE_BLOCK)) {
            Warning(LOCATION,
                    "%s is using the old special hitpoints method but does not appear to have variables for all the values",
                    p_objp->name);
            return;
        }
    }
...
int parse_object(mission *pm, int flag, p_object *p_objp) {
    ...
    while (optional_string("$Alt Ship Class:")) {
        ...
        if (new_alt_class.ship_class < 0) {
            if (!Fred_running) {
                Warning(LOCATION,
                        "Ship \"%s\" has an invalid Alternate Ship Class type (ships.tbl probably changed). Skipping this entry",
                        p_objp->name);
                continue;
            } else {
                ...
                if (new_alt_class.variable_index != -1) {
                    ...
                    Warning(LOCATION, "Ship \"%s\" has an invalid Alternate Ship Class type. Skipping this entry",
                            p_objp->name);
                    continue;
                }
            }
    ...
    if ((Game_mode & GM_MULTIPLAYER) && (Netgame.type_flags &
                                         NG_TYPE_DOGFIGHT) && (Ship_info[p_objp->ship_class].flags & SIF_SUPPORT))
        return 0;
    ...
    while (required_string_either("$Arrival Location:", "$Status Description:")) {
        ...
        find_and_stuff("$Status Description:", &p_objp->status_type[count], F_NAME, Status_desc_names,
                       Num_status_names, "Status Description");
    ...
    find_and_stuff("$Arrival Location:", &p_objp->arrival_location, F_NAME, Arrival_location_names, Num_arrival_names,
                   "Arrival Location");
    ...
    find_and_stuff("$Departure Location:", &p_objp->departure_location, F_NAME, Departure_location_names,
                   Num_arrival_names, "Departure Location");
    ...
    Assert(p_objp->ship_max_hull_strength >
    ...
    if (optional_string("$Texture Replace:") || optional_string("$Duplicate Model Texture Replace:")) {
        ...
        while ((p_objp->num_texture_replacements < MAX_REPLACEMENT_TEXTURES) && (optional_string("+old:"))) {
            stuff_string(p_objp->replacement_textures[p_objp->num_texture_replacements].old_texture, F_NAME,
                         MAX_FILENAME_LEN);
            required_string("+new:");
            stuff_string(p_objp->replacement_textures[p_objp->num_texture_replacements].new_texture, F_NAME,
                         MAX_FILENAME_LEN);
            ...
            // load the texture
            p_objp->replacement_textures[p_objp->num_texture_replacements].new_texture_id = bm_load_either(p_objp->
                    replacement_textures[p_objp->num_texture_replacements].new_texture);
            ...
            if (Fred_running) {
                ...
                strcpy_s(Fred_texture_replacements[Fred_num_texture_replacements].old_texture,
                         p_objp->replacement_textures[p_objp->num_texture_replacements].old_texture);
                strcpy_s(Fred_texture_replacements[Fred_num_texture_replacements].new_texture,
                         p_objp->replacement_textures[p_objp->num_texture_replacements].new_texture);
...
void mission_parse_handle_late_arrivals(p_object *p_objp) {
    ...
    ship_info *sip = &Ship_info[p_objp->ship_class];
...
void mission_parse_maybe_create_parse_object(p_object *pobjp) {
    ...
    if ((object_is_docked(pobjp) && !(pobjp->flags & P_SF_DOCK_LEADER)) || (!Fred_running &&
                                                                            (!eval_sexp(pobjp->arrival_cue) ||
                                                                             !timestamp_elapsed(pobjp->
                                                                                     arrival_delay) || (pobjp->flags &
                                                                                                        P_SF_REINFORCEMENT)))) {
        ...
        int real_objnum = parse_create_object(
                pobjp);    // this object may later get destroyed depending on wing status!!!!
        ...
        if (pobjp->destroy_before_mission_time >= 0) {
            ...
            if (!Fred_running) {
                int i;
                shipfx_blow_up_model(objp, Ship_info[Ships[objp->instance].ship_info_index].model_num, 0,
                                     0, &objp->pos);
                ...
                for (i = 0; i < MAX_DEBRIS_PIECES; i++) {
                    ...
                    physics_sim(&objp->pos, &objp->orient, &objp->phys_info, (float) pobjp->
    ...
    while (optional_string("+Subsystem:")) {
        ...
        if (stricmp("Pilot", Subsys_status[i].name)) {
            ...
            if (j == sip->n_subsystems)
                Warning(LOCATION, "Ship \"%s\", class \"%s\"\nUnknown subsystem \"%s\" found in mission!", objp->name,
                        sip->name, Subsys_status[i].name);
        }
        ...
        if (optional_string("+Cargo Name:")) {
            ...
            if (index == -1) {
                if (Num_cargo < MAX_CARGO) {
                    ...
                    WarningEx(LOCATION, "Maximum number of cargo names (%d) exceeded, defaulting to Nothing!",
                              MAX_CARGO);
                    index = 0;
                }
            }
...
int parse_wing_create_ships(wing *wingp, int num_to_create, int force, int specific_instance) {
    ...
    if (!force) {
        ...
        if (wingp->arrival_location == ARRIVE_FROM_DOCK_BAY) {
            ...
            int shipnum = ship_name_lookup(name);

            if (is_mothership_gone(wingp)) {
                // if the current wave is zero, it never existed
                if (wingp->current_wave == 0) {
                    mprintf(("The mothership '%s' of wing '%s' is gone. Marking wing as NEVER_EXISTED.\n", name, wingp->name));
                ...
                int num_remaining = ((wingp->num_waves - wingp->current_wave) * wingp->wave_count);
                ...
                maybe_handle_destroyed_or_departed(wingp, Ships[shipnum].team);

                mission_parse_mark_non_arrival(wingp);    // Goober5000
                return 0;
            }

            // Goober5000 - check status of fighter bays - if they're destroyed, we can't launch - but we want to re-eval later
        ...
        if (!timestamp_valid(wingp->wave_delay_timestamp)) {
            ...
            if ((wingp->wave_delay_min > 0) || (wingp->wave_delay_max > 0)) {
                ...
                time_to_arrive = wingp->wave_delay_min + (int) (frand() * (wingp->wave_delay_max -
    ...
    for (i = 0; i < (int) Parse_objects.size(); i++) {
        ...
        wingp->current_count++;
        mprintf(("Increased current ship count in wing '%s' to %i\n", wingp->name, wingp->current_count));
    ...
    if ((objnum != -1) && (Game_mode & GM_IN_MISSION)) {        // if true, we have created at least one new ship.
        ...
        for (int i = 0; i < wingp->current_count; i++) {
            ship_num = wingp->ship_index[i];
            bool ship_type_harmless = (Ship_info[Ships[ship_num].ship_info_index].flags & SIF_HARMLESS) != 0;
            bool ship_harmless = (Ships[ship_num].flags2 & SF2_IS_HARMLESS) != 0;
            bool is_harmless = (ship_type_harmless || ship_harmless);
            all_harmless &= is_harmless;
        }

        bool wing_has_no_arrival_music_flag = (wingp->flags & WF_NO_ARRIVAL_MUSIC) != 0;

        if (!all_harmless && !wing_has_no_arrival_music_flag) {
            if (timestamp_elapsed(Allow_arrival_music_timestamp)) {
                Allow_arrival_music_timestamp = timestamp(ARRIVAL_MUSIC_MIN_SEPARATION);
                event_music_arrival(Ships[wingp->ship_index[0]].team);
            }
        }
...
void parse_wing(mission *pm) {
    ...
    find_and_stuff("$Arrival Location:", &wingp->arrival_location, F_NAME, Arrival_location_names, Num_arrival_names,
                   "Arrival Location");
    ...
    find_and_stuff("$Departure Location:", &wingp->departure_location, F_NAME, Departure_location_names,
                   Num_arrival_names, "Departure Location");
    ...
    if (wing_goals != -1) {
        ...
        // already created.
        for (int sexp = CDR(wing_goals); sexp != -1; sexp = CDR(sexp)) {
...
void parse_messages(mission *pm, int flags) {
    ...
    if (optional_string("$Command Persona:")) {
        ...
        if (idx >= 0)
            ...
            Warning(LOCATION, "Supplied Command Persona is invalid!  Defaulting to %s.",
...
void parse_reinforcement(mission *pm) {
    ...
    find_and_stuff("$Type:", &ptr->type, F_NAME, Reinforcement_type_names, Num_reinforcement_type_names,
                   "reinforcement type");
...
void parse_variables() {
    ...
    if (!Fred_running && (Game_mode & GM_CAMPAIGN_MODE)) {
        ...
        for (int i = 0; i < Campaign.num_missions; i++) {
            ...
            for (int j = 0; j < Campaign.missions[i].num_saved_variables; j++) {
                ...
                for (int k = 0; k < num_variables; k++) {
                    ...
                    if (!(stricmp(Sexp_variables[k].variable_name,
                                  Campaign.missions[i].saved_variables[j].variable_name))) {
        ...
        for (int i = 0; i < Player->num_variables; i++) {
            // loop through the current mission's variables
...
int parse_mission(mission *pm, int flags) {
    ...
    *Player_start_shipname = 0;        // make the string 0 length for checking later
        ...
        if (Game_mode & GM_STANDALONE_SERVER) {
            ...
            if (Num_unknown_ship_classes > 0) {
                sprintf(text,
                        ...
                        (Game_mode & GM_CAMPAIGN_MODE) ? "campaign" : "mission");
            } else {
                sprintf(text,
                        ...
                        Num_unknown_loadout_classes, (Num_unknown_loadout_classes > 1) ? "es" : "",
                        (Game_mode & GM_CAMPAIGN_MODE) ? "campaign" : "mission");
            }
    ...
    if ((saved_warning_count - Global_warning_count) > 10 || (saved_error_count - Global_error_count) > 0) {
        char text[512];
        sprintf(text,
                "Warning!\n\nThe current mission has generated %d warnings and/or errors during load.  These are usually caused by corrupted ship models or syntax errors in the mission file.  While Wing Commander Saga will attempt to compensate for these issues, it cannot guarantee a trouble-free gameplay experience.  Source Code Project staff cannot provide assistance or support for these problems, as they are caused by the mission's data files, not Wing Commander Saga's source code.",
                (saved_warning_count - Global_warning_count) + (saved_error_count - Global_error_count));
...
void post_process_mission() {
    ...
    if (Fred_running) {
        ...
        for (i = 0; i < MAX_SHIPS; i++) {
            if ((Ships[i].objnum >= 0) && (Ships[i].arrival_anchor >= 0) && (Ships[i].arrival_anchor <
                                                                             SPECIAL_ARRIVAL_ANCHOR_FLAG))
        ...
        for (i = 0; i < MAX_WINGS; i++) {
            if (Wings[i].wave_count && (Wings[i].arrival_anchor >= 0) && (Wings[i].arrival_anchor <
                                                                          SPECIAL_ARRIVAL_ANCHOR_FLAG))
    ...
    for (so = GET_FIRST(&Ship_obj_list); so != END_OF_LIST(&Ship_obj_list); so = GET_NEXT(so)) {
        ...
        ship_add_ship_type_count(Ships[shipnum].ship_info_index, 1);
    }
    ...
    for (p_object *p_objp = GET_FIRST(&Ship_arrival_list); p_objp != END_OF_LIST(&Ship_arrival_list);
        ...
        int num = (p_objp->wingnum == -1) ?
                  1 :
                  Wings[p_objp->wingnum].num_waves - 1; // subtract one since we already counted the first wave
...
int get_mission_info(char *filename, mission *mission_p, bool basic) {
    ...
    do {
        CFILE *ftemp = cfopen(real_fname, "rt", CFILE_NORMAL, CF_TYPE_MISSIONS);
        if (!ftemp) {
            rval = -1;
            break;
        }
...
void mission_set_wing_arrival_location(wing *wingp, int num_to_set) {
    ...
    if ((wingp->arrival_location == ARRIVE_FROM_DOCK_BAY) || (wingp->arrival_location == ARRIVE_AT_LOCATION)) {
        while (index < wingp->current_count) {
            ...
            mission_set_arrival_location(wingp->arrival_anchor, wingp->arrival_location, wingp->arrival_distance,
                                         OBJ_INDEX(objp), wingp->arrival_path_mask, NULL, NULL);
        ...
        if (mission_set_arrival_location(wingp->arrival_anchor, wingp->arrival_location, wingp->arrival_distance,
                                         OBJ_INDEX(leader_objp), wingp->arrival_path_mask, &pos, &orient)) {
...
void mission_parse_set_arrival_locations() {
    ...
    for (objp = GET_FIRST(&obj_used_list); objp != END_OF_LIST(&obj_used_list); objp = GET_NEXT(objp)) {
        ...
        mission_set_arrival_location(shipp->arrival_anchor, shipp->arrival_location, shipp->arrival_distance,
                                     OBJ_INDEX(objp), shipp->arrival_path_mask, NULL, NULL);
    }
...
void mission_parse_set_up_initial_docks() {
    ...
    for (i = 0; i < (int) Parse_objects.size(); i++) {
        ...
        if (dfi.maintained_variables.int_value == 0) {
            ...
            Warning(LOCATION,
                    "There are multiple dock leaders in the docking group containing the leader %s!  Setting %s as the sole leader...\n",
...
int mission_set_arrival_location(int anchor, int location, int dist, int objnum, int path_mask, vec3d *new_pos,
                                 matrix *new_orient) {
    ...
    if (anchor & SPECIAL_ARRIVAL_ANCHOR_FLAG) {
        ...
        // get ship
        shipnum = ship_get_random_team_ship(iff_get_mask(iff_index),
                                            get_players ? SHIP_GET_ONLY_PLAYERS : SHIP_GET_ANY_SHIP);
    }
...
int mission_did_ship_arrive(p_object *objp) {
    ...
    if (should_arrive) {        // has the arrival criteria been met?
        ...
        bool no_arrival_music_flag = (Ships[Objects[object_num].instance].flags & SF_NO_ARRIVAL_MUSIC) != 0;
        bool ship_type_is_harmless = (Ship_info[Ships[Objects[object_num].instance].ship_info_index].flags &
                                      SIF_HARMLESS) != 0;
        bool ship_is_harmless = (Ships[Objects[object_num].instance].flags2 & SF2_IS_HARMLESS) != 0;

        if (!no_arrival_music_flag && !ship_type_is_harmless && !ship_is_harmless) {
            if (timestamp_elapsed(Allow_arrival_music_timestamp)) {
                Allow_arrival_music_timestamp = timestamp(ARRIVAL_MUSIC_MIN_SEPARATION);
...
void mission_eval_arrivals() {
    ...
    if (timestamp_valid(Arrival_message_delay_timestamp) && timestamp_elapsed(Arrival_message_delay_timestamp) &&
        !((Game_mode & GM_MULTIPLAYER) && (Netgame.type_flags & NG_TYPE_TEAM))) {
        ...
        if ((rship < 0) || use_terran_cmd)
            message_send_builtin_to_player(MESSAGE_ARRIVE_ENEMY, NULL, MESSAGE_PRIORITY_LOW, MESSAGE_TIME_SOON, 0,
                                           0, -1, -1);
        else if (rship >= 0)
            message_send_builtin_to_player(MESSAGE_ARRIVE_ENEMY, &Ships[rship], MESSAGE_PRIORITY_LOW,
                                           MESSAGE_TIME_SOON, 0, 0, -1, -1);
    ...
    for (i = 0; i < Num_wings; i++) {
        ...
        if (wingp->flags & WF_NEVER_EXISTED)
            continue;
            ...
            if ((Game_mode & GM_MULTIPLAYER) && (Netgame.type_flags & NG_TYPE_TEAM)) {
                ...
                if (timestamp_elapsed(Allow_arrival_message_timestamp_m[multi_team_filter])) {
                    if (!timestamp_valid(Arrival_message_delay_timestamp_m[multi_team_filter])) {
                        Arrival_message_delay_timestamp_m[multi_team_filter] =
                                timestamp_rand(ARRIVAL_MESSAGE_DELAY_MIN, ARRIVAL_MESSAGE_DELAY_MAX);
                    }
                    Allow_arrival_message_timestamp_m[multi_team_filter] = timestamp(ARRIVAL_MESSAGE_MIN_SEPARATION);

                    // send to the proper team
                    message_send_builtin_to_player(MESSAGE_ARRIVE_ENEMY, NULL, MESSAGE_PRIORITY_LOW, MESSAGE_TIME_SOON,
                                                   0, 0, -1, multi_team_filter);
                }
            }
                ...
                if (timestamp_elapsed(Allow_arrival_message_timestamp)) {
                    if (!timestamp_valid(Arrival_message_delay_timestamp)) {
                        Arrival_message_delay_timestamp = timestamp_rand(ARRIVAL_MESSAGE_DELAY_MIN,
                                                                         ARRIVAL_MESSAGE_DELAY_MAX);
                    }
                ...
                if (rship >= 0) {
                    ...
                    for (j = 0; j < MAX_BUILTIN_MESSAGE_TYPES; j++) {
                        if (!stricmp(message_name, Builtin_message_types[j])) {
                            message_send_builtin_to_player(j, &Ships[rship], MESSAGE_PRIORITY_LOW, MESSAGE_TIME_SOON,
                                                           0, 0, -1, -1);
                            break;
                        }
                    }
...
int mission_do_departure(object *objp) {
    ...
    if ((shipp->flags2 & SF2_NO_SUBSPACE_DRIVE) && (shipp->departure_location == DEPART_AT_LOCATION) &&
        (shipp->wingnum >= 0)) {
...
void mission_eval_departures() {
    ...
    for (objp = GET_FIRST(&obj_used_list); objp != END_OF_LIST(&obj_used_list); objp = GET_NEXT(objp)) {
        if (objp->type == OBJ_SHIP) {
            ...
            if ((shipp->flags & (SF_DEPARTING | SF_DYING)) || ((shipp->flags &
                                                                SF_CANNOT_WARP) &&
...
void copy_subsys_status_to_end(p_object *pobjp) {
    int new_index = Subsys_index;

    for (int i = 0; i < pobjp->subsys_count; i++) {
        int index = allocate_subsys_status();

        Subsys_status[index] = Subsys_status[pobjp->subsys_index + i];
    }

    pobjp->subsys_index = new_index;
}
...
void mission_parse_fixup_players() {
    ...
    for (objp = GET_FIRST(&obj_used_list); objp != END_OF_LIST(&obj_used_list); objp = GET_NEXT(objp)) {
        if ((objp->type == OBJ_SHIP) && (objp->flags & OF_PLAYER_SHIP)) {
            game_busy(
                    NOX("** fixing up player/ai stuff **"));    // animate the loading screen, doesn't nothing if the screen is not active
...
extern int pp_collide_any(vec3d *curpos, vec3d *goalpos, float radius, object *ignore_objp1, object *ignore_objp2,
                          int big_only_flag);
            ...
            for (ss = GET_FIRST(&Ships[i].subsys_list); ss != END_OF_LIST(&Ships[i].subsys_list); ss = GET_NEXT(ss)) {
                ...
                if (ss->system_info->type == SUBSYSTEM_TURRET) {
                    ...
                    if (Mp < strstr(subsys + 1, "$Subsystem")) {
                        Mp = strchr(Mp, '(');
                        restore_one_primary_bank(ss->weapons.primary_bank_weapons,
                                                 sip->subsystems[si_subsys].primary_banks);
                    }
                    ...
                    if (Mp < strstr(subsys + 1, "$Subsystem")) {
                        Mp = strchr(Mp, '(');
                        restore_one_secondary_bank(ss->weapons.secondary_bank_weapons,
                                                   sip->subsystems[si_subsys].secondary_banks);
                    }
                }
            }
        }

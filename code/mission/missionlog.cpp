...
void mission_log_obsolete_entries(int type, char* pname)
{
	...
	if (type == LOG_SHIP_DESTROYED)
	{
		for (i = 0; i < last_entry; i++)
		{
			...
			{
				if ((entry->type == LOG_SHIP_SUBSYS_DESTROYED) || (entry->type == LOG_SHIP_DISARMED) ||
...
void mission_log_add_entry(int type, char* pname, char* sname, int info_index)
{
	...
	if (pname)
	{
		...
		strcpy_s(entry->pdisplayname, pname);
		...
		if (index >= 0)
		{
			...
			{
				hud_stuff_ship_callsign(entry->pdisplayname, shipp);
			}
			else if (((Iff_info[shipp->team].flags & IFFF_WING_NAME_HIDDEN) && (shipp->wingnum != -1)) ||
					 (shipp->flags2 & SF2_HIDE_SHIP_NAME))
			{
				hud_stuff_ship_class(entry->pdisplayname, shipp);
			}
			else
			{
				end_string_at_first_hash_symbol(entry->pdisplayname, (shipp->wingnum != -1));
			}
		}
	}
	else
	{
		...
		strcpy_s(entry->pdisplayname, EMPTY_LOG_NAME);
	}
	...
	if (sname)
	{
		...
		strcpy_s(entry->sdisplayname, sname);
		...
		if (index >= 0)
		{
			...
			if ((shipp->flags2 & SF2_HIDE_LOG_ENTRIES) != 0)
			{
				entry->flags |= MLF_SECONDARY_HIDDEN;
			}

			if (shipp->callsign_index >= 0)
			{
				hud_stuff_ship_callsign(entry->sdisplayname, shipp);
			}
			else if (((Iff_info[shipp->team].flags & IFFF_WING_NAME_HIDDEN) && (shipp->wingnum != -1)) ||
					 (shipp->flags2 & SF2_HIDE_SHIP_NAME))
			{
				hud_stuff_ship_class(entry->sdisplayname, shipp);
			}
			else
			{
				end_string_at_first_hash_symbol(entry->sdisplayname, (shipp->wingnum != -1));
			}
		}
	}
	else
	{
		...
		strcpy_s(entry->sdisplayname, EMPTY_LOG_NAME);
	}
	...
	case LOG_SELF_DESTRUCTED:
		...
		if (((type == LOG_SHIP_ARRIVED) && ((Ships[index].flags2 & SF2_NO_ARRIVAL_LOG) != 0)) ||
		...
		else if ((type == LOG_SHIP_SUBSYS_DESTROYED) && (Ship_info[Ships[index].ship_info_index].flags &
														 SIF_SMALL_SHIP))
		{
	...
	case LOG_WING_ARRIVED:
		...
		{
			int si = Wings[index].ship_index[0];
			Assert(si != -1);

			char buf[NAME_LENGTH + 5];
			entry->primary_team = Ships[si].team;
			hud_stuff_ship_class(buf, &Ships[si]);
			strcat(buf, " Wing");
			strcpy(entry->pdisplayname, buf);
		}
		else
		{
			entry->primary_team = info_index;

			if ((Iff_info[entry->primary_team].flags & IFFF_WING_NAME_HIDDEN) != 0)
			{
				entry->flags |= MLF_HIDDEN;
			}
		}
	...
	for (i = 0; i < last_entry; i++, entry++)
	{
		...
		if (entry->type == type)
		{
			...
			if ((type == LOG_SHIP_DOCKED) || (type == LOG_SHIP_UNDOCKED))
			{
				...
				if ((!stricmp(entry->pname, pname) && !stricmp(entry->sname,
					 sname)) || (!stricmp(entry->pname, sname) && !stricmp(entry->sname, pname)))
				{
					found = 1;
				}
			}
...
void message_log_init_scrollback(int pw)
{
	...
	for (i = 0; i < last_entry; i++)
	{
		...
		{
			message_log_add_seg(Num_log_lines, OBJECT_X, c, entry->pdisplayname);
		}
		...
		case LOG_SHIP_DESTROYED:
			...
			if (strlen(entry->sdisplayname) && !(entry->flags & MLF_SECONDARY_HIDDEN))
			{
				message_log_add_segs(XSTR("  Kill: ", 405), LOG_COLOR_NORMAL);
				message_log_add_segs(entry->sdisplayname, c);
				if (entry->index >= 0)
				{
		...
		case LOG_SELF_DESTRUCTED:
			message_log_add_segs(XSTR("Destroyed", 1476), LOG_COLOR_NORMAL);
			break;
		...
		case LOG_SHIP_DOCKED:
			if ((entry->flags & MLF_SECONDARY_HIDDEN) != 0)
			{
				message_log_add_segs(XSTR("Docked", -1), LOG_COLOR_NORMAL);
			}
			else
			{
		...
		case LOG_SHIP_UNDOCKED:
			if ((entry->flags & MLF_SECONDARY_HIDDEN) != 0)
			{
				message_log_add_segs(XSTR("Undocked", -1), LOG_COLOR_NORMAL);
			}
			else
			{
		...
		case LOG_CAP_SUBSYS_CARGO_REVEALED:
			...
			if ((entry->flags & MLF_SECONDARY_HIDDEN) != 0)
			{
				message_log_add_segs(XSTR("target", -1), LOG_COLOR_NORMAL);
			}
			else
			{
				message_log_add_segs(entry->sdisplayname, LOG_COLOR_NORMAL);
			}
		...
		case LOG_GOAL_FAILED:
			{
				...
				message_log_add_segs(text,
					LOG_COLOR_BRIGHT, (entry->type == LOG_GOAL_SATISFIED?LOG_FLAG_GOAL_TRUE:LOG_FLAG_GOAL_FAILED));
				break;

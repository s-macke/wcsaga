...
static barracks_buttons Buttons[GR_NUM_RESOLUTIONS][BARRACKS_NUM_BUTTONS] =
{
	...
	{		// GR_1024
		...
		barracks_buttons("2_BAB_05", 914, 681, 949, 671, 5), // "Accept"
		barracks_buttons("2_BAB_06", 854, 681, 822, 704, 6), // "Help"
		barracks_buttons("2_BAB_07", 854, 724, 800, 743, 7), // "Options"
		barracks_buttons("2_BAB_08", 579, 681, 644, 743, 8),
		barracks_buttons("2_BAB_09", 0, 428, 0, 0, 9, 1),
		barracks_buttons("2_BAB_10", 0, 360, 0, 0, 10, 1),
		barracks_buttons("2_BAB_11", 196, 198, 214, 252, 11),
		barracks_buttons("2_BAB_12", 602, 200, 617, 187, 12),
		barracks_buttons("2_BAB_13", 107, 196, 123, 252, 13),
		barracks_buttons("2_BAB_14", 517, 0, 532, 40, 14),
		barracks_buttons("2_BAB_15", 596, 0, 614, 40, 15),
		barracks_buttons("2_BAB_16", 289, 196, 299, 252, 16),
...
namespace
{
typedef std::pair<std::string, int> Kill_pair;

class KillClassEqual
{
public:
	KillClassEqual(const std::string& ship_class_)
		: ship_class(ship_class_) {}

	bool operator()(const Kill_pair& lhs)
	{
		return lhs.first == ship_class;
	}
private:
...
void barracks_init_stats(scoring_struct* stats)
{
	...
	for (i = 0; i < Num_ship_classes; i++)
	{
		...
		strcpy(buffer, (Ship_info[i].alt_name[0]) ? Ship_info[i].alt_name : Ship_info[i].name);
		end_string_at_first_hash_symbol(buffer);
		std::string label(buffer);

		std::vector<Kill_pair>::iterator kill_pair = std::find_if(kills.begin(), kills.end(), KillClassEqual(label));

		if (kill_pair == kills.end())
		{
			kills.push_back(std::make_pair(label, stats->kills[i]));
		}
		else
		{
			kill_pair->second += stats->kills[i];
		}
	}

	for (std::vector<Kill_pair>::const_iterator kill = kills.begin(); kill != kills.end(); ++kill)
	{
		if (kill->second)
		{
			Assert(Num_stat_lines < Max_stat_lines);
			...
			Assert(strlen(kill->first.c_str()) + 1 < STAT_COLUMN1_W);
			sprintf(Stat_labels[Num_stat_lines], NOX("%s:"), kill->first.c_str());
			sprintf(Stats[Num_stat_lines], "%d", kill->second);
			Num_stat_lines++;
		}
	}
	...
	{
		gr_force_fit_string(Stat_labels[i], Stat_column1_w[gr_screen.res],
			Barracks_stats_coords[gr_screen.res][BARRACKS_W_COORD]);
		gr_force_fit_string(Stats[i], Stat_column2_w[gr_screen.res],
			Barracks_stats2_coords[gr_screen.res][BARRACKS_W_COORD]);
	}
}
...
void barracks_create_new_pilot()
{
	...
	Inputbox.update_dimensions(Barracks_list_coords[gr_screen.res][BARRACKS_X_COORD],
		Barracks_list_coords[gr_screen.res][BARRACKS_Y_COORD], Barracks_list_coords[gr_screen.res][BARRACKS_W_COORD],
		gr_get_font_height());
}
...
void barracks_delete_pilot()
{
	...
	int popup_rval = popup(PF_TITLE_BIG | PF_TITLE_RED, 2, POPUP_NO, POPUP_YES,
	...
	if (!del_rval)
	{
		popup(PF_USE_AFFIRMATIVE_ICON | PF_TITLE_BIG | PF_TITLE_RED, 1, POPUP_OK,
...
{
	// show a popup
	popup(PF_USE_AFFIRMATIVE_ICON | PF_NO_NETWORKING, 1, POPUP_OK,
		XSTR("You cannot change your squadron in Single Player mode.", 1445));
}
...
void barracks_init_player_stuff(int mode)
{
	...
	if (mode == PLAYER_SELECT_MODE_SINGLE)
	{
		...
		Num_pilots = cf_get_file_list_preallocated(MAX_PILOTS, Pilots_arr, Pilots, CF_TYPE_SINGLE_PLAYERS,
			NOX("*.pl2"), CF_SORT_TIME);
		old_pilot_num = cf_get_file_list_preallocated(MAX_PILOTS, old_pilots_arr, old_pilots, CF_TYPE_SINGLE_PLAYERS,
			NOX("*.plr"), CF_SORT_TIME);
	...
	else
	{
		...
		Num_pilots = cf_get_file_list_preallocated(MAX_PILOTS, Pilots_arr, Pilots, CF_TYPE_MULTI_PLAYERS, NOX("*.plr"),
			CF_SORT_TIME);
		...
		else
		{
			...
			if (Campaign_file_missing)
			{
				popup(PF_USE_AFFIRMATIVE_ICON, 1, POPUP_OK,
					XSTR("The currently active campaign cannot be found.  Please select another...", -1));
...
void barracks_display_pilot_callsigns(int prospective_pilot)
{
	...
	while (y + font_height <= Barracks_list_coords[gr_screen.res][BARRACKS_H_COORD])
	{
		...
		gr_printf(Barracks_list_coords[gr_screen.res][BARRACKS_X_COORD],
			Barracks_list_coords[gr_screen.res][BARRACKS_Y_COORD] + y, Pilots[cur_pilot_idx]);
...
void barracks_display_pilot_stats()
{
	...
	while (y + font_height <= Barracks_stats_coords[gr_screen.res][BARRACKS_H_COORD])
	{
		...
		if (*str == '*')
		{
			...
			gr_line(Barracks_stats_coords[gr_screen.res][BARRACKS_X_COORD], i,
				Barracks_stats_coords[gr_screen.res][BARRACKS_X_COORD] + Barracks_stats_coords[gr_screen.
				res][BARRACKS_W_COORD] - w - 2, i);
			gr_line(Barracks_stats_coords[gr_screen.res][BARRACKS_X_COORD] + Barracks_stats_coords[gr_screen.
				res][BARRACKS_W_COORD] + 1, i, Barracks_stats2_coords[gr_screen.res][BARRACKS_X_COORD] +
				Barracks_stats2_coords[gr_screen.res][BARRACKS_W_COORD], i);
		...
		gr_printf(Barracks_stats_coords[gr_screen.res][BARRACKS_X_COORD] + Barracks_stats_coords[gr_screen.
			res][BARRACKS_W_COORD] - w, Barracks_stats_coords[gr_screen.res][BARRACKS_Y_COORD] + y, "%s", str);
		str = Stats[z];
		if (*str)
		{
			gr_printf(Barracks_stats2_coords[gr_screen.res][BARRACKS_X_COORD],
				Barracks_stats_coords[gr_screen.res][BARRACKS_Y_COORD] + y, "%s", str);
		}
...
void barracks_draw_pilot_pic()
{
	...
	if (Cur_pilot->callsign[0] && (Pic_number >= 0) && (Pic_number < Num_pilot_images))
	{
		if (Pilot_images[Pic_number] >= 0)
		{
			...
			gr_bitmap(Barracks_image_coords[gr_screen.res][BARRACKS_X_COORD],
				Barracks_image_coords[gr_screen.res][BARRACKS_Y_COORD]);
			Palman_allow_any_color = 0;

			// print number of the current pic
			char buf[40];
			sprintf(buf, XSTR("%d of %d", 71), Pic_number + 1, Num_pilot_images);
			gr_printf(Barracks_image_number_coords[gr_screen.res][BARRACKS_X_COORD],
				Barracks_image_number_coords[gr_screen.res][BARRACKS_Y_COORD], buf);
		}
	}
	else
	{
...
void barracks_draw_squad_pic()
{
	...
	if (Cur_pilot->callsign[0] && (Pic_squad_number >= 0) && (Pic_squad_number < Num_pilot_squad_images))
	{
		if (Pilot_squad_images[Pic_squad_number] >= 0)
		{
			...
			gr_bitmap(Barracks_squad_coords[gr_screen.res][BARRACKS_X_COORD],
				Barracks_squad_coords[gr_screen.res][BARRACKS_Y_COORD]);
			Palman_allow_any_color = 0;
			...
			if (Player_sel_mode != PLAYER_SELECT_MODE_SINGLE)
			{
				...
				gr_printf(Barracks_squad_number_coords[gr_screen.res][BARRACKS_X_COORD],
					Barracks_squad_number_coords[gr_screen.res][BARRACKS_Y_COORD], buf);
			}
		}
	}
	else
...
void barracks_init()
{
	...
	{
		// create the object
		Buttons[gr_screen.res][i].button.create(&Ui_window, "", Buttons[gr_screen.res][i].x,
			Buttons[gr_screen.res][i].y, 60, 30, Buttons[gr_screen.res][i].repeat, 1);
	...
	// add all strings	
	w->add_XSTR("Create", 1034, Buttons[gr_screen.res][0].text_x, Buttons[gr_screen.res][0].text_y, &Buttons[gr_screen.
		res][0].button, UI_XSTR_COLOR_GREEN);
	w->add_XSTR("Accept", 1035, Buttons[gr_screen.res][5].text_x, Buttons[gr_screen.res][5].text_y, &Buttons[gr_screen.
		res][5].button, UI_XSTR_COLOR_PINK);
	w->add_XSTR("Help", 928, Buttons[gr_screen.res][6].text_x, Buttons[gr_screen.res][6].text_y, &Buttons[gr_screen.
		res][6].button, UI_XSTR_COLOR_PINK);
	w->add_XSTR("Options", 1036, Buttons[gr_screen.res][7].text_x,
		Buttons[gr_screen.res][7].text_y, &Buttons[gr_screen.res][7].button, UI_XSTR_COLOR_PINK);
	w->add_XSTR("Medals", 1037, Buttons[gr_screen.res][8].text_x, Buttons[gr_screen.res][8].text_y, &Buttons[gr_screen.
		res][8].button, UI_XSTR_COLOR_PINK);
	w->add_XSTR("Remove", 1038, Buttons[gr_screen.res][11].text_x,
		Buttons[gr_screen.res][11].text_y, &Buttons[gr_screen.res][11].button, UI_XSTR_COLOR_GREEN);
	w->add_XSTR("Select", 1552, Buttons[gr_screen.res][12].text_x,
		Buttons[gr_screen.res][12].text_y, &Buttons[gr_screen.res][12].button, UI_XSTR_COLOR_PINK);
	w->add_XSTR("Clone", 1040, Buttons[gr_screen.res][13].text_x,
		Buttons[gr_screen.res][13].text_y, &Buttons[gr_screen.res][13].button, UI_XSTR_COLOR_GREEN);
	w->add_XSTR("Single", 1041, Buttons[gr_screen.res][14].text_x,
		Buttons[gr_screen.res][14].text_y, &Buttons[gr_screen.res][14].button, UI_XSTR_COLOR_PINK);
	w->add_XSTR("Multi", 1042, Buttons[gr_screen.res][15].text_x,
		Buttons[gr_screen.res][15].text_y, &Buttons[gr_screen.res][15].button, UI_XSTR_COLOR_PINK);
	w->add_XSTR("Convert", 1043, Buttons[gr_screen.res][16].text_x,
		Buttons[gr_screen.res][16].text_y, &Buttons[gr_screen.res][16].button, UI_XSTR_COLOR_GREEN);
	...
	List_region.create(&Ui_window, "", Barracks_list_coords[gr_screen.res][BARRACKS_X_COORD],
		Barracks_list_coords[gr_screen.res][BARRACKS_Y_COORD], Barracks_list_coords[gr_screen.res][BARRACKS_W_COORD],
		Barracks_list_coords[gr_screen.res][BARRACKS_H_COORD], 0, 1);
	List_region.hide();

	// create input box (for new pilot)
	Inputbox.create(&Ui_window, Barracks_list_coords[gr_screen.res][BARRACKS_X_COORD],
		Barracks_list_coords[gr_screen.res][BARRACKS_Y_COORD], Barracks_list_coords[gr_screen.res][BARRACKS_W_COORD],
		CALLSIGN_LEN - 1, "", UI_INPUTBOX_FLAG_INVIS | UI_INPUTBOX_FLAG_KEYTHRU | UI_INPUTBOX_FLAG_LETTER_FIRST);
...
void barracks_do_frame(float frametime)
{
	...
	{
		Buttons[gr_screen.res][B_HELP_BUTTON].button.reset_status();
	...
	if ((Pic_number >= 0) && (Pic_number < Num_pilot_images))
	{
		if (Pilot_images[Pic_number] == BARRACKS_IMAGE_NOT_LOADED)
	...
	if ((Pic_squad_number >= 0) && (Pic_squad_number < Num_pilot_squad_images))
	{
		if (Pilot_squad_images[Pic_squad_number] == BARRACKS_IMAGE_NOT_LOADED)
		{  // haven't tried loading it yet

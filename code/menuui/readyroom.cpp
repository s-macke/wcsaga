...
static sim_room_buttons Buttons[GR_NUM_RESOLUTIONS][NUM_BUTTONS] =
{
	...
	{		// GR_1024
		...
		sim_room_buttons("2_LMB_06", 10, 701, 64, 714, 6),
		sim_room_buttons("2_LMB_07", 10, 732, 64, 741, 7),
		sim_room_buttons("2_LMB_08", 854, 681, 822, 704, 8), // "Help"
		sim_room_buttons("2_LMB_09", 914, 681, 949, 671, 9), // "Commit"
		sim_room_buttons("2_LMB_10", 854, 728, 800, 743, 10), // "Options"
...
int sim_room_campaign_mission_filter(char* filename)
{
	...
	num = mission_campaign_get_mission_list(filename, &Campaign_missions[Num_campaign_missions],
		MAX_MISSIONS - Num_campaign_missions);
	if (num < 0)
		return 0;
...
int build_standalone_mission_list_do_frame()
{
	...
	if (Num_standalone_missions > 0)
	{  // sanity check
		if (strlen(Mission_filenames[Num_standalone_missions_with_info]) < MAX_FILENAME_LEN - 4)
		{ // sanity check?
			...
			if (!get_mission_info(filename))
			{
				...
				// add the line
				sim_room_line_add(READYROOM_LINE_MISSION, Standalone_mission_names[Num_standalone_missions_with_info],
					Mission_filenames[Num_standalone_missions_with_info], list_x1 + M_TEXT_X, y, flags);
			}
		}
...
int build_campaign_mission_list_do_frame()
{
	...
	if (Campaign.missions[Num_campaign_missions_with_info].completed || Simroom_show_all)
	{
		if (!get_mission_info(Campaign.missions[Num_campaign_missions_with_info].name))
		{
			...
			// determine some extra information
			int flags = 0;
			fs_builtin_mission* fb = game_find_builtin_mission(Campaign.missions[Num_campaign_missions_with_info].
				name);
			if ((fb != NULL) && (fb->flags & FSB_FROM_VOLITION))
			{
				flags |= READYROOM_FLAG_FROM_VOLITION;
			...
			sim_room_line_add(READYROOM_LINE_CMISSION, Campaign_mission_names[Num_campaign_missions_with_info],
				Campaign.missions[Num_campaign_missions_with_info].name, list_x1 + C_SUBTEXT_X, y, flags);
...
void sim_room_build_listing()
{
	...
	if (Player->readyroom_listing_mode == MODE_MISSIONS)
	{
		if (Hash_table_inited)
		{
			...
			{  // Is this the first time through
				...
				popup_till_condition(build_standalone_mission_list_do_frame, POPUP_CANCEL, XSTR("Loading missions",
					991));
			}
			else
			{
				for (i = 0; i < Num_standalone_missions_with_info; i++)
				{
					if (Standalone_mission_names[i])
					{
						...
						sim_room_line_add(READYROOM_LINE_MISSION, Standalone_mission_names[i], Mission_filenames[i],
							list_x1 + M_TEXT_X, y, flags);
						y += font_height + 2;
					}
				}
			}
	...
	else
	{
		...
		{  // Is this the first time through
			...
			popup_till_condition(build_campaign_mission_list_do_frame, POPUP_CANCEL, XSTR("Loading campaign missions",
				992));
		}
		else
		{
			for (i = 0; i < Num_campaign_missions_with_info; i++)
			{
				if (Campaign_mission_names[i])
				{
					...
					sim_room_line_add(READYROOM_LINE_CMISSION, Campaign_mission_names[i], Campaign.missions[i].name,
						list_x1 + C_SUBTEXT_X, y, flags);
	...
	Sim_room_slider.set_numberItems((Num_lines > max_num_entries_viewable) ? (Num_lines - max_num_entries_viewable) :
		0);
}
...
int readyroom_continue_campaign()
{
	...
	{  // is campaign and next mission valid?
		...
		popup(0, 1, POPUP_OK,
			XSTR(
			"The campaign is over.  To replay the campaign, either create a new pilot or restart the campaign in the campaign room.", 112));
		return -1;
#endif
	}
	else if (mc_rval == -2)
...
//			gamesnd_play_iface(SND_GENERAL_FAIL);
	...
	case CAMPAIGN_TAB:
		...
		{
			popup(PF_NO_NETWORKING, 1, POPUP_OK,
				XSTR("The currently active campaign cannot be found, unable to switch to campaign mode!", -1));
			break;
		}
...
void sim_room_init()
{
	...
	*Game_current_mission_filename = 0;
	...
	Ui_window.add_XSTR("Single Missions", 1060, Buttons[gr_screen.res][MISSION_TAB].xt,
		Buttons[gr_screen.res][MISSION_TAB].yt, &Buttons[gr_screen.res][MISSION_TAB].button, UI_XSTR_COLOR_PINK);
	Ui_window.add_XSTR("Campaign Missions", 1061, Buttons[gr_screen.res][CAMPAIGN_TAB].xt,
		Buttons[gr_screen.res][CAMPAIGN_TAB].yt, &Buttons[gr_screen.res][CAMPAIGN_TAB].button, UI_XSTR_COLOR_PINK);
	Ui_window.add_XSTR("Help", 928, Buttons[gr_screen.res][HELP_BUTTON].xt,
		Buttons[gr_screen.res][HELP_BUTTON].yt, &Buttons[gr_screen.res][HELP_BUTTON].button, UI_XSTR_COLOR_PINK);
	Ui_window.add_XSTR("Commit", 1062, Buttons[gr_screen.res][COMMIT_BUTTON].xt,
		Buttons[gr_screen.res][COMMIT_BUTTON].yt, &Buttons[gr_screen.res][COMMIT_BUTTON].button, UI_XSTR_COLOR_PINK);
	Ui_window.add_XSTR("Options", 1036, Buttons[gr_screen.res][OPTIONS_BUTTON].xt,
		Buttons[gr_screen.res][OPTIONS_BUTTON].yt, &Buttons[gr_screen.res][OPTIONS_BUTTON].button, UI_XSTR_COLOR_PINK);

	// common tab button text
	Ui_window.add_XSTR("Technical Database", 1055, Buttons[gr_screen.res][TECH_DATABASE_BUTTON].xt,
		Buttons[gr_screen.res][TECH_DATABASE_BUTTON].yt, &Buttons[gr_screen.res][TECH_DATABASE_BUTTON].button,
		UI_XSTR_COLOR_PINK);
	Ui_window.add_XSTR("Mission Simulator", 1056, Buttons[gr_screen.res][SIMULATOR_BUTTON].xt,
		Buttons[gr_screen.res][SIMULATOR_BUTTON].yt, &Buttons[gr_screen.res][SIMULATOR_BUTTON].button,
		UI_XSTR_COLOR_PINK);
	Ui_window.add_XSTR("Cutscenes", 1057, Buttons[gr_screen.res][CUTSCENES_BUTTON].xt,
		Buttons[gr_screen.res][CUTSCENES_BUTTON].yt, &Buttons[gr_screen.res][CUTSCENES_BUTTON].button,
		UI_XSTR_COLOR_PINK);
	Ui_window.add_XSTR("Credits", 1058, Buttons[gr_screen.res][CREDITS_BUTTON].xt,
		Buttons[gr_screen.res][CREDITS_BUTTON].yt, &Buttons[gr_screen.res][CREDITS_BUTTON].button, UI_XSTR_COLOR_PINK);

	// misc text - not associated with any buttons
	Ui_window.add_XSTR("Mission", 1063, Sim_misc_text_coords[gr_screen.res][SIM_MISC_TEXT_MISSION][0],
		Sim_misc_text_coords[gr_screen.res][SIM_MISC_TEXT_MISSION][1], NULL, UI_XSTR_COLOR_PINK);
	Ui_window.add_XSTR("Filename", 1064, Sim_misc_text_coords[gr_screen.res][SIM_MISC_TEXT_FILENAME][0],
		Sim_misc_text_coords[gr_screen.res][SIM_MISC_TEXT_FILENAME][1], NULL, UI_XSTR_COLOR_PINK);
	...
	Num_standalone_missions = cf_get_file_list(MAX_MISSIONS, Mission_filenames, CF_TYPE_MISSIONS, wild_card,
		CF_SORT_NAME);

	// set up slider with 0 items to start
	Sim_room_slider.create(&Ui_window, Sim_room_slider_coords[gr_screen.res][X_COORD],
		Sim_room_slider_coords[gr_screen.res][Y_COORD], Sim_room_slider_coords[gr_screen.res][W_COORD],
		Sim_room_slider_coords[gr_screen.res][H_COORD], 0,
		Sim_room_slider_filename[gr_screen.res], &sim_room_scroll_screen_up, &sim_room_scroll_screen_down,
		&sim_room_scroll_capture);
...
void sim_room_do_frame(float frametime)
{
	...
	if (!Buttons[gr_screen.res][CAMPAIGN_TAB].button.button_down() && !Buttons[gr_screen.res][MISSION_TAB].button.
		button_down())
	{
		if (Player->readyroom_listing_mode == MODE_CAMPAIGNS)
		{
	...
	{ // GR_640
		{
			...
			UI_XSTR_COLOR_PINK,
			-1,
			&Cr_buttons[0][CR_RESET_BUTTON].button
		},
		{
			"Campaign",
			1404,
		...
		},
		{
			...
			UI_XSTR_COLOR_PINK,
			-1,
			&Cr_buttons[0][CR_COMMIT_BUTTON].button
		},
	},
	{ // GR_1024
		{
			...
			UI_XSTR_COLOR_PINK,
			-1,
			&Cr_buttons[1][CR_RESET_BUTTON].button
		},
		{
			"Campaign",
			1404,
...
void campaign_room_build_listing()
{
	...
	for (i = 0; i < Num_campaigns; i++)
	{
		if (Campaign_names[i] != NULL)
		{
			...
			sim_room_line_add(READYROOM_LINE_CAMPAIGN, Campaign_names[i], Campaign_file_names[i],
				Cr_list_coords[gr_screen.res][0], y, flags);
			y += font_height + 2;
		}
	}
}
...
void set_new_campaign_line(int n)
{
	...
	if (str)
	{
		Num_info_lines = split_str(str, Cr_info_coords[gr_screen.res][2], Info_text_line_size, Info_text_ptrs,
			MAX_INFO_LINES);
...
int campaign_room_reset_campaign(int n)
{
	...
	z = popup(PF_TITLE_BIG | PF_TITLE_RED, 2, POPUP_CANCEL, POPUP_OK,
		XSTR("Warning\nThis will cause all progress in your\ncurrent campaign to be lost", 110));
	if (z == 1)
	{
...
void campaign_room_commit()
{
	...
	{  // new campaign selected
		...
		*/
		...
		// Reset player rank if needed
		if ((Campaign.flags & CF_RESET_RANK) != 0)
		{
			Player->stats.rank = RANK_ENSIGN;
		}
...
void campaign_room_do_frame(float frametime)
{
	...
	while (sim_room_line_query_visible(line))
	{
		...
		List_buttons[line - Scroll_offset].update_dimensions(Cr_list_coords[gr_screen.res][0], y,
		...
		strcpy_s(buf, sim_room_lines[line].name);
		gr_force_fit_string(buf, 255, Cr_list_coords[gr_screen.res][0] + Cr_list_coords[gr_screen.res][2] -
			sim_room_lines[line].x);
		gr_printf(sim_room_lines[line].x, y, buf);
		line++;
	}

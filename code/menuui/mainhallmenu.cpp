...
//
#define MAX_RANDOM_INTERCOM_SOUNDS				18
...
void main_hall_do_multi_ready()
{
	...
	case NETWORK_ERROR_NO_TYPE:
		popup(PF_NO_NETWORKING, 1, POPUP_OK,
			XSTR(
			"You have not defined your type of Internet connection.  Please run the Launcher, hit the setup button, and go to the Network tab and choose your connection type.", 360));
		break;
	case NETWORK_ERROR_NO_WINSOCK:
		popup(PF_NO_NETWORKING, 1, POPUP_OK,
			XSTR("Winsock is not installed.  You must have TCP/IP and Winsock installed to play multiplayer FreeSpace."
			, 361));
		break;
	case NETWORK_ERROR_NO_PROTOCOL:
		...
		{
			popup(PF_NO_NETWORKING, 1, POPUP_OK,
				XSTR("TCP/IP protocol not found.  This protocol is required for multiplayer FreeSpace.", -1));
		}
		else
		{
			Assert(Multi_options_g.protocol == NET_IPX);
			popup(PF_NO_NETWORKING, 1, POPUP_OK,
				XSTR("IPX protocol not found.  This protocol is required for multiplayer FreeSpace.", -1));
		}
		break;
	case NETWORK_ERROR_CONNECT_TO_ISP:
		popup(PF_NO_NETWORKING, 1, POPUP_OK,
			XSTR(
			"You have selected Dial Up Networking as your type of connection to the Internet.  You are not currently connected.  You must connect to your ISP before continuing on past this point.", 363));
		break;
	case NETWORK_ERROR_LAN_AND_RAS:
		popup(PF_NO_NETWORKING, 1, POPUP_OK,
			XSTR(
			"You have indicated that you use a LAN for networking.  You also appear to be dialed into your ISP.  Please disconnect from your service provider, or choose Dial Up Networking.", 364));
		break;
	...
	if ((Multi_options_g.protocol == NET_TCP) && !Tcp_active)
	{
		if (Tcp_failure_code == WSAEADDRINUSE)
		{
			popup(PF_NO_NETWORKING, 1, POPUP_OK,
				XSTR(
				"You have selected TCP/IP for multiplayer FreeSpace, but the TCP socket is already in use.  Check for another instance and/or use the \"-port <port_num>\" command line option to select an available port.", -1));
		}
		else
		{
			popup(PF_NO_NETWORKING, 1, POPUP_OK,
				XSTR(
				"You have selected TCP/IP for multiplayer FreeSpace, but the TCP/IP protocol was not detected on your machine.", 362));
		}
	...
	if (Multi_connection_speed == CONNECTION_SPEED_NONE)
	{
		popup(PF_NO_NETWORKING, 1, POPUP_OK,
			XSTR(
			"You must define your connection speed.  Please run the Launcher, hit the setup button, and go to the Network tab and choose your connection speed.", 986));
		return;
	}
...
void main_hall_blit_table_status()
{
	...
	gr_line(Mh_ship_table_status[gr_screen.res][0], Mh_ship_table_status[gr_screen.res][1],
		Mh_ship_table_status[gr_screen.res][0], Mh_ship_table_status[gr_screen.res][1]);
...
void main_hall_init(int main_hall_num)
{
	...
	{
		Warning(LOCATION,
			"Tried to load a main hall %d, but valid main halls are only 0 through %d; defaulting to main hall 0",
			main_hall_num, Num_main_halls - 1);
		main_hall_num = 0;
	}
	...
	// tooltip strings
	Main_hall->region_descript[0] = XSTR("Exit Wing Commander Saga", 353);
	Main_hall->region_descript[1] = XSTR("Barracks - Manage yourWing Commander Saga pilots", 354);
	Main_hall->region_descript[2] = XSTR("Ready room - Start or continue a campaign", 355);
	Main_hall->region_descript[3] = XSTR("Tech room - View specifications of Wing Commander Saga ships and weaponry",
		356);
	Main_hall->region_descript[4] = XSTR("Options - Change your Wing Commander Saga options", 357);
...
void main_hall_do(float frametime)
{
	...
	snazzy_action = -1;
	code = snazzy_menu_do(Main_hall_mask_data, Main_hall_mask_w, Main_hall_mask_h, Main_hall_num_options,
		Main_hall_region, &snazzy_action, 1, &key);
	...
	case SNAZZY_CLICKED:
		...
		case READY_ROOM_REGION:
			if (Campaign_file_missing)
			{
				...
				popup(PF_NO_NETWORKING, 1, POPUP_OK,
					XSTR(
					"The currently active campaign cannot be found.\n\n Please select another in the Campaign Room.",
					-1));
				break;
			}
			else if (!(Player->flags & PLAYER_FLAGS_IS_MULTI) && !strlen(Campaign.filename))
			{
				// no campaign loaded...
				popup(PF_NO_NETWORKING, 1, POPUP_OK,
					XSTR("No active campaign is available.  Please choose one in the Campaign Room.", -1));
				break;
			}
	...
	{
		int rc = popup(0, 3, XSTR("Go to Campaign Room", -1), XSTR("Select another pilot", -1), XSTR("Exit Game", -1),
...
void main_hall_close()
{
	...
	for (idx = 0; idx < Main_hall->num_door_sounds - 2; idx++)
	{	// don't cut off the glow sounds (requested by Dan)
		...
		if (Main_hall_misc_anim[idx].num_frames > 0)
		{
			...
			if (Main_hall_misc_anim[idx].direction & GENERIC_ANIM_DIRECTION_PAUSED)
			{
				...
				if (Main_hall->misc_anim_delay[idx][0] == -1)
				{
					Main_hall->misc_anim_delay[idx][0] = timestamp(Main_hall->misc_anim_delay[idx][1] +
						(int)(((float)rand() / (float)RAND_MAX) * (float)(Main_hall->misc_anim_delay[idx][2] -
																		  Main_hall->misc_anim_delay[idx][1])));
			...
			else
			{
				for (s_idx = Main_hall->misc_anim_special_sounds[idx][0]; s_idx > 0; s_idx--)
				{
					...
					if ((Main_hall_misc_anim[idx].current_frame >= Main_hall->misc_anim_special_trigger[idx][s_idx]) &&
						!Main_hall->misc_anim_sound_flag[idx][s_idx])
					{
						...
						// play the sound
						Main_hall->misc_anim_sound_handles[idx][s_idx] = snd_play(&Snds_iface[Main_hall->
							misc_anim_special_sounds[idx][s_idx]], Main_hall->misc_anim_sound_pan[idx]);
						break;
					}
				}
			...
			if (Main_hall_frame_skip || Main_hall_paused)
				frametime = 0;
			generic_anim_render(&Main_hall_misc_anim[idx], frametime, Main_hall->misc_anim_coords[idx][0],
				Main_hall->misc_anim_coords[idx][1]);
		}
	}
}
...
void main_hall_render_door_anims(float frametime)
{
	...
	for (idx = 0; idx < MAX_DOOR_ANIMATIONS; idx++)
	{
		if (Main_hall_door_anim[idx].num_frames > 0)
		{
			// first pair : coords of where to play a given door anim
			// second pair : center of a given door anim in windowed mode
			generic_anim_render(&Main_hall_door_anim[idx], frametime, Main_hall->door_anim_coords[idx][0],
				Main_hall->door_anim_coords[idx][1]);
		}
	}
}
...
void main_hall_mouse_release_region(int region)
{
	...
	if ((Main_hall_door_anim[region].num_frames > 0))
	{
		...
		Main_hall_door_sound_handles[region] = snd_play(&Snds_iface[Main_hall->door_sounds[region][1]],
			Main_hall->door_sound_pan[region]);
		...
		snd_set_pos(Main_hall_door_sound_handles[region], &Snds_iface[SND_MAIN_HALL_DOOR_CLOSE],
			(float)((Main_hall_door_anim[region].keyframe) ? Main_hall_door_anim[region].keyframe :
					Main_hall_door_anim[region].num_frames - Main_hall_door_anim[region].
					current_frame) / (float)Main_hall_door_anim[region].num_frames, 1);
	}
}
...
void main_hall_mouse_grab_region(int region)
{
	...
	Main_hall_door_sound_handles[region] = snd_play(&Snds_iface[Main_hall->door_sounds[region][0]],
		Main_hall->door_sound_pan[region]);
...
void main_hall_handle_random_intercom_sounds()
{
	...
	if ((Main_hall_next_intercom_sound_stamp == -1) && (Main_hall_intercom_sound_handle == -1))
	{
		Main_hall_next_intercom_sound_stamp = timestamp((int)(((float)rand() / (float)RAND_MAX) *
															  (float)(Main_hall->
																	  intercom_delay[Main_hall_next_intercom_sound][1]
																	  -
																	  Main_hall->
																	  intercom_delay[Main_hall_intercom_sound_handle][
																	  0])));
	}
	...
	if (Main_hall_intercom_sound_handle == -1)
	{
		...
		{
			// play the sound
			Main_hall_intercom_sound_handle = snd_play(&Snds_iface[Main_hall->
				intercom_sounds[Main_hall_next_intercom_sound]]);
	...
	else
	{
		...
		if (!snd_is_playing(Main_hall_intercom_sound_handle))
		{
			...
			Main_hall_next_intercom_sound_stamp = timestamp((int)(((float)rand() / (float)RAND_MAX) *
																  (float)(Main_hall->
																		  intercom_delay[
																		  Main_hall_next_intercom_sound][1]
																		  -
																		  Main_hall->
																		  intercom_delay[
																		  Main_hall_next_intercom_sound][0])));
...
void main_hall_blit_version()
{
	...
	int version_y = (Main_hall->region_yval) - 2 * padding - Current_font->h;

	gr_set_shader(&Main_hall_tooltip_shader);
	gr_shade(0, version_y - padding, w + 2 * padding, Current_font->h + 2 * padding);

	// print the string near the lower left corner
	gr_set_color_fast(&Color_bright_blue);
	gr_string(5, version_y, version_string);
}
...
void main_hall_process_help_stuff()
{
	...
	if ((y_anim_offset >= (2 * Main_hall_tooltip_padding[gr_screen.res]) +
		 h) || (help_overlay_active(Main_hall_overlay_id)))
	{
...
void main_hall_read_table()
{
	...
	while (!optional_string("#end"))
	{
		...
		for (m_idx = 0; m_idx < GR_NUM_RESOLUTIONS; m_idx++)
		{
			...
			if (count >= MAIN_HALLS_MAX)
			{
				m = &temp;
				Warning(LOCATION,
					"Number of main halls in mainhall.tbl has exceeded max of %d. All further main halls will be ignored.", MAIN_HALLS_MAX);
	...
	if (Vasudan_funny)
	{
		...
		Main_hall_defines[GR_640][hall].door_sounds[OPTIONS_REGION][0] = SND_MAIN_HALL_PA_18;
		Main_hall_defines[GR_640][hall].door_sounds[OPTIONS_REGION][1] = SND_MAIN_HALL_PA_18;
		Main_hall_defines[GR_1024][hall].door_sounds[OPTIONS_REGION][0] = SND_MAIN_HALL_PA_18;
		Main_hall_defines[GR_1024][hall].door_sounds[OPTIONS_REGION][1] = SND_MAIN_HALL_PA_18;

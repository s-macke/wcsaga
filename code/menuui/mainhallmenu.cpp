...
void main_hall_do_multi_ready() {
    ...
    switch (error) {
        ...
        case NETWORK_ERROR_LAN_AND_RAS:
            popup(PF_NO_NETWORKING, 1, POPUP_OK,
                  XSTR(
                          "You have indicated that you use a LAN for networking.  You also appear to be dialed into your ISP.  Please disconnect from your service provider, or choose Dial Up Networking.",
                          364));
            break;
    ...
    if ((Multi_options_g.protocol == NET_TCP) && !Tcp_active) {
        if (Tcp_failure_code == WSAEADDRINUSE) {
            popup(PF_NO_NETWORKING, 1, POPUP_OK,
                  XSTR(
                          "You have selected TCP/IP for multiplayer FreeSpace, but the TCP socket is already in use.  Check for another instance and/or use the \"-port <port_num>\" command line option to select an available port.",
                          -1));
        } else {
            popup(PF_NO_NETWORKING, 1, POPUP_OK,
                  XSTR(
...
void main_hall_campaign_cheat() {
    char *ret = popup_input(0,
                            XSTR("Enter mission name.\n\n* This will destroy all legitimate progress in this campaign. *",
                                 -1));
            ...
            if (Main_hall_misc_anim[idx].direction & GENERIC_ANIM_DIRECTION_PAUSED) {
                ...
                if (Main_hall->misc_anim_delay[idx][0] == -1) {
                    Main_hall->misc_anim_delay[idx][0] = timestamp(Main_hall->misc_anim_delay[idx][1] +
                                                                   (int) (((float) rand() / (float) RAND_MAX) *
                                                                          (float) (Main_hall->misc_anim_delay[idx][2] -
...
void main_hall_mouse_grab_region(int region) {
    ...
    if ((Main_hall_door_anim[region].num_frames > 0) && (Main_hall_door_anim[region].current_frame != -1)) {
        snd_set_pos(Main_hall_door_sound_handles[region], &Snds_iface[SND_MAIN_HALL_DOOR_OPEN],
                    (float) Main_hall_door_anim[region].current_frame / (float) Main_hall_door_anim[region].num_frames,
                    1);
    }
}
...
void main_hall_maybe_blit_tooltips() {
    ...
    if (!help_overlay_active(Main_hall_overlay_id)) {
        int shader_y =
                (Main_hall->region_yval) - Main_hall_tooltip_padding[gr_screen.res];    // subtract more to pull higher
    ...
    while (!optional_string("#end")) {
        ...
        for (m_idx = 0; m_idx < GR_NUM_RESOLUTIONS; m_idx++) {
            ...
            if (count >= MAIN_HALLS_MAX) {
                m = &temp;
                Warning(LOCATION,
                        "Number of main halls in mainhall.tbl has exceeded max of %d. All further main halls will be ignored.",
                        MAIN_HALLS_MAX);

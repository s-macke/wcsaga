...
void physics_sim_vel(vec3d* position, physics_info* pi, float sim_time, matrix* orient)
{
	...
	apply_physics(damp.xyz.x, local_desired_vel.xyz.x, local_v_in.xyz.x,
		sim_time, &local_v_out.xyz.x, &local_disp.xyz.x);

	// find updated LOCAL velocity and position in the local y direction
	apply_physics(damp.xyz.y, local_desired_vel.xyz.y, local_v_in.xyz.y,
		sim_time, &local_v_out.xyz.y, &local_disp.xyz.y);
	...
	if (excess > 5 && (pi->flags & PF_SPECIAL_WARP_IN))
	{
		...
		local_disp.xyz.z = (pi->max_vel.xyz.z * sim_time) + excess * (float(SPECIAL_WARP_T_CONST) * (1.0f -
																									 exp_factor));
	}
	else if (pi->flags & PF_SPECIAL_WARP_OUT)
	{
		...
		local_disp.xyz.z = (local_v_in.xyz.z * sim_time) + deficeit * (sim_time - (float(SPECIAL_WARP_T_CONST) *
																				   (1.0f - exp_factor)));
	}
	else
	{
		apply_physics(damp.xyz.z, local_desired_vel.xyz.z, local_v_in.xyz.z,
			sim_time, &local_v_out.xyz.z, &local_disp.xyz.z);
	}
...
void physics_read_flying_controls(matrix* orient, physics_info* pi, control_info* ci, float sim_time, vec3d* wash_rot)
{
	...
	if (pi->flags & PF_ACCELERATES)
	{
		...
		if (pi->flags & PF_SLIDE_ENABLED)
		{
			...
			pi->prev_ramp_vel.xyz.x = velocity_ramp(pi->prev_ramp_vel.xyz.x, goal_vel.xyz.x, ramp_time_const,
				sim_time);
			...
			pi->prev_ramp_vel.xyz.y = velocity_ramp(pi->prev_ramp_vel.xyz.y, goal_vel.xyz.y, ramp_time_const,
				sim_time);
		}
		else
		{
		...
		if (pi->flags & PF_GLIDING)
		{
			...
			float xVal = glide_ramp(local_vel.xyz.x, goal_vel.xyz.x, pi->slide_accel_time_const, pi->glide_accel_mult,
				sim_time);
			float yVal = glide_ramp(local_vel.xyz.y, goal_vel.xyz.y, pi->slide_accel_time_const, pi->glide_accel_mult,
				sim_time);
			float zVal = 0.0;
			if (pi->flags & PF_AFTERBURNER_ON)
				zVal = glide_ramp(local_vel.xyz.z, goal_vel.xyz.z, pi->afterburner_forward_accel_time_const,
					pi->glide_accel_mult, sim_time);
			else
			{
				if (goal_vel.xyz.z >= 0.0f)
					zVal = glide_ramp(local_vel.xyz.z, goal_vel.xyz.z, pi->forward_accel_time_const,
						pi->glide_accel_mult, sim_time);
				else
					zVal = glide_ramp(local_vel.xyz.z, goal_vel.xyz.z, pi->forward_decel_time_const,
						pi->glide_accel_mult, sim_time);
			}
			...
			if (curGlideCap >= 0.0f)
			{	// so if negative, don't bother with speed cap
...
void physics_apply_whack(vec3d* impulse, vec3d* pos, physics_info* pi, matrix* orient, float mass)
{
	...
	//	Detect null vector.
	if ((fl_abs(impulse->xyz.x) <= WHACK_LIMIT) && (fl_abs(impulse->xyz.y) <= WHACK_LIMIT) &&
...
void physics_apply_shock(vec3d* direction_vec, float pressure, physics_info* pi, matrix* orient, vec3d* min,
						 vec3d* max, float radius)
{
	vec3d normal;
	...
	if (normal.xyz.x < 0.0f)		// normal < 0, hits the right face
		vm_vec_copy_scale(&impact_vec, &orient->vec.rvec, max->xyz.x * pressure * area.xyz.x * normal.xyz.x *
			sin.xyz.x / pi->mass);
	else								// normal > 0, hits the left face
		vm_vec_copy_scale(&impact_vec, &orient->vec.rvec, min->xyz.x * pressure * area.xyz.x * -normal.xyz.x *
			sin.xyz.x / pi->mass);
	...
	if (normal.xyz.y < 0.0f)
		vm_vec_copy_scale(&impact_vec, &orient->vec.uvec, max->xyz.y * pressure * area.xyz.y * normal.xyz.y *
			sin.xyz.y / pi->mass);
	else
		vm_vec_copy_scale(&impact_vec, &orient->vec.uvec, min->xyz.y * pressure * area.xyz.y * -normal.xyz.y *
			sin.xyz.y / pi->mass);
	...
	if (normal.xyz.z < 0.0f)
		vm_vec_copy_scale(&impact_vec, &orient->vec.fvec, max->xyz.z * pressure * area.xyz.z * normal.xyz.z *
			sin.xyz.z / pi->mass);
	else
		vm_vec_copy_scale(&impact_vec, &orient->vec.fvec, min->xyz.z * pressure * area.xyz.z * -normal.xyz.z *
			sin.xyz.z / pi->mass);
	...
	float scale;
	vm_vec_rotate(&local_torque, &torque, orient);
	vec3d delta_rotvel = local_torque;
	vm_vec_normalize_safe(&delta_rotvel);
...
int check_rotvel_limit(physics_info* pi)
{
	...
	if (!(pi->flags & PF_DEAD_DAMP))
	{
		...
		{
			pi->rotvel.xyz.x = (pi->rotvel.xyz.x / fl_abs(pi->rotvel.xyz.x)) * (pi->max_rotvel.xyz.
																				x - (float)ROTVEL_TOL);
			change_made = 1;
		}
		if (fl_abs(pi->rotvel.xyz.y) > pi->max_rotvel.xyz.y)
		{
			pi->rotvel.xyz.y = (pi->rotvel.xyz.y / fl_abs(pi->rotvel.xyz.y)) * (pi->max_rotvel.xyz.
																				y - (float)ROTVEL_TOL);
			change_made = 1;
		}
		if (fl_abs(pi->rotvel.xyz.z) > pi->max_rotvel.xyz.z)
		{
			pi->rotvel.xyz.z = (pi->rotvel.xyz.z / fl_abs(pi->rotvel.xyz.z)) * (pi->max_rotvel.xyz.
																				z - (float)ROTVEL_TOL);
			change_made = 1;
		}
	}
...
void avd_movement::setAVD(float final_position, float total_movement_time, float starting_accleration_time,
						  float ending_acceleration_time, float final_velocity)
{

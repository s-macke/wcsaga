...
TimeProc(unsigned int id, unsigned int msg, unsigned long userdata, unsigned long dw1, unsigned long dw2) {
    if (!Rtv_callback) {
        return;
    }
...
int rtvoice_init_recording(int qos) {
    if (!Rtv_recording_inited) {
        ...
        Rtv_capture_raw_buffer_size = Rtv_formats[Rtv_recording_format].frequency * (RTV_BUFFER_TIME) *
                                      fl2i(Rtv_formats[Rtv_recording_format].bits_per_sample / 8.0f);
        ...
        if (dscap_create_buffer(Rtv_formats[Rtv_recording_format].frequency,
                                Rtv_formats[Rtv_recording_format].bits_per_sample, 1, RTV_BUFFER_TIME)) {
            return -1;
        }
...
void rtvoice_get_data(unsigned char **outbuf, int *compressed_size, int *uncompressed_size, double *gain,
                      unsigned char **outbuf_raw, int *outbuf_size_raw) {
    ...
    *uncompressed_size = raw_size;
    ...
    if (Rtv_do_compression) {
        csize = rtvoice_compress(Rtv_capture_raw_buffer, raw_size, Rtv_capture_compressed_buffer,

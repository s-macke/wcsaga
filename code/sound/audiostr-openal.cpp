...
bool WaveFile::Open(char *pszFilename, bool keep_ext) {
    ...
    if (keep_ext) {
        ...
        rc = cf_find_file_location_ext(filename, NUM_EXT, audio_ext, CF_TYPE_ANY, sizeof(fullpath) - 1, fullpath,
                                       &FileSize, &FileOffset);
    }
...
//	nprintf(("Alan","Reqeusted: %d\n", cbSize));
    ...
    switch (m_wave_format) {
        ...
        case WAVE_FORMAT_ADPCM:
            if (!m_hStream_open) {
                if (!ACM_stream_open(m_pwfmt_original, &m_wfxDest, (void **) &m_hStream,
                                     m_bits_per_sample_uncompressed)) {
                    m_hStream_open = 1;
                } else {
                    Int3();
                }
...
//				nprintf(("Alan","Num bytes desired: %d\n", num_bytes_desired));
    ...
    if (m_wave_format == WAVE_FORMAT_ADPCM) {
        if (num_bytes_read > 0) {
            rc = ACM_convert((void *) m_hStream, (ubyte *) dest_buf, num_bytes_read, (ubyte *) uncompressed_wave_data,
    ...
    if (pszFilename) {
        ...
        if (m_pwavefile) {
            ...
            if (m_pwavefile->Open(pszFilename, (type == ASF_EVENTMUSIC))) {
                ...
                if ((m_cbBufSize == m_pwavefile->m_wfmt.nSamplesPerSec) || (m_cbBufSize == 11025) ||
                    (m_cbBufSize == 22050))
...
//				nprintf(("SOUND", "SOUND => Stream buffer created using %d bytes\n", m_cbBufSize));
                ...
                OpenAL_ErrorCheck(alGenBuffers(MAX_STREAM_BUFFERS, m_buffer_ids), {
                    fRtn = false;
                    goto ErrorExit;
                });
...
bool AudioStream::WriteWaveData(uint size, uint *num_bytes_written, int service) {
    ...
    *num_bytes_written = 0;
    ...
    if (num_bytes_read > 0) {
        ...
        OpenAL_ErrorCheck(alBufferData(m_buffer_ids[m_play_buffer_id], m_pwavefile->m_ALformat, uncompressed_wave_data,
                                       num_bytes_read, m_pwavefile->m_wfmt.nSamplesPerSec), {
                              fRtn = false;
                              goto ErrorExit;
                          });

        OpenAL_ErrorCheck(alSourceQueueBuffers(m_source_id, 1, &m_buffer_ids[m_play_buffer_id]), {
            fRtn = false;
            goto ErrorExit;
        });

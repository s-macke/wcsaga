...
HRESULT (__stdcall *pfn_DirectSoundCreate)(LPGUID lpGuid, LPDIRECTSOUND *ppDS, IUnknown FAR
...
GUID DSPROPSETID_EAX_ReverbProperties_Def =
        {
                0x4a4e6fc1,
                0xc341,
                0x11d1,
...
GUID DSPROPSETID_EAXBUFFER_ReverbProperties_Def =
        {
                0x4a4e6fc0,
                0xc341,
                0x11d1,
...
int ds_load_buffer(int *sid, int *hid, int *final_size, void *header, sound_info *si, int flags) {
    ...
    *sid = ds_get_sid();
    ...
    switch (si->format) {
        ...
        case WAVE_FORMAT_ADPCM:
            ...
            rc = ACM_convert_ADPCM_to_PCM(pwfx, si->data, si->size, &convert_buffer, 0, &convert_len, &src_bytes_used,
                                          16);
        ...
        case OGG_FORMAT_VORBIS:
            ...
            while (src_bytes_used < si->size) {
                rc = ov_read(&si->ogg_info, (char *) convert_buffer + src_bytes_used, si->size - src_bytes_used,
                             byte_order, si->bits / 8, 1, &section);
    ...
    *final_size = size;
    ...
    if ((ds_using_ds3d()) && (flags & DS_USE_DS3D)) {
        //	if (ds_using_ds3d()) {
        BufferDesc.dwFlags = DSBCAPS_STATIC | DSBCAPS_GETCURRENTPOSITION2 | DSBCAPS_CTRLVOLUME | DSBCAPS_CTRL3D |
    ...
    *sid = ds_get_sid();
    ...
    if (DSReturn == DS_OK && ds_software_buffers[*sid].pdsb != NULL) {
        ...
        if ((ds_software_buffers[*sid].pdsb)->Lock(0,
                                                   final_sound_size, (void **) (&pData), &DataSize, (void **) (&pData2),
                                                   &DataSize2, 0) == DS_OK) {
...
void ds_init_channels() {
    ...
    if (Channels == NULL) {
        Error(LOCATION, "Unable to allocate %d bytes for %d audio channels.", sizeof(channel) * MAX_CHANNELS,
              MAX_CHANNELS);
    }
...
void ds_dll_get_functions() {
    pfn_DirectSoundCreate = (HRESULT(__stdcall *)(LPGUID
    lpGuid, LPDIRECTSOUND * ppDS, IUnknown
    FAR * pUnkOuter))
    GetProcAddress(Ds_dll_handle, "DirectSoundCreate");
    pfn_DirectSoundCaptureCreate = (HRESULT(__stdcall *)(LPGUID
    lpGuid, LPDIRECTSOUNDCAPTURE * lplpDSC, IUnknown
    FAR * pUnkOuter))
    GetProcAddress(Ds_dll_handle, "DirectSoundCaptureCreate");
}
...
int ds_init_property_set(DWORD sample_rate, WORD sample_bits) {
    ...
    dsbd.dwFlags = DSBCAPS_CTRLDEFAULT | DSBCAPS_GETCURRENTPOSITION2 | DSBCAPS_STATIC | DSBCAPS_CTRL3D |
                   DSBCAPS_MUTE3DATMAXDISTANCE;
...
int ds_init(int use_a3d, int use_eax, unsigned int sample_rate, unsigned short sample_bits) {
    ...
    */
    ...
    if (Ds_use_ds3d) {
        ...
        if (hr != DS_OK) {
            nprintf(("Sound",
                    "SOUND ==> Primary Buffer create failed with DSBCAPS_CTRL3D property... disabling DirectSound3D\n"
                    ));
            Ds_use_ds3d = 0;
            Ds_use_eax = 0;
        } else {
...
//
int ds3d_play(int sid, int hid, int snd_id, vec3d *pos, vec3d *vel, int min, int max, int looping, int max_volume,
              int estimated_vol, int priority) {
#ifdef USE_OPENAL
    int ch_idx;
...
int ds_eax_set_volume(float volume) {
    ...
    hr = Ds_eax_reverb->Set(DSPROPSETID_EAX_ReverbProperties_Def, DSPROPERTY_EAX_VOLUME, NULL,
                            0, &volume, sizeof(float));
    if (SUCCEEDED(hr)) {
        return 0;
    } else {
        return -1;
...
int ds_eax_set_decay_time(float seconds) {
    ...
    hr = Ds_eax_reverb->Set(DSPROPSETID_EAX_ReverbProperties_Def, DSPROPERTY_EAX_DECAYTIME, NULL,
                            0, &seconds, sizeof(float));
    if (SUCCEEDED(hr)) {
        return 0;
    } else {
        return -1;
...
int ds_eax_set_damping(float damp) {
    ...
    hr = Ds_eax_reverb->Set(DSPROPSETID_EAX_ReverbProperties_Def, DSPROPERTY_EAX_DAMPING, NULL,
                            0, &damp, sizeof(float));
    if (SUCCEEDED(hr)) {
        return 0;
    } else {
...
int ds_eax_set_environment(unsigned long envid) {
    ...
    hr = Ds_eax_reverb->Set(DSPROPSETID_EAX_ReverbProperties_Def, DSPROPERTY_EAX_ENVIRONMENT, NULL,
                            0, &envid, sizeof(unsigned long));
    if (SUCCEEDED(hr)) {
        return 0;
    } else {
        return -1;
...
int ds_eax_set_preset(unsigned long envid) {
    ...
    hr = Ds_eax_reverb->Set(DSPROPSETID_EAX_ReverbProperties_Def, DSPROPERTY_EAX_ALL, NULL,
                            0, &Ds_eax_presets[envid], sizeof(EAX_REVERBPROPERTIES));
    if (SUCCEEDED(hr)) {
        return 0;
    } else {
        return -1;
...
int ds_eax_set_all(unsigned long id, float vol, float damping, float decay) {
    ...
    hr = Ds_eax_reverb->Set(DSPROPSETID_EAX_ReverbProperties_Def, DSPROPERTY_EAX_ALL, NULL,
                            0, &er, sizeof(EAX_REVERBPROPERTIES));
    if (SUCCEEDED(hr)) {
        return 0;
    } else {
        return -1;
...
int ds_eax_get_all(EAX_REVERBPROPERTIES *er) {
    ...
    hr = Ds_eax_reverb->Get(DSPROPSETID_EAX_ReverbProperties_Def, DSPROPERTY_EAX_ALL, NULL, 0,
                            er, sizeof(EAX_REVERBPROPERTIES), &outsize);
    if (SUCCEEDED(hr)) {
        return 0;
    } else {
        return -1;
...
int ds_eax_init() {
    ...
    if ((driver_support & (KSPROPERTY_SUPPORT_GET | KSPROPERTY_SUPPORT_SET)) != (KSPROPERTY_SUPPORT_GET |

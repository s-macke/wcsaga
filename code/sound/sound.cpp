    ...
    while (1) {
        ...
        if (num_tries++ > 5) {
            if (!gave_warning) {
                MessageBox(NULL,
                           XSTR(
                                   "Audio could not be initialized.  If you are running any applications playing sound in the background, you should stop them before continuing.",
                                   971), NULL, MB_OK);
                gave_warning = 1;
            } else {
                goto Failure;
...
int snd_play_3d(game_snd *gs, vec3d *source_pos, vec3d *listen_pos, float radius, vec3d *source_vel, int looping,
    ...
    if (play_using_ds3d) {
        ...
        handle = ds3d_play(snd->sid, snd->hid, gs->id_sig, source_pos, source_vel, min_range, max_range, looping,
                           ds_convert_volume(max_volume * Master_sound_volume), ds_convert_volume(volume),
...
void snd_rewind(int snd_handle, game_snd *gs, float seconds) {
    ...
    current_time = (float) current_offset /
                   bps;                                        // how many seconds we're into the sound

...
int beam_fire(beam_fire_info *fire_info) {
    ...
    if (fire_info->beam_info_override != NULL) {
        ...
        beam_get_binfo(new_item, fire_info->accuracy,
                       wip->b_info.beam_shots);            // to fill in b_info	- the set of directional aim vectors
    }
    ...
    if (MULTIPLAYER_MASTER) {
        ...
        send_beam_fired_packet(fire_info->shooter, fire_info->turret, fire_info->target,
                               fire_info->beam_info_index, &new_item->binfo, (ubyte) fire_info->fighter_beam,
                               bank_point);
    }
...
void beam_render(beam *b, float u_offset) {
    ...
    for (s_idx = 0; s_idx < bwi->beam_num_sections; s_idx++) {
        ...
        if (bwsi->tile_type == 1)
            u_scale = length / (bwsi->width / 2) /
                      bwsi->tile_factor;    // beam tileing, might make a tileing factor in beam index later -Bobboau
        else
...
int beam_collide_ship(obj_pair *pair) {
    ...
    if (!(ship_objp->flags & OF_NO_SHIELDS)) {
        ...
        if (quadrant_num >= 0) {
            // do the hit effect
            mc_info *mc = shield_collision ? &mc_shield : &mc_hull_enter;
                ...
                case OBJ_SHIP:
                    ...
                    if (!b->f_collisions[idx].is_exit_collision)
                        ship_apply_local_damage(&Objects[target], &Objects[b->objnum], &b->f_collisions[idx].cinfo.
                                                        hit_point_world, beam_get_ship_damage(b, &Objects[target]),
                                                b->f_collisions[idx].quadrant);

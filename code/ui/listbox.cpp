...
void UI_LISTBOX::create(UI_WINDOW *wnd, int _x, int _y, int _w, int _h, int _numitems, char **_list, char *_check_list,
                        int _max_items) {
                ...
                default:        // enter the key in the key buffer
                    ...
                    for (i = 0; i < num_items; i++) {
                        ...
                        for (j = 0; j < key_buffer_count; j++)
                            if ((current_text[j] != ascii_table[key_buffer[j]]) && (current_text[j] !=

...
public:
    ...
    int set_bmaps(char *ani_filename, int nframes = 3,
...
class UI_BUTTON : public UI_GADGET {
    ...
    void (*m_disabled_function)(
            void);                // callback that gets called when disabled button gets pressed (sound, popup, etc)
...
public:
    ...
    void create(UI_WINDOW *wnd, int _x, int _y, int _w, int _textlen, char *text, int _flags = 0, int pixel_lim = -1,
                color *clr = NULL);
...
class UI_CHECKBOX : public UI_GADGET {
    ...
    enum {
        CBOX_DISABLED_CLEAR = 4
    };
    enum {
        CBOX_DISABLED_MARKED = 5
    };
...
class UI_RADIO : public UI_GADGET {
    ...
    enum {
        RADIO_DISABLED_CLEAR = 4
    };
    enum {
        RADIO_DISABLED_MARKED = 5
    };
...
public:
    ...
    void
    force_currentItem(int _currentItem);        // force slider to new position manually, _not_ calling any callbacks
...
public:
    ...
    void create(UI_WINDOW *wnd, int _x, int _y, int num_pos, char *bm_slider, int slider_mask, char *bm_left = NULL,
                int left_mask = -1, int left_x = -1, int left_y = -1, char *bm_right = NULL, int right_mask = -1,
                int right_x = -1, int right_y = -1, int dot_width = 19);
...
public:
    void create(UI_WINDOW *wnd, int _x, int _y, int _w, int _h, int _numitem, char **_list, char *_check_list = NULL,
                int _max_items = -1);
    ...
    int
    toggled();    // toggled:  Returns which item was toggled with spacebr or mouse click of check_list is not NULL

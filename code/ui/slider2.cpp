...
void UI_SLIDER2::create(UI_WINDOW *wnd, int _x, int _y, int _w, int _h, int _numberItems, char *_bitmapSliderControl,
...
void UI_SLIDER2::process(int focus) {
    ...
    if ((OnMe && B1_PRESSED && ui_mouse.y < (currentPosition + y + slider_half_h -
                                             1)) || (mouse_locked && (ui_mouse.y < (currentPosition + y +
                                                                                    slider_half_h - 1)))) {
        ...
        if ((timer_get_milliseconds() > last_scrolled + 50) || B1_JUST_PRESSED || mouse_locked) {
            ...
            if (!mouse_locked) {
                ...
                mouse_lock_move = fl2i(((((float) ui_mouse.
                        y - (float) y - (float) slider_half_h) / (float) numberPositions) * (float)
                                                numberItems) - .49);

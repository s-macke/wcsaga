...
int mve_audio_data(ubyte major, ubyte *data) {
    ...
    if (mve_audio_canplay) {
#ifdef USE_OPENAL
        int chan = mve_get_ushort(data + 2);
        int nsamp = mve_get_ushort(data + 4);
...
void mve_video_display() {
    ...
    if (gr_screen.mode == GR_OPENGL) {
        glTexSubImage2D(GL_state.Texture.GetTarget(), 0, 0, 0, g_width, g_height, GL_BGRA,
                        GL_UNSIGNED_SHORT_1_5_5_5_REV, pixelbuf);
...
int mve_video_init(ubyte *data) {
    ...
    if (gr_screen.mode == GR_OPENGL) {
        ...
        glTexImage2D(GL_state.Texture.GetTarget(), 0, GL_RGB5_A1, wp2, hp2, 0, GL_BGRA_EXT,

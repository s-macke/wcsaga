...
static void OGG_audio_init(vorbis_info *vinfo) {
    ...
    OpenAL_ErrorCheck(alGenSources(1, &audio_sid),
    return );
...
static void OGG_audio_write(vorbis_info *vorbis, bool *ready) {
    ...
    OpenAL_ErrorCheck(alGetSourcei(audio_sid, AL_BUFFERS_PROCESSED, &processed),
    return );
    ...
    OpenAL_ErrorCheck(alGetSourcei(audio_sid, AL_BUFFERS_QUEUED, &queued),
    return );

    if (audiobuf_fill && (queued < OGG_AUDIO_BUFFERS)) {
        if (!audio_buffers[audio_buffer_tail])
            OpenAL_ErrorCheck(alGenBuffers(1, &audio_buffers[audio_buffer_tail]),
        return );

        ALenum format = (vorbis->channels == 2) ? AL_FORMAT_STEREO16 : AL_FORMAT_MONO16;

        OpenAL_ErrorCheck(alBufferData(audio_buffers[audio_buffer_tail], format, audiobuf, audiobuf_fill, vorbis->rate),
        return );

        OpenAL_ErrorCheck(alSourceQueueBuffers(audio_sid, 1, &audio_buffers[audio_buffer_tail]),
        return );
    ...
    OpenAL_ErrorCheck(alGetSourcei(audio_sid, AL_SOURCE_STATE, &status),
    return );

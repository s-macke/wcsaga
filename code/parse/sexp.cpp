		...
		case OPF_ORDER_RECIPIENT:
			...
			if ((ship_name_lookup(CTEXT(node), 1) < 0) && (wing_name_lookup(CTEXT(node), 1) < 0))
			{
				if (Fred_running || !mission_parse_get_arrival_ship(CTEXT(node)))
				{
					...
					{									// return invalid if not also looking for point
					...
					if (waypoint_lookup(CTEXT(node)) < 0)
					{
						...
						{  // non-zero on verify vector mean invalid!
		...
		case OPF_PRIORITY:
			{
				...
				if (Fred_running)
				{  // should still check in Fred though..
					char* name;
...
int sexp_is_destroyed(int n, fix* latest_time)
{
	...
	while (n != -1)
	{
		...
		if (mission_log_get_time(LOG_SHIP_DESTROYED, name, NULL, &time) || mission_log_get_time(LOG_WING_DESTROYED,
			name, NULL, &time) || mission_log_get_time(LOG_SELF_DESTRUCTED, name, NULL, &time))
		{
			num_destroyed++;
...
int sexp_get_energy_pct(int node, int op_num)
{
	...
	sindex = ship_name_lookup(shipname);
	if (sindex == -1)
	{					// has either not arrived yet or has departed
		return SEXP_NAN;
	}
...
int eval_any_of(int arg_handler_node, int condition_node)
{
	...
	num_valid_arguments = test_argument_list_for_condition(n,
		condition_node, &num_true, &num_false, &num_known_true, &num_known_false);
		...
		else
		{
			...
			for (objp = GET_FIRST(&obj_used_list); objp != END_OF_LIST(&obj_used_list); objp = GET_NEXT(objp))
			{
				...
				if (ship_explode_area_calc_damage(&origin, &objp->
					pos, (float)inner_radius, (float)outer_radius, (float)max_damage, (float)max_blast, &t_damage,
					&t_blast) == -1)
				{
					continue;
				}
	...
	*/
{
	...
	fireball_create(&origin, fireball_type, FIREBALL_WARP_EFFECT, -1, (float)radius, 0,
		NULL, (float)duration, -1, &m_orient, 0, extra_flags, warp_open_sound_index, warp_close_sound_index);
}
							  ...
							  bool include_players_in_ship_lookup = false)
{
	...
	for (; node >= 0; node = CDR(node))
	{
		...
		if (mission_log_get_time(LOG_SHIP_DEPARTED, ship_name, NULL, NULL) || mission_log_get_time(LOG_SELF_DESTRUCTED,
			ship_name, NULL, NULL) || mission_log_get_time(LOG_SHIP_DESTROYED, ship_name, NULL, NULL))
			continue;
...
int sexp_is_tagged(int node)
{
	...
	{ // This line and the one above were added.
...
int eval_sexp(int cur_node, int referenced_node)
{
	...
	else
	{
		...
		if (Sexp_nodes[cur_node].value == SEXP_NAN)
		{	// if we had a nan, but now don't, reset the value

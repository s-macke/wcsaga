...
extern bool end_string_at_first_hash_symbol(char *src, bool is_wing_ship_name = false);
...
extern void find_and_stuff_or_add(char *id, int *addr, int f_type, char *strlist[], int *total, int max,
                                  char *description);
...
inline void parse_advance(int s) {
    Mp += s;
}

// parse a modular table, returns the number of files matching the "name_check" filter or 0 if it did nothing
extern int parse_modular_table(char *name_check, void (*parse_callback)(char *filename), int path_type =
CF_TYPE_TABLES, int sort_type = CF_SORT_REVERSE);

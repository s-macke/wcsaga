...
bool script_state::CloseHookVarTable() {
    ...
    if (top_ldx >= ohvt_poststack) {
        ...
        Error(LOCATION,
              "CloseHookVarTable() was called with too few objects on the stack; get a coder. (Stack: %d OHVT post: %d OHVT num: %d",
              top_ldx, ohvt_poststack, ohvt_isopen);
        return false;
    }
}
...
int script_state::OutputMeta(char *filename) {
    ...
    if (FS_VERSION_BUILD == 0 && FS_VERSION_REVIS == 0) {
        ...
        fprintf(fp, "\t<h1>Script Output - FSO v%i.%i.%i.%i (%s)</h1>\n", FS_VERSION_MAJOR, FS_VERSION_MINOR,
                FS_VERSION_BUILD, FS_VERSION_REVIS, StateName);
    }
...
bool script_state::ParseCondition(char *filename) {
    ...
    for (condition = script_parse_condition(); condition != CHC_NONE; condition = script_parse_condition()) {
        ...
        if (!chp->AddCondition(sct)) {
            Warning(LOCATION, "Could not add condition to conditional hook in file '%s'; you may have more than %d",
                    filename, MAX_HOOK_CONDITIONS);
        }
    }

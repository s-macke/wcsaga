    ...
    if (*Mp == '@') {
        ...
        if (index > -1 && index < MAX_SEXP_VARIABLES) {
            if (Sexp_variables[index].type & SEXP_VARIABLE_NUMBER) {
                ...
                Error(LOCATION, "Invalid variable type \"%s\" found in mission. Variable must be a number variable!",
                      str);
            }
        } else {
...
int stuff_loadout_list(int *ilp, int max_ints, int lookup_type) {
    ...
    while (*Mp != ')') {
        ...
        if ((lookup_type == MISSION_LOADOUT_SHIP_LIST) && (!(Ship_info[index].flags & SIF_PLAYER_SHIP))) {
            clean_loadout_list_entry();
            Warning(LOCATION,
                    "Ship type \"%s\" found in loadout of mission file. This class is not marked as a player ship...skipping",
                    str);
            continue;
        } else if ((lookup_type == MISSION_LOADOUT_WEAPON_LIST) &&
                   (!(Weapon_info[index].wi_flags & WIF_PLAYER_ALLOWED))) {
            clean_loadout_list_entry();
            Warning(LOCATION,
                    "Weapon type \"%s\" found in loadout of mission file. This class is not marked as a player allowed weapon...skipping",
                    str);
            continue;
        }
...
char *stristr(const char *str, const char *substr) {
    ...
    for (char *start = (char *) str; start <= upper_bound; start++) {
        ...
        if ((*start == substr_ch_upper) || (*start == substr_ch_lower)) {
            ...
            for (char *str_ch = start + 1, *substr_ch = (char *) substr + 1;
        ...
        stristr_continue_outer_loop:/* NO-OP */ ;
    }
...
bool end_string_at_first_hash_symbol(char *src, bool is_wing_ship_name) {
    Assert(src);

    std::string ship_name(src);
    std::string::size_type found = ship_name.find('#');

    if (found != std::string::npos) {
        std::string corrected_ship_name = ship_name.substr(0, found);

        if (is_wing_ship_name) {
            found = ship_name.find_last_of(' ');

            if (found != std::string::npos) {
                corrected_ship_name += ship_name.substr(found);
            }

        }

        strcpy(src, corrected_ship_name.c_str());
...
int parse_modular_table(char *name_check, void (*parse_callback)(char *filename), int path_type, int sort_type) {
    ...
    num_files = cf_get_file_list_preallocated(MAX_TBL_PARTS, tbl_file_arr, tbl_file_names, path_type, name_check,
                                              sort_type);

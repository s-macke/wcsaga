...
// lifetime is calculated by the number of frames and fps.
#define PARTICLE_BITMAP_3D    6        // A bitmap in 3d space, optional data is the bitmap number.  If bitmap is an animation,
// lifetime is calculated by the number of frames and fps.


#define NUM_PARTICLE_TYPES    7
...
void particle_create(vec3d *pos, vec3d *vel, float lifetime, float rad, int type, int optional_data = -1,
                     float tracer_length = -1.0f, struct object *objp = NULL, bool reverse = false);

void particle_create(vec3d *pos, vec3d *norm, vec3d *vel, float lifetime, float rad, int type, int optional_data = -1,
                     int r = 255, int g = 255, int b = 255, float tracer_length = -1.0f, struct object *objp = NULL,
                     bool reverse = false);

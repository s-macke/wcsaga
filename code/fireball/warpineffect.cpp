...
void warpin_render(object* obj, matrix* orient, vec3d* pos, int texture_bitmap_num, float radius, float life_percent,
				   float max_radius, int warp_3d)
{
	vec3d center;
	vec3d vecs[5];
	vertex verts[5];
...
#define IN_PERCENT1 0.10f
	...
	if ((Warp_model >= 0) && (warp_3d || Cmdline_3dwarp))
	{
		...
		model_render(Warp_model, orient, pos, MR_LOCK_DETAIL | MR_NO_LIGHTING | MR_NORMAL | MR_NO_FOGGING |
			MR_NO_CULL);

...
void fireball_play_warphole_open_sound(int ship_class, fireball* fb)
{
	...
	snd_play_3d(&Snds[sound_index], &fireball_objp->pos, &Eye_position, fireball_objp->radius, NULL, 0, 1.0f,
		SND_PRIORITY_DOUBLE_INSTANCE, NULL, range_multiplier); // play warp sound effect
}
...
void fireball_load_data()
{
	...
	for (i = 0; i < Num_fireball_types; i++)
	{
		...
		for (idx = 0; idx < fd->lod_count; idx++)
		{
			...
			fd->lod[idx].bitmap_id = bm_load_animation(fd->lod[idx].filename, &fd->lod[idx].num_frames, &fd->lod[idx].
				fps, NULL, 1);
	...
	case FIREBALL_WARP_EFFECT:
		{
			...
			warpin_render(obj, &obj->orient, &obj->pos, Fireballs[num].current_bitmap, rad, percent_life,
				obj->radius, (Fireballs[num].flags & FBF_WARP_3D));
		}
		break;
...
int fireball_create(vec3d* pos, int fireball_type, int render_type, int parent_obj, float size, int reverse,
					vec3d* velocity, float warp_lifetime, int ship_class, matrix* orient_override, int low_res,
					int extra_flags, int warp_open_sound, int warp_close_sound)
{
	int n, objnum, fb_lod;
	object* obj;
	fireball* fb;

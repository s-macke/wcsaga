...
// Parses EFF file and gets basic information
extern int bm_load_and_parse_eff(char *filename, int dir_type, int *nframes, int *nfps, int *key, ubyte *type);

// Loads a single EFF frame, given the base EFF file name and frame number
extern int bm_load_eff_frame(char *filename, int frame, int dir_type, ubyte eff_type, int *bitmap_id, int *width,
                             int *height);
...
extern int bm_load_animation(char *filename, int *nframes = NULL, int *fps = NULL, int *keyframe = NULL,
                             int can_drop_frames = 0, int dir_type = CF_TYPE_ANY);

//Loads either animation (bm_load_animation) or still image (bm_load)
extern int bm_load_either(char *filename, int *nframes = NULL, int *fps = NULL, int *keyframe = NULL,
                          int can_drop_frames = 0, int dir_type = CF_TYPE_ANY);

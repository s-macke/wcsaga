...
void *bm_malloc(int n, int size) {
    ...
    mprintf(("Bitmap %s allocated %d bytes\n", bm_bitmaps[n].filename, size));
...
int bm_load_sub_slow(char *real_filename, const int num_ext, const char **ext_list, CFILE **img_cfp = NULL,
                     int dir_type = CF_TYPE_ANY) {
    char full_path[MAX_PATH];
    int size = 0, offset = 0;
    int rval = -1;

    rval = cf_find_file_location_ext(real_filename, num_ext, ext_list, dir_type, sizeof(full_path) - 1,
                                     full_path, &size, &offset, 0);
...
int bm_load(char *real_filename) {
    ...
    // Lets find out what type it is
    {
        ...
        const ubyte type_list[NUM_TYPES] =
                {
                        BM_TYPE_DDS,
                        BM_TYPE_TGA,
                        BM_TYPE_PNG,
...
int bm_load_animation(char *real_filename, int *nframes, int *fps, int *keyframe, int can_drop_frames, int dir_type) {
    ...
    for (i = 0; i < anim_frames; i++) {
        ...
        if (type == BM_TYPE_EFF) {
            ...
            if (gr_bm_load(eff_type, n + i, bm_bitmaps[n + i].info.ani.eff.filename,
                           NULL, &anim_width, &anim_height, &bpp, &c_type, &mm_lvl, &img_size)) {
...
int
bm_load_eff_frame(char *filename, int frame, int dir_type, ubyte eff_type, int *bitmap_id, int *width, int *height) {
    int anim_width;
    int anim_height;
    int bpp;
    ubyte c_type;
    ...
    int handle = bm_get_next_handle();

    Assert(strlen(filename) < MAX_FILENAME_LEN);

    memset(&bm_bitmaps[n], 0, sizeof(bitmap_entry));

    bm_bitmaps[n].info.ani.eff.type = eff_type;
    sprintf(bm_bitmaps[n].info.ani.eff.filename, "%s_%.4d", filename, frame);

    // gr_bm_load() returns non-0 on failure
    if (gr_bm_load(eff_type, n, bm_bitmaps[n].info.ani.eff.filename,
                   NULL, &anim_width, &anim_height, &bpp, &c_type, &mm_lvl, &img_size)) {
        return -1;
    }
    ...
    bm_bitmaps[n].info.ani.first_frame = n; // TODO: CONFIRM THIS WILL WORK FINE!
    bm_bitmaps[n].info.ani.num_frames = 1;    // TODO: CONFIRM THIS WILL WORK FINE!
    bm_bitmaps[n].info.ani.fps = (ubyte)/*anim_fps*/1; // FIXME: Irrelevant for EFF streaming... but review this later anyway.
    bm_bitmaps[n].info.ani.keyframe = n;    // TODO: CONFIRM THIS WILL WORK FINE!
    bm_bitmaps[n].bm.w = (short) anim_width;
    bm_bitmaps[n].bm.rowsize = (short) anim_width;
    bm_bitmaps[n].bm.h = (short) anim_height;
    bm_bitmaps[n].bm.flags = 0;
    bm_bitmaps[n].bm.bpp = 0;
    bm_bitmaps[n].bm.true_bpp = (ubyte) bpp;
    bm_bitmaps[n].bm.data = 0;
    bm_bitmaps[n].bm.palette = NULL;
    bm_bitmaps[n].type = BM_TYPE_EFF;
    bm_bitmaps[n].comp_type = c_type;
    bm_bitmaps[n].palette_checksum = 0;
    bm_bitmaps[n].signature = Bm_next_signature++;
    bm_bitmaps[n].handle = handle * MAX_BITMAPS + n;
    bm_bitmaps[n].last_used = -1;
    bm_bitmaps[n].num_mipmaps = mm_lvl;
    bm_bitmaps[n].mem_taken = img_size;
    bm_bitmaps[n].dir_type = dir_type;

    bm_bitmaps[n].load_count++;

    sprintf(bm_bitmaps[n].filename, "%s[%d]", filename, frame);

    if (bitmap_id) {
        *bitmap_id = bm_bitmaps[n].handle;
    }
...
int bm_get_info(int handle, int *w, int *h, ubyte *flags, int *nframes, int *fps) {
    ...
    if (bm_bitmaps[bitmapnum].handle != handle) {
        mprintf(("bm_get_info - %s: bm_bitmaps[%d].handle = %d, handle = %d\n", bm_bitmaps[bitmapnum].filename,
                bitmapnum, bm_bitmaps[bitmapnum].handle, handle));
    }
#endif
...
int bm_release(int handle, int clear_render_targets) {
    ...
    if (!clear_render_targets && ((be->type == BM_TYPE_RENDER_TARGET_STATIC) || (be->type ==
    ...
    if ((be->type == BM_TYPE_ANI) || (be->type == BM_TYPE_EFF)) {
        ...
        for (i = 0; i < total; i++) {
            ...
            bm_bitmaps[first +
                       i].signature = 0xDEADBEEF;                                    // a unique signature identifying the data
            bm_bitmaps[first +
                       i].palette_checksum = 0xDEADBEEF;                            // checksum used to be sure bitmap is in current palette

            // bookeeping
            bm_bitmaps[first +
                       i].ref_count = -1;                                    // Number of locks on bitmap.  Can't unload unless ref_count is 0.
...
void bm_page_in_texture(int bitmapnum, int nframes) {
    ...
    Assertion((bm_bitmaps[n].handle == bitmapnum), "bitmapnum = %i, n = %i, bm_bitmaps[n].handle = %i", bitmapnum, n,
              bm_bitmaps[n].handle);
...
void bm_page_in_stop() {
    ...
    for (i = 0; i < MAX_BITMAPS; i++) {
        if ((bm_bitmaps[i].type != BM_TYPE_NONE) && (bm_bitmaps[i].type != BM_TYPE_RENDER_TARGET_DYNAMIC) &&
            (bm_bitmaps[i].type != BM_TYPE_RENDER_TARGET_STATIC)) {
            if (bm_bitmaps[i].preloaded) {
                if (bm_preloading) {
                    ...
                    bm_lock(bm_bitmaps[i].handle, (bm_bitmaps[i].used_flags == BMP_AABITMAP) ? 8 : 16,
                            bm_bitmaps[i].used_flags);
...
void bm_get_components(ubyte *pixel, ubyte *r, ubyte *g, ubyte *b, ubyte *a) {
    ...
    if (r != NULL) {
        if (bit_32) {
            ...
            *r = ubyte(((((unsigned short *) pixel)[0] & Gr_current_red->mask) >> Gr_current_red->shift) *
                       Gr_current_red->scale);
        }
    }
    if (g != NULL) {
        if (bit_32) {
            ...
            *g = ubyte(((((unsigned short *) pixel)[0] & Gr_current_green->mask) >> Gr_current_green->shift) *
                       Gr_current_green->scale);
        }
    }
    if (b != NULL) {
        if (bit_32) {
            ...
            *b = ubyte(((((unsigned short *) pixel)[0] & Gr_current_blue->mask) >> Gr_current_blue->shift) *
                       Gr_current_blue->scale);
        }
    }
        ...
        if (bm_bitmaps[i].type != BM_TYPE_NONE) {
            if (bm_bitmaps[i].data_size) {
                nprintf(("BMP DEBUG", "BMPMAN = num: %d, name: %s, handle: %d - (%s) size: %.3fM\n", i,
                        bm_bitmaps[i].filename, bm_bitmaps[i].handle,
                        bm_bitmaps[i].data_size ? NOX("*LOCKED*") : NOX(""), ((float) bm_bitmaps[i].data_size /
                                                                              1024.0f) / 1024.0f));
            } else {
                nprintf(("BMP DEBUG", "BMPMAN = num: %d, name: %s, handle: %d\n", i, bm_bitmaps[i].filename,
                        bm_bitmaps[i].handle));
            }
        }
    }
...
int bm_make_render_target(int width, int height, int flags) {
    ...
    bm_bitmaps[n].type = (flags & BMP_FLAG_RENDER_TARGET_STATIC) ? BM_TYPE_RENDER_TARGET_STATIC :
                         BM_TYPE_RENDER_TARGET_DYNAMIC;
    bm_bitmaps[n].signature = Bm_next_signature++;
...
int bm_is_render_target(int bitmap_id) {
    ...
    if (!((bm_bitmaps[n].type == BM_TYPE_RENDER_TARGET_STATIC) || (bm_bitmaps[n].type ==
                                                                   BM_TYPE_RENDER_TARGET_DYNAMIC))) {
        return 0;
    }

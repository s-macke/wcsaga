...
Flag exe_params[] =
        {
...

                {
                        ...
                        EASY_DEFAULT,
                        "Game Speed",
                        "http://www.hard-light.net/wiki/index.php/Command-Line_Reference#-img2dds",
                },
                {
                        "-no_vsync",
                ...
                },
                {
                        ...
                        EASY_DEFAULT,
                        "Gameplay",
                        "http://www.hard-light.net/wiki/index.php/Command-Line_Reference#-tbp",
                }, // TBP warp effects -Et1
                {
                        ...
                        EASY_DEFAULT,
                        "Gameplay",
                        "http://www.hard-light.net/wiki/index.php/Command-Line_Reference#-wcsaga",
                },
...

                {
                        ...
                        EASY_DEFAULT,
                        "Troubleshoot",
                        "http://www.hard-light.net/wiki/index.php/Command-Line_Reference#-oldfire",
                },
                {
                        "-nohtl",
                ...
                },
                {
                        ...
                        EASY_DEFAULT,
                        "Troubleshoot",
                        "http://www.hard-light.net/wiki/index.php/Command-Line_Reference#-safeloading",
                },
                {
                        "-query_speech",
                ...
                {
                        "-d3d_bad_tsys",
                        "Enable inefficient textures",
                        true,
                        0,
                        EASY_DEFAULT,
                        "Troubleshoot",
                        "http://www.hard-light.net/wiki/index.php/Command-Line_Reference#-d3d_bad_tsys",
                },
                {
                        "-novbo",
                        "Disable OpenGL VBO",
                ...
                },
                {
                        ...
                        EASY_DEFAULT,
                        "Troubleshoot",
                        "http://www.hard-light.net/wiki/index.php/Command-Line_Reference#-noibx",
                },
                {
                        "-loadallweps",
                ...
                },
                {
                        ...
                        EASY_DEFAULT,
                        "Troubleshoot",
                        "http://scp.indiegames.us/mantis/view.php?id=1669",
                },
                ...
                },
                {
                        ...
                        EASY_DEFAULT,
                        "Dev Tool",
                        "http://www.hard-light.net/wiki/index.php/Command-Line_Reference#-timerbar",
                },
                {
                        "-stats",
                        "Show statistics",
...
cmdline_parm bit32_arg("-32bit",
                       NULL);                // (only here for retail compatibility reasons, doesn't actually do anything)
...
cmdline_parm nomotiondebris_arg("-nomotiondebris",
                                NULL); // Cmdline_nomotiondebris  -- Removes those ugly floating rocks -C
...
cmdline_parm noemissive_arg("-no_emissive_light",
                            NULL);        // Cmdline_no_emissive  -- don't use emissive light in OGL
...
float Cmdline_fov = gr_zoom_to_fov(Viewer_zoom_default);
float Cmdline_ogl_spec = 80.0f;
int Cmdline_ambient_factor = 95;
int Cmdline_cell = 0;
int Cmdline_decals = 0;
int Cmdline_env = 1;
int Cmdline_mipmap = 0;
int Cmdline_missile_lighting = 1;
int Cmdline_glow = 1;
...
int Cmdline_enable_3d_shockwave = 1; // WC Saga look
int Cmdline_postprocess = 0;
int Cmdline_bloom_intensity = 100; // WC Saga look
...
int Cmdline_cache_bitmaps = 1;    // caching of bitmaps between missions (faster loads, can hit swap on reload with <512 Meg RAM though) - taylor
...
cmdline_parm dualscanlines_arg("-dualscanlines",
                               NULL); // Cmdline_dualscanlines  -- Change to phreaks options including new targetting code
cmdline_parm orb_radar("-orbradar", NULL);            // Cmdline_orb_radar
cmdline_parm rearm_timer_arg("-rearm_timer", NULL);    // Cmdline_rearm_timer
cmdline_parm targetinfo_arg("-targetinfo",
                            NULL);    // Cmdline_targetinfo  -- Adds ship name/class to right of target box -C

int Cmdline_ballistic_gauge = 0;    // WMCoolmon's gauge thingy
int Cmdline_dualscanlines = 1;
...
int Cmdline_snd_preload = 1; // preload game sounds during mission load
...
cmdline_parm mpnoreturn_arg("-mpnoreturn",
                            NULL);    // Cmdline_mpnoreturn  -- Removes 'Return to Flight Deck' in respawn dialog -C
...
void parm_stuff_args(cmdline_parm *parm, char *cmdline) {
    ...
    if (size > 0) {
        ...
        if (saved_args != NULL) {
            size += (strlen(saved_args) +
                     1);    // an ',' is used as a separator when combining, so be sure to account for it
        }
    ...
    for (parmp = GET_FIRST(&Parm_list); parmp != END_OF_LIST(&Parm_list); parmp = GET_NEXT(parmp)) {
        ...
        do {
            ...
            while (true) {
                ...
                if ((*(cmdline_offset + strlen(parmp->name))) && !is_extra_space(*(cmdline_offset +
                                                                                   strlen(parmp->name)))) {
        ...
        if (token[0] == '-') {
            ...
            if (parm_found == 0) {
                ...
                if (!stricmp(token, "-help") || !stricmp(token, "--help") || !stricmp(token, "-h") || !stricmp(token,
                                                                                                               "-?")) {
                    if (SAGA_VERSION_REVIS == 0) {
                        printf("Wing Commander Saga, version %i.%i.%i\n", SAGA_VERSION_MAJOR, SAGA_VERSION_MINOR,
                               SAGA_VERSION_BUILD);
                    } else {
                        printf("Wing Commander Saga, version %i.%i.%i.%i\n", SAGA_VERSION_MAJOR, SAGA_VERSION_MINOR,
                               SAGA_VERSION_BUILD, SAGA_VERSION_REVIS);
                    }
                    ...
                    while (exe_params[p].name[0] == '-') {
                        ...
                        printf("    [ %s ]%*s- %s\n", exe_params[p].name, (STR_SIZE - sp - 1), NOX(" "),
                               exe_params[p].desc);
                        p++;
                    }
...
void os_init_cmdline(char *cmdline) {
    ...
    std::string data_path(full_path);

    size_t pos = data_path.rfind(".app");
    data_path = data_path.substr(0, pos-1);
    data_path += "/data/cmdline_fso.cfg";

    fp = fopen(data_path.c_str(), "rt");
#else
    fp = fopen("data/cmdline_fso.cfg", "rt");
#endif
    ...
    snprintf(cmdname, MAX_PATH - 1, "%s%s%s%s%s%s%s%s%s", detect_home(), DIR_SEPARATOR_STR, Osreg_company_name,
             DIR_SEPARATOR_STR, Osreg_user_dir, DIR_SEPARATOR_STR, "data", DIR_SEPARATOR_STR, "cmdline_fso.cfg");
    fp = fopen(cmdname, "rt");
    ...
    if (!fp) {
        ...
        memset(cmdname, 0, sizeof(cmdname));
        snprintf(cmdname, MAX_PATH - 1, "%s%s%s%s%s%s%s%s%s", detect_home(), DIR_SEPARATOR_STR, Osreg_company_name,
                 DIR_SEPARATOR_STR, Osreg_user_dir, DIR_SEPARATOR_STR, "data", DIR_SEPARATOR_STR, "cmdline.cfg");
        fp = fopen(cmdname, "rt");
    }
...
// Sets externed variables used for communication cmdline information
{
    ...
    if (fov_arg.found()) {
        ...
        if (Cmdline_fov > 0.1f && Cmdline_fov < (0.49f * PI)) {
            Viewer_zoom_default = gr_fov_to_zoom(Cmdline_fov);
        } else {
            Cmdline_fov = gr_zoom_to_fov(Viewer_zoom_default);
        }
    }
    ...
    if (get_flags_arg.found()) {
        char cmdname[MAX_PATH];
        memset(cmdname, 0, sizeof(cmdname));
        snprintf(cmdname, MAX_PATH - 1, "%s%s%s%s%s%s%s", detect_home(), DIR_SEPARATOR_STR, Osreg_company_name,
                 DIR_SEPARATOR_STR, Osreg_user_dir, DIR_SEPARATOR_STR, "flags.lch");

        FILE *fp = fopen(cmdname, "w");
    ...
    if (output_sexp_arg.found()) {
        ...
        memset(cmdname, 0, sizeof(cmdname));
        snprintf(cmdname, MAX_PATH - 1, "%s%s%s%s%s%s%s", detect_home(), DIR_SEPARATOR_STR, Osreg_company_name,
                 DIR_SEPARATOR_STR, Osreg_user_dir, DIR_SEPARATOR_STR, "sexps.html");

        output_sexps(cmdname);
    }
    ...
    if (wcsaga.found()) {
        // Activated by WC Saga per default
        //Cmdline_wcsaga = 1;
    }

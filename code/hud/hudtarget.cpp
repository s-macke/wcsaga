...
int TARGET_SHIP_IGNORE_FLAGS = (SF_EXPLODED | SF_DEPART_WARP | SF_DYING | SF_ARRIVING_STAGE_1 |
                ...
                { // normal HUD
                        { // GR_640
                                // based on the # of primaries
                                {
                                        509,
                                        273
                                },                // top of weapon gauge, first frame, always
                                {
                                        497,
                                        293
                ...
                { // normal HUD
                        { // GR_640
                                // based on the # of secondaries
                                {
                                        497,
                                        318
                                },                // bottom of gauge, 0 secondaries
                                {
                                        497,
                                        318
                                },                // bottom of gauge, 1 secondaries
                                {
                                        497,
                                        317
                                },                // middle of gauge, 2 secondaries AND middle of gauge, 3 secondaries
                                {
                                        497,
                                        326
                                },                // bottom of gauge, 2 secondaries AND middle of gauge, 3 secondaries
                                {
                                        497,
                                        335
                                }                // bottom of gauge, 3 secondaries
                        },
                        { // GR_1024
                                // based on the # of secondaries
                                {
                ...
                { // GR_640
                        {
                                530,
                                285
                        },                // fire-linked thingie, for the first primary
                        {
                                530,
                                295
                        }                // fire-linked thingie, for the second primary
                },
                { // GR_1024
                        {
                                913,
                ...
                { // GR_640
                        {
                                536,
                                285
                        },                // weapon name, first primary
                        {
                                536,
                                295
                        }                // weapon name, second primary
                },
                { // GR_1024
                        {
                                919,
...
int Weapon_secondary_y[GR_NUM_RESOLUTIONS][3] =
        {
                ...
                { // GR_1024
                        525,                        // y location of where to draw text for the third secondary
                        534,                        // y location of where to draw text for the third secondary
                        543                        // y location of where to draw text for the third secondary
                }
        };
int Weapon_primary_y[GR_NUM_RESOLUTIONS][2] =
        {
                ...
                { // GR_1024
                        501,                        // y location of where to draw text for the first primary
                        511                            // y location of where to draw text for the second primary
                }
        };
...
#define MAX_WEAPON_FLASH_LINES            MAX_SHIP_WEAPONS
...
int hud_target_untargetable_awacs(object *objp) {
    // if objp is ship object, first check if can be targeted with team info
    if (objp->type == OBJ_SHIP) {
        if (Player_ship != NULL) {
            if (ship_is_visible_by_team(objp, Player_ship)) {
    ...
    // check for invalid status
    if ((Player_ship != NULL) && (awacs_get_level(objp, Player_ship) < 0.0f)) {
        return 1;
    }
...
void hud_make_shader(shader *sh, int r, int g, int b, float dimmer = 1000.0f) {
    ...
    R = static_cast<ubyte>(fl2i(r * tmp));
    G = static_cast<ubyte>(fl2i(r * tmp)); // fl2i(g * tmp);  WTF??
    B = static_cast<ubyte>(fl2i(r * tmp)); // fl2i(b * tmp);  WTF??
    A = static_cast<ubyte>(fl2i((i2fl(r) / dimmer) * (i2fl(HUD_color_alpha) / 15.0f) * 255.0f));
    ...
    if (!Weapon_gauges_loaded) {
        ...
        for (hud_index = 0; hud_index < NUM_HUD_SETTINGS; hud_index++) {
            for (gauge_index = 0; gauge_index < NUM_WEAPON_GAUGES; gauge_index++) {
                Weapon_gauges[hud_index][gauge_index].first_frame = bm_load_animation(Weapon_gauge_fnames[hud_index][
                                                                                              gr_screen.res][gauge_index],
                                                                                      &Weapon_gauges[hud_index][gauge_index].num_frames);
                ...
                if (Weapon_gauges[hud_index][gauge_index].first_frame < 0) {
                    ...
                    if (hud_index) {
                        ...
                        Weapon_gauges[hud_index][gauge_index].first_frame = bm_load_animation(Weapon_gauge_fnames[0][
                                                                                                      gr_screen.res][gauge_index],
                                                                                              &Weapon_gauges[hud_index][gauge_index].num_frames);
            ...
            for (hud_index = 1; hud_index < NUM_HUD_SETTINGS; hud_index++) {
                for (i = 0; i < GR_NUM_RESOLUTIONS; i++) {
                    for (k = 0; k < 2; k++) {
                        ...
                        for (j = 0; j < 5; j++) {
                            Weapon_gauge_secondary_coords[hud_index][i][j][k] = Weapon_gauge_secondary_coords[0][i][j][
                                    k];
                        }
...
void hud_init_homing_beep() {
    ...
    Homing_beep.precalced_interp = (Homing_beep.max_cycle_dist -
                                    Homing_beep.min_cycle_dist) / (Homing_beep.max_cycle_time -
                                                                   Homing_beep.min_cycle_time);
}
...
void hud_init_targeting() {
    ...
    if (!Lead_indicator_gauge_loaded) {
        Lead_indicator_gauge.first_frame = bm_load_animation(Lead_fname[Hud_reticle_style][gr_screen.
                res], &Lead_indicator_gauge.num_frames);
...
void hud_target_missile(object *source_obj, int next_flag) {
    ...
    if (aip->target_objnum != -1) {
        ...
        if (target_objp->type == OBJ_WEAPON && Weapon_info[Weapons[target_objp->instance].weapon_info_index].subtype ==
                                               WP_MISSILE) {    // must be a missile
    ...
    if (!target_found) {
        ...
        for (so = advance_ship(startShip, next_flag); so != startShip; so = advance_ship(so, next_flag)) {
            ...
            if (!(Ship_info[Ships[A->instance].ship_info_index].flags &
                  SIF_BOMBER) && !(A->flags & OF_TARGETABLE_AS_BOMB)) {
                continue;
            }
...
void evaluate_ship_as_closest_target(esct *esct) {
    ...
    // bail if harmless
    if (Ship_info[esct->shipp->ship_info_index].class_type > -1 && !(Ship_types[Ship_info[esct->shipp->
            ship_info_index].class_type].hud_bools &
                                                                     STI_HUD_TARGET_AS_THREAT)) {
        return;
    }
    ...
    if (Ship_info[esct->shipp->ship_info_index].flags & (SIF_BIG_SHIP | SIF_HUGE_SHIP)) {
        for (ss = GET_FIRST(&esct->shipp->subsys_list); ss != END_OF_LIST(&esct->shipp->subsys_list);
             ss = GET_NEXT(ss)) {
            ...
            if ((ss->system_info->type == SUBSYSTEM_TURRET) && (ss->current_hits > 0)) {
                ...
                if (esct->check_all_turrets || (ss->turret_enemy_objnum == esct->attacked_objnum)) {
                    ...
                    if (!esct->turret_attacking_target || (esct->turret_attacking_target &&
                                                           (ss->turret_enemy_objnum == esct->attacked_objnum))) {
                        vec3d gsubpos;
...
void hud_render_tail_missile_triangle(float ang, float xpos, float ypos, float cur_dist, int draw_solid,
                                      int draw_inside) {
    float x1 = 0.0f;
    float x2 = 0.0f;
    float y1 = 0.0f;
...
void hud_render_split_missile_triangle(float ang, float xpos, float ypos, float cur_dist, int draw_solid,
                                       int draw_inside) {
    ...
    if (cur_dist < Min_warning_missile_dist) {
        ...
        triangle_sep = (cur_dist / Max_warning_missile_dist) * (Max_offscreen_tri_seperation[gr_screen.res] +
                                                                Max_front_seperation[gr_screen.res]);
    }
...
void hud_render_triangle(vec3d *hostile_pos, int aspect_flag, int show_interior, int split_tri) {
    ...
    if (hostile_vertex.codes == 0) {// on screen
        ...
        if (!(hostile_vertex.flags & PF_OVERFLOW)) {  // make sure point projected
            ...
            mag_squared = (projected_x - Hud_reticle_center[gr_screen.res][0]) * (projected_x -
                                                                                  Hud_reticle_center[gr_screen.
                                                                                          res][0]) +
                          (projected_y - Hud_reticle_center[gr_screen.res][1]) *
                          (projected_y - Hud_reticle_center[gr_screen.
                                  res][1]);
...
void hud_show_homing_missiles() {
    ...
    if (nearest_dist < Homing_beep.max_cycle_dist) {
        ...
        cycle_time = Homing_beep.min_cycle_time + (nearest_dist - Homing_beep.min_cycle_dist) *
                                                  Homing_beep.precalced_interp;
        ...
        if ((delta_time * 1000) > cycle_time) {
            ...
            if (closest_is_aspect) {
                Homing_beep.snd_handle = snd_play(&Snds[SND_PROXIMITY_ASPECT_WARNING], 0.0f, 1.0f,
                                                  SND_PRIORITY_MUST_PLAY);
            } else {
                Homing_beep.snd_handle = snd_play(&Snds[SND_PROXIMITY_WARNING], 0.0f, 1.0f, SND_PRIORITY_MUST_PLAY);
            }
        }
    }
}
                ...
                if (!(target_point.flags & PF_OVERFLOW)) {  // make sure point projected
                    ...
                    switch (mobjp->type) {
                        case OBJ_WEAPON:
                            ...
                            bound_rval = model_find_2d_bound_min(modelnum, &mobjp->orient, &mobjp->
                                    pos, &x1, &y1, &x2, &y2);
                            break;
...
void hud_show_message_sender() {
    ...
    if (hud_gauge_active(HUD_OFFSCREEN_INDICATOR)) {
        if (target_point.codes != 0) { // target center is not on screen
            ...
            if ((OBJ_INDEX(targetp) != Player_ai->target_objnum) || (Message_shipnum == Objects[Player_ai->
                    target_objnum].instance)) {
    ...
    if (Player->target_is_dying <= 0) {
        ...
        switch (targetp->type) {
            ...
            case OBJ_DEBRIS:
                ...
                bound_rc = submodel_find_2d_bound_min(modelnum, Debris[targetp->instance].submodel_num, &targetp->
                        orient, &targetp->pos, &x1, &y1, &x2, &y2);
                if (bound_rc != 0) {
                    draw_box = FALSE;
                }
                break;
...
void hud_show_targeting_gauges(float frametime, int in_cockpit) {
    ...
    if (in_cockpit) {
        ...
        if (targetp->type == OBJ_SHIP) {
            if ((Ships[targetp->instance].flags & SF_SCANNABLE) != 0) {
...
void hud_show_hostile_triangle() {
    ...
    for (so = GET_FIRST(&Ship_obj_list); so != END_OF_LIST(&Ship_obj_list); so = GET_NEXT(so)) {
        ...
        if (Ship_info[sp->ship_info_index].class_type > -1 && !(Ship_types[Ship_info[sp->ship_info_index].class_type].
                hud_bools & STI_HUD_SHOW_ATTACK_DIRECTION)) {
            continue;
        }
...
void hud_show_lead_indicator(vec3d *target_world_pos) {
    ...
    if (vm_vec_mag_quick(&target_moving_direction) < 0.1f)        // Find distance!
        ...
        polish_predicted_target_pos(wip, targetp, target_world_pos, &Players[Player_num].lead_target_pos,
                                    dist_to_target, &last_delta_vector, 1); // Not used:, float time_to_enemy)
    }
        ...
        if (!(lead_target_vertex.flags & PF_OVERFLOW)) {
            ...
            if (Hud_lead_alternate) {
                ...
                if (target_vertex.codes == 0) { // on screen
                    ...
                    if (!(target_vertex.flags & PF_OVERFLOW)) {
                        ...
                        if (indicator_frame >= 0) {
                            ...
                            float reticle2_target_sx = target_vertex.sx -
                                                       Lead_indicator_half[Hud_reticle_style][gr_screen.res][0] -
                                                       lead_target_vertex.sx;
                            float reticle2_target_sy = target_vertex.sy -
                                                       Lead_indicator_half[Hud_reticle_style][gr_screen.res][1] -
                                                       lead_target_vertex.sy;
                            ...
                            hud_aabitmap(indicator_frame, fl2i(reticle2_target_sx) + HUD_nose_x,
                                         fl2i(reticle2_target_sy) + HUD_nose_y);
                        }
                    }
                }
            } else {
                if (indicator_frame >= 0) {
                    ...
                    hud_aabitmap(indicator_frame, fl2i(sx - Lead_indicator_half[Hud_reticle_style][gr_screen.res][0]),
                                 fl2i(sy - Lead_indicator_half[Hud_reticle_style][gr_screen.res][1]));
                }
            }
        }
    }
...
void hud_draw_offscreen_indicator(vertex *target_point, vec3d *tpos, float distance, int draw_solid) {
    ...
    if (dist_behind < 0) {    // still in front of player, but not in view
        ...
        triangle_sep = dist_behind * Max_offscreen_tri_seperation[gr_screen.res] +
                       Max_offscreen_tri_seperation[gr_screen.res];
    }
...
void hud_show_afterburner_gauge() {
    ...
    if (!(Ship_info[Player_ship->ship_info_index].flags & SIF_AFTERBURNER)) {
        ...
        percent_left = Player_ship->afterburner_fuel / Ship_info[Player_ship->ship_info_index].
    ...
    if (clip_h > 0) {
        hud_aabitmap_ex(Aburn_bar_gauge.first_frame, current_hud->Aburn_coords[0] + nose_offset_x,
                        current_hud->Aburn_coords[1] + nose_offset_y, w, clip_h, 0, 0);
    }

    if (clip_h <= current_hud->Aburn_size[0]) {
        hud_aabitmap_ex(Aburn_bar_gauge.first_frame + 1, current_hud->Aburn_coords[0] + nose_offset_x,
                        current_hud->Aburn_coords[1] + clip_h + nose_offset_y, w, h - clip_h, 0, clip_h);
    }
}
...
void hud_show_weapon_energy_gauge() {
    ...
    if (use_new_gauge) {
        ...
        remaining = max_w * ((float) Player_ship->weapon_energy / (float) Ship_info[Player_ship->ship_info_index].
                max_weapon_reserve);
        if (remaining > 0) {
            hud_set_gauge_color(HUD_WEAPONS_ENERGY, HUD_C_BRIGHT);
            for (y = 0; y < 10; y++) {
        ...
        for (x = 0; x < Player_ship->weapons.num_primary_banks; x++) {
            ...
            //Draw the bar graph
            remaining = (max_w - 4) * ((float) Player_ship->weapons.primary_bank_ammo[x] / (float) Player_ship->weapons.
                    primary_bank_start_ammo[x]);
            if (remaining > 0) {
        ...
        if (percent_left <= 0.3) {
            ...
            gr_string(current_hud->Wenergy_text_coords[0] + nose_offset_x, current_hud->Wenergy_text_coords[1] +
                                                                           nose_offset_y, buf);
        }
        ...
        if (clip_h > 0) {
            hud_aabitmap_ex(Wenergy_bar_gauge.first_frame + 2, current_hud->Wenergy_coords[0] + nose_offset_x,
                            current_hud->Wenergy_coords[1] + nose_offset_y, w, clip_h, 0, 0);
        }

        if (clip_h <= current_hud->Wenergy_size[0]) {
            hud_aabitmap_ex(Wenergy_bar_gauge.first_frame + 3, current_hud->Wenergy_coords[0] + nose_offset_x,
                            current_hud->Wenergy_coords[1] + clip_h + nose_offset_y, w, h - clip_h, 0, clip_h);
        }
...
void hud_show_cmeasure_gauge() {
    ...
    // blit the background
    hud_aabitmap(Cmeasure_gauge.first_frame, Cm_coords[gr_screen.res][0], Cm_coords[gr_screen.res][1]);
    ...
    gr_printf(Cm_text_val_coords[gr_screen.res][0], Cm_text_val_coords[gr_screen.res][1], NOX("%02d"),
...
void hud_show_weapons() {
    ...
    hud_aabitmap(Weapon_gauges[ballistic_hud_index][0].first_frame,
                 Weapon_gauge_primary_coords[ballistic_hud_index][gr_screen.res][0][0],
                 Weapon_gauge_primary_coords[ballistic_hud_index][gr_screen.res][0][1]);

    emp_hud_string(Weapon_title_coords[ballistic_hud_index][gr_screen.res][0],
                   Weapon_title_coords[ballistic_hud_index][gr_screen.res][1], EG_WEAPON_TITLE, XSTR("weapons", 328));

    char ammo_str[32];
    int i, w, h;
    int res_diff = 12;
    int name_res_diff = 10;
    int y = gr_screen.res == 0 ? 293 : 509;
    int name_y = gr_screen.res == 0 ? 285 : 501;
    ...
    for (i = 0; i < np; i++) {
        hud_set_gauge_color(HUD_WEAPONS_GAUGE);
        if (i == 1) {
            hud_aabitmap(Weapon_gauges[ballistic_hud_index][1].first_frame,
                         Weapon_gauge_primary_coords[ballistic_hud_index][gr_screen.res][1][0], y);

        } else if (i != 0) {
            if (New_weapon.first_frame != -1)
                hud_aabitmap(New_weapon.first_frame, Weapon_gauge_primary_coords[ballistic_hud_index][gr_screen.
                        res][1][0], y);
        }

        strcpy_s(name, (Weapon_info[sw->primary_bank_weapons[i]].alt_name[0]) ? Weapon_info[sw->
                primary_bank_weapons[i]].alt_name : Weapon_info[sw->primary_bank_weapons[i]].name);
        if (Lcl_gr) {
        ...
        if (Weapon_info[sw->primary_bank_weapons[0]].hud_image_index != -1) {
            hud_aabitmap(Weapon_info[sw->primary_bank_weapons[i]].hud_image_index,
                         Weapon_pname_coords[gr_screen.res][0][0], name_y);
        } else {
    ...
    hud_aabitmap(Weapon_gauges[ballistic_hud_index][2].first_frame,
                 Weapon_gauge_primary_coords[ballistic_hud_index][gr_screen.res][1][0], y);
    ...
    for (i = 0; i < ns; i++) {
        ...
        if (i != 0)
            hud_aabitmap(Weapon_gauges[ballistic_hud_index][3].first_frame,
                         Weapon_gauge_secondary_coords[ballistic_hud_index][gr_screen.res][3][0], y);
        ...
        if (sw->current_secondary_bank == i) {
            ...
            if (wip->hud_image_index != -1) {
                hud_aabitmap(wip->hud_image_index, Weapon_secondary_name_x[gr_screen.res], name_y);
            } else {
                emp_hud_string(Weapon_secondary_name_x[gr_screen.res], name_y, i ? EG_WEAPON_S1 : EG_WEAPON_S2,
                               weapon_name);
            }
            ...
            if ((sw->secondary_bank_ammo[i] > 0) && (sw->current_secondary_bank >= 0)) {
                ...
                if ((ms_till_fire >= 500) && ((wip->fire_wait >= 1) || (ms_till_fire > wip->fire_wait * 1000))) {
                    emp_hud_printf(Weapon_secondary_reload_x[gr_screen.res], name_y, EG_NULL, "%d",
                                   fl2i(ms_till_fire / 1000.0f + 0.5f));
                }
            }
        } else {
            emp_hud_string(Weapon_secondary_name_x[gr_screen.res], name_y, i ? EG_WEAPON_S1 : EG_WEAPON_S2,
    ...
    y -= 0;
    hud_aabitmap(Weapon_gauges[ballistic_hud_index][4].first_frame,
                 Weapon_gauge_secondary_coords[ballistic_hud_index][gr_screen.res][0][0], y);
...
int hud_sensors_ok(ship *sp, int show_msg) {
    ...
    if ((Game_skill_level == 0) || (Game_mode & GM_DEAD) || ((Game_mode & GM_MULTIPLAYER) && (Net_player->flags &
...
void hud_auto_target_icon() {
    ...
    hud_aabitmap(Toggle_gauge.first_frame + frame_offset, Toggle_target_gauge_coords[gr_screen.res][0],
                 Toggle_target_gauge_coords[gr_screen.res][1]);
    ...
    gr_string(Hud_toggle_coords[gr_screen.res][TOGGLE_TEXT_AUTOT][0],
              Hud_toggle_coords[gr_screen.res][TOGGLE_TEXT_AUTOT][1], XSTR("auto", 1463));
    gr_string(Hud_toggle_coords[gr_screen.res][TOGGLE_TEXT_TARGET][0],
              Hud_toggle_coords[gr_screen.res][TOGGLE_TEXT_TARGET][1], XSTR("target", 1465));
}
...
void hud_auto_speed_match_icon() {
    ...
    hud_aabitmap(Toggle_gauge.first_frame + frame_offset, Toggle_speed_gauge_coords[gr_screen.res][0],
                 Toggle_speed_gauge_coords[gr_screen.res][1]);
    ...
    gr_string(Hud_toggle_coords[gr_screen.res][TOGGLE_TEXT_AUTOS][0],
              Hud_toggle_coords[gr_screen.res][TOGGLE_TEXT_AUTOS][1], XSTR("auto", 1463));
    gr_string(Hud_toggle_coords[gr_screen.res][TOGGLE_TEXT_SPEED][0],
              Hud_toggle_coords[gr_screen.res][TOGGLE_TEXT_SPEED][1], XSTR("speed", 1464));
}

// display the auto-targeting and auto-speed-matching icons on the HUD
void hud_show_auto_icons() {
    ActivateAlignment right(ALIGNMENT_RIGHT);
    int show_flag;
...
int hud_target_closest_repair_ship(int goal_objnum) {
    ...
    if (nearest_obj != &obj_used_list) {
        ...
        if (goal_objnum == -1) {
            HUD_sourced_printf(HUD_SOURCE_HIDDEN, XSTR("No support ships in area.  Use messaging to call one in.",
                                                       332));
        }
        rval = 0;
    }
    ...
    for (i = 0; i < MAX_TRANSMIT_TARGETS; i++) {
        if (Transmit_target_list[transmit_index].objnum >= 0) {
            ...
            if (Player_ai->target_objnum == transmit_objnum) {
                ...
                if (Transmit_target_list[transmit_index].objsig == Objects[Transmit_target_list[transmit_index].
                        objnum].signature) {
...
void hud_stuff_ship_name(char *ship_name_text, ship *shipp) {
    // print ship name
    if (((Iff_info[shipp->team].flags & IFFF_WING_NAME_HIDDEN) && (shipp->wingnum != -1)) || (shipp->flags2 &
                                                                                              SF2_HIDE_SHIP_NAME)) {
        *ship_name_text = 0;
    } else {
        ...
        // handle hash symbol
        end_string_at_first_hash_symbol(ship_name_text, (shipp->wingnum != -1));
...
void hud_stuff_ship_class(char *ship_class_text, ship *shipp) {
    ...
    if (!*ship_class_text) {
        strcpy(ship_class_text, (Ship_info[shipp->ship_info_index].alt_name[0]) ? Ship_info[shipp->ship_info_index].
                alt_name : Ship_info[shipp->ship_info_index].name);
    }

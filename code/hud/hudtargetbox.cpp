...
bool Highlight_tagged_ships = true;
...
void hud_targetbox_init() {
    if (!Target_view_gauge_loaded) {
        Target_view_gauge.first_frame = bm_load_animation(Target_view_fname[gr_screen.res], &Target_view_gauge.
                num_frames);
    ...
    if (!Target_view_integrity_gauge_loaded) {
        Target_view_integrity_gauge.first_frame = bm_load_animation(Target_integ_fname[gr_screen.
                res], &Target_view_integrity_gauge.num_frames);
    ...
    if (!Target_view_extra_loaded) {
        Target_view_extra.first_frame = bm_load_animation(Target_extra_fname[gr_screen.res], &Target_view_extra.
                num_frames);
...
void hud_render_target_background() {
    ...
    hud_aabitmap(Target_view_gauge.first_frame, Targetbox_coords[gr_screen.res][TBOX_BACKGROUND][0],
                 Targetbox_coords[gr_screen.res][TBOX_BACKGROUND][1]);
...
void hud_render_target_setup(vec3d *camera_eye, matrix *camera_orient, float zoom) {
    ...
    int window_x = Target_window_coords[gr_screen.res][0];
    int window_y = Target_window_coords[gr_screen.res][1];
    int window_width = Target_window_coords[gr_screen.res][2];
    int window_height = Target_window_coords[gr_screen.res][3];

    gr_unsize_screen_pos(&window_x, &window_y);
    gr_unsize_screen_pos(&window_width, &window_height);
    gr_resize_screen_pos(&window_x, &window_y);
    gr_resize_screen_pos(&window_width, &window_height, true);

    if (Keeping_aspect_ratio) {
        zoom *= (float) Target_window_coords[gr_screen.res][2] / (float) window_width;
    }
    ...
    {
        KeepAspectRatio keep(false);
        HUD_set_clip(window_x, window_y, window_width, window_height);
    }
...
void hud_blit_target_foreground() {
    ...
    hud_aabitmap(Target_view_gauge.first_frame + 1, Targetbox_coords[gr_screen.res][TBOX_BACKGROUND][0],
                 Targetbox_coords[gr_screen.res][TBOX_BACKGROUND][1]);
}
...
void hud_targetbox_show_extra_ship_info(ship *target_shipp, object *target_objp) {
    ...
    if (not_training && (hud_gauge_active(HUD_TARGET_MONITOR_EXTRA_DATA)) && (Targetbox_show_extra_info)) {
        ...
        bool is_flyable_type = !(ship_get_SIF(target_shipp) & SIF_NOT_FLYABLE);
        bool is_player_team = (Player_ship->team == target_shipp->team);
        bool is_orders_shown_flag = (Iff_info[target_shipp->team].flags & IFFF_ORDERS_SHOWN) != 0;
        bool is_orders_hidden_flag = (Iff_info[target_shipp->team].flags & IFFF_ORDERS_HIDDEN) != 0;

        if (is_flyable_type && !is_orders_hidden_flag && (is_player_team || is_orders_shown_flag)) {
            extra_data_shown = 1;
            if (ship_return_orders(outstr, target_shipp)) {
                gr_force_fit_string(outstr, 255, 256);
                has_orders = 1;
            } else {
                strcpy_s(outstr, XSTR("no orders", 337));
            }

            emp_hud_string(Targetbox_coords[gr_screen.res][TBOX_EXTRA_ORDERS][0],
                           Targetbox_coords[gr_screen.res][TBOX_EXTRA_ORDERS][1], EG_TBOX_EXTRA1, outstr);
        }
        ...
        if (has_orders) {
            ...
            if (ship_return_time_to_goal(tmpbuf, target_shipp)) {
                ...
                emp_hud_string(Targetbox_coords[gr_screen.res][TBOX_EXTRA_TIME][0],
                               Targetbox_coords[gr_screen.res][TBOX_EXTRA_TIME][1], EG_TBOX_EXTRA2, outstr);
            }
        }
    }
    ...
    if (object_is_docked(target_objp)) {
        ...
        if (dock_count == 1) {
            ship *shipp = &Ships[dock_get_first_docked_object(target_objp)->instance];
            sprintf(outstr, XSTR("Docked: %s", 339), shipp->ship_name);
            end_string_at_first_hash_symbol(outstr, (shipp->wingnum != -1));
        }
        ...
        emp_hud_string(Targetbox_coords[gr_screen.res][TBOX_EXTRA_DOCK][0],
                       Targetbox_coords[gr_screen.res][TBOX_EXTRA_DOCK][1], EG_TBOX_EXTRA3, outstr);
        extra_data_shown = 1;
    }
    ...
    if (extra_data_shown) {
        ...
        hud_aabitmap(Target_view_extra.first_frame, Targetbox_coords[gr_screen.res][TBOX_EXTRA][0],
                     Targetbox_coords[gr_screen.res][TBOX_EXTRA][1]);
    }
}
...
void hud_render_target_jump_node(object *target_objp) {
    ...
    if (Detail.targetview_model) {
        ...
        hud_render_target_setup(&camera_eye, &camera_orient, Viewer_zoom_target);
    ...
    emp_hud_string(Targetbox_coords[gr_screen.res][TBOX_NAME][0], Targetbox_coords[gr_screen.res][TBOX_NAME][1],
                   EG_TBOX_NAME, target_objp->jnp->get_name_ptr());
    ...
    emp_hud_printf(Targetbox_coords[gr_screen.res][TBOX_DIST][0] + hx, Targetbox_coords[gr_screen.res][TBOX_DIST][1] +
                                                                       hy, EG_TBOX_DIST, outstr);
}
...
void hud_render_target_asteroid(object *target_objp) {
    ...
    if (Detail.targetview_model) {
        ...
        model_render(Asteroid_info[asteroidp->asteroid_type].model_num[pof], &target_objp->orient, &obj_pos,
                     flags | MR_NO_LIGHTING | MR_LOCK_DETAIL | MR_NO_FOGGING);
        hud_render_target_close();
    }
    ...
    switch (asteroidp->asteroid_type) {
        ...
        default:
            sprintf(hud_name, NOX("Debris"));
            break;
    }

    emp_hud_printf(Targetbox_coords[gr_screen.res][TBOX_NAME][0], Targetbox_coords[gr_screen.res][TBOX_NAME][1],
                   EG_TBOX_NAME, hud_name);


    if (time_to_impact >= 0.0f) {
        emp_hud_printf(Targetbox_coords[gr_screen.res][TBOX_CLASS][0], Targetbox_coords[gr_screen.res][TBOX_CLASS][1],
...
void hud_render_target_ship_info(object *target_objp) {
    ...
    // print lines
    emp_hud_string(Targetbox_coords[gr_screen.res][TBOX_NAME][0], Targetbox_coords[gr_screen.res][TBOX_NAME][1],
                   EG_TBOX_NAME, outstr_name);
    emp_hud_string(Targetbox_coords[gr_screen.res][TBOX_CLASS][0], Targetbox_coords[gr_screen.res][TBOX_CLASS][1],
                   EG_TBOX_CLASS, outstr_class);
    ...
    emp_hud_printf(Targetbox_coords[gr_screen.res][TBOX_HULL][0] - w, Targetbox_coords[gr_screen.res][TBOX_HULL][1],
                   EG_TBOX_HULL, "%s", outstr);
    hud_set_gauge_color(HUD_TARGET_MONITOR);
    ...
    if (Player_ai->targeted_subsys != NULL) {
        ...
        if (Player_ai->targeted_subsys->system_info->type == SUBSYSTEM_TURRET &&
            !ship_subsys_has_instance_name(Player_ai->targeted_subsys)) {
        ...
        if (n_linebreaks) {
            ...
            while (p_line != NULL) {
                gr_printf(Target_window_coords[gr_screen.res][0] + 2, Target_window_coords[gr_screen.res][1] +
                                                                      Target_window_coords[gr_screen.res][3] - h -
                                                                      (10 * n_linebreaks), p_line);
            ...
            gr_printf(Target_window_coords[gr_screen.res][0] + 2, Target_window_coords[gr_screen.res][1] +
                                                                  Target_window_coords[gr_screen.res][3] - h, outstr);
        }
        ...
        if (ship_subsys_takes_damage(Player_ai->targeted_subsys)) {
            sprintf(outstr, XSTR("%d%%", 341), screen_integrity);
            gr_get_string_size(&w, &h, outstr);
            gr_printf(Target_window_coords[gr_screen.res][0] + Target_window_coords[gr_screen.res][2] - w - 1,
                      Target_window_coords[gr_screen.res][1] + Target_window_coords[gr_screen.res][3] - h, "%s",
                      outstr);
        }
    ...
    if ((target_shipp->flags & SF_DISABLED) || (ship_subsys_disrupted(target_shipp, SUBSYSTEM_ENGINE))) {
        ...
        gr_get_string_size(&w, &h, outstr);
        gr_printf(Target_window_coords[gr_screen.res][0] + Target_window_coords[gr_screen.res][2] / 2 - w / 2 - 1,
                  Target_window_coords[gr_screen.res][1] + Target_window_coords[gr_screen.res][3] - 2 * h, "%s",
                  outstr);
    }
...
void hud_blit_target_integrity(int disabled, int force_obj_num) {
    ...
    if (disabled) {
        hud_aabitmap(Target_view_integrity_gauge.first_frame, Integrity_bar_coords[gr_screen.res][0],
                     Integrity_bar_coords[gr_screen.res][1]);
        return;
    }
    ...
    if ((Ships[objp->instance].flags & SF_DISABLED) || (ship_subsys_disrupted(&Ships[objp->instance],
                                                                              SUBSYSTEM_ENGINE))) {
    ...
    emp_hud_string(Integrity_string_coords[gr_screen.res][0], Integrity_string_coords[gr_screen.res][1], EG_TBOX_INTEG,
                   buf);
    ...
    if (clip_h > 0) {
        ...
        hud_aabitmap_ex(Target_view_integrity_gauge.first_frame, Integrity_bar_coords[gr_screen.res][0],
                        Integrity_bar_coords[gr_screen.res][1], w, clip_h, 0, 0);
    }

    if (clip_h <= Integrity_bar_coords[gr_screen.res][3]) {
        // draw the bright portion
        hud_aabitmap_ex(Target_view_integrity_gauge.first_frame + 1, Integrity_bar_coords[gr_screen.res][0],
                        Integrity_bar_coords[gr_screen.res][1] + clip_h, w, h - clip_h, 0, clip_h);
    }
}
...
void hud_maybe_render_cargo_scan(ship_info *target_sip) {
    ...
    y1 = fl2i(0.5f + Cargo_scan_coords[gr_screen.res][1] + ((i2fl(Player->cargo_inspect_time) / scan_time) *
                                                            Cargo_scan_coords[gr_screen.res][3]));
    ...
    if (Cmdline_dualscanlines) {
        ...
        y1 = fl2i(Cargo_scan_coords[gr_screen.res][1] + Cargo_scan_coords[gr_screen.
                res][3] - ((i2fl(Player->cargo_inspect_time) / scan_time) * Cargo_scan_coords[gr_screen.res][3]));
        gr_line(x1, y1, x2, y1);
    }

    // draw vertical scan line
    x1 = fl2i(0.5f + Cargo_scan_coords[gr_screen.res][0] + ((i2fl(Player->cargo_inspect_time) / scan_time) *
                                                            Cargo_scan_coords[gr_screen.res][2]));
    ...
    if (Cmdline_dualscanlines) {
        ...
        x1 = fl2i(0.5f + Cargo_scan_coords[gr_screen.res][2] + Cargo_scan_coords[gr_screen.res][0] - ((i2fl(Player->
                cargo_inspect_time) / scan_time) * Cargo_scan_coords[gr_screen.res][2]));
        gr_line(x1, y1 - 3, x1, y2 - 1);
    }
}
...
void hud_render_target_ship(object *target_objp) {
    ...
    if (Detail.targetview_model) {
        ...
        if (Targetbox_wire != 0) {
            ...
            if (Highlight_tagged_ships && ship_is_tagged(target_objp))
                model_set_outline_color_fast(iff_get_color(IFF_COLOR_TAGGED, is_bright));
            else {
                model_set_outline_color_fast(iff_get_color_by_team_and_object(target_shipp->team, Player_ship->team,
                                                                              is_bright, target_objp));
            }
        ...
        if (target_sip->model_num_hud >= 0) {
            model_render(target_sip->model_num_hud, &target_objp->orient, &obj_pos, flags | MR_NO_LIGHTING |
                                                                                    MR_LOCK_DETAIL | MR_AUTOCENTER |
                                                                                    MR_NO_FOGGING);
        } else {
            model_render(target_sip->model_num, &target_objp->orient, &obj_pos, flags | MR_NO_LIGHTING |
                                                                                MR_LOCK_DETAIL | MR_AUTOCENTER |
                                                                                MR_NO_FOGGING, -1, -1,
                         target_shipp->ship_replacement_textures);
        }
...
void hud_render_target_debris(object *target_objp) {
    ...
    if (Detail.targetview_model) {
        ...
        submodel_render(debrisp->model_num, debrisp->submodel_num, &target_objp->orient, &obj_pos,
                        flags | MR_NO_LIGHTING | MR_LOCK_DETAIL | MR_NO_FOGGING);
        hud_render_target_close();
    }
    ...
    emp_hud_string(Targetbox_coords[gr_screen.res][TBOX_CLASS][0], Targetbox_coords[gr_screen.res][TBOX_CLASS][1],
                   EG_TBOX_CLASS, printable_ship_class);
    emp_hud_string(Targetbox_coords[gr_screen.res][TBOX_NAME][0], Targetbox_coords[gr_screen.res][TBOX_NAME][1],
                   EG_TBOX_NAME, XSTR("Debris", 348));
}
...
void hud_render_target_weapon(object *target_objp) {
    ...
    if (Detail.targetview_model) {
        ...
        if (Targetbox_wire != 0) {
            ...
            model_set_outline_color_fast(iff_get_color_by_team_and_object(target_team, Player_ship->team, is_bright,
                                                                          target_objp));
        ...
        hud_render_target_setup(&camera_eye, &camera_orient, View_zoom / 3.114f);
        ...
        model_render(viewed_model_num, &viewed_obj->orient, &obj_pos, flags | MR_NO_LIGHTING | MR_LOCK_DETAIL |
                                                                      MR_AUTOCENTER | MR_IS_MISSILE | MR_NO_FOGGING, -1,
                     -1, replacement_textures);
        hud_render_target_close();
    }
    ...
    if (targetp->type == OBJ_SHIP) {
        if (player_inspect_cargo(frametime, outstr)) {
            if (hud_gauge_active(HUD_TARGET_MONITOR)) {
                ...
                emp_hud_string(Targetbox_coords[gr_screen.res][TBOX_CARGO][0] + hx,
                               Targetbox_coords[gr_screen.res][TBOX_CARGO][1] + hy, EG_TBOX_CARGO, outstr);
                hud_set_gauge_color(HUD_TARGET_MONITOR);
            }
        }
    }    // end if (is_ship)
...
void hud_show_target_data(float frametime) {
    ...
    sprintf(outstr, XSTR("d: %.0f%s", 350), displayed_target_distance,
            modifiers[Player_ai->current_target_dist_trend]);

    hud_num_make_mono(outstr);
    gr_get_string_size(&w, &h, outstr);

    emp_hud_string(Targetbox_coords[gr_screen.res][TBOX_DIST][0] + hx, Targetbox_coords[gr_screen.res][TBOX_DIST][1] +
                                                                       hy, EG_TBOX_DIST, outstr);
    ...
    sprintf(outstr, XSTR("s: %.0f%s", 351), displayed_target_speed, (displayed_target_speed > 1) ? modifiers[Player_ai->
            current_target_speed_trend] : "");
    hud_num_make_mono(outstr);

    emp_hud_string(Targetbox_coords[gr_screen.res][TBOX_SPEED][0] + hx,
                   Targetbox_coords[gr_screen.res][TBOX_SPEED][1] + hy, EG_TBOX_SPEED, outstr);
    ...
    if (Show_target_debug_info && (is_ship == 1)) {
        ...
        if (shipp->ai_index >= 0) {
            ...
            gr_printf(sx, sy, "Max speed = %d, (%d%%)", (int) shipp->current_max_speed, (int) (100.0f *
                                                                                               vm_vec_mag(&target_objp->
                                                                                                       phys_info.vel) /
                                                                                               shipp->
                                                                                                       current_max_speed));
            sy += dy;
            ...
            sprintf(outstr, "G/S/E: %.2f/%.2f/%.2f", Energy_levels[shipp->weapon_recharge_index],
                    Energy_levels[shipp->shield_recharge_index], Energy_levels[shipp->engine_recharge_index]);
            gr_printf(sx, sy, outstr);
            sy += dy;
                ...
                if (Enemy_attacker != NULL)
                    if (Enemy_attacker->type == OBJ_SHIP) {
                        ...
                        if (eaip->target_objnum == Player_obj - Objects) {
                            ...
                            gr_printf(sx, sy, "#%i: %s", Enemy_attacker - Objects,
                                      Ships[Enemy_attacker->instance].ship_name);
                            sy += dy;

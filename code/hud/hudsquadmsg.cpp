...
LOCAL
int Msg_key_used;                                // local variable which tells if the key being processed
// with the messaging system was actually used
LOCAL
int Msg_key;                                    // global which indicates which key was currently pressed
LOCAL
int Msg_mode_timestamp;
...
LOCAL
int Msg_target_objnum;                // id of the current target of the player
LOCAL
        ship_subsys
*
Msg_targeted_subsys;// pointer to current subsystem which is targeted
//#ifndef NDEBUG
LOCAL
int Msg_enemies;                        // tells us whether or not to message enemy ships or friendlies
//#endif

LOCAL
int Msg_eat_key_timestamp;            // used to temporarily "eat" keys
    ...
    struct {
        ...
        int item;
    } temp_comm_orders[] =
            {
                    // common stuff
                    {
                            XSTR("Destroy my target", 299),
                            ATTACK_TARGET_ITEM
                    },
                    {
                            XSTR("Disable my target", 300),
                            DISABLE_TARGET_ITEM
                    },
                    {
                            XSTR("Disarm my target", 301),
                            DISARM_TARGET_ITEM
                    },
                    {
                            XSTR("Destroy subsystem", 302),
                            DISABLE_SUBSYSTEM_ITEM
                    },
                    {
                            XSTR("Protect my target", 303),
                            PROTECT_TARGET_ITEM
                    },
                    {
                            XSTR("Ignore my target", 304),
                            IGNORE_TARGET_ITEM
                    },
                    {
                            XSTR("Form on my wing", 305),
                            FORMATION_ITEM
                    },
                    {
                            XSTR("Cover me", 306),
                    ...
                    {
                            XSTR("Engage enemy", 307),
                            ENGAGE_ENEMY_ITEM
                    },

                    // transports mostly
                    {
                            XSTR("Capture my target", 308),
                            CAPTURE_TARGET_ITEM
                    },
                    ...
                    {
                            XSTR("Stay near my target", -1),
                            STAY_NEAR_TARGET_ITEM
                    },
                    {
                            XSTR("Keep safe distance", -1),
                            KEEP_SAFE_DIST_ITEM
                    },
            };
...
int hud_squadmsg_count_ships(int add_to_menu) {
    ...
    for (so = GET_FIRST(&Ship_obj_list); so != END_OF_LIST(&Ship_obj_list); so = GET_NEXT(so)) {
        ...
        if (Ship_info[shipp->ship_info_index].class_type < 0 || !(Ship_types[Ship_info[shipp->ship_info_index].
                class_type].ai_bools & STI_AI_ACCEPT_PLAYER_ORDERS))
            continue;
        ...
        if (add_to_menu) {
            ...
            end_string_at_first_hash_symbol(MsgItems[Num_menu_items].text,
                                            (shipp->wingnum != -1)); // truncate the name if it has a # in it
        ...
        if (k == keys_used[i]) {
            ...
            if (k == KEY_PAGEDOWN) {            // pageup and pagedown scroll the menu -- deal with these seperately!!
                ...
                return -1;
                       ...
                       (Squad_msg_mode == SM_MODE_REINFORCEMENTS))        // return any key if selecting reinforcement
                return i;
                ...
                gamesnd_play_iface(SND_GENERAL_FAIL);
                     ...
                     (MsgItems[i].active))    // only return keys that are associated with menu items
                return i;
...
void hud_squadmsg_display_menu(char *title) {
    ...
    for (i = 0; i < nitems; i++) {
        ...
        if (Mbox_gauge[1].first_frame >= 0) {
            hud_aabitmap(Mbox_gauge[1].first_frame, bx, by);
        }
    ...
    // display
    if (!messaging_allowed || none_valid || ((First_menu_item + nitems) < Num_menu_items) ||
    ...
    if (Mbox_gauge[2].first_frame >= 0) {
        ...
        hud_aabitmap(Mbox_gauge[2].first_frame, bx, by);
    }
...
void hud_squadmsg_repair_rearm(int toggle_state, object *objp) {
    ...
    if (is_support_allowed(tobj)) {
        if (mission_is_repair_scheduled(tobj)) {
            message_send_builtin_to_player(MESSAGE_REARM_ON_WAY, NULL, MESSAGE_PRIORITY_NORMAL, MESSAGE_TIME_SOON, 0,
                                           0, multi_player_num, multi_player_team);
        } else {
            robjnum = hud_support_find_closest(OBJ_INDEX(tobj));
            if (robjnum != -1) {
                message_send_builtin_to_player(MESSAGE_REARM_ON_WAY, &Ships[Objects[robjnum].instance],
                                               MESSAGE_PRIORITY_NORMAL, MESSAGE_TIME_SOON, 0, 0, multi_player_num,
                                               multi_player_team);
            } else {
                ...
                if (robjnum != -1) {
                    ...
                    message_send_builtin_to_player(MESSAGE_ON_WAY, &Ships[robjp->instance], MESSAGE_PRIORITY_NORMAL,
                    ...
                    message_send_builtin_to_player(MESSAGE_REARM_WARP, NULL, MESSAGE_PRIORITY_NORMAL,
                                                   MESSAGE_TIME_SOON, 0, 0, multi_player_num, multi_player_team);
                }
...
int hud_squadmsg_is_target_order_valid(int order, int find_order, ai_info *aip) {
    ...
    if ((Comm_orders[order].item == DISARM_TARGET_ITEM) && ((shipp->subsys_info[SUBSYSTEM_TURRET].num == 0) ||
                                                            (shipp->subsys_info[SUBSYSTEM_TURRET].current_hits ==
                                                             0.0f))) {
        return 0;
    }

    // if order is disable subsystem, and no subsystem targeted or no hits, then order not valid
    if ((Comm_orders[order].item == DISABLE_SUBSYSTEM_ITEM) && ((aip->targeted_subsys == NULL) ||
                                                                (aip->targeted_subsys->current_hits <= 0.0f))) {
        return 0;
    }
...
void hud_squadmsg_send_to_all_fighters(int command, int player_num) {
    ...
    */
    ...
    for (objp = GET_FIRST(&obj_used_list); objp != END_OF_LIST(&obj_used_list); objp = GET_NEXT(objp)) {
        ...
        if (send_message) {
            hud_add_issued_order("All Fighters", command);
            if (hud_squadmsg_send_ship_command(objp->instance, command, send_message, SQUADMSG_HISTORY_UPDATE,
                                               player_num)) {
                send_message = 0;
            }
        } else {
            hud_squadmsg_send_ship_command(objp->instance, command, send_message, SQUADMSG_HISTORY_NO_UPDATE,
                                           player_num);
        }
    }
...
int hud_squadmsg_send_ship_command(int shipnum, int command, int send_message, int update_history, int player_num) {
    ...
    if (hud_squadmsg_is_target_order_valid(command, 1, ainfo)) {
        ...
        switch (command) {                                    // value of k matches the #defines for ship messages
            case ATTACK_TARGET_ITEM:
                if (Objects[ainfo->target_objnum].type == OBJ_SHIP) {
                    ...
                    if (Ship_info[Ships[Objects[ainfo->target_objnum].instance].ship_info_index].
                            flags & OVERRIDE_PROTECT_SHIP_TYPE) {
            ...
            case DISABLE_SUBSYSTEM_ITEM:
                ...
                ai_submode = ship_get_subsys_index(&Ships[Objects[ainfo->target_objnum].instance],
                                                   ainfo->targeted_subsys->system_info->subobj_name);
            ...
            case REARM_REPAIR_ME_ITEM:
                if ((Game_mode & GM_MULTIPLAYER) && (Net_player->flags & NETINFO_FLAG_AM_MASTER) &&
                    ...
                    hud_squadmsg_repair_rearm(
                            0);                // note we return right away.  repair/rearm code handles messaging, etc
                }
        ...
        if (ai_mode != AI_GOAL_NONE) {
            ...
            ai_add_ship_goal_player(AIG_TYPE_PLAYER_SHIP, ai_mode, ai_submode,
                                    target_shipname, &Ai_info[Ships[shipnum].ai_index]);
    ...
    if (send_message && (!(Ships[shipnum].flags2 & SF2_NO_BUILTIN_MESSAGES))) {
        message_send_builtin_to_player(message, &Ships[shipnum], MESSAGE_PRIORITY_NORMAL, MESSAGE_TIME_ANYTIME, 0, 0,
                                       player_num, message_team_filter);
    }
...
int hud_squadmsg_send_wing_command(int wingnum, int command, int send_message, int update_history, int player_num) {
        ...
        switch (command) {                                    // value of k matches the #defines for ship messages
            case ATTACK_TARGET_ITEM:
                if (Objects[ainfo->target_objnum].type == OBJ_SHIP) {
                    ...
                    if ((Ships[Objects[ainfo->target_objnum].instance].wingnum != -1) &&
                        (Ships[Objects[ainfo->target_objnum].instance].wingnum == wingnum)) {
                        message = MESSAGE_NOSIR;
    ...
    if (send_message) {
        ...
        if (ship_num != -1) {
            message_send_builtin_to_player(message, &Ships[ship_num], MESSAGE_PRIORITY_NORMAL, MESSAGE_TIME_ANYTIME, 0,
                                           0, player_num, message_team_filter);
            message_sent = 1;
        }
    }
...
void hud_squadmsg_type_select() {
    ...
    if ((hud_communications_state(Player_ship) != COMM_OK) || ((Game_mode &
                                                                GM_MULTIPLAYER) && !multi_can_message(Net_player))) {
        for (i = 0; i < MAX_MENU_ITEMS; i++) {
            MsgItems[i].active = 0;
        }
...
void hud_squadmsg_wing_select() {
    ...
    if (k != -1) {                        // if true, we have selected a ship.
        if (Msg_shortcut_command ==
            -1) {                                    // do normal menu stuff when no hoykey active
            Msg_instance = MsgItems[First_menu_item + k].instance;    // store the instance id in a global
...
void hud_squadmsg_reinforcement_select() {
    ...
    hud_squadmsg_display_menu(
            XSTR("Select Ship/Wing", 319));    // AL 11-14-97: Reinforcement didn't fit, so using this for now
    k = hud_squadmsg_get_key();
    if (k != -1) {
        int rnum;
...
void hud_squadmsg_ship_command() {
    ...
    for (i = 0; i < NUM_COMM_ORDER_ITEMS; i++) {
        ...
        if (default_orders & Comm_orders[i].item) {
            ...
            if ((Msg_instance != MESSAGE_ALL_FIGHTERS) && !hud_squadmsg_ship_order_valid(Msg_instance,
                                                                                         Comm_orders[i].item))
                MsgItems[Num_menu_items].active = 0;
            ...
            if (Msg_instance == MESSAGE_ALL_FIGHTERS) {
                ...
                for (so = GET_FIRST(&Ship_obj_list); so != END_OF_LIST(&Ship_obj_list); so = GET_NEXT(so)) {
                    ...
                    partial_accept |= (shipp->orders_accepted &
                                       Comm_orders[i].item);    // 'or'ing will tell us if at least one accepts
                }
...
int hud_squadmsg_do_frame() {
    ...
    if ((Game_mode & GM_MULTIPLAYER) && !(Net_player->flags & NETINFO_FLAG_AM_MASTER) &&
        (Squad_msg_mode == SM_MODE_REPAIR_REARM || Squad_msg_mode == SM_MODE_REPAIR_REARM_ABORT)) {
        char *subsys_name;
        //		int who_to_sig;
        ushort net_sig;
    ...
    if (Mbox_gauge[0].first_frame >= 0) {
        ActivateAlignment left(ALIGNMENT_RIGHT);
        hud_aabitmap(Mbox_gauge[0].first_frame, Mbox_top_coords[gr_screen.res][0], Mbox_top_coords[gr_screen.res][1]);
    }
    ...
    switch (Squad_msg_mode) {
        ...
        case SM_MODE_REPAIR_REARM:
            ...
            //} else {
            hud_squadmsg_repair_rearm(
                    1);                // note we return right away.  repair/rearm code handles messaging, etc
            //}
            break;
...
int hud_query_order_issued(char *to, char *order_name, char *target_name, int timestamp, char *from,
            ...
            if (ship_or_wing == Squadmsg_history[i].order_to) {
                if (target == -1 || Squadmsg_history[i].target == target) {
                    if ((!timestamp) || (Squadmsg_history[i].order_time + i2f(timestamp) >= Missiontime)) {
                        ...
                        if ((order & DISABLE_SUBSYSTEM_ITEM) && (special_argument != NULL)) {

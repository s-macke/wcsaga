...
typedef struct hud_info {
    ...
    Alignment custom_gauge_alignments[MAX_CUSTOM_HUD_GAUGES];
    ...
    hud_info()
        ...
        memset(custom_gauge_alignments, 0, sizeof(custom_gauge_alignments));

        for (i = 0; i < MAX_CUSTOM_HUD_GAUGES; ++i) {
            custom_gauge_color_parents[i] = -1;
            custom_gauge_alignments[i] = ALIGNMENT_CENTER;
        }
    }
} hud_info;

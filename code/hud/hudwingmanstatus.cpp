                ...
                { // GR_640
                        {
                                550,
                                144
                        },                // where to draw the left part of gauge if we have 1 wing
                        {
                                550,
                                144
                        },                // "" 2 wings
                ...
                { // GR_640
                        {
                                0,
                                0
                        },                    // we never draw this for 1 wing
                        {
                                0,
                                0
                        },                    // we never draw this for 2 wings
                        {
                                586,
                                144
                        },                // where to draw the _first_ middle gauge for 3 wings
                        {
                                551,
                                144
                        },                // "" 4 wings
                ...
                { // GR_640
                        {
                                589,
                                159
                        },                // where to draw dots if we have only one wing present (special case)
                        {
                                582,
                                167
                        },
...
void hud_set_wingman_status_none(int wing_index, int wing_pos) {
    ...
    for (i = 0; i < MAX_SHIPS_PER_WING; i++) {
        if (HUD_wingman_status[wing_index].status[i] == HUD_WINGMAN_STATUS_ALIVE) {
            used = 1;
            break;
        }
    }
...
void hud_init_wingman_status_gauge() {
    ...
    if (!Wingman_status_gauge_loaded) {
        ...
        for (i = 0; i < HUD_WINGMAN_STATUS_NUM_FRAMES; i++) {
            Wingman_status_frames[i].first_frame = bm_load_animation(Wingman_status_filenames[gr_screen.
                    res][i], &Wingman_status_frames[i].num_frames);
    ...
    if (timestamp_elapsed(HUD_wingman_update_timer)) {
        ...
        for (so = GET_FIRST(&Ship_obj_list); so != END_OF_LIST(&Ship_obj_list); so = GET_NEXT(so)) {
            ...
            if ((wing_index >= 0) && (wing_pos >= 0)) {
                ...
                // KeldorKatarn: Do not resurrect ships here.
                if (HUD_wingman_status[wing_index].status[wing_pos] != HUD_WINGMAN_STATUS_DEAD) {
        ...
        case 1:
            for (i = 0; i < MAX_SQUADRON_WINGS; i++) {
                if (HUD_wingman_status[i].used > 0) {
                    for (j = 0; j < MAX_SHIPS_PER_WING; j++) {
                        if (HUD_wingman_status[i].status[j] == HUD_WINGMAN_STATUS_ALIVE) {
                            count++;
                        }
                    }
                }

...
extern int HUD_draw;
extern int HUD_draw_user;
extern int HUD_contrast;

extern int Steering_cursor_bitmap;
...
// hud function for maybe flickering all gauges
void hud_aabitmap(int bitmap_num, int x, int y, bool resize = true, bool mirror = false);

// hud function for maybe flickering all gauges
void hud_aabitmap_ex(int bitmap_num, int x, int y, int w, int h, int sx, int sy, bool resize = true, bool mirror =
false);
...
int hud_anim_render(hud_anim *ha, float frametime, int draw_alpha = 0, int loop = 1, int hold_last = 0, int reverse = 0,
                    bool resize = true, bool mirror = false);
...
void hud_set_custom_gauge_color(int gauge_index, int bright_index = HUD_C_NONE);

void hud_set_gauge_color_specific(int flash_status, color *use_color, int bright_index = HUD_C_NONE);

...
bool Custom_gauges_loaded = false;
bool Main_gauges_loaded = false;
...
int Support_text_dock_coords[GR_NUM_RESOLUTIONS][2] =
        {            // "dock in" x coord
                { // GR_640
                        270,
                        -1
...
int Support_text_dock_val_coords[GR_NUM_RESOLUTIONS][2] =
        {        // time value for "dock in" x coord
                { // GR_640
                        328,
                        -1
                },
...
void hud_reset_throttle_sound_check_id() {
...
void hud_init_mission_time_gauge() {
    ...
    if (!Mission_time_gauge_loaded) {
        Mission_time_gauge.first_frame = bm_load_animation(Mission_time_fname[gr_screen.res], &Mission_time_gauge.
                num_frames);
...
void HUD_init() {
    ...
    hud_anim_init(&Target_static, Target_window_coords[gr_screen.res][0], Target_window_coords[gr_screen.res][1],
                  NOX("TargetStatic"));
    ...
    if (Steering_cursor_bitmap < 0) {
        Steering_cursor_bitmap = bm_load("cursor_hud");
    }
}

void hud_toggle_draw() {
    HUD_draw_user = !HUD_draw_user;
}
...
void hud_update_frame() {
    ...
    if ((!retarget) && (Player_ai->target_objnum != -1)) {
        if (Objects[Player_ai->target_objnum].type == OBJ_SHIP) {
            if (!(Ships[Objects[Player_ai->target_objnum].instance].flags & SF_DYING)) {
                if (Ship_info[Ships[Objects[Player_ai->target_objnum].instance].ship_info_index].
...
void HUD_render_3d(float frametime) {
    ...
    if (!(Viewer_mode & (VM_EXTERNAL | VM_DEAD_VIEW | VM_WARP_CHASE | VM_PADLOCK_ANY))) {
        ...
        if (!(Viewer_mode &
              VM_OTHER_SHIP)) { // Added to prevent the offscreen crosshair indicator from drawing when viewing other ships (Swifty)
...
void hud_force_reload_ship_gauges() {
    Custom_gauges_loaded = false;
    Main_gauges_loaded = false;
    Shield_mini_loaded = false;
}
...
void hud_show_mission_time() {
    ...
    if (Mission_time_gauge.first_frame >= 0) {
        hud_aabitmap(Mission_time_gauge.first_frame, Mission_time_coords[gr_screen.res][0],
                     Mission_time_coords[gr_screen.res][1]);
    }

    // print out mission time in MM:SS format
    gr_printf(Mission_time_text_coords[gr_screen.res][0], Mission_time_text_coords[gr_screen.res][1], NOX("%02d:%02d"),
              minutes, seconds);

    // display time compression as xN
    time_comp = f2fl(Game_time_compression);
    if (time_comp < 1) {
        gr_printf(Mission_time_text_val_coords[gr_screen.res][0], Mission_time_text_val_coords[gr_screen.res][1],
                /*XSTR( "x%.1f", 215), time_comp)*/ NOX("%.2f"), time_comp);
    } else {
        gr_printf(Mission_time_text_val_coords[gr_screen.res][0], Mission_time_text_val_coords[gr_screen.res][1],
                  XSTR("x%.2f", 216), time_comp);
    }
}
...
void hud_maybe_blit_head_border() {
    ...
    if (message_anim_is_playing()) {
        ...
        hud_aabitmap(Head_frame_gauge.first_frame, Head_frame_coords[gr_screen.res][0],
                     Head_frame_coords[gr_screen.res][1]);
...
void hud_maybe_display_supernova() {
    ...
    gr_printf(Supernova_coords[gr_screen.res][0], Supernova_coords[gr_screen.res][1], "Supernova Warning: %.2f s",
              time_left);
}
...
void HUD_render_2d(float frametime) {
    ...
    if (!(Viewer_mode & (VM_EXTERNAL | VM_DEAD_VIEW | VM_WARP_CHASE | VM_PADLOCK_ANY | VM_OTHER_SHIP))) {
        //Custom hud stuff
        int i;
        static hud_frames image_ids[MAX_CUSTOM_HUD_GAUGES];
        if (!Custom_gauges_loaded) {
            for (i = 0; i < Num_custom_gauges; i++) {
                if (strlen(current_hud->custom_gauge_images[i])) {
                    image_ids[i].first_frame = bm_load_animation(current_hud->custom_gauge_images[i], &image_ids[i].
                            num_frames);
            ...
            Custom_gauges_loaded = true;
        }

        // Display the gauges
        for (i = 0; i < Num_custom_gauges; i++) {
            ActivateAlignment align(current_hud->custom_gauge_alignments[i]);

            hud_set_custom_gauge_color(i);

            if (strlen(current_hud->custom_gauge_text[i])) {
                hud_num_make_mono(current_hud->custom_gauge_text[i]);
                gr_string(current_hud->custom_gauge_coords[i][0], current_hud->custom_gauge_coords[i][1],
                          current_hud->custom_gauge_text[i]);
            }
            if (image_ids[i].first_frame != -1) {
                if (current_hud->custom_gauge_moveflags[i]) {
                    hud_aabitmap(image_ids[i].first_frame + current_hud->custom_gauge_frames[i],
                                 current_hud->custom_gauge_coords[i][0] + HUD_nose_x,
                                 current_hud->custom_gauge_coords[i][1] +
                                 HUD_nose_y);
                } else {
                    hud_aabitmap(image_ids[i].first_frame + current_hud->custom_gauge_frames[i],
                                 current_hud->custom_gauge_coords[i][0], current_hud->custom_gauge_coords[i][1]);
                }
            }
    ...
    if (!(Viewer_mode & (VM_EXTERNAL | VM_DEAD_VIEW | VM_WARP_CHASE | VM_PADLOCK_ANY))) {
        if (!Main_gauges_loaded) {
            hud_load_main_gauges();
        }
...
void update_throttle_sound() {
    ...
    // do the same if we are in external camera view
    if (((Game_mode & GM_MULTIPLAYER) && (Net_player->flags & NETINFO_FLAG_OBSERVER)) ||
        (Viewer_mode & VM_NOT_COCKPIT)) {
    ...
    if (timestamp_elapsed(throttle_sound_check_id)) {
        ...
        if (object_get_gliding(Player_obj)) {    // Backslash
            ...
            percent_throttle = Player_obj->phys_info.fspeed / Ship_info[Ships[Player_obj->instance].ship_info_index].
                    max_speed;
        } else {
        ...
        if (percent_throttle != last_percent_throttle || Player_engine_snd_loop == -1) {
            ...
            if (percent_throttle < ZERO_PERCENT) {
                ...
                if (Player_engine_snd_loop == -1) {
                    Player_engine_snd_loop = snd_play_looping(&Snds[SND_ENGINE], 0.0f, -1, -1, percent_throttle);
                } else {
                    ...
                    if (snd_is_playing(Player_engine_snd_loop)) {
                        snd_set_volume(Player_engine_snd_loop, percent_throttle * Snds[SND_ENGINE].default_volume);
                    } else {
                        Player_engine_snd_loop = -1;
                    }
                }
...
void hud_damage_popup_init() {
    ...
    if (!Damage_gauges_loaded) {
        for (i = 0; i < NUM_DAMAGE_GAUGES; i++) {
            Damage_gauges[i].first_frame = bm_load_animation(Damage_gauge_fnames[gr_screen.res][i], &Damage_gauges[i].
                    num_frames);
...
void hud_show_damage_popup() {
    ...
    hud_aabitmap(Damage_gauges[0].first_frame, Damage_gauge_coords[gr_screen.res][0][0],
                 Damage_gauge_coords[gr_screen.res][0][1]);
    ...
    if (screen_integrity < 100) {
        ...
        gr_string(Hull_integ_coords[gr_screen.res][0], Hull_integ_coords[gr_screen.res][1], XSTR("Hull Integrity",
                                                                                                 220));
    ...
    for (int i = 0; i < num; i++) {
        ...
        hud_aabitmap(Damage_gauges[1].first_frame, bx, by);
    ...
    hud_aabitmap(Damage_gauges[2].first_frame, bx, by);
}
...
void hud_show_text_flash_icon(char *txt, int y, int bright) {
    ...
    // draw the box
    gr_rect((int) ((((float) gr_screen.max_w_unscaled / 2.0f) - ((float) w / 2.0f)) - 1.0f) + HUD_nose_x,
            (int) ((float) y -
                   1.0f) +
            HUD_nose_y, w + 2, h + 1);
...
void hud_show_kills_gauge() {
    ...
    ActivateAlignment left(ALIGNMENT_RIGHT);

    // hud_set_default_color();
    hud_set_gauge_color(HUD_KILLS_GAUGE);

    // draw background
    hud_aabitmap(Kills_gauge.first_frame, Kills_gauge_coords[gr_screen.res][0], Kills_gauge_coords[gr_screen.res][1]);
    ...
    if (Lcl_gr) {
        gr_string(Kills_text_val_coords_gr[gr_screen.res][0] - w, Kills_text_val_coords_gr[gr_screen.res][1],
                  num_kills_string);
    } else {
        gr_string(Kills_text_val_coords[gr_screen.res][0] - w, Kills_text_val_coords[gr_screen.res][1],
                  num_kills_string);
    }
}
...
void hud_support_view_init() {
    ...
    if (!Support_view_gauge_loaded) {
        Support_view_gauge.first_frame = bm_load_animation(Support_fname[gr_screen.res], &Support_view_gauge.
                num_frames);
...
void hud_support_view_blit() {
    ...
    if (Hud_support_objnum == -1) {
        ...
        if ((Objects[Hud_support_objnum].signature != Hud_support_obj_sig) || (Hud_support_target_sig != Player_obj->
                signature)) {
    ...
    hud_aabitmap(Support_view_gauge.first_frame, Support_view_coords[gr_screen.res][0],
                 Support_view_coords[gr_screen.res][1]);
    ...
    if (Player_ai->ai_flags & AIF_BEING_REPAIRED) {
        ...
        if (Hud_support_objnum == -1) {
            ...
            if (!show_time) {
                gr_string(Support_text_dock_coords[gr_screen.res][0], Support_text_val_coords[gr_screen.res][1],
                          outstr);
            } else {
                gr_string(Support_text_dock_coords[gr_screen.res][0], Support_text_val_coords[gr_screen.res][1],
                          outstr);
    ...
    if (show_time) {
        ...
        gr_printf(Support_text_dock_val_coords[gr_screen.res][0], Support_text_val_coords[gr_screen.res][1],
                  NOX("%02d:%02d"), minutes, seconds);
    }
}
...
void hud_set_iff_color(object *objp, int is_bright) {
    ...
    if (Highlight_tagged_ships && ship_is_tagged(objp)) {
...
void hud_set_gauge_color(int gauge_index, int bright_index) {
    ...
    hud_set_gauge_color_specific(flash_status, use_color, bright_index);
}

void hud_set_custom_gauge_color(int gauge_index, int bright_index) {
    color *gauge_color;
    int flash_status = -1;

    // Inherit color if color parent is specified.
    if (current_hud->custom_gauge_color_parents[gauge_index] >= 0 &&
        current_hud->custom_gauge_color_parents[gauge_index] < NUM_HUD_GAUGES) {
        gauge_color = &HUD_config.clr[current_hud->custom_gauge_color_parents[gauge_index]];
        flash_status = hud_gauge_maybe_flash(current_hud->custom_gauge_color_parents[gauge_index]);
    } else {
        gauge_color = &current_hud->custom_gauge_colors[gauge_index];
    }

    hud_set_gauge_color_specific(-1, gauge_color, bright_index);
}

void hud_set_gauge_color_specific(int flash_status, color *use_color, int bright_index) {
    int alpha;
...
void hud_objective_message_init() {
    ...
    if (!Objective_display_gauge_inited) {
        Objective_display_gauge.first_frame = bm_load_animation(Objective_fname[gr_screen.
                res], &Objective_display_gauge.num_frames);
...
void hud_maybe_display_subspace_notify() {
    ...
    hud_aabitmap(Objective_display_gauge.first_frame, Objective_display_coords[gr_screen.res][0],
                 Objective_display_coords[gr_screen.res][1]);
    ...
    switch (Objective_display.goal_type) {
        ...
        case SECONDARY_GOAL:
            switch (Objective_display.goal_status) {
                case GOAL_FAILED:
                    sprintf(buf, XSTR("failed (%d/%d)", 240), Objective_display.goal_nresolved,
                            Objective_display.goal_ntotal);
                    gr_string(0x8000, Objective_text_val_coords[gr_screen.res][1], buf);
                    break;
                default:
                    sprintf(buf, XSTR("complete (%d/%d)", 241), Objective_display.goal_nresolved,
                            Objective_display.goal_ntotal);
                    gr_string(0x8000, Objective_text_val_coords[gr_screen.res][1], buf);
                    break;
            }
...
void HUD_reset_clip() {
    KeepAspectRatio ignoreForReset(false);
...
void hud_aabitmap(int bitmap_num, int x, int y, bool resize, bool mirror) {
    if (emp_should_blit_gauge()) {
        gr_set_bitmap(bitmap_num);
        int jx = x;
        int jy = y;

        emp_hud_jitter(&jx, &jy);
        gr_aabitmap(jx, jy, resize, mirror);
    }
}

void hud_aabitmap_ex(int bitmap_num, int x, int y, int w, int h, int sx, int sy, bool resize, bool mirror) {
    if (emp_should_blit_gauge()) {
        gr_set_bitmap(bitmap_num);
        int jx = x;
        int jy = y;
        emp_hud_jitter(&jx, &jy);
        gr_aabitmap_ex(jx, jy, w, h, sx, sy, resize, mirror);
    }
}

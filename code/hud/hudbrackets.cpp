...
void draw_bounding_brackets_subobject() {
    if (Player_ai->targeted_subsys_parent == Player_ai->target_objnum)
        if (Player_ai->targeted_subsys != NULL) {
            ...
            bound_rc = subobj_find_2d_bound(subsys->system_info->radius, &targetp->orient, &subobj_pos, &x1, &y1, &x2,
                                            &y2);
            if (bound_rc != 0)
                return;
            ...
            if ((Player_ai->targeted_subsys->current_hits <= 0) && (ship_subsys_takes_damage(Player_ai->
                    targeted_subsys))) {
...
void draw_bounding_brackets(int x1, int y1, int x2, int y2, int w_correction, int h_correction, float distance,
                            int target_objnum) {
    int width, height;
        ...
        if (num_attacking > 0) {
            ...
            if (Ships_attacking_bitmap >= 0) {
                ...
                for (i = 0; i < num_blips; i++) {
                    hud_aabitmap(Ships_attacking_bitmap, x2 + 3, y1 + i * 7);
                }
            }

...
void update_ets(object *objp, float fl_frametime) {
    ...
    if (objp->flags & OF_PLAYER_SHIP) {
        ship_p->weapon_energy += Energy_levels[ship_p->weapon_recharge_index] * max_new_weapon_energy *
                                 The_mission.ai_profile->weapon_energy_scale[Game_skill_level];
    } else {
        ship_p->weapon_energy += Energy_levels[ship_p->weapon_recharge_index] * max_new_weapon_energy;
    }
    ...
    if (objp->flags & OF_PLAYER_SHIP) {
        shield_delta = Energy_levels[ship_p->shield_recharge_index] * max_new_shield_energy * The_mission.ai_profile->
                shield_energy_scale[Game_skill_level];
    } else {
    ...
    if (INTIAL_ENGINE_RECHARGE_INDEX == ship_p->engine_recharge_index) {
        ship_p->current_max_speed = sinfo_p->max_speed;
    } else if (0 == ship_p->engine_recharge_index) {
        ship_p->current_max_speed = 0.5f * sinfo_p->max_speed;
    } else if (MAX_ENERGY_INDEX == ship_p->engine_recharge_index) {
        ...
        if (y < Energy_levels[INTIAL_ENGINE_RECHARGE_INDEX]) {
            ship_p->current_max_speed = 0.5f * sinfo_p->max_speed + (y - Energy_levels[0]) * (sinfo_p->max_speed -
                                                                                              0.5f *
                                                                                              sinfo_p->max_speed) /
                                                                    (Energy_levels[INTIAL_ENGINE_RECHARGE_INDEX] -
                                                                     Energy_levels[0]);
        } else {
            ship_p->current_max_speed = sinfo_p->max_speed + (y -
                                                              Energy_levels[INTIAL_ENGINE_RECHARGE_INDEX]) * (sinfo_p->
                    max_overclocked_speed - sinfo_p->max_speed) / (Energy_levels[MAX_ENERGY_INDEX] -
                                                                   Energy_levels[INTIAL_ENGINE_RECHARGE_INDEX]);
        }
    }
    ...
    if (!(obj->flags & OF_NO_SHIELDS)) {
        if (shield_left_percent < SHIELDS_EMERG_LEVEL_PERCENT) {
            ...
            if (shield_left_percent > SHIELDS_MIN_LEVEL_PERCENT ||
                weapon_left_percent <= 0.01)    // dampen ai enthusiasm for sucking energy to weapons
...
void transfer_energy_weapon_common(object *objp, float from_field, float to_field, float *from_delta, float *to_delta,
...
void transfer_energy_to_shields(object *obj) {
    ...
    transfer_energy_weapon_common(obj, ship_p->weapon_energy,
                                  shield_get_strength(obj), &ship_p->target_weapon_energy_delta,
                                  &ship_p->target_shields_delta,
                                  ship_p->ship_max_shield_strength, 0.5f);
}
...
void transfer_energy_to_weapons(object *obj) {
    ...
    transfer_energy_weapon_common(obj, shield_get_strength(obj),
                                  ship_p->weapon_energy, &ship_p->target_shields_delta,
                                  &ship_p->target_weapon_energy_delta,
                                  sinfo_p->max_weapon_reserve, 1.0f);
}

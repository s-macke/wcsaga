...
#define HUD_MESSAGE_TOTAL_LIFE    20000    // total time a HUD message is alive (in milliseconds)
...
static scrollback_buttons Buttons[GR_NUM_RESOLUTIONS][NUM_BUTTONS] =
        {
                ...
                {    // GR_1024
                        ...
                        scrollback_buttons("2_LB_02", 177, 602, 167, 661, 2), // "Messages"
                        scrollback_buttons("2_LB_03", 335, 602, 339, 661, 3), // "Events"
                        scrollback_buttons("2_LB_04", 20, 602, 8, 661, 4),            // "Objectives"
                        scrollback_buttons("2_CB_05a", 914, 681, 938, 661, 5)        // "Continue"
                        //XSTR:ON
                }
        };
...
void hud_init_msg_window() {
    ...
    MSG_WINDOW_WIDTH = gr_screen.clip_width_unscaled - MSG_WINDOW_X_START - 15;
...
void hud_show_msg_window() {
    ...
    {
        KeepAspectRatio doNotKeep(false);
    ...
    if (Hud_list_start != Hud_list_end && !Scroll_needed) {
        ...
        Active_index++;
        if (Active_index >= ACTIVE_BUFFER_LINES)
            Active_index = 0;
        ...
        if (!timestamp_elapsed(HUD_active_msgs_list[i].total_life)) {
            ...
            if (hud_gauge_active(HUD_MESSAGE_LINES)) {
                if (!(Player->flags & PLAYER_FLAGS_MSG_MODE)) {
                    ...
                    gr_printf(MSG_WINDOW_X_START + HUD_active_msgs_list[i].msg.x - 2, HUD_active_msgs_list[i].y, "%s",
                              HUD_active_msgs_list[i].msg.text);
                }
            }
        }
...
void hud_show_fixed_text() {
    HUD_ft *hp;
    ActivateAlignment center(ALIGNMENT_CENTER);
...
void hud_sourced_print(int source, char *msg) {
    ...
    float pretend_width = (float) MSG_WINDOW_WIDTH - MSG_WINDOW_X_START;
    float old_aspect = (float) gr_screen.max_w_unscaled / gr_screen.max_h_unscaled;
    float new_aspect = (float) gr_screen.max_w / gr_screen.max_h;
    pretend_width /= old_aspect;
    pretend_width *= new_aspect;

    int split_width = (int) pretend_width;

    while ((ptr = split_str_once(str, split_width - x - 7)) != NULL) {        // the 7 is a fudge hack
...
void hud_scrollback_init() {
    ...
    // add all strings
    Ui_window.add_XSTR("Continue", 1069, Buttons[gr_screen.res][ACCEPT_BUTTON].xt,
                       Buttons[gr_screen.res][ACCEPT_BUTTON].yt, &Buttons[gr_screen.res][ACCEPT_BUTTON].button,
                       UI_XSTR_COLOR_PINK);
    Ui_window.add_XSTR("Events", 1070, Buttons[gr_screen.res][SHOW_EVENTS_BUTTON].xt,
                       Buttons[gr_screen.res][SHOW_EVENTS_BUTTON].yt,
                       &Buttons[gr_screen.res][SHOW_EVENTS_BUTTON].button,
                       UI_XSTR_COLOR_PINK);
    Ui_window.add_XSTR("Objectives", 1071, Buttons[gr_screen.res][SHOW_OBJS_BUTTON].xt,
                       Buttons[gr_screen.res][SHOW_OBJS_BUTTON].yt, &Buttons[gr_screen.res][SHOW_OBJS_BUTTON].button,
                       UI_XSTR_COLOR_PINK);
    Ui_window.add_XSTR("Messages", 1072, Buttons[gr_screen.res][SHOW_MSGS_BUTTON].xt,
                       Buttons[gr_screen.res][SHOW_MSGS_BUTTON].yt, &Buttons[gr_screen.res][SHOW_MSGS_BUTTON].button,
                       UI_XSTR_COLOR_PINK);
    ...
    Num_obj_lines = ML_objectives_init(Hud_mission_log_list_coords[gr_screen.res][0],
                                       Hud_mission_log_list_coords[gr_screen.res][1],
                                       Hud_mission_log_list_coords[gr_screen.res][2],
                                       Hud_mission_log_list_objective_x_coord[gr_screen.res]);
    hud_scroll_reset();
}
...
void hud_scrollback_do_frame(float frametime) {
    ...
    */
    ...
    if (Scrollback_mode == SCROLLBACK_MODE_EVENT_LOG) {
        ...
        mission_log_scrollback(Scroll_offset, Hud_mission_log_list_coords[gr_screen.res][0],
                               Hud_mission_log_list_coords[gr_screen.res][1],
                               Hud_mission_log_list_coords[gr_screen.res][2],
                               Hud_mission_log_list_coords[gr_screen.res][3]);
        ...
        if (!EMPTY(&Msg_scrollback_used_list) && HUD_msg_inited) {
            ...
            while (node_ptr != END_OF_LIST(&Msg_scrollback_used_list)) {
                if ((node_ptr->source == HUD_SOURCE_HIDDEN) || (i++ < Scroll_offset)) {
                    ...
                    if (team >= 0) {
                        gr_set_color_fast(iff_get_color_by_team(team, Player_ship->team, 1));
                    } else {
                    ...
                    if (node_ptr->time)
                        gr_print_timestamp(Hud_mission_log_list_coords[gr_screen.res][0],
                                           Hud_mission_log_list_coords[gr_screen.res][1] + y, node_ptr->time);
                    ...
                    if (node_ptr->underline_width)
                        gr_line(x, Hud_mission_log_list_coords[gr_screen.res][1] + y + font_height - 1,
                                x + node_ptr->underline_width, Hud_mission_log_list_coords[gr_screen.res][1] + y +
                                                               font_height - 1);
                    ...
                    if ((node_ptr->source == HUD_SOURCE_FAILED) || (node_ptr->source == HUD_SOURCE_SATISFIED)) {
                        ...
                        gr_line(Hud_mission_log_list2_coords[gr_screen.res][0] - 10, i,
                                Hud_mission_log_list2_coords[gr_screen.res][0] - 8, i);
                        gr_line(Hud_mission_log_list2_coords[gr_screen.res][0] - 6, i - 4,
                                Hud_mission_log_list2_coords[gr_screen.res][0] - 6, i - 2);
                        gr_line(Hud_mission_log_list2_coords[gr_screen.res][0] - 4, i,
                                Hud_mission_log_list2_coords[gr_screen.res][0] - 2, i);
                        gr_line(Hud_mission_log_list2_coords[gr_screen.res][0] - 6, i + 2,
                                Hud_mission_log_list2_coords[gr_screen.res][0] - 6, i + 4);
                    }
    ...
    gr_print_timestamp(Hud_mission_log_time_coords[gr_screen.res][0], Hud_mission_log_time_coords[gr_screen.res][1] -
                                                                      font_height, (int) (f2fl(Missiontime) * 1000));
    gr_string(Hud_mission_log_time2_coords[gr_screen.res][0], Hud_mission_log_time_coords[gr_screen.res][1] -
                                                              font_height, XSTR("Current time", 289));
    gr_flip();
}

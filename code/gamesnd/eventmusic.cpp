...
const int BATTLE_START_INTERVAL = 1000;
const int ARRIVAL_INTERVAL_TIMESTAMP = 3000;
const int HOSTILE_PRESENCE_CHECK_INTERVAL = 1000;
const int MISSION_OVER_CHECK_INTERVAL = 3000;
const int BATTLE_OVER_INTERVAL = 5000;
static int Hostile_presence_timestamp;
    ...
    if (timestamp_elapsed(Pattern_timer_id)) {
        ...
        if (Current_pattern != -1 && Patterns[Current_pattern].handle >= 0) {
            ...
            audiostream_set_sample_cutoff(Patterns[Current_pattern].handle,
                                          fl2i(Patterns[Current_pattern].num_measures *
...
void event_music_level_init(int force_soundtrack) {
    ...
    for (i = 0; i < MAX_PATTERNS; i++) {
        ...
        Patterns[i].forcing_pattern = SONG_NONE;
...
void parse_menumusic() {
    ...
    if (idx < 0 && (nocreate || Num_music_files >= MAX_SPOOLED_MUSIC)) {
        if (Num_music_files >= MAX_SPOOLED_MUSIC) {
            Warning(LOCATION,
                    "Could not load spooled music file after '%s' as maximum number of spooled music was reached (Max is %d)",
